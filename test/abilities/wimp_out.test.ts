...
describe("Abilities - Wimp Out/Emergency Exit", () => {
  ...
  it.each<{ name: string; ability: AbilityId }>([
    ...
    confirmSwitch();
    expect(game.field.getEnemyPokemon().getInverseHp()).toBe(0);
  ...
  it.todo("should trigger separately for each Pokemon hit in speed order", async () => {
    game.override.battleStyle("double").enemyLevel(600).enemyMoveset(MoveId.DRAGON_ENERGY);
    ...
    game.move.use(MoveId.ENDURE, BattlerIndex.PLAYER);
    game.move.use(MoveId.ENDURE, BattlerIndex.PLAYER_2);

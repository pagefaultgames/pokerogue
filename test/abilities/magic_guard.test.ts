...
describe("Abilities - Magic Guard", () => {
  ...
  it("Magic Guard prevents damage caused by entry hazards", async () => {
    ...
    const newTag = getArenaTag(ArenaTagType.SPIKES, 5, Moves.SPIKES, 0, ArenaTagSide.BOTH)!;
  ...
  it("Magic Guard does not prevent poison from Toxic Spikes", async () => {
    //Adds and applies Spikes to both sides of the arena
    const playerTag = getArenaTag(ArenaTagType.TOXIC_SPIKES, 5, Moves.TOXIC_SPIKES, 0, ArenaTagSide.PLAYER)!;
    const enemyTag = getArenaTag(ArenaTagType.TOXIC_SPIKES, 5, Moves.TOXIC_SPIKES, 0, ArenaTagSide.ENEMY)!;

...
export class MoveHelper extends GameManagerHelper {
   ...
   * @param move - The {@linkcode Moves | move} being learnt
  ...
  public async learnMove(move: Moves | number, partyIndex = 0, moveSlotIndex = 0) {
    return new Promise<void>((resolve, _reject) => {
      ...
      if (this.game.scene.getPlayerParty()[partyIndex].moveset.length < 4) {
        resolve();
      }
      ...
      resolve();
    }).then(async () => await this.game.phaseInterceptor.to(LearnMovePhase));
  }
}

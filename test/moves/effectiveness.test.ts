...
describe("Moves - Type Effectiveness", () => {
  ...
  it("Ghost-type attacks have no effect on Normal-type Pokemon", () =>
    testMoveEffectiveness(game, MoveId.SHADOW_BALL, Species.URSALUNA, 0));
  ...
  it("Electric-type attacks are super-effective against Tera-Water Pokemon", () =>
    testMoveEffectiveness(game, MoveId.THUNDERBOLT, Species.EXCADRILL, 2, AbilityId.BALL_FETCH, PokemonType.WATER));
  ...
  it("Powder moves have no effect on Tera-Grass Pokemon", () =>
    testMoveEffectiveness(game, MoveId.SLEEP_POWDER, Species.SNORLAX, 0, AbilityId.BALL_FETCH, PokemonType.GRASS));
  ...
  it("Prankster-boosted status moves have no effect on Tera-Dark Pokemon", () => {
    game.override.ability(AbilityId.PRANKSTER);
    testMoveEffectiveness(game, MoveId.BABY_DOLL_EYES, Species.SNORLAX, 0, AbilityId.BALL_FETCH, PokemonType.DARK);
  });
});

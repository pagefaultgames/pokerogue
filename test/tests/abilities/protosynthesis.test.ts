...
describe("Abilities - Protosynthesis", () => {
  ...
  it("should not consider temporary items when determining which stat to boost", async () => {
    ...
    const def_before_boost = mew.getEffectiveStat(Stat.DEF, {
      ignoreAbility: false,
      isCritical: false,
      simulated: false,
      ignoreHeldItems: true,
    });
    const atk_before_boost = mew.getEffectiveStat(Stat.ATK, {
      ignoreAbility: false,
      isCritical: false,
      simulated: false,
      ignoreHeldItems: true,
    ...
    enemy.hp = initialHp;
    const def_after_boost = mew.getEffectiveStat(Stat.DEF, {
      ignoreAbility: false,
      isCritical: false,
      simulated: false,
      ignoreHeldItems: true,
    });
    const atk_after_boost = mew.getEffectiveStat(Stat.ATK, {
      ignoreAbility: false,
      isCritical: false,
      simulated: false,
      ignoreHeldItems: true,

...
export class FormChangePhase extends EvolutionPhase {
  ...
  setMode(): Promise<void> {
    if (!this.modal) return super.setMode();
    ...
    this.pokemon.getPossibleForm(this.formChange).then((transformedPokemon) => {
      [this.pokemonEvoSprite, this.pokemonEvoTintSprite].map((sprite) => {
        ...
        ["spriteColors", "fusionSpriteColors"].map((k) => {
          if (transformedPokemon.summonData?.speciesForm) k += "Base";
          sprite.pipelineData[k] =
            transformedPokemon.getSprite().pipelineData[k];
        });
      });
                                      ...
                                      this.scene.time.delayedCall(250, () => {
                                        ...
                                        this.scene.time.delayedCall(
                                          ...
                                          () => {
                                            ...
                                            const delay = playEvolutionFanfare
                                              ? 4000
                                              : 1750;
...
export class QuietFormChangePhase extends BattlePhase {
  ...
  start(): void {
    ...
    if (
      ...
      this.pokemon.species.forms.findIndex(
        (f) => f.formKey === this.formChange.formKey,
      )
    )
      return this.end();
    ...
    const [pokemonTintSprite, pokemonFormTintSprite] = [
      getPokemonSprite(),
      getPokemonSprite(),
    ];
    ...
    this.pokemon.getSprite().on("animationupdate", (_anim, frame) => {
      ...
      else pokemonFormTintSprite.setFrame(frame.textureFrame);
    });

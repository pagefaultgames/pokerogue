  ...
  start() {
    ...
    applyMoveAttrs(OverrideMoveEffectAttr, user, this.getTarget() ?? null, move, overridden, this.move.virtual).then(
      () => {
        ...
        if (
          !hasActiveTargets ||
          (!move.hasAttr(VariableTargetAttr) && !move.isMultiTarget() && !targetHitChecks[this.targets[0]])
        ) {
          this.stopMultiHit();
          if (hasActiveTargets) {
            this.scene.queueMessage(
              i18next.t("battle:attackMissed", {
                pokemonNameWithAffix: this.getTarget() ? getPokemonNameWithAffix(this.getTarget()!) : "",
              }),
            );
        ...
        new MoveAnim(move.id as Moves, user, this.getTarget()?.getBattlerIndex()!).play(this.scene, () => {
              ...
              new Promise((resolve) => {
                ...
                applyFilteredMoveAttrs(
                  ...
                  if (hitResult !== HitResult.FAIL) {
                    ...
                    const chargeEffect = !!move
                      .getAttrs(ChargeAttr)
                      .find((ca) => ca.usedChargeEffect(user, this.getTarget() ?? null, move));
                    /**
                              ...
                              return Utils.executeIf(!target.isFainted() || target.canApplyAbility(), () =>
                                applyPostDefendAbAttrs(
                                  ...
                                  if (!user.isPlayer() && this.move.getMove() instanceof AttackMove) {
                                    user.scene.applyShuffledModifiers(
                                      this.scene,
                                      EnemyAttackStatusEffectChanceModifier,
                                      false,
                                      target,
          ...
          const postTarget =
            user.turnData.hitsLeft === 1 || !this.getTarget()?.isActive()
              ? applyFilteredMoveAttrs(

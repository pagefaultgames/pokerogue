...
export class MovePhase extends BattlePhase {
  ...
  protected useMove(): void {
    ...
    if (!move.hasAttr(CopyMoveAttr) && !this.reflected) {
    ...
    this.showMoveText();
    globalScene.phaseManager.unshiftNew("MoveEffectPhase", this.pokemon.getBattlerIndex(), this.targets, move, this.reflected, this.followUp,
    );

    // Handle Dancer, which triggers immediately after a move is used (rather than waiting on `this.end()`).
    // Note that the `!this.followUp` check here prevents an infinite Dancer loop.
    if (move.hasFlag(MoveFlags.DANCE_MOVE) && !this.followUp) {
  ...
  protected chargeMove() {
    ...
    this.showMoveText();
    globalScene.phaseManager.unshiftNew("MoveChargePhase", this.pokemon.getBattlerIndex(), this.targets[0], this.move);
  }

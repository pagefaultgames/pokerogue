...
export class CommandPhase extends FieldPhase {
   ...
   */
  private computeMoveId(playerPokemon: PlayerPokemon, cursor: number, move?: TurnMove): Moves {
    return move?.move ?? (cursor > -1 ? playerPokemon.getMoveset()[cursor]?.moveId : Moves.NONE);
  }
  ...
  private handleFightCommand(
    ...
    const moveId = useStruggle ? Moves.STRUGGLE : this.computeMoveId(playerPokemon, cursor, move);
   ...
   *  For {@linkcode Command.POKEMON}, must be `true` if the player is switching via the Baton item.
  ...
  handleCommand(command: Command, cursor: number, ignorePP = false, move?: TurnMove): boolean {
    ...
    switch (command) {
      case Command.TERA:
      case Command.FIGHT:
        success = this.handleFightCommand(command, cursor, ignorePP, move);
        break;
      case Command.BALL:
      ...
      case Command.POKEMON:
        this.isSwitch = true;
        success = this.tryLeaveField(cursor, ignorePP);
        this.isSwitch = false;
        break;
      case Command.RUN:

...
export class SelectTargetPhase extends PokemonPhase {
  ...
  start() {
    ...
    globalScene.ui.setMode(UiMode.TARGET_SELECT, this.fieldIndex, move, (targets: BattlerIndex[]) => {
      ...
      if (user.isMoveTargetRestricted(move, fieldSide[targets[0]])) {
        const errorMessage = user.getRestrictingTag(move, fieldSide[targets[0]])!.selectionDeniedText(user, move);

...
export abstract class FormModalUiHandler extends ModalUiHandler {
  ...
  setup(): void {
    ...
    fields.forEach((field, f) => {
      ...
      const inputContainer = this.scene.add.container(
        70,
        (hasTitle ? 28 : 2) + 20 * f,
      );
      inputContainer.setVisible(false);

      const inputBg = addWindow(
        this.scene,
        0,
        0,
        80,
      ...
      const input = addTextInputObject(
        this.scene,
        4,
        -2,
        440,
    ...
    this.errorMessage.setShadowColor(
      this.getTextColor(TextStyle.SUMMARY_PINK, true),
    );
    ...
    if (super.show(args)) {
      ...
      if (this.buttonBgs.length) {
        ...
        this.buttonBgs[0].on("pointerdown", () => {
          if (this.submitAction) this.submitAction();
        });
      }
  ...
  sanitizeInputs(): void {
    for (const input of this.inputs) input.text = input.text.trim();
  }
  ...
  updateContainer(config?: ModalConfig): void {
    ...
    this.errorMessage.setText(
      this.getReadableErrorMessage(
        (config as FormModalConfig)?.errorMessage || "",
      ),
    );

...
const displayStats: DisplayStats = {
  playTime: {
    sourceFunc: (gameData) =>
      Utils.getPlayTimeString(gameData.gameStats.playTime),
  },
  ...
  startersUnlocked: {
    ...
    sourceFunc: (gameData) => {
      ...
      return `${starterCount} (${
        Math.floor(
          (starterCount / Object.keys(speciesStarters).length) * 1000,
        ) / 10
      }%)`;
    },
  },
  ...
  dexSeen: {
    ...
    sourceFunc: (gameData) => {
      ...
      return `${seenCount} (${
        Math.floor((seenCount / Object.keys(gameData.dexData).length) * 1000) /
        10
      }%)`;
    },
  },
  dexCaught: {
    ...
    sourceFunc: (gameData) => {
      ...
      return `${caughtCount} (${
        Math.floor(
          (caughtCount / Object.keys(gameData.dexData).length) * 1000,
        ) / 10
      }%)`;
    },
  },
...
export default class GameStatsUiHandler extends UiHandler {
  ...
  setup() {
    ...
    this.gameStatsContainer = this.scene.add.container(
      1,
  ...
  updateStats(): void {
    const statKeys = Object.keys(displayStats).slice(
      this.cursor * 2,
      this.cursor * 2 + 18,
    );
  ...
  processInput(button: Button): boolean {
    ...
    if (button === Button.CANCEL) {
      ...
      switch (button) {
        ...
        case Button.DOWN:
          if (
            this.cursor < Math.ceil((Object.keys(displayStats).length - 18) / 2)
          )
...
(() => {
  ...
  for (const key of statKeys) {
    ...
    if (!(displayStats[key] as DisplayStat).label) {
      const splittableKey = key.replace(
        /([a-z]{2,})([A-Z]{1}(?:[^A-Z]|$))/g,
        "$1_$2",
      );
      (displayStats[key] as DisplayStat).label = Utils.toReadableString(

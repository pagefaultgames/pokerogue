...
import type { PartyUiHandlerParams } from "#types/ui/ui-handler-params";
...
import {
  type PartyModifierSpliceSelectCallback,
  type PartyModifierTransferSelectCallback,
  PartyOption,
  type PartySelectCallback,
  PartyUiMode,
  type PokemonModifierTransferSelectFilter,
  type PokemonMoveSelectFilter,
  type PokemonSelectFilter,
  SummaryUiMode,
} from "#ui/ui-types";
...
export class PartyUiHandler extends MessageUiHandler {
  ...
  show(args: PartyUiHandlerParams): boolean {
    if (!args.partyUiMode || this.active) {
      return false;
    }
    super.show(args);
    ...
    this.partyUiMode = args.partyUiMode;
    this.fieldIndex = args.fieldIndex ?? -1;

    this.selectCallback = args.selectCallback ?? args.itemSelectCallback ?? null;
    this.selectFilter = args.selectFilter ?? args.itemSelectFilter ?? PartyUiHandler.FilterAll;
    this.moveSelectFilter = args.moveSelectFilter ?? PartyUiHandler.FilterAllMoves;
    this.tmMoveId = args.tmMoveId ?? MoveId.NONE;
    this.showMovePp = !!args.showMovePp;
  ...
  private processSummaryOption(pokemon: Pokemon): boolean {
    const ui = this.getUi();
    ui.playSelect();
    ui.setModeWithoutClear(UiMode.SUMMARY, { uiMode: SummaryUiMode.DEFAULT, pokemon: pokemon as PlayerPokemon }).then(
      () => this.clearOptions(),
    );
    return true;
  }
  ...
  private processPokedexOption(pokemon: Pokemon): boolean {
    ...
    ui.setOverlayMode(UiMode.POKEDEX_PAGE, { species: pokemon.species, savedStarterAttributes: attributes }).then(() =>
      this.clearOptions(),
    );
    return true;
  }

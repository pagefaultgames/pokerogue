...
export class LoginFormUiHandler extends FormModalUiHandler {
	...
	private buildInfoContainer() {
		...
		this.usernameInfoImage = this.buildInteractableImage(
			"settings_icon",
			"username-info-icon",
			{
				x: 20,
		...
		this.saveDownloadImage = this.buildInteractableImage(
			"saving_icon",
			"save-download-icon",
			{
				x: 0,
		...
		this.changeLanguageImage = this.buildInteractableImage(
			"settings_icon",
	...
	private processExternalProvider(config: ModalConfig): void {
		...
		this.externalPartyContainer.setPositionRelative(
			this.modalContainer,
			175,
			0,
		);
		...
		this.externalPartyBg.setSize(
			this.externalPartyTitle.text.length + 50,
			this.modalBg.height,
		);
		this.getUi().moveTo(this.externalPartyContainer, this.getUi().length - 1);
		this.googleImage.setPosition(
			this.externalPartyBg.width / 3.1,
			this.externalPartyBg.height - 60,
		);
		this.discordImage.setPosition(
			this.externalPartyBg.width / 3.1,
			this.externalPartyBg.height - 40,
		);
		...
		this.discordImage.on("pointerdown", () => {
			const redirectUri = encodeURIComponent(
				`${import.meta.env.VITE_SERVER_URL}/auth/discord/callback`,
			);
		...
		const onFail = (error) => {
			globalScene.ui.setMode(UiMode.LOADING, { buttonActions: [] });
			globalScene.ui.setModeForceTransition(
				UiMode.LOGIN_FORM,
				Object.assign(config, { errorMessage: error?.trim() }),
			);
			globalScene.ui.playError();
		...
		this.usernameInfoImage.on("pointerdown", () => {
			if (globalScene.tweens.getTweensOf(this.infoContainer).length === 0) {
				...
				const dataKeys = localStorageKeys.filter(
					(ls) => ls.indexOf(keyToFind) >= 0,
				);
		...
		this.saveDownloadImage.on("pointerdown", async () => {
			...
			const sessionKeys = localStorageKeys.filter(
				(ls) => ls.indexOf(sessionKeyToFind) >= 0,
			);
	...
	private buildInteractableImage(
		texture: string,
		name: string,

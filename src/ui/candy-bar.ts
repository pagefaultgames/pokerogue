...
export default class CandyBar extends Phaser.GameObjects.Container {
  ...
  setup(): void {
    ...
    this.candyOverlayIcon = this.scene.add.sprite(
      14,
      0,
      "items",
      "candy_overlay",
  ...
  showStarterSpeciesCandy(
    starterSpeciesId: Species,
    count: integer,
  ): Promise<void> {
    return new Promise<void>((resolve) => {
      if (this.shown) {
        if (this.speciesId === starterSpeciesId) return resolve();
        else
          return this.hide()
      ...
      this.candyOverlayIcon.setTint(
        argbFromRgba(Utils.rgbHexToRgba(colorScheme[1])),
      );

      this.countText.setText(
        `${
          (this.scene as BattleScene).gameData.starterData[starterSpeciesId]
            .candyCount + count
        } (+${count.toString()})`,
      );
  ...
  hide(): Promise<void> {
    return new Promise<void>((resolve) => {
      ...
      if (this.autoHideTimer) clearInterval(this.autoHideTimer);

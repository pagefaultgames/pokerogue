...
export default class RunInfoUiHandler extends UiHandler {
  ...
  private async parseRunResult() {
    ...
    const runResultText = addBBCodeTextObject(
      this.scene,
      6,
      5,
  ...
  private async parseRunInfo(windowX: number, windowY: number) {
    ...
    runInfoText.appendText(
      `[color=${getTextColor(TextStyle.MONEY)}]${i18next.t("battleScene:moneyOwned", { formattedMoney: runMoney })}[/color]`,
    );
  ...
  private parsePartyInfo(): void {
    ...
    party.forEach((p: PokemonData, i: integer) => {
      ...
      const speedLabel =
        currentLanguage === "es" || currentLanguage === "pt_BR"

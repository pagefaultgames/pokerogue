...
export default class AchvsUiHandler extends MessageUiHandler {
  ...
  setup() {
    ...
    this.achvsContainer = this.scene.add.container(
      1,
    ...
    const titleBg = addWindow(
      this.scene,
      0,
      headerBg.height + this.achvIconsBg.height,
      174,
      24,
    );
    ...
    const scoreBg = addWindow(
      this.scene,
      titleBg.x + titleBg.width,
      titleBg.y,
      46,
      24,
    );
    ...
    const descriptionText = addTextObject(
      this.scene,
      0,
      0,
      "",
  ...
  show(args: any[]): boolean {
    ...
    Object.values(achvs).forEach((achv: Achv, i: integer) => {
      ...
      const hidden =
        !unlocked &&
        achv.secret &&
        (!achv.parentId || !achvUnlocks.hasOwnProperty(achv.parentId));
      const tinted = !hidden && !unlocked;

      icon.setFrame(!hidden ? achv.iconImage : "unknown");
      if (tinted) icon.setTintFill(0);
      else icon.clearTint();
    });
  ...
  protected showAchv(achv: Achv) {
    ...
    this.unlockText.setText(
      unlocked ? new Date(achvUnlocks[achv.id]).toLocaleDateString() : "Locked",
    );
  }
  ...
  processInput(button: Button): boolean {
    ...
    if (button === Button.CANCEL) {
      ...
      switch (button) {
        case Button.UP:
          if (this.cursor >= 17) success = this.setCursor(this.cursor - 17);
          break;
        case Button.DOWN:
  ...
  setCursor(cursor: integer): boolean {
    ...
    if (updateAchv) this.showAchv(achvs[Object.keys(achvs)[cursor]]);

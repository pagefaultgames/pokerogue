...
export default abstract class AbstractOptionSelectUiHandler extends UiHandler {
  ...
  setup() {
    ...
    this.optionSelectContainer = this.scene.add.container(
  ...
  protected setupOptions() {
    ...
    this.optionSelectBg.width = Math.max(
      this.optionSelectText.displayWidth + 24,
      this.getWindowWidth(),
    );
    ...
    options.forEach((option: OptionSelectItem, i: integer) => {
      if (option.item) {
        ...
        if (option.item === "candy") {
          const itemOverlayIcon = this.scene.add.sprite(
            0,
            0,
            "items",
            "candy_overlay",
          ...
          itemIcon.setTint(
            argbFromRgba(Utils.rgbHexToRgba(option.itemArgs[0])),
          );
          itemOverlayIcon.setTint(
            argbFromRgba(Utils.rgbHexToRgba(option.itemArgs[1])),
          );
        }
      }
    });
  ...
  show(args: any[]): boolean {
    if (
      !args.length ||
      !args[0].hasOwnProperty("options") ||
      !args[0].options.length
    )
      return false;
  ...
  processInput(button: Button): boolean {
    ...
    if (button === Button.ACTION || button === Button.CANCEL) {
      ...
      if (button === Button.CANCEL) {
        if (
          this.config?.maxOptions &&
          this.config.options.length > this.config.maxOptions
        ) {
          this.scrollCursor =
            this.config.options.length - this.config.maxOptions + 1;
          this.cursor = options.length - 1;
        } else if (!this.config?.noCancel) this.setCursor(options.length - 1);
        else return false;
      }
      const option =
        this.config.options[
          this.cursor + (this.scrollCursor - (this.scrollCursor ? 1 : 0))
        ];
      if (option.handler()) {
        if (!option.keepOpen) this.clear();
    ...
    if (success && playSound) ui.playSelect();
  ...
  getOptionsWithScroll(): OptionSelectItem[] {
    ...
    const optionEndIndex = Math.min(
      ...
      optionStartIndex +
        (!optionStartIndex ||
        this.scrollCursor + (this.config.maxOptions - 1) >= optionsScrollTotal
    ...
    if (
      ...
      if (Math.abs(cursor - this.cursor) === options.length - 1) {
        this.scrollCursor = cursor
          ? optionsScrollTotal - (this.config.maxOptions - 1)
          : 0;
        this.setupOptions();
      } else {

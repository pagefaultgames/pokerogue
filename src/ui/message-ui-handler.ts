...
export default abstract class MessageUiHandler extends AwaitableUiHandler {
  ...
  private showTextInternal(
    ...
    if (delay === null || delay === undefined) delay = 20;
    ...
    if (text) {
      ...
      for (let w = 0; w < textWords.length; w++) {
        ...
        if (textWords[w].includes("\n")) {
          ...
          const lineCount = this.message
            .runWordWrap(nextWordText)
            .split(/\n/g).length;
          if (lineCount > lastLineCount) {
            lastLineCount = lineCount;
            newText = `${newText}\n${textWords[w]}`;
          } else newText = nextWordText;
        }
      }
    ...
    if (this.textTimer) {
      ...
      if (this.textCallbackTimer) this.textCallbackTimer.callback();
    }
    if (prompt) {
      const originalCallback = callback;
      callback = () => {
        const showPrompt = () =>
          this.showPrompt(originalCallback, callbackDelay);
        if (promptDelay) this.scene.time.delayedCall(promptDelay, showPrompt);
        else showPrompt();
      };
    }
    if (delay) {
      this.clearText();
      if (prompt) this.pendingPrompt = true;
      this.textTimer = this.scene.time.addEvent({
        ...
        callback: () => {
          ...
          const advance = () => {
            if (charVar) this.scene.charSprite.setVariant(charVar);
            if (charSound) this.scene.playSound(charSound);
            if (callback && !this.textTimer.repeatCount) {
              if (callbackDelay && !prompt) {
                this.textCallbackTimer = this.scene.time.delayedCall(
                  callbackDelay,
                  () => {
  ...
  showPrompt(callback: Function, callbackDelay: integer) {
    const wrappedTextLines = this.message
      .runWordWrap(this.message.text)
      .split(/\n/g);
    const textLinesCount = wrappedTextLines.length;
    const lastTextLine = wrappedTextLines[wrappedTextLines.length - 1];
    const lastLineTest = this.scene.add.text(0, 0, lastTextLine, {
      font: "96px emerald",
    });

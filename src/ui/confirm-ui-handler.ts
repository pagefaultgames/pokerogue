...
export default class ConfirmUiHandler extends AbstractOptionSelectUiHandler {
  ...
  show(args: any[]): boolean {
    if (args.length >= 2 && args.length !== 3 && args[0] instanceof Function && args[1] instanceof Function) {
      ...
      return true;
    } else if (args.length === 3 && args[0] instanceof Function && args[1] instanceof Function && args[2] instanceof Function) {
      const config: OptionSelectConfig = {
        options: [
          {
            label: i18next.t("menu:Summary"),
            handler: () => {
              args[0]();
            }
          },

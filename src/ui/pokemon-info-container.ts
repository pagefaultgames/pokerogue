...
export default class PokemonInfoContainer extends Phaser.GameObjects.Container {
  ...
  constructor(scene: BattleScene, x = 372, y = 66) {
    super(scene, x, y);
    this.initialX = x;
  }
  ...
  setup(): void {
    ...
    for (let m = 0; m < 4; m++) {
      ...
      const moveBg = this.scene.add.nineslice(
        0,
        0,
        "type_bgs",
        "unknown",
    ...
    this.pokemonGenderLabelText = addTextObject(
      this.scene,
      -18,
      18,
      "Gender:",
    ...
    this.pokemonGenderText = addTextObject(
      this.scene,
      -14,
      18,
      "",
    ...
    this.pokemonNatureText = addBBCodeTextObject(
      ...
      "",
      TextStyle.WINDOW,
      { fontSize: "64px", lineSpacing: 3, maxLines: 2 },
    );
  ...
  show(
    ...
    return new Promise<void>((resolve) => {
      if (pokemon.gender > Gender.GENDERLESS) {
        ...
        this.pokemonGenderText.setShadowColor(
          getGenderColor(pokemon.gender, true),
        );
        this.pokemonGenderLabelText.setVisible(true);
        this.pokemonGenderText.setVisible(true);
      } else this.pokemonGenderText.setVisible(false);
      ...
      this.pokemonAbilityText.setColor(
        getTextColor(abilityTextStyle, false, this.scene.uiTheme),
      );
      ...
      if (this.pokemonShinyIcon.visible) {
        const shinyDescriptor =
          doubleShiny || baseVariant
            ? `${
                ...
                doubleShiny
                  ? `/${
                      pokemon.fusionVariant === 2
                        ? "Epic"
                        : pokemon.fusionVariant === 1
                          ? "Rare"
                          : "Common"
                    }`
                  : ""
        ...
        this.pokemonShinyIcon.on("pointerout", () =>
          (this.scene as BattleScene).ui.hideTooltip(),
        );
      }

      this.pokemonFusionShinyIcon.setPosition(
        this.pokemonShinyIcon.x,
        this.pokemonShinyIcon.y,
      );
      this.pokemonFusionShinyIcon.setVisible(doubleShiny);
      if (isFusion)
        this.pokemonFusionShinyIcon.setTint(
          getVariantTint(pokemon.fusionVariant),
        );

      const starterSpeciesId = pokemon.species.getRootSpeciesId(true);
      const originalIvs: integer[] = this.scene.gameData.dexData[
        starterSpeciesId
      ].caughtAttr
      ...
      for (let m = 0; m < 4; m++) {
        const move =
          m < pokemon.moveset.length ? pokemon.moveset[m].getMove() : null;

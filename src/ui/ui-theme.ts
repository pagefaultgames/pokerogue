...
export function addWindow(
  ...
  if (windowVariant === undefined) windowVariant = WindowVariant.NORMAL;
...
export function updateWindowType(
  ...
  const traverse = (object: any) => {
    if (
      ...
      for (const child of children.getAll()) traverse(child);
    } else if (object instanceof Phaser.GameObjects.Container) {
      for (const child of object.getAll()) traverse(child);
    } else if (object instanceof Phaser.GameObjects.NineSlice) {
      if (object.texture.key.startsWith("window_"))
        windowObjects.push([
          object,
          object.texture.key.endsWith(
            getWindowVariantSuffix(WindowVariant.XTHIN),
          )
            ? WindowVariant.XTHIN
            : object.texture.key.endsWith(
                  getWindowVariantSuffix(WindowVariant.THIN),
                )
              ? WindowVariant.THIN
              : WindowVariant.NORMAL,
        ]);
      else if (object.texture?.key === "namebox") themedObjects.push(object);
    } else if (object instanceof Phaser.GameObjects.Sprite) {
      if (object.texture?.key === "bg") themedObjects.push(object);
    }
  };
  ...
  for (const obj of themedObjects) obj.setFrame(windowTypeIndex);
}
...
export function addUiThemeOverrides(scene: BattleScene): void {
  ...
  scene.add.image = function (
    ...
    const ret: Phaser.GameObjects.Image = originalAddImage.apply(this, [
      x,
      y,
      texture,
      frame,
  ...
  scene.add.sprite = function (
    ...
    const ret: Phaser.GameObjects.Sprite = originalAddSprite.apply(this, [
      x,
      y,
      texture,
      frame,
  ...
  scene.add.nineslice = function (
    ...
    if (legacy) {
      ...
      ret.setTexture = function (
        ...
        key += "_legacy";
        return originalSetTexture.apply(this, [
          key,
          frame,
          updateSize,
          updateOrigin,

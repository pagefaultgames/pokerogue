...
export default class CommandUiHandler extends UiHandler {
  ...
  show(args: any[]): boolean {
    ...
    if (currentPhase instanceof CommandPhase) commandPhase = currentPhase;
    else commandPhase = this.scene.getStandbyPhase() as CommandPhase;
  ...
  processInput(button: Button): boolean {
    ...
    if (button === Button.CANCEL || button === Button.ACTION) {
      ...
      } else (this.scene.getCurrentPhase() as CommandPhase).cancel();
    } else {
      switch (button) {
        case Button.UP:

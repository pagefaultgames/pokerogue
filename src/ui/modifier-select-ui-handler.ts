...
export default class ModifierSelectUiHandler extends AwaitableUiHandler {
  ...
  show(args: any[]): boolean {
    ...
    const partyHasHeldItem =
      this.player &&
      !!this.scene.findModifiers((m) => m instanceof PokemonHeldItemModifier && m.isTransferrable).length;
    ...
    for (let m = 0; m < shopTypeOptions.length; m++) {
      ...
      const option = new ModifierOption(
        ...
        -this.scene.game.canvas.height / 12 - this.scene.game.canvas.height / 32 - (40 - (28 * row - 1)),
        shopTypeOptions[m],
      );

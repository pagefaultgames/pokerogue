...
export default class ModifierSelectUiHandler extends AwaitableUiHandler {
  ...
  setup() {
    ...
    this.transferButtonContainer = this.scene.add.container(
      this.scene.game.canvas.width / 6 - 1,
      -64,
    );
    ...
    this.rerollCostText.setPositionRelative(
      rerollButtonText,
      rerollButtonText.displayWidth + 5,
      1,
    );
  ...
  show(args: any[]): boolean {
    ...
    const partyHasHeldItem =
      ...
      !!this.scene.findModifiers(
        (m) =>
          ...
          (m as PokemonHeldItemModifier).getTransferrable(true),
      ).length;
    ...
    for (let m = 0; m < typeOptions.length; m++) {
      const sliceWidth =
        this.scene.game.canvas.width / 6 / (typeOptions.length + 2);
    ...
    this.scene.time.delayedCall(1000 + maxUpgradeCount * 2000, () => {
      for (const shopOption of this.shopOptionsRows.flat()) shopOption.show(0, 0);
    });
  ...
  processInput(button: Button): boolean {
    ...
    if (!this.awaitingActionInput) return false;
    ...
    if (button === Button.ACTION) {
      ...
      switch (button) {
        case Button.UP:
          if (!this.rowCursor && this.cursor === 2) success = this.setCursor(0);
          else if (this.rowCursor < this.shopOptionsRows.length + 1)
            success = this.setRowCursor(this.rowCursor + 1);
          break;
        case Button.DOWN:
          if (this.rowCursor) success = this.setRowCursor(this.rowCursor - 1);
        ...
        case Button.LEFT:
          if (!this.rowCursor) {
            success =
              this.cursor === 1 &&
              this.rerollButtonContainer.visible &&
              this.setCursor(0);
          } else if (this.cursor) success = this.setCursor(this.cursor - 1);
        ...
        case Button.RIGHT:
          if (!this.rowCursor)
            success =
              this.cursor !== 1 &&
              this.transferButtonContainer.visible &&
              this.setCursor(1);
  ...
  setCursor(cursor: integer): boolean {
    ...
    const options =
      this.rowCursor === 1
        ? this.options
        : this.shopOptionsRows[
            this.shopOptionsRows.length - (this.rowCursor - 1)
          ];

    this.cursorObj.setScale(
      this.rowCursor === 1 ? 2 : this.rowCursor >= 2 ? 1.5 : 1,
    );
    ...
    if (this.rowCursor) {
      ...
      if (this.rowCursor < 2)
        this.cursorObj.setPosition(
          ...
          -this.scene.game.canvas.height / 12 -
            (this.shopOptionsRows.length > 1 ? 6 : 22),
        );
      else
        this.cursorObj.setPosition(
      ...
      ui.showText(
        options[this.cursor].modifierTypeOption.type.getDescription(this.scene),
      );
    } else if (!cursor) {
      this.cursorObj.setPosition(
        6,
        this.lockRarityButtonContainer.visible ? -72 : -60,
      );
  ...
  setRowCursor(rowCursor: integer): boolean {
    ...
    if (
      ...
      if (!rowCursor) {
        if (!newCursor && !this.rerollButtonContainer.visible) newCursor = 1;
  ...
  private getRowItems(rowCursor: integer): integer {
    switch (rowCursor) {
      ...
      default:
        return this.shopOptionsRows[
          this.shopOptionsRows.length - (rowCursor - 1)
        ].length;
    }
  }
  ...
  updateCostText(): void {
    ...
    for (const shopOption of shopOptions) shopOption.updateCostText();
  ...
  updateLockRaritiesText(): void {
    const textStyle = this.scene.lockModifierTiers
      ? TextStyle.SUMMARY_BLUE
      : TextStyle.PARTY;
    this.lockRarityButtonText.setColor(this.getTextColor(textStyle));
    this.lockRarityButtonText.setShadowColor(
      this.getTextColor(textStyle, true),
    );
  }
    ...
    [
      ...
      if (container.visible) {
        this.scene.tweens.add({
          ...
          ease: "Cubic.easeIn",
          onComplete: () => {
            if (!this.options.length) container.setVisible(false);
            else container.setAlpha(1);
          },
        });
      }
    });
...
class ModifierOption extends Phaser.GameObjects.Container {
    ...
    if (!this.modifierTypeOption.cost) {
      ...
      this.scene.tweens.addCounter({
        ...
        onUpdate: (t) => {
          ...
          if (!bounce && value > lastValue) {
            (this.scene as BattleScene).playSound("pb_bounce_1", {
              volume: 1 / ++bounceCount,
            });
            bounce = true;
          } else if (bounce && value < lastValue) bounce = false;
          lastValue = value;
        },
      });
      ...
      for (let u = 0; u < this.modifierTypeOption.upgradeCount; u++) {
        ...
        this.scene.time.delayedCall(
          remainingDuration -
            2000 *
              (this.modifierTypeOption.upgradeCount -
                (upgradeIndex + 1 + upgradeCountOffset)),
          () => {
            ...
            this.scene.tweens.add({
              ...
              onComplete: () => {
                this.pb.setTexture(
                  "pb",
                  this.getPbAtlasKey(
                    -this.modifierTypeOption.upgradeCount + (upgradeIndex + 1),
                  ),
                );
                this.scene.tweens.add({
                  targets: this.pbTint,
  ...
  getPbAtlasKey(tierOffset: integer = 0) {
    return getPokeballAtlasKey(
      (this.modifierTypeOption.type.tier +
        tierOffset) as integer as PokeballType,
    );
  }
  ...
  updateCostText(): void {
    ...
    const textStyle =
      this.modifierTypeOption.cost <= scene.money
        ? TextStyle.MONEY
        : TextStyle.PARTY_RED;

    this.itemCostText.setText(
      `₽${this.modifierTypeOption.cost.toLocaleString("en-US")}`,
    );
    this.itemCostText.setColor(getTextColor(textStyle, false, scene.uiTheme));
    this.itemCostText.setShadowColor(
      getTextColor(textStyle, true, scene.uiTheme),
    );
  }
}

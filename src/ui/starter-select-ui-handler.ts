...
export default class StarterSelectUiHandler extends MessageUiHandler {
  ...
  setup() {
    ...
    this.starterSelectContainer = this.scene.add.container(
      0,
    ...
    this.starterSelectContainer.add(
      addWindow(this.scene, 107, 145, 34, 34, true),
    );
    this.starterSelectContainer.add(starterContainerWindow);

    if (!this.scene.uiTheme) starterContainerWindow.setVisible(false);

    this.iconAnimHandler = new PokemonIconAnimHandler();
    this.iconAnimHandler.setup(this.scene);

    this.pokemonNumberText = addTextObject(
      this.scene,
      17,
      1,
      "0000",
    ...
    this.pokemonNameText = addTextObject(
      this.scene,
      6,
      112,
      "",
    ...
    this.genOptionsText = addTextObject(
      this.scene,
      124,
      7,
      "",
      TextStyle.WINDOW,
      { fontSize: 72, lineSpacing: 39, align: "center" },
    );
    ...
    this.starterSelectGenIconContainers = new Array(gens.length)
      .fill(null)
      .map((_, i) => {
        const container = this.scene.add.container(151, 9);
        if (i) container.setVisible(false);
    ...
    this.genCursorHighlightObj = this.scene.add.image(
      111,
      5,
      "select_gen_cursor_highlight",
    );
    ...
    this.valueLimitLabel = addTextObject(
      this.scene,
      124,
      150,
      "0/10",
    ...
    for (let g = 0; g < this.starterSelectGenIconContainers.length; g++) {
      ...
      for (const species of allSpecies) {
        ...
        const defaultProps = this.scene.gameData.getSpeciesDexAttrProps(
          species,
          defaultDexAttr,
        );
        const x = (s % 9) * 18;
    ...
    this.pokemonCandyOverlayIcon = this.scene.add.sprite(
      4.5,
      18,
      "candy_overlay",
    );
    ...
    this.pokemonCandyCountText = addTextObject(
      this.scene,
      14,
      18,
      "x0",
    ...
    this.pokemonCaughtCountText = addTextObject(
      this.scene,
      24,
      4,
      "0",
    ...
    const pokemonHatchedIcon = this.scene.add.sprite(
      1,
      14,
      "items",
      "mystery_egg",
    ...
    this.pokemonHatchedCountText = addTextObject(
      this.scene,
      24,
      19,
      "0",
    ...
    for (let m = 0; m < 4; m++) {
      ...
      const eggMoveBg = this.scene.add.nineslice(
        0,
        0,
        "type_bgs",
        "unknown",
    ...
    this.instructionsText = addTextObject(
      this.scene,
      4,
      156,
      "",
      ...
      () => {
        for (let c = 0; c < 3; c++) {
          ...
          const generateSpecies = () => {
            ...
            pokerusCursor =
              this.genSpecies[species.generation - 1].indexOf(species);
          };
    ...
    if (
      ...
      for (let g = 0; g < this.genSpecies.length; g++) {
        this.genSpecies[g].forEach((species, s) => {
          ...
          const icon = this.starterSelectGenIconContainers[g].getAt(
            s,
          ) as Phaser.GameObjects.Sprite;
          if (dexEntry.caughtAttr) icon.clearTint();
          else if (dexEntry.seenAttr) icon.setTint(0x808080);
        });
      }
  ...
  processInput(button: Button): boolean {
    ...
    if (button === Button.SUBMIT) {
      if (this.tryStart(true)) success = true;
      else error = true;
      ...
      switch (button) {
        case Button.UP:
          if (this.genCursor) success = this.setCursor(this.genCursor - 1);
          break;
        case Button.DOWN:
          if (this.genCursor < 2) success = this.setCursor(this.genCursor + 1);
          else {
      ...
      if (button === Button.ACTION) {
        if (!this.speciesStarterDexEntry?.caughtAttr) error = true;
          ...
          const options = [
            {
              ...
              handler: () => {
                ...
                const species =
                  this.genSpecies[this.getGenCursorWithScroll()][this.cursor];
                if (
                  !isDupe &&
                  this.tryUpdateValue(
                    this.scene.gameData.getSpeciesStarterValue(
                      species.speciesId,
                    ),
                  )
                ) {
                  const cursorObj =
                    this.starterCursorObjs[this.starterCursors.length];
                  ...
                  this.starterNatures.push(
                    this.natureCursor as unknown as Nature,
                  );
                  this.starterMovesets.push(
                  ...
                  if (
                    this.starterCursors.length === 6 ||
                    this.value === this.getValueLimit()
                  )
                    this.tryStart();
                  this.updateInstructions();
                  ui.playSelect();
          ...
          if (this.speciesStarterMoves.length > 1) {
            const showSwapOptions = (moveset: StarterMoveset) => {
              ui.setMode(Mode.STARTER_SELECT).then(() => {
                ui.showText(
                  i18next.t("starterSelectUiHandler:selectMoveSwapOut"),
                  null,
                  () => {
                          ...
                          const option: OptionSelectItem = {
                            ...
                            handler: () => {
                              ui.setMode(Mode.STARTER_SELECT).then(() => {
                                ui.showText(
                                  `${i18next.t(
                                    "starterSelectUiHandler:selectMoveSwapWith",
                                  )} ${allMoves[m].name}.`,
                                  null,
                                  () => {
                                    ui.setModeWithoutClear(Mode.OPTION_SELECT, {
                                      options: this.speciesStarterMoves
                                        .filter((sm: Moves) => sm !== m)
                                        .map((sm) => {
          ...
          const starterData =
            this.scene.gameData.starterData[this.lastSpecies.speciesId];
          ...
          const showUseCandies = () => {
            ...
            if (!(passiveAttr & PassiveAttr.UNLOCKED)) {
              ...
              options.push({
                label: `x${passiveCost} ${i18next.t(
                  "starterSelectUiHandler:unlockPassive",
                )} (${
                  allAbilities[
                    starterPassiveAbilities[this.lastSpecies.speciesId]
                  ].name
                })`,
                handler: () => {
                  if (candyCount >= passiveCost) {
                    starterData.passiveAttr |=
                      PassiveAttr.UNLOCKED | PassiveAttr.ENABLED;
                    starterData.candyCount -= passiveCost;
                    this.pokemonCandyCountText.setText(
                      `x${starterData.candyCount}`,
                    );
            ...
            if (valueReduction < 2) {
              ...
              options.push({
                label: `x${reductionCost} ${i18next.t(
                  "starterSelectUiHandler:reduceCost",
                )}`,
                handler: () => {
        ...
        switch (button) {
          ...
          case Button.CYCLE_ABILITY:
            if (this.canCycleAbility) {
              ...
              do {
                ...
                if (!newAbilityIndex) {
                  if (abilityAttr & AbilityAttr.ABILITY_1) break;
                } else if (newAbilityIndex === 1) {
                  if (
                    abilityAttr &
                    (this.lastSpecies.ability2
                      ? AbilityAttr.ABILITY_2
                      : AbilityAttr.ABILITY_HIDDEN)
                  )
                    break;
                } else {
                  if (abilityAttr & AbilityAttr.ABILITY_HIDDEN) break;
                }
          ...
          case Button.CYCLE_NATURE:
            if (this.canCycleNature) {
              ...
              const newNature =
                natures[natureIndex < natures.length - 1 ? natureIndex + 1 : 0];
              this.setSpeciesDetails(
                this.lastSpecies,
                undefined,
                undefined,
          ...
          case Button.CYCLE_VARIANT:
            ...
            break;
          case Button.UP:
            if (row) success = this.setCursor(this.cursor - 9);
            break;
          case Button.DOWN:
            if (
              row < rows - 2 ||
              (row < rows - 1 && this.cursor % 9 <= (genStarters - 1) % 9)
            )
              success = this.setCursor(this.cursor + 9);
            break;
          case Button.LEFT:
            if (this.cursor % 9) success = this.setCursor(this.cursor - 1);
            else {
  ...
  switchMoveHandler(i: number, newMove: Moves, move: Moves) {
    ...
    if (existingMoveIndex > -1) this.starterMoveset[existingMoveIndex] = move;
  ...
  updateInstructions(): void {
    ...
    for (const cil of cycleInstructionLines) instructionLines.push(cil);
  ...
  setCursor(cursor: integer): boolean {
    ...
    if (this.genMode) {
      ...
      if (genCursorWithScroll !== undefined)
        this.starterSelectGenIconContainers[genCursorWithScroll].setVisible(
          false,
        );
      this.cursor = 0;
      ...
      for (let s = 0; s < this.starterCursorObjs.length; s++)
        this.starterCursorObjs[s].setVisible(
          this.starterGens[s] === genCursorWithScroll,
        );
      for (let s = 0; s < this.pokerusCursorObjs.length; s++)
        this.pokerusCursorObjs[s].setVisible(
          this.pokerusGens[s] === genCursorWithScroll,
        );
      ...
      for (let s = 0; s < 81; s++) {
        const speciesId =
          s < genLimit
            ? this.genSpecies[genCursorWithScroll][s].speciesId
            : (0 as Species);
        const slotVisible = !!speciesId;
        if (slotVisible) this.updateStarterValueLabel(s);
        this.starterValueLabels[s].setVisible(slotVisible);
        const speciesVariants =
          speciesId &&
          this.scene.gameData.dexData[speciesId].caughtAttr & DexAttr.SHINY
            ? [
                DexAttr.DEFAULT_VARIANT,
                DexAttr.VARIANT_2,
                DexAttr.VARIANT_3,
                ...
                (v) =>
                  !!(this.scene.gameData.dexData[speciesId].caughtAttr & v),
              )
            : [];
        for (let v = 0; v < 3; v++) {
        ...
        this.classicWinIcons[s].setVisible(
          slotVisible &&
            this.scene.gameData.starterData[speciesId].classicWinCount > 0,
        );
      }
    } else {
  ...
  updateGenOptions(): void {
    ...
    for (let g = this.genScrollCursor; g <= this.genScrollCursor + 2; g++) {
      ...
      if (g === this.genScrollCursor && this.genScrollCursor) optionText = "↑";
      else if (
        g === this.genScrollCursor + 2 &&
  ...
  setGenMode(genMode: boolean): boolean {
    ...
    if (genMode !== this.genMode) {
      ...
      if (genMode) this.setSpecies(null);
  ...
  setSpecies(species: PokemonSpecies) {
    this.speciesStarterDexEntry = species
      ? this.scene.gameData.dexData[species.speciesId]
      : null;
    ...
    if (this.lastSpecies) {
      const dexAttr = this.scene.gameData.getSpeciesDefaultDexAttr(
        this.lastSpecies,
        false,
        true,
      );
      ...
      lastSpeciesIcon.setTexture(
        this.lastSpecies.getIconAtlasKey(
          props.formIndex,
          props.shiny,
          props.variant,
        ),
        this.lastSpecies.getIconId(
          props.female,
          props.formIndex,
          props.shiny,
          props.variant,
    ...
    if (
      ...
      if (this.speciesStarterDexEntry?.caughtAttr) {
        ...
        const luck = this.scene.gameData.getDexAttrLuck(
          this.speciesStarterDexEntry.caughtAttr,
        );
        this.pokemonLuckText.setVisible(!!luck);
        this.pokemonLuckText.setText(luck.toString());
        this.pokemonLuckText.setTint(
          getVariantTint(Math.min(luck - 1, 2) as Variant),
        );
        this.pokemonLuckLabelText.setVisible(this.pokemonLuckText.visible);

        this.pokemonGrowthRateText.setText(
          Utils.toReadableString(GrowthRate[species.growthRate]),
        );
        this.pokemonGrowthRateText.setColor(
          getGrowthRateColor(species.growthRate),
        );
        this.pokemonGrowthRateText.setShadowColor(
          getGrowthRateColor(species.growthRate, true),
        );
        ...
        this.pokemonCaughtCountText.setText(
          `${this.speciesStarterDexEntry.caughtCount}`,
        );
        this.pokemonHatchedCountText.setText(
          `${this.speciesStarterDexEntry.hatchedCount}`,
        );
        ...
        if (pokemonPrevolutions.hasOwnProperty(species.speciesId)) {
          ...
          this.pokemonCandyIcon.setTint(
            argbFromRgba(Utils.rgbHexToRgba(colorScheme[0])),
          );
          this.pokemonCandyIcon.setVisible(true);
          this.pokemonCandyOverlayIcon.setTint(
        ...
        this.iconAnimHandler.addOrUpdate(
          this.starterSelectGenIconContainers[species.generation - 1].getAt(
            this.genSpecies[species.generation - 1].indexOf(species),
          ) as Phaser.GameObjects.Sprite,
  ...
  setSpeciesDetails(
    ...
    if (species?.forms?.find((f) => f.formKey === "female")) {
      if (female !== undefined) formIndex = female ? 1 : 0;
      else if (formIndex !== undefined) female = formIndex === 1;
    }
    ...
    if (species) {
      const dexEntry = this.scene.gameData.dexData[species.speciesId];
      const abilityAttr =
        this.scene.gameData.starterData[species.speciesId].abilityAttr;
      if (!dexEntry.caughtAttr) {
        const props = this.scene.gameData.getSpeciesDexAttrProps(
          species,
          this.scene.gameData.getSpeciesDefaultDexAttr(
            species,
            forSeen,
            !forSeen,
          ),
        ...
        const defaultNature =
          this.scene.gameData.getSpeciesDefaultNature(species);
        if (shiny === undefined || shiny !== props.shiny) shiny = props.shiny;
      ...
      this.pokemonNumberText.setColor(
        this.getTextColor(
          shiny ? TextStyle.SUMMARY_GOLD : TextStyle.SUMMARY,
          false,
        ),
      );
      ...
      if (
        ...
        species
          .loadAssets(this.scene, female, formIndex, shiny, variant, true)
          .then(() => {
            if (assetLoadCancelled.value) return;
            this.assetLoadCancelled = null;
            this.speciesLoaded.set(species.speciesId, true);
            this.pokemonSprite.play(
        ...
        (
          this.starterSelectGenIconContainers[
            this.getGenCursorWithScroll()
          ].getAt(this.cursor) as Phaser.GameObjects.Sprite
        ).setTexture(
        ...
        this.canCycleShiny = !!(
          dexEntry.caughtAttr & DexAttr.NON_SHINY &&
        ...
        this.canCycleForm =
          species.forms
            .filter(
              (f) =>
                !f.formKey ||
                !pokemonFormChanges[species.speciesId]?.find(
                  (fc) => fc.formKey,
                ),
            )
            .map(
              (_, f) =>
                dexEntry.caughtAttr & this.scene.gameData.getFormAttr(f),
            )
            .filter((f) => f).length > 1;
        this.canCycleNature =
          this.scene.gameData.getNaturesForAttr(dexEntry.natureAttr).length > 1;
      ...
      } else this.pokemonGenderText.setText("");
      ...
      if (dexEntry.caughtAttr) {
        ...
        this.pokemonAbilityText.setColor(
          this.getTextColor(
            !isHidden ? TextStyle.SUMMARY_ALT : TextStyle.SUMMARY_GOLD,
          ),
        );
        ...
        const passiveAttr =
          this.scene.gameData.starterData[species.speciesId].passiveAttr;
        ...
        this.pokemonNatureText.setText(
          getNatureName(
            natureIndex as unknown as Nature,
            true,
            true,
            false,
            this.scene.uiTheme,
        ...
        else levelMoves = pokemonSpeciesLevelMoves[species.speciesId];
        this.speciesStarterMoves.push(
          ...levelMoves.filter((lm) => lm[0] <= 5).map((lm) => lm[1]),
        );
        if (speciesEggMoves.hasOwnProperty(species.speciesId)) {
          for (let em = 0; em < 4; em++) {
            if (
              this.scene.gameData.starterData[species.speciesId].eggMoves &
              Math.pow(2, em)
            )
              this.speciesStarterMoves.push(
                speciesEggMoves[species.speciesId][em],
              );
          }
        }

        const speciesMoveData =
          this.scene.gameData.starterData[species.speciesId].moveset;
        ...
        for (let i = 0; i < formText?.length; i++)
          formText[i] =
            formText[i].charAt(0).toUpperCase() + formText[i].substring(1);
      ...
      this.pokemonNumberText.setShadowColor(
        this.getTextColor(TextStyle.SUMMARY, true),
      );
    ...
    if (!this.starterMoveset)
      this.starterMoveset = this.speciesStarterMoves.slice(
        0,
        4,
      ) as StarterMoveset;
    ...
    for (let m = 0; m < 4; m++) {
      const move =
        m < this.starterMoveset.length
          ? allMoves[this.starterMoveset[m]]
          : null;
    ...
    const hasEggMoves =
      species && speciesEggMoves.hasOwnProperty(species.speciesId);

    for (let em = 0; em < 4; em++) {
      const eggMove = hasEggMoves
        ? allMoves[speciesEggMoves[species.speciesId][em]]
        : null;
      ...
      this.pokemonEggMoveLabels[em].setText(
        eggMove && eggMoveUnlocked ? eggMove.name : "???",
      );
    }

    this.pokemonEggMovesContainer.setVisible(
      this.speciesStarterDexEntry?.caughtAttr && hasEggMoves,
    );

    this.pokemonAdditionalMoveCountLabel.setText(
      `(+${Math.max(this.speciesStarterMoves.length - 4, 0)})`,
    );
    this.pokemonAdditionalMoveCountLabel.setVisible(
      this.speciesStarterMoves.length > 4,
    );
  ...
  setTypeIcons(type1: Type, type2: Type): void {
    ...
    } else this.type1Icon.setVisible(false);
    if (type2 !== null) {
      this.type2Icon.setVisible(true);
      this.type2Icon.setFrame(Type[type2].toLowerCase());
    } else this.type2Icon.setVisible(false);
  }
  ...
  updateStarterValueLabel(cursor: integer): void {
    const speciesId =
      this.genSpecies[this.getGenCursorWithScroll()][cursor].speciesId;
    ...
    if (valueStr.startsWith("0.")) valueStr = valueStr.slice(1);
    ...
    this.starterValueLabels[cursor].setShadowColor(
  ...
  tryUpdateValue(add?: integer): boolean {
    ...
    if (newValueStr.startsWith("0.")) newValueStr = newValueStr.slice(1);
    this.valueLimitLabel.setText(`${newValueStr}/${valueLimit}`);
    this.valueLimitLabel.setColor(
      this.getTextColor(
        !overLimit ? TextStyle.TOOLTIP_CONTENT : TextStyle.SUMMARY_PINK,
      ),
    );
    ...
    for (let g = 0; g < this.genSpecies.length; g++) {
      for (let s = 0; s < this.genSpecies[g].length; s++)
        (
          this.starterSelectGenIconContainers[g].getAt(
            s,
          ) as Phaser.GameObjects.Sprite
        ).setAlpha(
          newValue +
  ...
  tryStart(manualTrigger = false): boolean {
    if (!this.starterGens.length) return false;

    const ui = this.getUi();

    const cancel = () => {
      ui.setMode(Mode.STARTER_SELECT);
      if (!manualTrigger) this.popStarter();
      this.clearText();
    };
          ...
          () => {
            const startRun = (gameMode: GameModes) => {
              ...
              originalStarterSelectCallback(
                new Array(this.starterGens.length).fill(0).map((_, i) => {
                  const starterSpecies =
                    thisObj.genSpecies[thisObj.starterGens[i]][
                      thisObj.starterCursors[i]
                    ];
                  return {
                    ...
                    passive: !(
                      thisObj.scene.gameData.starterData[
                    ...
                    pokerus: !![0, 1, 2].filter(
                      (n) =>
                        thisObj.pokerusGens[n] ===
                          starterSpecies.generation - 1 &&
                        thisObj.pokerusCursors[n] ===
                          thisObj.genSpecies[
                            starterSpecies.generation - 1
                          ].indexOf(starterSpecies),
                    ).length,
                  };
                }),
              );
  ...
  toggleStatsMode(on?: boolean): void {
    if (on === undefined) on = !this.statsMode;
    if (on) {
      this.showStats();
      this.statsMode = true;
      this.pokemonSprite.setVisible(false);
  ...
  showStats(): void {
    if (!this.speciesStarterDexEntry) return;
  ...
  clear(): void {
    ...
    while (this.starterCursors.length) this.popStarter();

    if (this.statsMode) this.toggleStatsMode(false);
  }
}

...
export default class VouchersUiHandler extends MessageUiHandler {
  ...
  setup() {
    ...
    this.vouchersContainer = this.scene.add.container(
      1,
    ...
    this.voucherIconsContainer = this.scene.add.container(
      6,
      headerBg.height + 6,
    );
    ...
    const titleBg = addWindow(
      this.scene,
      0,
      headerBg.height + this.voucherIconsBg.height,
      220,
      24,
    );
    ...
    if (button === Button.CANCEL) {
      ...
      switch (button) {
        case Button.UP:
          ...
          } else success = this.setCursor(this.cursor - itemCols);
          break;
        case Button.DOWN:
          const canMoveDown =
            this.cursor + itemOffset + itemCols < this.itemsTotal;
          if (rowIndex >= itemRows - 1) {
            if (
              this.scrollCursor <
                Math.ceil(this.itemsTotal / itemCols) - itemRows &&
              canMoveDown
            )
              success = this.setScrollCursor(this.scrollCursor + 1);
          } else if (canMoveDown)
        ...
        case Button.RIGHT:
          if (
            ...
            this.cursor + itemOffset <
            Object.keys(vouchers).length - 1
          )
  ...
  setCursor(cursor: integer): boolean {
    ...
    if (updateVoucher)
      this.showVoucher(
        vouchers[Object.keys(vouchers)[cursor + this.scrollCursor * itemCols]],
      );
    ...
    this.showVoucher(
      vouchers[
        Object.keys(vouchers)[
          Math.min(
            this.cursor + this.scrollCursor * itemCols,
            Object.values(vouchers).length - 1,
          )
        ]
      ],
    );
  ...
  updateVoucherIcons(): void {
    ...
    const voucherRange = Object.values(vouchers).slice(
      itemOffset,
      itemLimit + itemOffset,
    );
    ...
    voucherRange.forEach((voucher: Voucher, i: integer) => {
      ...
      if (!unlocked) icon.setTintFill(0);
      else icon.clearTint();
    });

    if (voucherRange.length < this.voucherIcons.length)
      this.voucherIcons
        .slice(voucherRange.length)

...
import {
  ...
  Egg,
  GachaType,
  getEggTierDefaultHatchWaves,
  getEggDescriptor,
  getLegendaryGachaSpeciesForTimestamp,
} from "../data/egg";
...
export default class EggGachaUiHandler extends MessageUiHandler {
  ...
  setup() {
    ...
    this.eggGachaContainer = this.scene.add.container(
      0,
    ...
    const hatchFrameNames = this.scene.anims.generateFrameNames("gacha_hatch", {
      suffix: ".png",
      start: 1,
      end: 4,
    });
    ...
    Utils.getEnumValues(GachaType).forEach((gachaType, g) => {
      ...
      const gachaUnderlay = this.scene.add.sprite(
        115,
        80,
        `gacha_underlay_${gachaTypeKey}`,
      );
      ...
      const gachaUpLabel = addTextObject(
        this.scene,
        4,
        0,
        "UP!",
      ...
      gachaHatch.on("animationupdate", (_anim, frame) =>
        gachaUnderlay.setFrame(
          frame.textureFrame === "4.png" ? "open_hatch" : "default",
        ),
      );
    ...
    new Array(Utils.getEnumKeys(VoucherType).length).fill(null).map((_, i) => {
      const container = this.scene.add.container(
        this.scene.game.canvas.width / 6 - 56 * i,
        0,
      );
      ...
      const countLabel = addTextObject(
        this.scene,
        -48,
        3,
        "0",
    ...
    this.eggGachaOverlay = this.scene.add.rectangle(
      0,
      0,
      bg.displayWidth,
      bg.displayHeight,
    ...
    const gachaMessageText = addTextObject(
      this.scene,
      8,
      8,
      "",
  ...
  pull(pullCount?: integer, count?: integer, eggs?: Egg[]): void {
    ...
    const doPull = () => {
      if (this.transitionCancelled) return this.showSummary(eggs);

      const egg = this.scene.add.sprite(
        127,
        75,
        "egg",
        `egg_${eggs[count].getKey()}`,
      );
      egg.setScale(0.5);
      ...
      const doPullAnim = () => {
        ...
        this.scene.time.delayedCall(
          this.getDelayValue(count ? 500 : 1250),
          () => {
    ...
    if (!eggs) {
      ...
      const tiers = new Array(pullCount).fill(null).map(() => {
        ...
        return tierValue >= 52 + tierValueOffset
          ? EggTier.COMMON
          : tierValue >= 8 + tierValueOffset
            ? EggTier.GREAT
            : tierValue >= 1 + tierValueOffset
              ? EggTier.ULTRA
              : EggTier.MASTER;
      });
      if (pullCount >= 25 && !tiers.filter((t) => t >= EggTier.ULTRA).length)
        ...
        pullCount >= 10 &&
        !tiers.filter((t) => t >= EggTier.GREAT).length
      )
      ...
      for (const tier of tiers) {
        const egg = new Egg(
          Utils.randInt(EGG_SEED, EGG_SEED * tier),
          this.gachaCursor,
          getEggTierDefaultHatchWaves(tier),
          timestamp,
        );
        if (egg.isManaphyEgg()) {
  ...
  showSummary(eggs: Egg[]): void {
    ...
    this.scene.tweens.add({
      ...
      onComplete: () => {
        ...
        const height =
          this.eggGachaOverlay.displayHeight -
          this.eggGachaMessageBox.displayHeight;
        const eggContainers = eggs.map((egg, t) => {
          ...
          const yOffset =
            (sliceHeight / 2) * (row / Math.max(rows - 1, 1)) + sliceHeight / 4;
          ...
          const eggSprite = this.scene.add.sprite(
            0,
            0,
            "egg",
            `egg_${egg.getKey()}`,
          ...
          const eggText = addTextObject(
            this.scene,
            0,
            14,
            getEggDescriptor(egg),
            TextStyle.PARTY,
            { align: "center" },
          );
          eggText.setOrigin(0.5, 0);
          eggText.setTint(
            getEggTierTextTint(!egg.isManaphyEgg() ? egg.tier : EggTier.ULTRA),
          );
          ret.add(eggText);
  ...
  updateGachaInfo(gachaType: GachaType): void {
    ...
    switch (gachaType as GachaType) {
      case GachaType.LEGENDARY:
        ...
        pokemonIcon.setTexture(
          species.getIconAtlasKey(),
          species.getIconId(false),
        );
        break;
    }
  }
  ...
  setTransitioning(transitioning: boolean): void {
    if (this.transitioning === transitioning) return;
      ...
      if (this.eggGachaSummaryContainer.visible) {
        ...
        switch (button) {
            ...
            switch (this.cursor) {
              ...
              case 3:
                if (
                  ...
                  else this.consumeVouchers(VoucherType.REGULAR, 10);
                  this.pull(10);
                  success = true;
                } else {
                  error = true;
          ...
          case Button.UP:
            ...
            break;
          case Button.DOWN:
            if (this.cursor < 5) success = this.setCursor(this.cursor + 1);
            break;
          case Button.LEFT:
            if (this.gachaCursor)

...
export default class SummaryUiHandler extends UiHandler {
  ...
  setup() {
    ...
    this.tabSprite = this.scene.add.sprite(
      134,
      -summaryBg.displayHeight + 16,
      "summary_tabs_1",
    );
    this.tabSprite.setOrigin(1, 1);
    this.summaryContainer.add(this.tabSprite);

    const summaryLabel = addTextObject(
      this.scene,
      4,
      -165,
      "Pokémon Info",
    ...
    this.splicedIcon.setInteractive(
      new Phaser.Geom.Rectangle(0, 0, 12, 15),
    ...
    this.candyShadow.setInteractive(
    ...
    this.movePowerText = addTextObject(
      this.scene,
      99,
      27,
      "0",
    ...
    this.moveAccuracyText = addTextObject(
      this.scene,
      99,
      43,
      "0",
  ...
  getPageKey(page?: integer) {
    if (page === undefined) page = this.cursor;
  ...
  show(args: any[]): boolean {
    ...
    this.summaryUiMode =
      args.length > 1 ? (args[1] as SummaryUiMode) : SummaryUiMode.DEFAULT;
    ...
    this.numberText.setColor(
      this.getTextColor(
        !this.pokemon.isShiny() ? TextStyle.SUMMARY : TextStyle.SUMMARY_GOLD,
      ),
    );
    ...
    this.pokemonSprite.setPipelineData(
      "teraColor",
      getTypeRgb(this.pokemon.getTeraType()),
    );
    ...
    ["spriteColors", "fusionSpriteColors"].map((k) => {
      ...
      if (this.pokemon.summonData?.speciesForm) k += "Base";
      this.pokemonSprite.pipelineData[k] =
    ...
    if (this.splicedIcon.visible) {
      this.splicedIcon.on("pointerover", () =>
        (this.scene as BattleScene).ui.showTooltip(
          null,
          `${this.pokemon.species.getName(
            this.pokemon.formIndex,
          )}/${this.pokemon.fusionSpecies.getName(
            this.pokemon.fusionFormIndex,
          )}`,
          true,
        ),
      );
      this.splicedIcon.on("pointerout", () =>
        (this.scene as BattleScene).ui.hideTooltip(),
      );
    }

    var currentFriendship =
      this.scene.gameData.starterData[this.pokemon.species.getRootSpeciesId()]
        .friendship;
    if (!currentFriendship || currentFriendship === undefined)
      currentFriendship = 0;

    const friendshipCap = getStarterValueFriendshipCap(
      speciesStarters[this.pokemon.species.getRootSpeciesId()],
    );
    ...
    if (this.candyShadow.visible) {
      this.candyShadow.on("pointerover", () =>
        (this.scene as BattleScene).ui.showTooltip(
          null,
          `${currentFriendship}/${friendshipCap}`,
          true,
        ),
      );
      this.candyShadow.on("pointerout", () =>
        (this.scene as BattleScene).ui.hideTooltip(),
      );
    }
    ...
    const doubleShiny =
      isFusion && this.pokemon.shiny && this.pokemon.fusionShiny;
    const baseVariant = !doubleShiny
      ? this.pokemon.getVariant()
    ...
    if (this.shinyIcon.visible) {
      const shinyDescriptor =
        doubleShiny || baseVariant
          ? `${
              baseVariant === 2 ? "Epic" : baseVariant === 1 ? "Rare" : "Common"
            }${
              doubleShiny
                ? `/${
                    this.pokemon.fusionVariant === 2
                      ? "Epic"
                      : this.pokemon.fusionVariant === 1
                        ? "Rare"
                        : "Common"
                  }`
                : ""
      ...
      this.shinyIcon.on("pointerover", () =>
        (this.scene as BattleScene).ui.showTooltip(
          null,
          `Shiny${shinyDescriptor ? ` (${shinyDescriptor})` : ""}`,
          true,
        ),
      );
      this.shinyIcon.on("pointerout", () =>
        (this.scene as BattleScene).ui.hideTooltip(),
      );
    }
    ...
    this.genderText.setShadowColor(
      getGenderColor(this.pokemon.getGender(true), true),
    );
    ...
    } else this.hideStatus(!fromSummary);
  ...
  processInput(button: Button): boolean {
    ...
    if (this.moveSelect) {
          ...
          if (this.summaryUiMode === SummaryUiMode.LEARN_MOVE)
            ...
            if (this.selectedMoveIndex === -1) {
              ...
              if (this.selectedMoveIndex !== this.moveCursor) {
                ...
                this.pokemon.moveset[this.selectedMoveIndex] =
                  this.pokemon.moveset[this.moveCursor];
                ...
                const switchMoveRow = this.moveRowsContainer.getAt(
                  this.moveCursor,
                ) as Phaser.GameObjects.Container;

                this.moveRowsContainer.moveTo(selectedMoveRow, this.moveCursor);
                this.moveRowsContainer.moveTo(
                  switchMoveRow,
                  this.selectedMoveIndex,
                );
        ...
        switch (button) {
          ...
          case Button.DOWN:
            success = this.setCursor(
              this.moveCursor < 4 ? this.moveCursor + 1 : 0,
            );
            break;
          case Button.LEFT:
      ...
      if (button === Button.ACTION) {
        ...
        switch (button) {
          case Button.UP:
          case Button.DOWN:
            if (this.summaryUiMode === SummaryUiMode.LEARN_MOVE) break;
            const isDown = button === Button.DOWN;
            const party = this.scene.getParty();
            const partyMemberIndex = party.indexOf(this.pokemon);
            if (
              (isDown && partyMemberIndex < party.length - 1) ||
              (!isDown && partyMemberIndex)
            ) {
              const page = this.cursor;
              this.clear();
              this.show([
                party[partyMemberIndex + (isDown ? 1 : -1)],
                this.summaryUiMode,
                page,
              ]);
            }
    ...
    else if (error) ui.playError();

    return success || error;
  }

  setCursor(cursor: integer, overrideChanged = false): boolean {
    ...
    if (this.moveSelect) {
      ...
      if (selectedMove) {
        ...
        this.movePowerText.setText(
          selectedMove.power >= 0 ? selectedMove.power.toString() : "---",
        );
        this.moveAccuracyText.setText(
          selectedMove.accuracy >= 0 ? selectedMove.accuracy.toString() : "---",
        );
        this.moveCategoryIcon.setFrame(
          MoveCategory[selectedMove.category].toLowerCase(),
        );
        this.showMoveEffect();
      } else this.hideMoveEffect();

      this.moveDescriptionText.setText(selectedMove?.effect || "");
      const moveDescriptionLineCount = Math.floor(
        this.moveDescriptionText.displayHeight / 14.83,
      );
      ...
      if (!this.moveCursorObj) {
        this.moveCursorObj = this.scene.add.sprite(
          -2,
          0,
      ...
      if (this.moveCursorBlinkTimer) this.moveCursorBlinkTimer.destroy();
      ...
      if (this.selectedMoveIndex > -1) {
        if (!this.selectedMoveCursorObj) {
          this.selectedMoveCursorObj = this.scene.add.sprite(
            -2,
            0,
            "summary_moves_cursor",
            "select",
          ...
          this.movesContainer.moveBelow(
            this.selectedMoveCursorObj,
            this.moveCursorObj,
          );
        }
      ...
      if (changed) {
        ...
        if (this.summaryPageContainer.visible) {
          ...
          if (forward) this.summaryPageTransitionContainer.x += 214;
          else this.populatePageContainer(this.summaryPageContainer);
          this.scene.tweens.add({
            ...
            onComplete: () => {
              ...
              } else this.summaryPageTransitionContainer.x -= 214;
              this.summaryPageTransitionContainer.setVisible(false);
  ...
  populatePageContainer(
    ...
    if (pageContainer.getAll().length > 1) {
      pageContainer.each((o: Phaser.GameObjects.GameObject) => {
        if (o instanceof Phaser.GameObjects.Container) o.removeAll(true);
      });
    ...
    switch (page) {
      case Page.PROFILE:
        ...
        const trainerLabel = addTextObject(
          this.scene,
          7,
          12,
          "OT/",
        ...
        const trainerText = addTextObject(
          this.scene,
          25,
          12,
        ...
        const getTypeIcon = (
          ...
          if (tera) {
            ...
            typeIcon.setTint(
              Phaser.Display.Color.GetColor(typeRgb[0], typeRgb[1], typeRgb[2]),
            );
          }
        ...
        if (types.length > 1) profileContainer.add(getTypeIcon(1, types[1]));
        if (this.pokemon.isTerastallized())
          profileContainer.add(
            getTypeIcon(types.length, this.pokemon.getTeraType(), true),
          );
        ...
        if (this.pokemon.getLuck()) {
          ...
          luckText.setTint(
            getVariantTint(Math.min(this.pokemon.getLuck() - 1, 2) as Variant),
          );
        ...
        const abilityNameText = addTextObject(
          this.scene,
          7,
          66,
          ability.name,
        ...
        const abilityDescriptionText = addTextObject(
          this.scene,
          7,
          69,
          ability.description,
        ...
        const abilityDescriptionTextMask =
          abilityDescriptionTextMaskRect.createGeometryMask();

        abilityDescriptionText.setMask(abilityDescriptionTextMask);

        const abilityDescriptionLineCount = Math.floor(
          abilityDescriptionText.displayHeight / 14.83,
        );
        ...
        const memoString = `${getBBCodeFrag(
          Utils.toReadableString(Nature[this.pokemon.getNature()]),
          TextStyle.SUMMARY_RED,
        )}${getBBCodeFrag(" nature,", TextStyle.WINDOW_ALT)}\n${getBBCodeFrag(
          `${this.pokemon.metBiome === -1 ? "apparently " : ""}met at Lv`,
          TextStyle.WINDOW_ALT,
        )}${getBBCodeFrag(
          this.pokemon.metLevel.toString(),
          TextStyle.SUMMARY_RED,
        )}${getBBCodeFrag(",", TextStyle.WINDOW_ALT)}\n${getBBCodeFrag(
          getBiomeName(this.pokemon.metBiome),
          TextStyle.SUMMARY_RED,
        )}${getBBCodeFrag(".", TextStyle.WINDOW_ALT)}`;

        const memoText = addBBCodeTextObject(
          this.scene,
          7,
          113,
          memoString,
      ...
      case Page.STATS:
        ...
        stats.forEach((stat, s) => {
          const statName = stat !== Stat.HP ? getStatName(stat) : "HP";
          const rowIndex = s % 3;
          const colIndex = Math.floor(s / 3);

          const natureStatMultiplier = getNatureStatMultiplier(
            this.pokemon.getNature(),
            s,
          );
          ...
          const statValueText =
            stat !== Stat.HP
              ...
              : `${Utils.formatStat(this.pokemon.hp, true)}/${Utils.formatStat(
        ...
        const itemModifiers = this.scene.findModifiers(
          (m) =>
            ...
            (m as PokemonHeldItemModifier).pokemonId === this.pokemon.id,
          true,
        ...
        itemModifiers.forEach((item, i) => {
          ...
          icon.on("pointerover", () =>
            (this.scene as BattleScene).ui.showTooltip(
              item.type.name,
              item.type.getDescription(this.scene),
              true,
            ),
          );
          icon.on("pointerout", () =>
        ...
        const relLvExp = getLevelRelExp(
          this.pokemon.level + 1,
          this.pokemon.species.growthRate,
        );
        const expRatio =
          this.pokemon.level < this.scene.getMaxExpLevel()
            ? this.pokemon.levelExp / relLvExp
            : 0;
        ...
        const nextLvExpLabel = addTextObject(
          this.scene,
          6,
          128,
          "Next Lv.",
        ...
        const expText = addTextObject(
          this.scene,
          208,
          112,
          this.pokemon.exp.toString(),
          TextStyle.WINDOW_ALT,
        );
        ...
        const nextLvExp =
          this.pokemon.level < this.scene.getMaxExpLevel()
            ...
            : 0;
        const nextLvExpText = addTextObject(
          this.scene,
          208,
          128,
          nextLvExp.toString(),
        ...
        const expOverlay = this.scene.add.image(
          140,
          145,
          "summary_stats_overlay_exp",
        );
      ...
      case Page.MOVES:
        ...
        const extraRowOverlay = this.scene.add.image(
          -2,
          1,
          "summary_moves_overlay_row",
        );
        ...
        if (this.summaryUiMode === SummaryUiMode.LEARN_MOVE) {
          this.extraMoveRowContainer.setVisible(true);
          const newMoveTypeIcon = this.scene.add.sprite(
            0,
            0,
            "types",
            Type[this.newMove.type].toLowerCase(),
          );
          newMoveTypeIcon.setOrigin(0, 1);
          this.extraMoveRowContainer.add(newMoveTypeIcon);

          const ppOverlay = this.scene.add.image(
            163,
            -1,
            "summary_moves_overlay_pp",
          );
          ppOverlay.setOrigin(0, 1);
          this.extraMoveRowContainer.add(ppOverlay);

          const pp = Utils.padInt(this.newMove.pp, 2, "  ");
          const ppText = addTextObject(
            this.scene,
            173,
            1,
            `${pp}/${pp}`,
        ...
        for (let m = 0; m < 4; m++) {
          const move =
            m < this.pokemon.moveset.length ? this.pokemon.moveset[m] : null;
          const moveRowContainer = this.scene.add.container(0, 16 * m);
          this.moveRowsContainer.add(moveRowContainer);

          if (move) {
            const typeIcon = this.scene.add.sprite(
              0,
              0,
              "types",
              Type[move.getMove().type].toLowerCase(),
            );
            typeIcon.setOrigin(0, 1);
          ...
          if (move) {
            ...
            ppText.setText(
              `${Utils.padInt(pp, 2, "  ")}/${Utils.padInt(maxPP, 2, "  ")}`,
            );
          }
        ...
        this.moveDescriptionText = addTextObject(
          this.scene,
          2,
          84,
          "",
        ...
        const moveDescriptionTextMask =
          moveDescriptionTextMaskRect.createGeometryMask();
  ...
  getSelectedMove(): Move {
    if (this.cursor !== Page.MOVES) return null;
  ...
  showMoveEffect(instant?: boolean) {
    if (this.moveEffectsVisible) return;

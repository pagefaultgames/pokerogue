...
import PokemonIconAnimHandler, {
  PokemonIconAnimMode,
} from "./pokemon-icon-anim-handler";
...
export type PartySelectCallback = (
  cursor: integer,
  option: PartyOption,
) => void;
...
export type PartyModifierSpliceSelectCallback = (
  fromCursor: integer,
  toCursor?: integer,
) => void;
...
export default class PartyUiHandler extends MessageUiHandler {
  ...
  public static FilterItemMaxStacks = (
    ...
    if (
      matchingModifier &&
      matchingModifier.stackCount ===
        matchingModifier.getMaxStackCount(pokemon.scene)
    )
  ...
  setup() {
    ...
    const partyMessageText = addTextObject(
      this.scene,
      8,
      10,
      defaultMessage,
  ...
  show(args: any[]): boolean {
    if (!args.length || this.active) return false;
    ...
    this.selectCallback =
      args.length > 2 && args[2] instanceof Function ? args[2] : undefined;
    this.selectFilter =
    ...
    this.partyBg.setTexture(
      `party_bg${this.scene.currentBattle.double ? "_double" : ""}`,
    );
    ...
    if (this.optionsMode) {
      if (button === Button.ACTION) {
        ...
        if (
          ...
          const filterResult = (this.selectFilter as PokemonSelectFilter)(
            pokemon,
          );
          ...
          (option !== PartyOption.SUMMARY &&
            ...
            option !== PartyOption.CANCEL) ||
          (option === PartyOption.RELEASE &&
            this.partyUiMode === PartyUiMode.RELEASE)
        ) {
          let filterResult: string;
          if (
            option !== PartyOption.TRANSFER &&
            option !== PartyOption.SPLICE
          ) {
            ...
            if (
              ...
              filterResult = this.moveSelectFilter(
                pokemon.moveset[this.optionsCursor],
              );
          } else {
            ...
            const itemModifiers = this.scene.findModifiers(
              (m) =>
                ...
                (m as PokemonHeldItemModifier).pokemonId === transferPokemon.id,
            ) as PokemonHeldItemModifier[];
            filterResult = (
              this.selectFilter as PokemonModifierTransferSelectFilter
            )(pokemon, itemModifiers[this.transferOptionCursor]);
          }
          if (filterResult === null) {
            if (this.partyUiMode !== PartyUiMode.SPLICE) this.clearOptions();
            if (this.selectCallback) {
              ...
              if (
                ...
                this.scene.getCurrentPhase() instanceof CommandPhase
              ) {
          ...
          ui.playSelect();
          ui.setModeWithoutClear(Mode.SUMMARY, pokemon).then(() =>
            this.clearOptions(),
          );
          return true;
          ...
          if (this.cursor >= this.scene.currentBattle.getBattlerCount()) {
            this.showText(
              `Do you really want to release ${pokemon.name}?`,
              null,
              () => {
                ui.setModeWithoutClear(
                  Mode.CONFIRM,
        ...
        switch (button) {
          ...
          case Button.DOWN:
            success = this.setCursor(
              this.optionsCursor < this.options.length - 1
                ? this.optionsCursor + 1
                : 0,
            );
            break;
      ...
      if (button === Button.ACTION) {
        if (this.cursor < 6) {
          ...
          ui.playError();
        else return this.processInput(Button.CANCEL);
        return true;
      } else if (button === Button.CANCEL) {
        if (
          (this.partyUiMode === PartyUiMode.MODIFIER_TRANSFER ||
  ...
  populatePartySlots() {
    ...
    else if (this.cursor === 6) this.partyCancelButton.select();
    ...
    for (const p in party) {
      ...
      if (this.cursor === slotIndex) partySlot.select();
    }
  }
  ...
  setCursor(cursor: integer): boolean {
    ...
    if (this.optionsMode) {
      ...
      if (isScroll) this.updateOptions();
      else this.optionsCursor = cursor;
      ...
      if (changed) {
        ...
        this.cursor = cursor;
        if (this.lastCursor < 6) this.partySlots[this.lastCursor].deselect();
        else if (this.lastCursor === 6) this.partyCancelButton.deselect();
        if (cursor < 6) this.partySlots[cursor].select();
        else if (cursor === 6) this.partyCancelButton.select();
      }
    }
  ...
  showOptions() {
    ...
    this.partyMessageBox.setSize(
      262 - Math.max(this.optionsBg.displayWidth - 56, 0),
      30,
    );
  ...
  updateOptions(): void {
    ...
    if (
      ...
      switch (this.partyUiMode) {
            ...
            if (
              ...
              this.scene.findModifier(
                (m) =>
                  ...
                  (m as SwitchEffectTransferModifier).pokemonId ===
                    this.scene.getPlayerField()[this.fieldIndex].id,
              )
            )
        ...
        case PartyUiMode.SPLICE:
          ...
          } else this.options.push(PartyOption.APPLY);
          break;
      ...
      for (let m = 0; m < learnableMoves.length; m++) this.options.push(m);
    } else {
      for (let im = 0; im < itemModifiers.length; im++) this.options.push(im);
    }
    ...
    let optionEndIndex = Math.min(
      ...
      optionStartIndex +
        (!optionStartIndex ||
        this.optionsScrollCursor + 8 >= this.optionsScrollTotal
          ? 8
          : 7),
    );
    ...
    if (this.optionsScroll) {
      ...
      if (optionStartIndex) this.options.unshift(PartyOption.SCROLL_UP);
    ...
    for (let o = optionStartIndex; o < optionEndIndex; o++) {
      ...
      if (option === PartyOption.SCROLL_UP) optionName = "↑";
      else if (option === PartyOption.SCROLL_DOWN) optionName = "↓";
      else if (
        (this.partyUiMode !== PartyUiMode.REMEMBER_MOVE_MODIFIER &&
          (this.partyUiMode !== PartyUiMode.MODIFIER_TRANSFER ||
            this.transferMode)) ||
        ...
        switch (option) {
          ...
          default:
            if (
              ...
              const modifier =
                formChangeItemModifiers[option - PartyOption.FORM_CHANGE_ITEM];
              optionName = `${modifier.active ? "Deactivate" : "Activate"} ${
                modifier.type.name
              }`;
            } else optionName = Utils.toReadableString(PartyOption[option]);
            break;
        }
      } else if (this.partyUiMode === PartyUiMode.REMEMBER_MOVE_MODIFIER) {
  ...
  getReleaseMessage(pokemonName: string): string {
    ...
    if (rand < 20) return `Goodbye, ${pokemonName}!`;
    else if (rand < 40) return `Byebye, ${pokemonName}!`;
    else if (rand < 60) return `Farewell, ${pokemonName}!`;
    else if (rand < 80) return `So long, ${pokemonName}!`;
    else if (rand < 100) return `This is where we part, ${pokemonName}!`;
    else if (rand < 108) return `I'll miss you, ${pokemonName}!`;
    else if (rand < 116) return `I'll never forget you, ${pokemonName}!`;
    else if (rand < 124) return `Until we meet again, ${pokemonName}!`;
    else if (rand < 127) return `Sayonara, ${pokemonName}!`;
    else return `Smell ya later, ${pokemonName}!`;
  }
  ...
  eraseOptionsCursor() {
    if (this.optionsCursorObj) this.optionsCursorObj.destroy();
...
class PartySlot extends Phaser.GameObjects.Container {
  ...
  setup(partyUiMode: PartyUiMode, tmMoveId: Moves) {
    ...
    ).currentBattle.getBattlerCount();

    const slotKey = `party_slot${
      this.slotIndex >= battlerCount ? "" : "_main"
    }`;
    ...
    const nameSizeTest = addTextObject(
      this.scene,
      0,
      0,
      displayName,
    ...
    while (
      nameTextWidth >
      (this.slotIndex >= battlerCount
        ? 52
        : 76 - (this.pokemon.fusionSpecies ? 8 : 0))
    ) {
      displayName = `${displayName
        .slice(0, displayName.endsWith(".") ? -2 : -1)
        .trimEnd()}.`;
    ...
    if (genderSymbol) {
      const slotGenderText = addTextObject(
        this.scene,
        0,
        0,
        genderSymbol,
      ...
      else slotGenderText.setPositionRelative(slotName, 76, 3);
    ...
    if (this.pokemon.fusionSpecies) {
      ...
      if (this.slotIndex >= battlerCount)
        splicedIcon.setPositionRelative(
          slotLevelLabel,
          36 - (genderSymbol ? 8 : 0),
          0.5,
        );
    ...
    if (this.pokemon.isShiny()) {
      const doubleShiny =
        this.pokemon.isFusion() &&
        this.pokemon.shiny &&
        this.pokemon.fusionShiny;

      const shinyStar = this.scene.add.image(
        0,
        0,
        `shiny_star_small${doubleShiny ? "_1" : ""}`,
      );
      shinyStar.setOrigin(0, 0);
      shinyStar.setPositionRelative(slotName, -9, 3);
      shinyStar.setTint(
        getVariantTint(
          !doubleShiny ? this.pokemon.getVariant() : this.pokemon.variant,
        ),
      );
      ...
      if (doubleShiny) {
        const fusionShinyStar = this.scene.add.image(
          0,
          0,
          `shiny_star_small_2`,
        );
    ...
    if (partyUiMode !== PartyUiMode.TM_MODIFIER) {
      ...
      switch (true) {
        case this.pokemon.compatibleTms.indexOf(tmMoveId) === -1:
          slotTmText = "Not Able";
          break;
        case this.pokemon.getMoveset().filter((m) => m?.moveId === tmMoveId)
          .length > 0:
          slotTmText = "Learned";
          break;
        default:
      ...
      const slotTmLabel = addTextObject(
        this.scene,
        0,
        0,
        slotTmText,
  ...
  private updateSlotTexture(): void {
    ...
    this.slotBg.setTexture(
      ...
      `party_slot${this.slotIndex >= battlerCount ? "" : "_main"}${
        this.transfer ? "_swap" : this.pokemon.hp ? "" : "_fnt"
      }${this.selected ? "_sel" : ""}`,
    );
  }
}

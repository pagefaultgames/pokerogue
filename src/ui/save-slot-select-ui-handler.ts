...
class SessionSlot extends Phaser.GameObjects.Container {
  ...
  async setupWithData(data: SessionSaveData) {
    ...
    const gameModeLabel = addTextObject(this.scene, 8, 5, `${gameModes[data.gameMode]?.getName() || i18next.t("gameMode:unknown")} - ${i18next.t("saveSlotSelectUiHandler:wave")} ${data.waveIndex}`, TextStyle.WINDOW);

...
export default class SaveSlotSelectUiHandler extends MessageUiHandler {
  ...
  setup() {
    ...
    this.sessionSlotsContainer = this.scene.add.container(
      8,
      this.sessionSlotsContainerInitialY,
    );
  ...
  show(args: any[]): boolean {
    if (args.length < 2 || !(args[1] instanceof Function)) return false;
  ...
  processInput(button: Button): boolean {
    ...
    if (button === Button.ACTION || button === Button.CANCEL) {
      ...
      if (button === Button.ACTION) {
        ...
        if (
          this.uiMode === SaveSlotUiMode.LOAD &&
          !this.sessionSlots[cursor].hasData
        )
          error = true;
        else {
          switch (this.uiMode) {
            ...
            case SaveSlotUiMode.SAVE:
              ...
              if (this.sessionSlots[cursor].hasData) {
                ui.showText(
                  "Overwrite the data in the selected slot?",
                  null,
                  () => {
                    ui.setOverlayMode(
                      Mode.CONFIRM,
      ...
      switch (button) {
        case Button.UP:
          ...
          break;
        case Button.DOWN:
          if (this.cursor < 2) success = this.setCursor(this.cursor + 1);
...
class SessionSlot extends Phaser.GameObjects.Container {
  ...
  async setupWithData(data: SessionSaveData) {
    ...
    const gameModeLabel = addTextObject(
      ...
      8,
      5,
      `${gameModes[data.gameMode]?.getName() || "Unknown"} - Wave ${
        data.waveIndex
      }`,
      TextStyle.WINDOW,
    );
    ...
    data.party.forEach((p: PokemonData, i: integer) => {
      ...
      const text = addTextObject(
        ...
        32,
        20,
        `Lv${Utils.formatLargeNumber(pokemon.level, 1000)}`,
        TextStyle.PARTY,

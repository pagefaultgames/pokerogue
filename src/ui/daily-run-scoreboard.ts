...
export class DailyRunScoreboard extends Phaser.GameObjects.Container {
  ...
  setup() {
    ...
    this.prevCategoryButton.on("pointerup", () => {
      this.update(
        this.category
          ? this.category - 1
          : Utils.getEnumKeys(ScoreboardCategory).length - 1,
      );
    });

    this.nextCategoryButton = this.scene.add.sprite(
    ...
    this.nextCategoryButton.on("pointerup", () => {
      this.update(
        this.category < Utils.getEnumKeys(ScoreboardCategory).length - 1
          ? this.category + 1
          : 0,
      );
    });
  ...
  updateRankings(rankings: RankingEntry[]) {
    const getEntry = (
      ...
      const usernameLabel = addTextObject(
        this.scene,
        12,
        0,
        username,
      ...
      const scoreLabel = addTextObject(
        this.scene,
        84,
        0,
        score,
  ...
  update(
    category: ScoreboardCategory = this.category,
    ...
    if (category !== this.category) this.page = page = 1;

    Utils.executeIf(
      category !== this.category || this.pageCount === undefined,
      () =>
        ...
        Utils.apiFetch(`daily/rankings?category=${category}&page=${page}`)
          ...
          .then((jsonResponse) => {
            ...
            this.titleLabel.setText(
              `${i18next.t(
                `menu:${ScoreboardCategory[category].toLowerCase()}Rankings`,
              )}`,
            );
            ...
            } else this.loadingLabel.setText(i18next.t("menu:noRankings"));
          });
      })
      .catch((err) => {

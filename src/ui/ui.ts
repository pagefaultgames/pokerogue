...
export default class UI extends Phaser.GameObjects.Container {
  ...
  setup(): void {
    for (const handler of this.handlers) handler.setup();
  ...
  private setupTooltip() {
    ...
    this.tooltipTitle = addTextObject(
      this.scene,
      64,
      4,
      "",
    ...
    this.tooltipContent = addTextObject(
      this.scene,
      6,
      16,
      "",
    ...
    if (prompt && text.indexOf("$") > -1) {
      ...
      for (let p = messagePages.length - 1; p >= 0; p--) {
        ...
        showMessageAndCallback = () =>
          this.showText(messagePages[p], null, originalFunc, null, true);
      }
      showMessageAndCallback();
    } else {
      const handler = this.getHandler();
      if (handler instanceof MessageUiHandler)
        (handler as MessageUiHandler).showText(
          text,
          delay,
          callback,
          callbackDelay,
        ...
        this.getMessageHandler().showText(
          text,
          delay,
          callback,
          callbackDelay,
  ...
  showDialogue(
    ...
    if (text.indexOf("$") > -1) {
      ...
      for (let p = messagePages.length - 1; p >= 0; p--) {
        const originalFunc = showMessageAndCallback;
        showMessageAndCallback = () =>
          this.showDialogue(messagePages[p], name, null, originalFunc);
      }
      showMessageAndCallback();
    } else {
      const handler = this.getHandler();
  ...
  showTooltip(title: string, content: string, overlap?: boolean): void {
    ...
    this.tooltipBg.width = Math.min(
      Math.max(
        this.tooltipTitle.displayWidth,
        this.tooltipContent.displayWidth,
      ) + 12,
      684,
    );
    this.tooltipBg.height =
      (title ? 31 : 19) + 10.5 * (wrappedContent.split("\n").length - 1);
    if (overlap)
      (this.scene as BattleScene).uiContainer.moveAbove(
        this.tooltipContainer,
        this,
      );
    else
      (this.scene as BattleScene).uiContainer.moveBelow(
        this.tooltipContainer,
        this,
      );
  }
  ...
  update(): void {
    if (this.tooltipContainer.visible) {
      const reverse =
        ...
        this.scene.game.canvas.width - this.tooltipBg.width * 6 - 12;
      this.tooltipContainer.setPosition(
        !reverse
          ? this.scene.game.input.mousePointer.x / 6 + 2
          : this.scene.game.input.mousePointer.x / 6 - this.tooltipBg.width - 2,
        this.scene.game.input.mousePointer.y / 6 + 2,
      );
    }
  }
  ...
  clearText(): void {
    ...
    else this.getMessageHandler().clearText();
  }

  setCursor(cursor: integer): boolean {
    const changed = this.getHandler().setCursor(cursor);
    if (changed) this.playSelect();
  ...
  fadeOut(duration: integer): Promise<void> {
    return new Promise((resolve) => {
      if (this.overlayActive) return resolve();
    ...
    return new Promise((resolve) => {
      ...
      const doSetMode = () => {
        if (this.mode !== mode) {
          ...
          if (chainMode && this.mode && !clear) this.modeChain.push(this.mode);
          this.mode = mode;
          const touchControls = document.getElementById("touchControls");
          if (touchControls) touchControls.dataset.uiMode = Mode[mode];
      ...
      if (
        (!chainMode &&
          (transitionModes.indexOf(this.mode) > -1 ||
            transitionModes.indexOf(mode) > -1) &&
  ...
  revertMode(): Promise<boolean> {
    return new Promise<boolean>((resolve) => {
      if (!this?.modeChain?.length) return resolve(false);
      ...
      const doRevertMode = () => {
        ...
        const touchControls = document.getElementById("touchControls");
        if (touchControls) touchControls.dataset.uiMode = Mode[this.mode];
        resolve(true);
      };
  ...
  revertModes(): Promise<void> {
    return new Promise<void>((resolve) => {
      if (!this?.modeChain?.length) return resolve();
      this.revertMode().then((success) =>
        Utils.executeIf(success, this.revertModes).then(() => resolve()),
      );
    });
  }
}

...
export default class MenuUiHandler extends MessageUiHandler {
  ...
  setup() {
    ...
    this.optionSelectText = addTextObject(
      ...
      0,
      this.menuOptions
        .map((o) => `${i18next.t(`menuUiHandler:${MenuOptions[o]}`)}`)
        .join("\n"),
      TextStyle.WINDOW,
    ...
    this.menuBg = addWindow(
      this.scene,
      this.scene.game.canvas.width / 6 -
        (this.optionSelectText.displayWidth + 25),
      0,
      ...
      manageDataOptions.push({
        ...
        handler: () => {
          confirmSlot(
            ...
            () => true,
            (slotId) =>
              this.scene.gameData.importData(GameDataType.SESSION, slotId),
          );
          return true;
        },
        keepOpen: true,
      ...
      handler: () => {
        ...
        Promise.all(
          ...
          confirmSlot(
            ...
            (slotId) =>
              this.scene.gameData.tryExportData(GameDataType.SESSION, slotId),
          );
        });
        return true;
      },
  ...
  processInput(button: Button): boolean {
    ...
    if (button === Button.ACTION) {
      ...
      for (const imo of this.ignoredMenuOptions) {
        if (adjustedCursor >= imo) adjustedCursor++;
        else break;
      }
        ...
        case MenuOptions.LOG_OUT:
          ...
          const doLogout = () => {
            Utils.apiFetch("account/logout", true).then((res) => {
              if (!res.ok)
                console.error(
                  `Log out failed (${res.status}: ${res.statusText})`,
                );
      ...
      ui.revertMode().then((result) => {
        if (!result) ui.setMode(Mode.MESSAGE);
      });
    } else {
      switch (button) {
        case Button.UP:
          if (this.cursor) success = this.setCursor(this.cursor - 1);
          else success = this.setCursor(this.menuOptions.length - 1);
          break;
        case Button.DOWN:
          if (this.cursor + 1 < this.menuOptions.length)
            success = this.setCursor(this.cursor + 1);
          else success = this.setCursor(0);
          break;
      }
    }

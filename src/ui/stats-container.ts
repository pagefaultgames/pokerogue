...
export class StatsContainer extends Phaser.GameObjects.Container {
  ...
  setup() {
    ...
    const ivChartBg = this.scene.add.polygon(
      48,
      44,
      ivChartBgData,
      0xd8e0f0,
    ...
    const ivChartBorder = this.scene.add
    ...
    this.ivChart = this.scene.add.polygon(
      ivChartBg.x,
      ivChartBg.y,
      defaultIvChartData,
      0x98d8a0,
    ...
    new Array(6).fill(null).map((_, i: integer) => {
      const statLabel = addTextObject(
        this.scene,
        ivChartBg.x + ivChartSize * ivChartStatCoordMultipliers[i][0] * 1.325,
        ivChartBg.y +
          ivChartSize * ivChartStatCoordMultipliers[i][1] * 1.325 -
          4,
        getStatName(i as Stat),
        TextStyle.TOOLTIP_CONTENT,
      );
      statLabel.setOrigin(0.5);

      this.ivStatValueTexts[i] = addBBCodeTextObject(
        this.scene,
        statLabel.x,
        statLabel.y + 8,
        "0",
      ...
      this.ivStatValueTexts.map((t: BBCodeText, i: integer) => {
        ...
        if (this.showDiff && originalIvs) {
          if (originalIvs[i] < ivs[i])
            label += ` ([color=${getTextColor(
              TextStyle.SUMMARY_BLUE,
              false,
              (this.scene as BattleScene).uiTheme,
            )}][shadow=${getTextColor(
              TextStyle.SUMMARY_BLUE,
              true,
              (this.scene as BattleScene).uiTheme,
            )}]+${ivs[i] - originalIvs[i]}[/shadow][/color])`;
          else label += " (-)";
        }
      ...
      this.scene.tweens.addCounter({
        ...
        onUpdate: (tween: Phaser.Tweens.Tween) => {
          ...
          const interpolatedData = ivChartData.map(
            (v: number, i: integer) =>
              v * progress + lastIvChartData[i] * (1 - progress),
          );

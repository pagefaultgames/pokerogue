...
export default class PokeballTray extends Phaser.GameObjects.Container {
  ...
  setup(): void {
    ...
    this.balls = new Array(6).fill(null).map((_, i) => this.scene.add.sprite((this.player ? -83 : 76) + (this.scene.game.canvas.width / (this.scene as BattleScene).resolutionScale) * (this.player ? -1 : 1) + 10 * i * (this.player ? 1 : -1), -8, "pb_tray_ball", "empty"));
  ...
  showPbTray(party: Pokemon[]): Promise<void> {
    return new Promise(resolve => {
      ...
      this.balls.forEach((ball, b) => {
        ball.x += (this.scene.game.canvas.width / (this.scene as BattleScene).resolutionScale + 104) * (this.player ? 1 : -1);
        let ballFrame = "ball";
        if (b >= party.length) {
          ballFrame = "empty";
        } else if (!party[b].hp) {
  ...
  hide(): Promise<void> {
    return new Promise(resolve => {
      ...
      this.balls.forEach((ball, b) => {
        this.scene.tweens.add({
          targets: ball,
          x: `${this.player ? "-" : "+"}=${this.scene.game.canvas.width / (this.scene as BattleScene).resolutionScale}`,
          duration: 250,
          delay: b * 100,
          ease: "Sine.easeIn"
        });

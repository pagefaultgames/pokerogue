  ...
  showPbTray(party: Pokemon[]): Promise<void> {
    return new Promise((resolve) => {
      ...
      this.balls.forEach((ball, b) => {
        ball.x +=
          (this.scene.game.canvas.width / 6 + 104) * (this.player ? 1 : -1);
        let ballFrame = "ball";
        if (b >= party.length) ballFrame = "empty";
        else if (!party[b].hp) ballFrame = "faint";
        else if (party[b].status) ballFrame = "status";

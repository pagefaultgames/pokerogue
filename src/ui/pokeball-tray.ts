...
export default class PokeballTray extends Phaser.GameObjects.Container {
  ...
  showPbTray(party: Pokemon[]): Promise<void> {
    return new Promise(resolve => {
      ...
      (this.scene as BattleScene).audioHandler.playSound("pb_tray_enter");
        ...
        onComplete: () => {
          this.balls.forEach((ball, b) => {
            this.scene.tweens.add({
              ...
              duration: b * 100,
              ease: "Sine.easeIn",
              onComplete: () => (this.scene as BattleScene).audioHandler.playSound(b < party.length ? "pb_tray_ball" : "pb_tray_empty")
            });
          });
        }
      });

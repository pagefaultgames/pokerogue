...
export default class FightUiHandler extends UiHandler {
  ...
  processInput(button: Button): boolean {
    ...
    if (button === Button.CANCEL || button === Button.ACTION) {
      ...
      switch (button) {
        case Button.UP:
          if (cursor >= 2) success = this.setCursor(cursor - 2);
          break;
        case Button.DOWN:
          if (cursor < 2) success = this.setCursor(cursor + 2);
          break;
        case Button.LEFT:
          if (cursor % 2 === 1) success = this.setCursor(cursor - 1);
          break;
        case Button.RIGHT:
          if (cursor % 2 === 0) success = this.setCursor(cursor + 1);
          break;
      }
    }
  ...
  setCursor(cursor: integer): boolean {
    ...
    if (changed) {
      if (!this.fieldIndex) this.cursor = cursor;
      else this.cursor2 = cursor;
    }
    ...
    const moveset = (this.scene.getCurrentPhase() as CommandPhase)
      .getPokemon()
      .getMoveset();
    ...
    if (hasMove) {
      ...
      this.typeIcon
        .setTexture("types", Type[pokemonMove.getMove().type].toLowerCase())
        .setScale(0.8);
  ...
  displayMoves() {
    ...
    for (let m = 0; m < 4; m++) {
      ...
      if (m < moveset.length) moveText.setText(moveset[m].getName());

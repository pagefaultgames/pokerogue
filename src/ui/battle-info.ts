...
export default class BattleInfo extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number, player: boolean) {
    ...
    this.levelContainer = this.scene.add.container(
      player ? -41 : -50,
      player ? -10 : -5,
    );
    ...
    this.statsBox = this.scene.add.sprite(
      0,
      0,
      `${this.getTextureName()}_stats`,
    );
    ...
    battleStatOrder.map((s, i) => {
      ...
      const statY =
        -this.statsBox.height / 2 +
        4 +
        (i < battleStatOrder.length - 1 ? (i % 2 ? 10 : 0) : 5);
      const statLabel = this.scene.add.sprite(
        statX,
        statY,
        "pbinfo_stat",
        BattleStat[s],
  ...
  initInfo(pokemon: Pokemon) {
      ...
      this.splicedIcon.on("pointerover", () =>
        (this.scene as BattleScene).ui.showTooltip(
          ...
          `${pokemon.species.getName(
            pokemon.formIndex,
          )}/${pokemon.fusionSpecies.getName(pokemon.fusionFormIndex)}`,
        ),
      );
    ...
    if (!this.player) {
      const dexEntry =
        pokemon.scene.gameData.dexData[pokemon.species.speciesId];
      this.ownedIcon.setVisible(!!dexEntry.caughtAttr);
      const dexAttr = pokemon.getDexAttr();
      if (
        (dexEntry.caughtAttr & dexAttr) < dexAttr ||
        !(
          pokemon.scene.gameData.starterData[pokemon.species.getRootSpeciesId()]
            .abilityAttr & Math.pow(2, pokemon.abilityIndex)
        )
      )
        this.ownedIcon.setTint(0x808080);

      if (this.boss) this.updateBossSegmentDividers(pokemon as EnemyPokemon);
    }
    ...
    this.lastHpFrame =
      this.hpBar.scaleX > 0.5
        ? "high"
        : this.hpBar.scaleX > 0.25
          ? "medium"
          : "low";
    this.hpBar.setFrame(this.lastHpFrame);
    if (this.player) this.setHpNumbers(pokemon.hp, pokemon.getMaxHp());
    ...
    this.type1Icon.setTexture(
      `pbinfo_${this.player ? "player" : "enemy"}_type${
        types.length > 1 ? "1" : ""
      }`,
    );
    ...
    if (types.length > 1) this.type2Icon.setFrame(Type[types[1]].toLowerCase());
    if (types.length > 2) this.type3Icon.setFrame(Type[types[2]].toLowerCase());

    if (this.player) {
      this.expMaskRect.x =
        (pokemon.levelExp /
          getLevelTotalExp(pokemon.level, pokemon.species.growthRate)) *
        510;
  ...
  getTextureName(): string {
    return `pbinfo_${this.player ? "player" : "enemy"}${
      !this.player && this.boss ? "_boss" : this.mini ? "_mini" : ""
    }`;
  }
  ...
  setMini(mini: boolean): void {
    ...
    if (this.player) this.y -= 12 * (mini ? 1 : -1);
  ...
  updateBossSegmentDividers(pokemon: EnemyPokemon): void {
    ...
    if (this.boss && this.bossSegments > 1) {
      ...
      for (let s = 1; s < this.bossSegments; s++) {
        const dividerX =
          (Math.round((maxHp / this.bossSegments) * s) / maxHp) *
          this.hpBar.width;
        ...
        this.add(divider);
        this.moveBelow(
          divider as Phaser.GameObjects.GameObject,
          this.statsContainer,
        );
  ...
  updateInfo(pokemon: Pokemon, instant?: boolean): Promise<void> {
    return new Promise((resolve) => {
      ...
      if (nameUpdated) {
        this.updateNameText(pokemon);
        this.genderText.setPositionRelative(
          this.nameText,
      ...
      if (teraTypeUpdated) {
        ...
        this.teraIcon.setTintFill(
          Phaser.Display.Color.GetColor(...getTypeRgb(teraType)),
        );
      ...
      if (this.lastStatus !== (pokemon.status?.effect || StatusEffect.NONE)) {
        ...
        if (this.lastStatus !== StatusEffect.NONE)
          this.statusIndicator.setFrame(
            StatusEffect[this.lastStatus].toLowerCase(),
          );
      ...
      const updatePokemonHp = () => {
        ...
        if (speed) duration = speed >= 3 ? 0 : duration / Math.pow(2, speed);
        this.scene.tweens.add({
          targets: this.hpBar,
          ease: "Sine.easeOut",
      ...
      if (this.player) {
        const isLevelCapped =
          pokemon.level >= (this.scene as BattleScene).getMaxExpLevel();
        ...
        if (this.lastExp !== pokemon.exp || this.lastLevel !== pokemon.level) {
          ...
          resolve = () =>
            this.updatePokemonExp(pokemon, false, durationMultipler).then(() =>
              originalResolve(),
            );
  ...
  updatePokemonExp(
    ...
    return new Promise((resolve) => {
      ...
      const relLevelExp = getLevelRelExp(
        this.lastLevel + 1,
      ...
      const durationMultiplier = Phaser.Tweens.Builders.GetEaseFunction(
        "Sine.easeIn",
      ...
      if (duration) (this.scene as BattleScene).playSound("exp");
      this.scene.tweens.add({
        ...
        onComplete: () => {
          if (!this.scene) return resolve();
          if (duration) this.scene.sound.stopByKey("exp");
          if (ratio === 1) {
  ...
  setLevel(level: integer): void {
    ...
    this.levelContainer.setX(
      (this.player ? -41 : -50) - 8 * Math.max(levelStr.length - 3, 0),
    );
  }
  ...
  setHpNumbers(hp: integer, maxHp: integer): void {
    ...
    for (let i = maxHpStr.length - 1; i >= 0; i--)
      this.hpNumbersContainer.add(
        this.scene.add.image(offset++ * -8, 0, "numbers", maxHpStr[i]),
      );
    this.hpNumbersContainer.add(
      this.scene.add.image(offset++ * -8, 0, "numbers", "/"),
    );
    for (let i = hpStr.length - 1; i >= 0; i--)
      this.hpNumbersContainer.add(
        this.scene.add.image(offset++ * -8, 0, "numbers", hpStr[i]),
      );
  }

...
export default class BattleInfo extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number, player: boolean) {
    ...
    if (this.player) {
      ...
      expMaskRect.setScale((this.scene as BattleScene).resolutionScale);
  ...
  initInfo(pokemon: Pokemon) {
    ...
    if (this.player) {
      this.expMaskRect.x = (pokemon.levelExp / getLevelTotalExp(pokemon.level, pokemon.species.growthRate)) * 85 * (this.scene as BattleScene).resolutionScale;
  ...
  updatePokemonExp(pokemon: Pokemon, instant?: boolean, levelDurationMultiplier: number = 1): Promise<void> {
    return new Promise(resolve => {
      ...
      this.scene.tweens.add({
        ...
        ease: "Sine.easeIn",
        x: ratio * 85 * (this.scene as BattleScene).resolutionScale,
        duration: duration,
        onComplete: () => {
          if (!this.scene) {
            return resolve();
...
export class PlayerBattleInfo extends BattleInfo {
  constructor(scene: Phaser.Scene) {
    super(scene, Math.floor(scene.game.canvas.width / (scene as BattleScene).resolutionScale) - 10, -72, true);
  }
}

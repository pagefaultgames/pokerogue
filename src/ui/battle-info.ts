...
export default class BattleInfo extends Phaser.GameObjects.Container {
  ...
  constructor(x: number, y: number, player: boolean) {
    ...
    this.box = gScene.add.sprite(0, 0, this.getTextureName());
    ...
    if (!this.player) {
      this.ownedIcon = gScene.add.sprite(0, 0, "icon_owned");
      ...
      this.championRibbon = gScene.add.sprite(0, 0, "champion_ribbon");
    ...
    this.teraIcon = gScene.add.sprite(0, 0, "icon_tera");
    ...
    this.shinyIcon = gScene.add.sprite(0, 0, "shiny_star");
    ...
    this.fusionShinyIcon = gScene.add.sprite(0, 0, "shiny_star_2");
    ...
    this.statusIndicator = gScene.add.sprite(0, 0, Utils.getLocalizedSpriteKey("statuses"));
    ...
    this.levelContainer = gScene.add.container(player ? -41 : -50, player ? -10 : -5);
    this.levelContainer.setName("container_level");
    this.add(this.levelContainer);

    const levelOverlay = gScene.add.image(0, 0, "overlay_lv");
    this.levelContainer.add(levelOverlay);

    this.hpBar = gScene.add.image(player ? -61 : -71, player ? -1 : 4.5, "overlay_hp");
    ...
    this.levelNumbersContainer = gScene.add.container(9.5, gScene.uiTheme ? 0 : -0.5);
    this.levelNumbersContainer.setName("container_level");
    this.levelContainer.add(this.levelNumbersContainer);

    if (this.player) {
      this.hpNumbersContainer = gScene.add.container(-15, 10);
      this.hpNumbersContainer.setName("container_hp");
      this.add(this.hpNumbersContainer);

      const expBar = gScene.add.image(-98, 18, "overlay_exp");
    ...
    this.statsBox = gScene.add.sprite(0, 0, `${this.getTextureName()}_stats`);
    ...
    this.statValuesContainer = gScene.add.container(0, 0);
    ...
    this.statOrder.map((s, i) => {
      ...
      const statLabel = gScene.add.sprite(statX, statY, "pbinfo_stat", Stat[s]);
      statLabel.setName("icon_stat_label_" + i.toString());
      statLabel.setOrigin(0, 0);
      statLabels.push(statLabel);
      this.statValuesContainer.add(statLabel);

      const statNumber = gScene.add.sprite(statX + statLabel.width, statY, "pbinfo_stat_numbers", this.statOrder[i] !== Stat.HP ? "3" : "empty");
    ...
    this.type1Icon = gScene.add.sprite(player ? -139 : -15, player ? -17 : -15.5, `pbinfo_${player ? "player" : "enemy"}_type1`);
    this.type1Icon.setName("icon_type_1");
    this.type1Icon.setOrigin(0, 0);
    this.add(this.type1Icon);

    this.type2Icon = gScene.add.sprite(player ? -139 : -15, player ? -1 : -2.5, `pbinfo_${player ? "player" : "enemy"}_type2`);
    this.type2Icon.setName("icon_type_2");
    this.type2Icon.setOrigin(0, 0);
    this.add(this.type2Icon);

    this.type3Icon = gScene.add.sprite(player ? -154 : 0, player ? -17 : -15.5, `pbinfo_${player ? "player" : "enemy"}_type`);
    this.type3Icon.setName("icon_type_3");
    this.type3Icon.setOrigin(0, 0);
    this.add(this.type3Icon);

    if (!this.player) {
      this.effectivenessContainer = gScene.add.container(0, 0);
  ...
  initInfo(pokemon: Pokemon) {
    ...
    this.teraIcon.on("pointerover", () => {
      if (this.lastTeraType !== Type.UNKNOWN) {
        gScene.ui.showTooltip("", i18next.t("fightUiHandler:teraHover", { type: i18next.t(`pokemonInfo:Type.${Type[this.lastTeraType]}`) }));
      }
    });
    this.teraIcon.on("pointerout", () => gScene.ui.hideTooltip());
    ...
    if (this.splicedIcon.visible) {
      this.splicedIcon.on("pointerover", () => gScene.ui.showTooltip("", `${pokemon.species.getName(pokemon.formIndex)}/${pokemon.fusionSpecies?.getName(pokemon.fusionFormIndex)}`));
    ...
    if (this.shinyIcon.visible) {
      const shinyDescriptor = doubleShiny || baseVariant ?
        ...
        : "";
      this.shinyIcon.on("pointerover", () => gScene.ui.showTooltip("", `${i18next.t("common:shinyOnHover")}${shinyDescriptor ? ` (${shinyDescriptor})` : ""}`));
    ...
    if (!this.player) {
      if (this.nameText.visible) {
        this.nameText.on("pointerover", () => gScene.ui.showTooltip("", i18next.t("battleInfo:generation", { generation: i18next.t(`starterSelectUiHandler:gen${pokemon.species.generation}`) })));
        this.nameText.on("pointerout", () => gScene.ui.hideTooltip());
      }

      const dexEntry = gScene.gameData.dexData[pokemon.species.speciesId];
      this.ownedIcon.setVisible(!!dexEntry.caughtAttr);
      const opponentPokemonDexAttr = pokemon.getDexAttr();
      if (gScene.gameMode.isClassic) {
        if (gScene.gameData.starterData[pokemon.species.getRootSpeciesId()].classicWinCount > 0 && gScene.gameData.starterData[pokemon.species.getRootSpeciesId(true)].classicWinCount > 0) {
      ...
      const ownedAbilityAttrs = gScene.gameData.starterData[pokemon.species.getRootSpeciesId()].abilityAttr;
  ...
  updateBossSegmentDividers(pokemon: EnemyPokemon): void {
    ...
    if (this.boss && this.bossSegments > 1) {
      const uiTheme = gScene.uiTheme;
      const maxHp = pokemon.getMaxHp();
      for (let s = 1; s < this.bossSegments; s++) {
        const dividerX = (Math.round((maxHp / this.bossSegments) * s) /  maxHp) * this.hpBar.width;
        const divider = gScene.add.rectangle(0, 0, 1, this.hpBar.height - (uiTheme ? 0 : 1), pokemon.bossSegmentIndex >= s ? 0xFFFFFF : 0x404040);
  ...
  updateInfo(pokemon: Pokemon, instant?: boolean): Promise<void> {
    return new Promise(resolve => {
      ...
      if (this.player) {
        const isLevelCapped = pokemon.level >= gScene.getMaxExpLevel();
  ...
  updatePokemonExp(pokemon: Pokemon, instant?: boolean, levelDurationMultiplier: number = 1): Promise<void> {
    return new Promise(resolve => {
      ...
      if (this.lastLevel >= gScene.getMaxExpLevel(true)) {
        if (levelUp) {
          ratio = 1;
        } else {
          ratio = 0;
      ...
      const speed = gScene.expGainsSpeed;
      ...
      gScene.tweens.add({
        ...
        onComplete: () => {
          ...
          if (duration) {
            gScene.sound.stopByKey("se/exp");
          }
          if (ratio === 1) {
            gScene.playSound("se/level_up");
            this.setLevel(this.lastLevel);
            gScene.time.delayedCall(500 * levelDurationMultiplier, () => {
  ...
  setHpNumbers(hp: integer, maxHp: integer): void {
    ...
    for (let i = maxHpStr.length - 1; i >= 0; i--) {
      this.hpNumbersContainer.add(gScene.add.image(offset++ * -8, 0, "numbers", maxHpStr[i]));
    }
    this.hpNumbersContainer.add(gScene.add.image(offset++ * -8, 0, "numbers", "/"));
    for (let i = hpStr.length - 1; i >= 0; i--) {
      this.hpNumbersContainer.add(gScene.add.image(offset++ * -8, 0, "numbers", hpStr[i]));
    }
  }
  ...
  updateEffectiveness(effectiveness?: string) {
    ...
    if (!gScene.typeHints || effectiveness === undefined || this.flyoutMenu?.flyoutVisible) {
      this.effectivenessContainer.setVisible(false);
      return;
    }
...
export class PlayerBattleInfo extends BattleInfo {
  constructor() {
    super(Math.floor(gScene.game.canvas.width / 6) - 10, -72, true);
  }
}

...
export default class AchvBar extends Phaser.GameObjects.Container {
  ...
  setup(): void {
    ...
    this.titleText = addTextObject(this.scene, 40, 3, "", TextStyle.MESSAGE, {
      fontSize: "72px",
    });
    this.titleText.setOrigin(0, 0);
    this.add(this.titleText);

    this.scoreText = addTextObject(this.scene, 150, 3, "", TextStyle.MESSAGE, {
      fontSize: "72px",
    });
    this.scoreText.setOrigin(1, 0);
    this.add(this.scoreText);

    this.descriptionText = addTextObject(
      this.scene,
      43,
      16,
      "",
  ...
  protected hide(): void {
    ...
    this.scene.tweens.add({
      ...
      onComplete: () => {
        ...
        if (this.queue.length) this.showAchv(this.queue.shift());
      },
    });
  }
}

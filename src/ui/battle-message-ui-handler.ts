...
export default class BattleMessageUiHandler extends MessageUiHandler {
  ...
  setup(): void {
    ...
    const moveDetailsWindow = addWindow(
      this.scene,
      238,
      0,
      80,
    ...
    for (const s of stats) levelUpStatsLabelText += `${getStatName(s)}\n`;
  ...
  promptLevelUpStats(
    ...
    return new Promise((resolve) => {
      if (!this.scene.showLevelUpStats) return resolve();
      const newStats = (this.scene as BattleScene).getParty()[partyMemberIndex]
        .stats;
      let levelUpStatsValuesText = "";
      const stats = Utils.getEnumValues(Stat);
      for (const s of stats)
        levelUpStatsValuesText += `${
          showTotals ? newStats[s] : newStats[s] - prevStats[s]
        }\n`;
      ...
      this.onActionInput = () => {
        if (!showTotals)
          return this.promptLevelUpStats(partyMemberIndex, null, true).then(
            () => resolve(),
          );
        else {
  ...
  promptIvs(
    ...
    return new Promise((resolve) => {
      this.scene.executeWithSeedOffset(() => {
        ...
        } else shownStats = stats;
        for (const s of stats)
          levelUpStatsValuesText += `${
            shownStats.indexOf(s) > -1
              ? this.getIvDescriptor(ivs[s], s, pokemonId)
              : "???"
          }\n`;
        this.levelUpStatsValuesContent.text = levelUpStatsValuesText;
  ...
  getIvDescriptor(value: integer, typeIv: integer, pokemonId: integer): string {
    const starterSpecies = this.scene
      .getPokemonById(pokemonId)
      .species.getRootSpeciesId(true);
    const starterIvs: number[] =
      this.scene.gameData.dexData[starterSpecies].ivs;
    ...
    const coloredText = (text: string, isBetter: boolean) => {
      const textStyle: TextStyle = isBetter
        ? TextStyle.SUMMARY_GREEN
        : TextStyle.SUMMARY;
      const color = getTextColor(textStyle, false, uiTheme);
      return `[color=${color}][shadow=${getTextColor(
        textStyle,
        true,
        uiTheme,
      )}]${text}[/shadow][/color]`;
    ...
    if (value > 30) return coloredText("Best", value > starterIvs[typeIv]);
    if (value === 30)
      return coloredText("Fantastic", value > starterIvs[typeIv]);
    if (value > 20) return coloredText("Very Good", value > starterIvs[typeIv]);
    if (value > 10)
      return coloredText("Pretty Good", value > starterIvs[typeIv]);
    if (value > 0) return coloredText("Decent", value > starterIvs[typeIv]);

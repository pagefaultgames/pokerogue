...
export default class PokemonSpriteSparkleHandler {
  ...
  onLapse(): void {
    ...
    for (const s of this.sprites.values()) {
      ...
      if (
        !s.visible ||
        (s.parentContainer instanceof Pokemon &&
          !s.parentContainer.parentContainer)
      )
        continue;
      const pokemon =
        s.parentContainer instanceof Pokemon
          ? (s.parentContainer as Pokemon)
          : null;
      ...
      const pixel = texture.manager.getPixel(
        pixelX,
        pixelY,
        texture.key,
        "__BASE",
      ...
      if (pixel.alpha) {
        ...
        sparkle.pipelineData["ignoreTimeTint"] =
          s.pipelineData["ignoreTimeTint"];
        sparkle.play("tera_sparkle");
        parent.add(sparkle);
        s.scene.time.delayedCall(
          Utils.fixedInt(Math.floor((1000 / 12) * 13)),
          () => sparkle.destroy(),
        );
      }
    }
  ...
  add(sprites: Phaser.GameObjects.Sprite | Phaser.GameObjects.Sprite[]): void {
    ...
    for (const s of sprites) {
      if (this.sprites.has(s)) continue;
      this.sprites.add(s);
    }
  }
  ...
  removeAll(): void {
    for (const s of this.sprites.values()) this.sprites.delete(s);
  }
}

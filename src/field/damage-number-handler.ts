...
export default class DamageNumberHandler {
  ...
  add(
    ...
    if (!scene.damageNumbersMode) return;
    ...
    if (textColor) damageNumber.setColor(textColor);
    if (shadowColor) {
      if (critical) {
        damageNumber.setShadowOffset(0, 0);
        damageNumber.setStroke(shadowColor, 12);
      } else damageNumber.setShadowColor(shadowColor);
    }
    ...
    if (yOffset) damageNumber.y += yOffset;
      ...
      scene.tweens.add({
        ...
        onComplete: () => {
          this.damageNumbers
            .get(battlerIndex)
            .splice(
              this.damageNumbers.get(battlerIndex).indexOf(damageNumber),
              1,
            );

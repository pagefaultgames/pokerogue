...
import {
  ...
  MOVE_OVERRIDE,
  MOVE_OVERRIDE_2,
  OPP_ABILITY_OVERRIDE,
  OPP_MOVE_OVERRIDE,
  OPP_MOVE_OVERRIDE_2,
  OPP_PASSIVE_ABILITY_OVERRIDE,
  OPP_SHINY_OVERRIDE,
  OPP_VARIANT_OVERRIDE,
...
export default abstract class Pokemon extends Phaser.GameObjects.Container {
  ...
  constructor(
    ...
    if (!species.isObtainable() && this.isPlayer())
      throw `Cannot create a player Pokemon for species '${species.getName(
        formIndex,
      )}'`;
    ...
    this.abilityIndex =
      abilityIndex !== undefined
        ? abilityIndex
        : species.abilityHidden && hasHiddenAbility
          ? species.ability2
            ? 2
            : 1
          : species.ability2
            ? randAbilityIndex
            : 0;
    if (formIndex !== undefined) this.formIndex = formIndex;
    ...
    if (dataSource) {
      ...
      if (this.variant === undefined) this.variant = 0;
      this.nature = dataSource.nature || (0 as Nature);
      this.natureOverride =
        dataSource.natureOverride !== undefined
          ? dataSource.natureOverride
          : -1;
      ...
      if (this.gender === undefined) this.generateGender();

      if (this.formIndex === undefined)
        this.formIndex = this.scene.getSpeciesFormIndex(
          species,
          this.gender,
          this.nature,
          this.isPlayer(),
      ...
      if (this.shiny === undefined) this.trySetShiny();

      if (this.variant === undefined)
        this.variant = this.shiny ? this.generateVariant() : 0;

      if (nature !== undefined) this.setNature(nature);
      else this.generateNature();
    ...
    if (!species.isObtainable()) this.shiny = false;
  ...
  init(): void {
    ...
    if (this.isShiny() && !this.shinySparkle) this.initShinySparkle();
  }
  ...
  isFainted(checkStatus?: boolean): boolean {
    return (
      !this.hp && (!checkStatus || this.status?.effect === StatusEffect.FAINT)
    );
  }
  ...
  generateName(): void {
    ...
    if (this.battleInfo) this.updateInfo(true);
  }
        ...
        () => {
          ...
          this.scene.load.once(Phaser.Loader.Events.COMPLETE, () => {
            if (this.isPlayer()) {
              ...
              console.warn = () => {};
              const battleFrameNames = this.scene.anims.generateFrameNames(
            ...
            const updateFusionPaletteAndResolve = () => {
              ...
              if (this.summonData?.speciesForm) this.updateFusionPalette(true);
              resolve();
            };
            if (this.shiny) {
              const populateVariantColors = (
                ...
                return new Promise((resolve) => {
                  ...
                  const useExpSprite =
                    ...
                    this.scene.hasExpSprite(
                      this.getBattleSpriteKey(back, ignoreOverride),
                    );
                  battleSpritePath
                    .split("/")
                  ...
                  if (variantSet && variantSet[this.variant] === 1) {
                    if (variantColorCache.hasOwnProperty(key)) return resolve();
                    this.scene
                      .cachedFetch(
                        `./images/pokemon/variant/${
                          useExpSprite ? "exp/" : ""
                        }${battleSpritePath}.json`,
                      )
                      .then((res) => res.json())
                      .then((c) => {
              ...
              if (this.isPlayer())
                Promise.all([
                  populateVariantColors(this.getBattleSpriteKey(false)),
                  populateVariantColors(this.getBattleSpriteKey(true), true),
                ]).then(() => updateFusionPaletteAndResolve());
              else
                populateVariantColors(this.getBattleSpriteKey(false)).then(() =>
                  updateFusionPaletteAndResolve(),
                );
            } else updateFusionPaletteAndResolve();
          });
  ...
  getFusionFormKey(): string {
    if (!this.fusionSpecies) return null;
    if (
  ...
  getBattleSpriteId(back?: boolean, ignoreOverride?: boolean): string {
    if (back === undefined) back = this.isPlayer();
    return this.getSpeciesForm(ignoreOverride).getSpriteId(
      this.getGender(ignoreOverride) === Gender.FEMALE,
      this.formIndex,
      this.shiny,
  ...
  getSpeciesForm(ignoreOverride?: boolean): PokemonSpeciesForm {
    ...
    if (!this.species.forms?.length) return this.species;
  ...
  getSpriteScale(): number {
    ...
    if (
      formKey.indexOf(SpeciesFormKey.GIGANTAMAX) > -1 ||
      formKey.indexOf(SpeciesFormKey.ETERNAMAX) > -1
    )
      return 1.5;
    return 1;
  }
  ...
  getHeldItems(): PokemonHeldItemModifier[] {
    ...
    return this.scene.findModifiers(
      (m) =>
        ...
        (m as PokemonHeldItemModifier).pokemonId === this.id,
      this.isPlayer(),
  ...
  updateSpritePipelineData(): void {
    [this.getSprite(), this.getTintSprite()].map(
      (s) => (s.pipelineData["teraColor"] = getTypeRgb(this.getTeraType())),
    );
  ...
  getBattleStat(
    ...
    if (stat === Stat.HP) return this.getStat(Stat.HP);
    const battleStat = (stat - 1) as BattleStat;
    const statLevel = new Utils.IntegerHolder(
      this.summonData.battleStats[battleStat],
    );
    if (opponent) {
      if (isCritical) {
        switch (stat) {
    ...
    if (this.isPlayer())
      this.scene.applyModifiers(
        TempBattleStatBoosterModifier,
        this.isPlayer(),
        battleStat as integer as TempBattleStat,
        statLevel,
      );
    ...
    let ret =
      statValue.value *
      (Math.max(2, 2 + statLevel.value) / Math.max(2, 2 - statLevel.value));
    switch (stat) {
      case Stat.ATK:
        if (this.getTag(BattlerTagType.SLOW_START)) ret >>= 1;
        break;
      case Stat.DEF:
        if (
          this.isOfType(Type.ICE) &&
          this.scene.arena.weather?.weatherType === WeatherType.SNOW
        )
          ret *= 1.5;
        break;
      case Stat.SPATK:
      ...
      case Stat.SPDEF:
        if (
          this.isOfType(Type.ROCK) &&
          this.scene.arena.weather?.weatherType === WeatherType.SANDSTORM
        )
          ret *= 1.5;
        break;
      case Stat.SPD:
    ...
    if (highestStatBoost) ret *= highestStatBoost.multiplier;

    return Math.floor(ret);
  }

  calculateStats(): void {
    if (!this.stats) this.stats = [0, 0, 0, 0, 0, 0];
    ...
    for (const s of stats) {
      ...
      if (isHp) {
        ...
        if (this.hasAbility(Abilities.WONDER_GUARD, false, true)) value = 1;
        if (this.hp > value || this.hp === undefined) this.hp = value;
        else if (this.hp) {
          const lastMaxHp = this.getMaxHp();
          if (lastMaxHp && value > lastMaxHp) this.hp += value - lastMaxHp;
        }
      } else {
        value += 5;
        ...
        if (natureStatMultiplier.value !== 1)
          value = Math.max(
            Math[natureStatMultiplier.value > 1 ? "ceil" : "floor"](
              value * natureStatMultiplier.value,
            ),
            1,
          );
      }
  ...
  generateNature(naturePool?: Nature[]): void {
    if (naturePool === undefined) naturePool = Utils.getEnumValues(Nature);
  ...
  generateGender(): void {
    if (this.species.malePercent === null) this.gender = Gender.GENDERLESS;
    else {
      const genderChance = (this.id % 256) * 0.390625;
      if (genderChance < this.species.malePercent) this.gender = Gender.MALE;
  ...
  getMoveset(ignoreOverride?: boolean): PokemonMove[] {
    ...
    if (MOVE_OVERRIDE && this.isPlayer())
      this.moveset[0] = new PokemonMove(
        MOVE_OVERRIDE,
        Math.min(this.moveset[0].ppUsed, allMoves[MOVE_OVERRIDE].pp),
      );
    else if (OPP_MOVE_OVERRIDE && !this.isPlayer())
      this.moveset[0] = new PokemonMove(
        OPP_MOVE_OVERRIDE,
        Math.min(this.moveset[0].ppUsed, allMoves[OPP_MOVE_OVERRIDE].pp),
      );
    if (MOVE_OVERRIDE_2 && this.isPlayer())
      this.moveset[1] = new PokemonMove(
        MOVE_OVERRIDE_2,
        Math.min(this.moveset[1].ppUsed, allMoves[MOVE_OVERRIDE_2].pp),
      );
    else if (OPP_MOVE_OVERRIDE_2 && !this.isPlayer())
      this.moveset[1] = new PokemonMove(
        OPP_MOVE_OVERRIDE_2,
        Math.min(this.moveset[1].ppUsed, allMoves[OPP_MOVE_OVERRIDE_2].pp),
      );
  ...
  getLearnableLevelMoves(): Moves[] {
    return this.getLevelMoves(1, true)
      .map((lm) => lm[1])
      .filter((lm) => !this.moveset.filter((m) => m.moveId === lm).length)
      .filter(
        (move: Moves, i: integer, array: Moves[]) => array.indexOf(move) === i,
      );
  }
  ...
  getTypes(
    ...
    if (includeTeraType) {
      ...
      if (teraType != Type.UNKNOWN) types.push(teraType);
    }
    ...
    if (!types.length || !includeTeraType) {
      if (!ignoreOverride && this.summonData?.types)
        ...
        if (fusionSpeciesForm) {
          if (
            fusionSpeciesForm.type2 !== null &&
            fusionSpeciesForm.type2 !== speciesForm.type1
          )
    ...
    if (
      ...
      (this.getTag(BattlerTagType.IGNORE_FLYING) ||
        this.scene.arena.getTag(ArenaTagType.GRAVITY) ||
        this.getTag(BattlerTagType.GROUNDED))
    ) {
      const flyingIndex = types.indexOf(Type.FLYING);
      if (flyingIndex > -1) types.splice(flyingIndex, 1);
    }
  ...
  isOfType(type: Type, forDefend = false): boolean {
  ...
  getAbility(ignoreOverride?: boolean): Ability {
    ...
    if (abilityId === Abilities.NONE) abilityId = this.species.ability1;
  ...
  hasPassive(): boolean {
    if (
      (PASSIVE_ABILITY_OVERRIDE !== Abilities.NONE && this.isPlayer()) ||
      (OPP_PASSIVE_ABILITY_OVERRIDE !== Abilities.NONE && !this.isPlayer())
    )
      return true;
    return this.passive || this.isBoss();
  }

  canApplyAbility(passive = false): boolean {
    if (passive && !this.hasPassive()) return false;
    const ability = !passive ? this.getAbility() : this.getPassiveAbility();
    if (this.isFusion() && ability.hasAttr(NoFusionAbilityAbAttr)) return false;
    if (this.scene?.arena.ignoreAbilities && ability.isIgnorable) return false;
    if (
    ...
    if (this.isOnField() && !ability.hasAttr(SuppressFieldAbilitiesAbAttr)) {
      ...
      this.scene.getField(true).map((p) => {
        if (
          p.getAbility().hasAttr(SuppressFieldAbilitiesAbAttr) &&
          p.canApplyAbility()
        )
          p.getAbility()
        ...
        if (
          p.getPassiveAbility().hasAttr(SuppressFieldAbilitiesAbAttr) &&
          p.canApplyAbility(true)
        )
          p.getPassiveAbility()
            .getAttrs(SuppressFieldAbilitiesAbAttr)
            .map((a) => a.apply(this, true, suppressed, [ability]));
      });
      if (suppressed.value) return false;
    }
    return (
      (this.hp || ability.isBypassFaint) &&
  ...
  getTeraType(): Type {
    ...
    if (teraModifier) return teraModifier.teraType;
  ...
  isGrounded(): boolean {
    return (
      !this.isOfType(Type.FLYING, true) &&
      this.getAbility().id !== Abilities.LEVITATE
    );
  }
  ...
  getAttackMoveEffectiveness(
    ...
    return (
      !cancelled.value ? typeMultiplier.value : 0
  ...
  getAttackTypeEffectiveness(
    ...
    if (moveType === Type.STELLAR) return this.isTerastallized() ? 2 : 1;
    ...
    let multiplier = types
      .map((defType) => {
        if (source) {
          ...
          if (ignoreImmunity.value) return 1;
        }
    ...
    if (
      this.scene.arena.weather?.weatherType === WeatherType.STRONG_WINDS &&
      !this.scene.arena.weather.isEffectSuppressed(this.scene) &&
      multiplier >= 2 &&
      this.isOfType(Type.FLYING) &&
      getTypeDamageMultiplier(moveType, Type.FLYING) === 2
    )
      multiplier /= 2;
    return multiplier;
  }
  ...
  getMatchupScore(pokemon: Pokemon): number {
    ...
    const outspeed =
      (this.isActive(true)
        ? this.getBattleStat(Stat.SPD, pokemon)
        : this.getStat(Stat.SPD)) <= pokemon.getBattleStat(Stat.SPD, this);
    let atkScore =
      pokemon.getAttackTypeEffectiveness(types[0], this) *
      (outspeed ? 1.25 : 1);
    let defScore =
      1 /
      Math.max(this.getAttackTypeEffectiveness(enemyTypes[0], pokemon), 0.25);
    if (types.length > 1)
      atkScore *= pokemon.getAttackTypeEffectiveness(types[1], this);
    if (enemyTypes.length > 1)
      defScore *=
        1 /
        Math.max(this.getAttackTypeEffectiveness(enemyTypes[1], pokemon), 0.25);
    ...
    if (pokemonEvolutions.hasOwnProperty(this.species.speciesId)) {
      ...
      for (const e of evolutions) {
        if (
          !e.item &&
          this.level >= e.level &&
          (!e.preFormKey || this.getFormKey() === e.preFormKey)
        ) {
          if (
            e.condition === null ||
    ...
    if (
      ...
      for (const fe of fusionEvolutions) {
        if (
          !fe.item &&
          this.level >= fe.level &&
          (!fe.preFormKey || this.getFusionFormKey() === fe.preFormKey)
        ) {
          if (
            fe.condition === null ||
  ...
  getLevelMoves(
    ...
    if (!startingLevel) startingLevel = this.level;
    if (simulateEvolutionChain) {
      ...
      for (let e = 0; e < evolutionChain.length; e++) {
        ...
        const speciesLevelMoves = getPokemonSpeciesForm(
          evolutionChain[e][0] as Species,
          this.formIndex,
        ).getLevelMoves();
        levelMoves.push(
          ...speciesLevelMoves.filter(
            (lm) =>
              (includeEvolutionMoves && !lm[0]) ||
              ((!e || lm[0] > 1) &&
      ...
      levelMoves = levelMoves.filter((lm) => {
        if (uniqueMoves.find((m) => m === lm[1])) return false;
        uniqueMoves.push(lm[1]);
        return true;
      });
    } else levelMoves = this.getSpeciesForm(true).getLevelMoves();
    if (this.fusionSpecies) {
      const evolutionLevelMoves = levelMoves.slice(
        0,
        Math.max(
          levelMoves.findIndex((lm) => !!lm[0]),
          0,
        ),
      );
      ...
      if (includeEvolutionMoves) {
        for (const elm of evolutionLevelMoves.reverse()) levelMoves.unshift(elm);
      }
      for (
        let l = includeEvolutionMoves ? 0 : startingLevel;
        l <= this.level;
        l++
      ) {
        if (l === 1 && startingLevel > 1) l = startingLevel;
    ...
    if (levelMoves) {
      for (const lm of levelMoves) {
        ...
        if ((!includeEvolutionMoves || level) && level < startingLevel)
          continue;
        else if (level > this.level) break;
        ret.push(lm);
      }
    }
  ...
  setMove(moveIndex: integer, moveId: Moves): void {
    ...
    if (this.summonData?.moveset) this.summonData.moveset[moveIndex] = move;
  }
  ...
  trySetShiny(thresholdOverride?: integer): boolean {
    ...
    } else shinyThreshold.value = thresholdOverride;

    this.shiny = (E ^ F) < shinyThreshold.value;
    if ((E ^ F) < 32) console.log("REAL SHINY!!");

    if (this.shiny) this.initShinySparkle();
  ...
  generateFusionSpecies(forStarter?: boolean): void {
    ...
    this.fusionSpecies = this.scene.randomSpecies(
    ...
    if (this.fusionSpecies.malePercent === null)
      ...
      else this.fusionGender = Gender.FEMALE;
    }
  ...
  generateAndPopulateMoveset(): void {
    ...
    if (attackMovePool.length) {
      const randomAttackMove = Utils.randSeedEasedWeightedItem(
        attackMovePool,
        easeType,
      );
      ...
      movePool.splice(
        movePool.findIndex((m) => m === randomAttackMove),
        1,
      );
    }
    ...
    this.scene.triggerPokemonFormChange(
      this,
      SpeciesFormChangeMoveLearnedTrigger,
    );
  }
  ...
  showInfo(): void {
    if (!this.battleInfo.visible) {
      ...
      else this.scene.fieldUI.moveAbove(this.battleInfo, otherBattleInfo);
      this.battleInfo.setX(
        this.battleInfo.x +
          (this.isPlayer() ? 150 : !this.isBoss() ? -150 : -198),
      );
      this.battleInfo.setVisible(true);
      if (this.isPlayer()) this.battleInfo.expMaskRect.x += 150;
      this.scene.tweens.add({
  ...
  getOpponents(): Pokemon[] {
    return (
      (this.isPlayer()
        ...
        : this.scene.getPlayerField()) as Pokemon[]
    ).filter((p) => p.isActive());
  }

  getOpponentDescriptor(): string {
    const opponents = this.getOpponents();
    if (opponents.length === 1) return opponents[0].name;
    return this.isPlayer() ? "the opposing team" : "your team";
  }

  getAlly(): Pokemon {
    return (
      this.isPlayer() ? this.scene.getPlayerField() : this.scene.getEnemyField()
  ...
  apply(source: Pokemon, battlerMove: PokemonMove): HitResult {
    ...
    const typeMultiplier = new Utils.NumberHolder(
      !typeless &&
        (moveCategory !== MoveCategory.STATUS ||
          move
            .getAttrs(StatusMoveTypeImmunityAttr)
            .find((attr) =>
              types.includes((attr as StatusMoveTypeImmunityAttr).immuneType),
            ))
    ...
    if (typeless) typeMultiplier.value = 1;
    if (types.find((t) => move.isTypeImmune(t))) typeMultiplier.value = 0;
    ...
    switch (moveCategory) {
      ...
      case MoveCategory.SPECIAL:
        ...
        if (
          ...
          power.value < 60 &&
          move.priority <= 0 &&
          !move.getAttrs(MultiHitAttr).length &&
        ...
        if (cancelled.value) result = HitResult.NO_EFFECT;
        else {
          const typeBoost = source.findTag(
            (t) =>
              t instanceof TypeBoostTag &&
              (t as TypeBoostTag).boostedType === type,
          ) as TypeBoostTag;
          if (typeBoost) {
          ...
          const arenaAttackTypeMultiplier =
            this.scene.arena.getAttackTypeMultiplier(type, source.isGrounded());
          if (
            this.scene.arena.getTerrainType() === TerrainType.GRASSY &&
            this.isGrounded() &&
            type === Type.GROUND &&
          ...
          if (source.getTag(HelpingHandTag)) power.value *= 1.5;
          let isCritical: boolean;
          const critOnly = new Utils.BooleanHolder(false);
          const critAlways = source.getTag(BattlerTagType.ALWAYS_CRIT);
          applyMoveAttrs(CritOnlyAttr, source, this, move, critOnly);
          if (critOnly.value || critAlways) isCritical = true;
          else {
            ...
            if (applyAbAttrs(BonusCritAbAttr, source, null, bonusCrit)) {
              if (bonusCrit.value) critLevel.value += 1;
            }
            if (source.getTag(BattlerTagType.CRIT_BOOST)) critLevel.value += 2;
            const critChance = [24, 8, 2, 1][
              Math.max(0, Math.min(critLevel.value, 3))
            ];
            isCritical =
              !source.getTag(BattlerTagType.NO_CRIT) &&
              (critChance === 1 || !this.scene.randBattleSeedInt(critChance));
            if (isCritical) {
              const blockCrit = new Utils.BooleanHolder(false);
              applyAbAttrs(BlockCritAbAttr, this, null, blockCrit);
              if (blockCrit.value) isCritical = false;
            }
          }
          const sourceAtk = new Utils.IntegerHolder(
            source.getBattleStat(
          ...
          const targetDef = new Utils.IntegerHolder(
            this.getBattleStat(
          ...
          const criticalMultiplier = new Utils.NumberHolder(
            isCritical ? 1.5 : 1,
          );
          ...
          const isTypeImmune =
            typeMultiplier.value * arenaAttackTypeMultiplier === 0;
          const sourceTypes = source.getTypes();
          const matchesSourceType =
            sourceTypes[0] === type ||
            (sourceTypes.length > 1 && sourceTypes[1] === type);
          ...
          if (!isTypeImmune) {
            damage.value = Math.ceil(
              ((((2 * source.level) / 5 + 2) * power.value * sourceAtk.value) /
                targetDef.value /
                50 +
                2) *
                ...
                ((this.scene.randBattleSeedInt(15) + 85) / 100) *
            ...
            if (
              ...
              const burnDamageReductionCancelled = new Utils.BooleanHolder(
                false,
              );
              applyAbAttrs(
                BypassBurnDamageReductionAbAttr,
            ...
            move
              ...
              .map((hta) => hta as HitsTagAttr)
              .filter((hta) => hta.doubleDamage)
              .forEach((hta) => {
                if (this.getTag(hta.tagType)) damage.value *= 2;
              });
          }

          if (
            this.scene.arena.terrain?.terrainType === TerrainType.MISTY &&
            this.isGrounded() &&
            type === Type.DRAGON
          )
            damage.value = Math.floor(damage.value / 2);
          ...
          if (!result) {
            if (!typeMultiplier.value) result = HitResult.NO_EFFECT;
            else {
              ...
              else if (typeMultiplier.value >= 1) result = HitResult.EFFECTIVE;
              else result = HitResult.NOT_VERY_EFFECTIVE;
            }
          }
          ...
          if (damage.value) {
            ...
            damage.value = this.damageAndUpdate(
              damage.value,
          ...
          if (damage) this.scene.clearPhaseQueueSplice();
        }
        break;
      case MoveCategory.STATUS:
        ...
        if (!typeMultiplier.value)
          this.scene.queueMessage(
            i18next.t("battle:hitResultNoEffect", { pokemonName: this.name }),
          );
        result =
          cancelled.value || !typeMultiplier.value
            ? HitResult.NO_EFFECT
            : HitResult.STATUS;
        break;
    }
  ...
  damage(
    ...
    if (!preventEndure && this.hp - damage <= 0) {
      ...
      if (surviveDamage.value) damage = this.hp - 1;
    }
    ...
    if (this.isFainted()) {
      this.scene.unshiftPhase(
        new FaintPhase(this.scene, this.getBattlerIndex(), preventEndure),
      );
  ...
  isMax(): boolean {
    ...
    return (
      maxForms.includes(this.getFormKey()) ||
      maxForms.includes(this.getFusionFormKey())
    );
  }
  ...
  getTag(
    tagType: BattlerTagType | { new (...args: any[]): BattlerTag },
  ): BattlerTag {
    ...
    tags
      .filter(
        (t) =>
          ...
          (t.lapseType === lapseType && !t.lapse(this, lapseType)) ||
          (lapseType === BattlerTagLapseType.TURN_END && t.turnCount < 1),
      )
      .forEach((t) => {
        t.onRemove(this);
  ...
  findAndRemoveTags(tagFilter: (tag: BattlerTag) => boolean): boolean {
    if (!this.summonData) return false;
    const tags = this.summonData.tags;
  ...
  removeTagsBySourceId(sourceId: integer): void {
    this.findAndRemoveTags(
      (t) => t.isSourceLinked() && t.sourceId === sourceId,
    );
  }
  ...
  transferSummon(source: Pokemon): void {
    ...
    for (const tag of source.summonData.tags) this.summonData.tags.push(tag);
    if (
      this instanceof PlayerPokemon &&
      source.summonData.battleStats.find((bs) => bs === 6)
    )
  ...
  getLastXMoves(turnCount?: integer): TurnMove[] {
    ...
    return moveHistory
      .slice(
        turnCount >= 0 ? Math.max(moveHistory.length - (turnCount || 1), 0) : 0,
        moveHistory.length,
      )
      .reverse();
  }
  ...
  changeForm(formChange: SpeciesFormChange): Promise<void> {
    return new Promise((resolve) => {
      ...
      this.loadAssets().then(() => {
        ...
        Promise.all([this.updateInfo(), this.scene.updateFieldScale()]).then(
          () => resolve(),
        );
      });
    });
      ...
      scene.time.delayedCall(Utils.fixedInt(Math.ceil(duration * 0.4)), () => {
        try {
          ...
          SoundFade.fadeIn(
            ...
            fusionCry,
            Utils.fixedInt(Math.ceil(duration * 0.2)),
            scene.masterVolume * scene.seVolume,
            0,
          );
        } catch (err) {
          console.error(err);
  ...
  faintCry(callback: Function): void {
    if (this.fusionSpecies) return this.fusionFaintCry(callback);
    ...
    this.scene.time.delayedCall(Utils.fixedInt(3000), () => {
      if (!faintCryTimer || !this.scene) return;
      if (cry?.isPlaying) cry.stop();
      faintCryTimer.destroy();
      if (callback) callback();
    });
  }
  ...
  private fusionFaintCry(callback: Function): void {
    ...
    let fusionCry = this.scene.playSound(
      this.getFusionSpeciesForm().getCryKey(this.fusionFormIndex),
      { rate: rate },
    ) as AnySound;
    fusionCry.stop();
    duration = Math.min(duration, fusionCry.totalDuration * 1000);
      ...
      callback: () => {
        ...
        if (i === transitionIndex) {
          ...
          SoundFade.fadeIn(
            ...
            fusionCry,
            Utils.fixedInt(Math.ceil((duration / rate) * 0.2)),
            this.scene.masterVolume * this.scene.seVolume,
            0,
          );
        }
        rate *= 0.99;
        if (cry && !cry.pendingRemove) cry.setRate(rate);
        if (fusionCry && !fusionCry.pendingRemove) fusionCry.setRate(rate);
        if (
    ...
    this.scene.time.delayedCall(Utils.fixedInt(3000), () => {
      if (!faintCryTimer || !this.scene) return;
      if (cry?.isPlaying) cry.stop();
      if (fusionCry?.isPlaying) fusionCry.stop();
      faintCryTimer.destroy();
      if (callback) callback();
    });
  }
  ...
  canSetStatus(
    ...
    switch (effect) {
      ...
      case StatusEffect.SLEEP:
        if (
          this.isGrounded() &&
          this.scene.arena.terrain?.terrainType === TerrainType.ELECTRIC
        )
          return false;
        break;
      ...
      case StatusEffect.BURN:
        if (this.isOfType(Type.FIRE)) return false;
        break;
    }
  ...
  trySetStatus(
    ...
    if (!this.canSetStatus(effect, asPhase)) return false;
  ...
  resetSummonData(): void {
    ...
    if (!this.battleData) this.resetBattleData();
  ...
  tint(color: number, alpha?: number, duration?: integer, ease?: string) {
    ...
    } else tintSprite.setAlpha(alpha);
  }
  ...
  sparkle(): void {
    if (this.shinySparkle) {
      this.shinySparkle.play(
        `sparkle${this.variant ? `_${this.variant + 1}` : ""}`,
      );
  ...
  updateFusionPalette(ignoreOveride?: boolean): void {
    if (!this.getFusionSpeciesForm(ignoreOveride)) {
      [this.getSprite(), this.getTintSprite()].map((s) => {
        s.pipelineData[
          `spriteColors${
            ignoreOveride && this.summonData?.speciesForm ? "Base" : ""
          }`
        ] = [];
        s.pipelineData[
          `fusionSpriteColors${
    ...
    [canvas, backCanvas, fusionCanvas, fusionBackCanvas].forEach(
      (canv: HTMLCanvasElement, c: integer) => {
          ...
          for (let c = keys.length - 1; c >= 0; c--) {
            ...
            for (let c2 = 0; c2 < c; c2++) {
              ...
              if (diff < 30 || diff >= 330) {
                ...
                else mappedColors.set(keys[c], [keys[c2]]);
                break;
              }
            }
          }
...
export class PlayerPokemon extends Pokemon {
  ...
  constructor(
    ...
    if (!dataSource) this.generateAndPopulateMoveset();
  ...
  generateCompatibleTms(): void {
    ...
    for (const tm of tms) {
      ...
      if (reverseCompatibleTms.indexOf(moveId) > -1) compatible = !compatible;
      if (compatible) this.compatibleTms.push(moveId);
    }
  }
  ...
  tryPopulateMoveset(moveset: StarterMoveset): boolean {
    if (
      !this.getSpeciesForm().validateStarterMoveset(
        moveset,
        this.scene.gameData.starterData[this.species.getRootSpeciesId()]
          .eggMoves,
      )
    )
      return false;
  ...
  switchOut(
    ...
    return new Promise((resolve) => {
      this.resetTurnData();
      if (!batonPass) this.resetSummonData();
      this.hideInfo();
      ...
      this.scene.ui.setMode(
        ...
        (slotIndex: integer, option: PartyOption) => {
          if (
            slotIndex >= this.scene.currentBattle.getBattlerCount() &&
            slotIndex < 6
          )
            this.scene.unshiftPhase(
  ...
  addFriendship(friendship: integer): void {
    ...
    const fusionStarterSpeciesId = this.isFusion()
      ? this.fusionSpecies.getRootSpeciesId()
      : 0;
    const starterData = [
      this.scene.gameData.starterData[starterSpeciesId],
      fusionStarterSpeciesId
        ? this.scene.gameData.starterData[fusionStarterSpeciesId]
        : null,
    ].filter((d) => d);
    const amount = new Utils.IntegerHolder(friendship);
    const starterAmount = new Utils.IntegerHolder(
      Math.floor(
        (friendship * (this.scene.gameMode.isClassic ? 2 : 1)) /
    ...
    if (amount.value > 0) {
      ...
      starterData.forEach((sd: StarterDataEntry, i: integer) => {
        ...
        if (
          sd.friendship >=
          getStarterValueFriendshipCap(speciesStarters[speciesId])
        ) {
  ...
  evolve(evolution: SpeciesFormEvolution): Promise<void> {
    return new Promise((resolve) => {
      ...
      const isFusion = evolution instanceof FusionSpeciesFormEvolution;
      if (!isFusion) this.species = getPokemonSpecies(evolution.speciesId);
      else this.fusionSpecies = getPokemonSpecies(evolution.speciesId);
      if (evolution.preFormKey !== null) {
        const formIndex = Math.max(
          (!isFusion ? this.species : this.fusionSpecies).forms.findIndex(
            (f) => f.formKey === evolution.evoFormKey,
          ),
          0,
        );
        if (!isFusion) this.formIndex = formIndex;
        else this.fusionFormIndex = formIndex;
      }
      this.generateName();
      if (!isFusion) {
        const abilityCount = this.getSpeciesForm().getAbilityCount();
  ...
  fuse(pokemon: PlayerPokemon): Promise<void> {
    return new Promise((resolve) => {
      ...
      const newHpPercent =
        (pokemon.hp / pokemon.stats[Stat.HP] + this.hp / this.stats[Stat.HP]) /
        2;
      ...
      if (partyMemberIndex > fusedPartyMemberIndex) partyMemberIndex--;
      const fusedPartyMemberHeldModifiers = this.scene.findModifiers(
        (m) =>
          m instanceof PokemonHeldItemModifier &&
          (m as PokemonHeldItemModifier).pokemonId === pokemon.id,
        true,
      ) as PokemonHeldItemModifier[];
...
export class EnemyPokemon extends Pokemon {
  ...
  constructor(
    ...
    super(
      ...
      dataSource?.gender,
      dataSource ? dataSource.shiny : false,
      dataSource ? dataSource.variant : undefined,
      null,
    ...
    if (!dataSource) {
      ...
      if (this.shiny) {
        ...
        if (OPP_VARIANT_OVERRIDE) this.variant = OPP_VARIANT_OVERRIDE;
      }
  ...
  initBattleInfo(): void {
    if (!this.battleInfo) {
      ...
      this.battleInfo.initInfo(this);
    } else this.battleInfo.updateBossSegments(this);
  }

  setBoss(boss = true, bossSegments: integer = 0): void {
    if (boss) {
      this.bossSegments =
        bossSegments ||
  ...
  getNextMove(): QueuedMove {
    ...
    if (movePool.length) {
      ...
      switch (this.aiType) {
        case AiType.RANDOM:
          const moveId =
            movePool[this.scene.randBattleSeedInt(movePool.length)].moveId;
        ...
        case AiType.SMART:
          ...
          for (const m in movePool) {
            ...
            for (const mt of moveTargets[move.id]) {
              ...
              if (mt === BattlerIndex.ATTACKER) break;
              ...
              if (
                ...
                if (target.isPlayer() !== this.isPlayer()) {
                  ...
                  if (this.isOfType(moveType)) targetScore *= 1.5;
          ...
          if (this.aiType === AiType.SMART_RANDOM) {
            while (
              ...
              this.scene.randBattleSeedInt(8) >= 5
            )
              r++;
  ...
  getNextTargets(moveId: Moves): BattlerIndex[] {
    ...
    if (moveTargets.multiple) return targets.map((p) => p.getBattlerIndex());
  ...
  handleBossSegmentCleared(segmentIndex: integer): void {
    while (segmentIndex - 1 < this.bossSegmentIndex) {
      ...
      const statWeights = new Array()
        .fill(battleStats.length)
        .filter((bs: BattleStat) => this.summonData.battleStats[bs] < 6)
        .map((bs: BattleStat) => this.getStat(bs + 1));
      ...
      switch (segmentIndex) {
        case 1:
          if (this.bossSegments >= 3) statLevels++;
          break;
        case 2:
          if (this.bossSegments >= 5) statLevels++;
          break;
      }
  ...
  heal(amount: integer): integer {
    if (this.isBoss()) {
      ...
      const segmentBypassCount = Math.floor(
        amountRatio / (1 / this.bossSegments),
      );
      const segmentSize = this.getMaxHp() / this.bossSegments;
      for (let s = 1; s < this.bossSegments; s++) {
        ...
        if (this.hp <= Math.round(hpThreshold)) {
          const healAmount = Math.min(
            amount,
            this.getMaxHp() - this.hp,
            Math.round(
              hpThreshold + segmentSize * segmentBypassCount - this.hp,
            ),
          );
          this.hp += healAmount;
          return healAmount;
        } else if (s >= this.bossSegmentIndex) return super.heal(amount);
      }
    }
...
export class PokemonMove {
  ...
  getMovePp(): integer {
    return (
      this.getMove().pp +
      this.ppUp * Math.max(Math.floor(this.getMove().pp / 5), 1)
    );
  }

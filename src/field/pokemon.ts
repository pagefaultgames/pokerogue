...
export default abstract class Pokemon extends Phaser.GameObjects.Container {
            ...
            const populateVariantColors = (key: string, back: boolean = false): Promise<void> => {
              return new Promise((resolve) => {
                ...
                const useExpSprite =
                  ...
                  this.scene.hasExpSprite(this.getBattleSpriteKey(back, ignoreOverride));
                ...
                if (variantSet && variantSet[this.variant] === 1) {
                  ...
                  this.scene
                    .cachedFetch(`./images/pokemon/variant/${useExpSprite ? "exp/" : ""}${battleSpritePath}.json`)
                    .then((res) => {
  ...
  getEffectiveStat(stat: EffectiveStat, opponent?: Pokemon, move?: Move, isCritical: boolean = false): integer {
    ...
    switch (stat) {
      ...
      case Stat.SPD:
        ...
        if (
          (this.isPlayer() && this.scene.arena.getTagOnSide(ArenaTagType.TAILWIND, ArenaTagSide.PLAYER)) ||
    ...
    for (const s of PERMANENT_STATS) {
      ...
      if (s === Stat.HP) {
        ...
        if (natureStatMultiplier.value !== 1) {
          value = Math.max(
            Math[natureStatMultiplier.value > 1 ? "ceil" : "floor"](value * natureStatMultiplier.value),
            1,
          );
        }
      }
  ...
  isGrounded(): boolean {
    return (
      ...
      (!this.isOfType(Type.FLYING, true, true) &&
        ...
        !this.getTag(BattlerTagType.MAGNET_RISEN) &&
  ...
  getLevelMoves(
    ...
    if (simulateEvolutionChain) {
      ...
      levelMoves = this.getSpeciesForm(true)
        .getLevelMoves()
        .filter((lm) => (includeEvolutionMoves && lm[0] === 0) || (includeRelearnerMoves && lm[0] === -1) || lm[0] > 0);
    }
    if (this.fusionSpecies) {
      if (simulateEvolutionChain) {
        ...
        levelMoves.push(
          ...this.getFusionSpeciesForm(true)
            .getLevelMoves()
            .filter(
              (lm) => (includeEvolutionMoves && lm[0] === 0) || (includeRelearnerMoves && lm[0] === -1) || lm[0] > 0,
            ),
        );
      }
    }
  ...
  generateAndPopulateMoveset(): void {
    ...
    if (this.hasTrainer()) {
      ...
      movePool = movePool.map((m) => [
        m[0],
        m[1] * (!!allMoves[m[0]].hasAttr(ChargeAttr) || !!allMoves[m[0]].hasAttr(RechargeAttr) ? 0.7 : 1),
      ]);
    }
        ...
        movePool = baseWeights
          ...
          .map((m) => [
            ...
            this.moveset.some(
              ...
              ? Math.ceil(Math.sqrt(m[1]))
              : allMoves[m[0]].category !== MoveCategory.STATUS
                ? Math.ceil(
                    (m[1] /
                      Math.max(Math.pow(4, this.moveset.filter((mo) => mo?.getMove().power! > 1).length) / 8, 0.5)) *
      ...
      case MoveCategory.SPECIAL:
        ...
        if (cancelled.value) {
          ...
          const typeBoost = source.findTag(
            (t) => t instanceof TypeBoostTag && t.boostedType === moveType,
          ) as TypeBoostTag;
          if (typeBoost?.oneUse) {
          ...
          const sourceAtk = new Utils.IntegerHolder(
            source.getEffectiveStat(isPhysical ? Stat.ATK : Stat.SPATK, this, undefined, isCritical),
          );
          const targetDef = new Utils.IntegerHolder(
            this.getEffectiveStat(isPhysical ? Stat.DEF : Stat.SPDEF, source, move, isCritical),
          );
          ...
          const matchesSourceType =
            sourceTypes[0] === moveType || (sourceTypes.length > 1 && sourceTypes[1] === moveType);
  ...
  getLastXMoves(turnCount: integer = 0): TurnMove[] {
    ...
    return moveHistory
      .slice(turnCount >= 0 ? Math.max(moveHistory.length - (turnCount || 1), 0) : 0, moveHistory.length)
      .reverse();
  }
  ...
  canSetStatus(
    ...
    if (
      sourcePokemon &&
      sourcePokemon !== this &&
      this.scene.arena.getTagOnSide(ArenaTagType.SAFEGUARD, defendingSide)
    ) {
      return false;
    }
  ...
  trySetStatus(
    ...
    if (asPhase) {
      this.scene.unshiftPhase(
        new ObtainStatusEffectPhase(this.scene, this.getBattlerIndex(), effect, cureTurn, sourceText!, sourcePokemon!),
...
export class PlayerPokemon extends Pokemon {
        ...
        (slotIndex: integer, option: PartyOption) => {
          if (slotIndex >= this.scene.currentBattle.getBattlerCount() && slotIndex < 6) {
            this.scene.prependToPhase(
              new SwitchSummonPhase(this.scene, this.getFieldIndex(), slotIndex, false, batonPass),
              MoveEndPhase,
            );
          }
  ...
  addFriendship(friendship: integer): void {
    ...
    const starterAmount = new Utils.IntegerHolder(
      Math.floor(
        (friendship * (this.scene.gameMode.isClassic && friendship > 0 ? 2 : 1)) / (fusionStarterSpeciesId ? 2 : 1),
      ),
    );
    if (amount.value > 0) {
          ...
          if (slotIndex >= 0 && slotIndex < 6) {
            ...
            if (this.scene.currentBattle.double && this.scene.getParty().length > 1) {
              ...
              if (slotIndex <= 1) {
                ...
                this.scene.unshiftPhase(
                  new SwitchSummonPhase(this.scene, pokemon.getFieldIndex(), slotIndex, false, false, true),
                );
                this.scene.unshiftPhase(new ToggleDoublePositionPhase(this.scene, true));
              } else if (allyPokemon.isFainted()) {
                // Revived party pokemon, and ally pokemon is fainted
                this.scene.unshiftPhase(
                  new SwitchSummonPhase(this.scene, allyPokemon.getFieldIndex(), slotIndex, false, false, true),
                );
                this.scene.unshiftPhase(new ToggleDoublePositionPhase(this.scene, true));
              }
            }
    ...
    return new Promise((resolve) => {
      ...
      Promise.allSettled(transferModifiers).then(() => {
        this.scene.updateModifiers(true, true).then(() => {
          ...
          pokemon
            .getMoveset(true)
            .map((m) => this.scene.unshiftPhase(new LearnMovePhase(this.scene, newPartyMemberIndex, m!.getMove().id))); // TODO: is the bang correct?
          pokemon.destroy();
          this.updateFusionPalette();
          resolve();
        });
  ...
  copyMoveset(): PokemonMove[] {
    ...
    this.moveset.forEach((move) => newMoveset.push(new PokemonMove(move!.moveId, 0, move!.ppUp, move!.virtual))); // TODO: are those bangs correct?
      ...
      for (let s = 1; s < this.bossSegments; s++) {
        ...
        if (this.hp <= Math.round(hpThreshold)) {
          const healAmount = Math.min(
            ...
            Math.round(hpThreshold + segmentSize * segmentBypassCount - this.hp),
          );
          this.hp += healAmount;
          return healAmount;

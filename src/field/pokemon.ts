...
export default abstract class Pokemon extends Phaser.GameObjects.Container {
  ...
  getAttackTypeEffectiveness(moveOrType: Move | Type, source?: Pokemon, ignoreStrongWinds: boolean = false, simulated: boolean = true): TypeDamageMultiplier {
    ...
    if (!ignoreStrongWinds && this.scene.arena.weather?.weatherType === WeatherType.STRONG_WINDS && !this.scene.arena.weather.isEffectSuppressed(this.scene) && this.isOfType(Type.FLYING) && getTypeDamageMultiplier(moveType, Type.FLYING, this.scene.gameMode.isInverseBattleChallenge()) === 2) {
      multiplier /= 2;
      if (!simulated) {

...
export default abstract class Pokemon extends Phaser.GameObjects.Container {
  ...
  getCritStage(source: Pokemon, move: Move): number {
    ...
    if (critBoostTag) {
      ...
      critStage.value += critBoostTag instanceof DragonCheerTag && !critBoostTag.typesOnAdd.includes(PokemonType.DRAGON)
        ? 1
        : 2;
    }
  ...
  */
  getCriticalHitResult(source: Pokemon, move: Move): boolean {
    ...
    const critChance = [24, 8, 2, 1][
      Phaser.Math.Clamp(this.getCritStage(source, move), 0, 3)
    ];

...
function doDefaultPbOpenParticles(
  scene: BattleScene,
  x: number,
  y: number,
  radius: number,
) {
  const pbOpenParticlesFrameNames = scene.anims.generateFrameNames(
    "pb_particles",
    { start: 0, end: 3, suffix: ".png" },
  );
  scene.anims.create({
...
function doMbOpenParticles(scene: BattleScene, x: number, y: number) {
  ...
  for (let j = 0; j < 2; j++) {
    for (let i = 0; i < 8; i++)
      particles.push(
        doFanOutParticle(scene, i * 32, x, y, j ? 1 : 2, j ? 2 : 1, 8, 4),
      );
...
function doFanOutParticle(
  ...
  const updateParticle = () => {
    if (!particle.scene) return particleTimer.remove();

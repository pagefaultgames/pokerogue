...
export default class PokemonData {
  ...
  constructor(source: Pokemon | any, forHistory = false) {
    const sourcePokemon =
      source instanceof Pokemon ? (source as Pokemon) : null;
    this.id = source.id;
    this.player = sourcePokemon ? sourcePokemon.isPlayer() : source.player;
    this.species = sourcePokemon
      ? sourcePokemon.species.speciesId
      : source.species;
    this.formIndex = Math.max(
      Math.min(
        source.formIndex,
        getPokemonSpecies(this.species).forms.length - 1,
      ),
      0,
    );
    ...
    this.exp = source.exp;
    if (!forHistory) this.levelExp = source.levelExp;
    this.gender = source.gender;
    if (!forHistory) this.hp = source.hp;
    ...
    this.luck =
      source.luck !== undefined
        ...
        : source.shiny
          ? source.variant + 1
          : 0;
    if (!forHistory) this.pauseEvolutions = !!source.pauseEvolutions;
    ...
    if (sourcePokemon) {
      ...
      if (!forHistory) {
        ...
        if (this.player) this.summonData = sourcePokemon.summonData;
      }
    } else {
      this.moveset = (
        source.moveset || [
      ...
      if (!forHistory) {
        this.status = source.status
          ? new Status(
              ...
              source.status.turnCount,
              source.status.cureTurn,
            )
          : undefined;
      }
  ...
  toPokemon(
    ...
    if (this.summonData) ret.primeSummonData(this.summonData);
    return ret;
  }
}

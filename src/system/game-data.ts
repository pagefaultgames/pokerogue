...
export function getDataTypeKey(
...
export class GameData {
  ...
  public saveSystem(): Promise<boolean> {
    return new Promise<boolean>((resolve) => {
      ...
      if (!bypassLogin) {
        Utils.apiPost(
          `savedata/update?datatype=${GameDataType.SYSTEM}`,
          systemData,
          undefined,
          true,
        )
  ...
  public loadSystem(): Promise<boolean> {
    return new Promise<boolean>((resolve) => {
        ...
        try {
          ...
          if (initStarterData) {
            ...
            Object.keys(systemData.starterData).forEach((sd) => {
              if (
                systemData.dexData[sd].caughtAttr &&
      ...
      } else handleSystemData(atob(localStorage.getItem("data")));
    });
  }

  private parseSystemData(dataStr: string): SystemSaveData {
    return JSON.parse(dataStr, (k: string, v: any) => {
      if (k === "gameStats") return new GameStats(v);
      else if (k === "eggs") {
        const ret: EggData[] = [];
        if (v === null) v = [];
        for (const e of v) ret.push(new EggData(e));
        return ret;
      }
  ...
  public saveSetting(setting: Setting, valueIndex: integer): boolean {
    ...
    Object.keys(settingDefaults).forEach((s) => {
      if (s === setting) settings[s] = valueIndex;
    });
  ...
  private loadSettings(): boolean {
    ...
    if (!localStorage.hasOwnProperty("settings")) return false;
  ...
  public saveTutorialFlag(tutorial: Tutorial, flag: boolean): boolean {
    ...
    Object.keys(Tutorial)
      ...
      .forEach((t) => {
        ...
        if (key === tutorial) tutorials[key] = flag;
        else tutorials[key] ??= false;
      });
  ...
  public getTutorialFlags(): TutorialFlags {
    ...
    if (!localStorage.hasOwnProperty("tutorials")) return ret;
  ...
  saveSession(
    ...
    return new Promise<boolean>((resolve) => {
      Utils.executeIf(!skipVerification, updateUserInfo).then((success) => {
        if (success !== null && !success) return resolve(false);

        const sessionData = this.getSessionSaveData(scene);

        if (!bypassLogin) {
          Utils.apiPost(
            `savedata/update?datatype=${GameDataType.SESSION}&slot=${scene.sessionSlotId}&trainerId=${this.trainerId}&secretId=${this.secretId}`,
  ...
  getSession(slotId: integer): Promise<SessionSaveData> {
    return new Promise(async (resolve, reject) => {
      if (slotId < 0) return resolve(null);
      ...
      if (!bypassLogin) {
        Utils.apiFetch(
          `savedata/get?datatype=${GameDataType.SESSION}&slot=${slotId}`,
          true,
        )
          .then((response) => response.text())
          .then(async (response) => {
        ...
        if (sessionData) await handleSessionData(atob(sessionData));
      ...
      try {
        const initSessionFromData = async (sessionData) => {
          ...
          for (const modifierData of sessionData.modifiers) {
            ...
            if (modifier) scene.addModifier(modifier, true);
          }
          ...
          for (const enemyModifierData of sessionData.enemyModifiers) {
            ...
            if (modifier) scene.addEnemyModifier(modifier, true);
          }
        ...
        if (sessionData) initSessionFromData(sessionData);
        else {
  ...
  deleteSession(slotId: integer): Promise<boolean> {
    return new Promise<boolean>((resolve) => {
      ...
      updateUserInfo().then((success) => {
        if (success !== null && !success) return resolve(false);
        Utils.apiFetch(
          `savedata/delete?datatype=${GameDataType.SESSION}&slot=${slotId}`,
          true,
        )
          .then((response) => {
            if (response.ok) {
  ...
  tryClearSession(
    ...
    return new Promise<[boolean, boolean]>((resolve) => {
      ...
      updateUserInfo().then((success) => {
        if (success !== null && !success) return resolve([false, false]);
        const sessionData = this.getSessionSaveData(scene);
        Utils.apiPost(
          `savedata/clear?slot=${slotId}&trainerId=${this.trainerId}&secretId=${this.secretId}`,
          JSON.stringify(sessionData),
          undefined,
          true,
        )
          .then((response) => {
            if (response.ok) loggedInUser.lastSessionSlot = -1;
          ...
          .then((jsonResponse) => {
            ...
            if (
              jsonResponse &&
              jsonResponse.error.startsWith("session out of date")
            ) {
              this.scene.clearPhaseQueue();
              this.scene.unshiftPhase(new ReloadSessionPhase(this.scene));
            }
  ...
  parseSessionData(dataStr: string): SessionSaveData {
    return JSON.parse(dataStr, (k: string, v: any) => {
      ...
      if (k === "party" || k === "enemyParty") {
        ...
        if (v === null) v = [];
        for (const pd of v) ret.push(new PokemonData(pd));
        return ret;
      }

      if (k === "trainer") return v ? new TrainerData(v) : null;

      if (k === "modifiers" || k === "enemyModifiers") {
        const player = k === "modifiers";
        const ret: PersistentModifierData[] = [];
        if (v === null) v = [];
        for (const md of v) ret.push(new PersistentModifierData(md, player));
        return ret;
      }

      if (k === "arena") return new ArenaData(v);

      return v;
    }) as SessionSaveData;
  }

  public tryExportData(
    dataType: GameDataType,
    slotId: integer = 0,
  ): Promise<boolean> {
    return new Promise<boolean>((resolve) => {
      ...
      if (!bypassLogin && dataType < GameDataType.SETTINGS) {
        Utils.apiFetch(
          `savedata/get?datatype=${dataType}${
            dataType === GameDataType.SESSION ? `&slot=${slotId}` : ""
          }`,
          true,
        )
          .then((response) => response.text())
                ...
                () => {
                  if (!bypassLogin && dataType < GameDataType.SETTINGS) {
                    updateUserInfo().then((success) => {
                      ...
                      Utils.apiPost(
                        `savedata/update?datatype=${dataType}${
  ...
  private initDexData(): void {
    ...
    for (let ds = 0; ds < defaultStarterSpecies.length; ds++) {
      ...
      for (const i in entry.ivs) entry.ivs[i] = 10;
    }
  ...
  setPokemonSeen(
    ...
    if (incrementCount) {
      ...
      if (!trainer && pokemon.isShiny()) this.gameStats.shinyPokemonSeen++;
    }
  }
    ...
    return new Promise<void>((resolve) => {
      ...
      if (incrementCount) {
        if (!fromEgg) {
          ...
          if (pokemon.isShiny()) this.gameStats.shinyPokemonCaught++;
        } else {
          ...
          if (pokemon.isShiny()) this.gameStats.shinyPokemonHatched++;
        }

        if (
          !hasPrevolution &&
          (!pokemon.scene.gameMode.isDaily || hasNewAttr || fromEgg)
        )
  ...
  incrementRibbonCount(
    ...
    if (ribbonsInStats >= 100) this.scene.validateAchv(achvs._100_RIBBONS);
    if (ribbonsInStats >= 75) this.scene.validateAchv(achvs._75_RIBBONS);
    if (ribbonsInStats >= 50) this.scene.validateAchv(achvs._50_RIBBONS);
    if (ribbonsInStats >= 25) this.scene.validateAchv(achvs._25_RIBBONS);
    if (ribbonsInStats >= 10) this.scene.validateAchv(achvs._10_RIBBONS);
  ...
  setEggMoveUnlocked(
    ...
    return new Promise<boolean>((resolve) => {
      ...
      this.scene.playSound("level_up_fanfare");
      this.scene.ui.showText(
        `${eggMoveIndex === 3 ? "Rare " : ""}Egg Move unlocked: ${
          allMoves[speciesEggMoves[speciesId][eggMoveIndex]].name
        }`,
        null,
        () => resolve(true),
        null,
  ...
  updateSpeciesDexIvs(speciesId: Species, ivs: integer[]): void {
    ...
    do {
      ...
      for (let i = 0; i < dexIvs.length; i++) {
        if (dexIvs[i] < ivs[i]) dexIvs[i] = ivs[i];
      }
  ...
  getSpeciesCount(dexEntryPredicate: (entry: DexEntry) => boolean): integer {
    ...
    for (const s of dexKeys) {
      if (dexEntryPredicate(this.dexData[s])) speciesCount++;
    }
    return speciesCount;
  }
  ...
  getStarterCount(dexEntryPredicate: (entry: DexEntry) => boolean): integer {
    ...
    for (const s of starterKeys) {
      ...
      if (dexEntryPredicate(starterDexEntry)) starterCount++;
    }
    return starterCount;
  }
  ...
  getSpeciesDefaultNature(species: PokemonSpecies): Nature {
    const dexEntry = this.dexData[species.speciesId];
    for (let n = 0; n < 25; n++) {
      if (dexEntry.natureAttr & Math.pow(2, n + 1)) return n as Nature;
    }
    return 0 as Nature;
  }
  ...
  getNaturesForAttr(natureAttr: integer): Nature[] {
    const ret: Nature[] = [];
    for (let n = 0; n < 25; n++) {
      if (natureAttr & Math.pow(2, n + 1)) ret.push(n);
    }
    return ret;
  }
  ...
  getFormIndex(attr: bigint): integer {
    if (!attr || attr < DexAttr.DEFAULT_FORM) return 0;
    let f = 0;
    while (!(attr & this.getFormAttr(f))) f++;
    return f;
  }
  ...
  consolidateDexData(dexData: DexData): void {
    for (const k of Object.keys(dexData)) {
      ...
      if (!entry.hasOwnProperty("hatchedCount")) entry.hatchedCount = 0;
      if (
        !entry.hasOwnProperty("natureAttr") ||
        (entry.caughtAttr && !entry.natureAttr)
      )
        entry.natureAttr =
          this.defaultDexData[k].natureAttr ||
          Math.pow(2, Utils.randInt(25, 1));
    }
  }
      ...
      for (const s of starterIds) {
        ...
        if (variantData[s]) {
          ...
          if (dexData[s].caughtAttr & DexAttr.VARIANT_3 && !seenVariant3)
            dexData[s].caughtAttr ^= DexAttr.VARIANT_3;
          starterData[s].abilityAttr =
            (tempCaughtAttr & DexAttr.DEFAULT_VARIANT
              ? AbilityAttr.ABILITY_1
              : 0) |
            (tempCaughtAttr & DexAttr.VARIANT_2 && species.ability2
  ...
  fixLegendaryStats(systemData: SystemSaveData): void {
    ...
    allSpecies
      ...
      .forEach((s) => {
        ...
        systemData.gameStats.subLegendaryPokemonHatched +=
          dexEntry.hatchedCount;

...
export class DamageAchv extends Achv {
  ...
  constructor(localizationKey: string, name: string, damageAmount: integer, iconImage: string, score: integer) {
    super(
      ...
      iconImage,
      score,
      (_scene: BattleScene, args: any[]) => (args[0] as Utils.NumberHolder).value >= this.damageAmount,
    );
...
export class HealAchv extends Achv {
  ...
  constructor(localizationKey: string, name: string, healAmount: integer, iconImage: string, score: integer) {
    super(
      ...
      iconImage,
      score,
      (_scene: BattleScene, args: any[]) => (args[0] as Utils.NumberHolder).value >= this.healAmount,
    );
...
export const achvs = {
  ...
  UNEVOLVED_CLASSIC_VICTORY: new Achv(
    ...
    "eviolite",
    175,
    (c) => c.getParty().some((p) => p.getSpeciesForm(true).speciesId in pokemonEvolutions),
  ),

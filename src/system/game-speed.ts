...
export function initGameSpeed() {
  ...
  const transformValue = (value: number | Utils.FixedInt): number => {
    if (value instanceof Utils.FixedInt) return (value as Utils.FixedInt).value;
    return thisArg.gameSpeed === 1
      ? value
      : Math.ceil((value /= thisArg.gameSpeed));
  };
  ...
  this.time.addEvent = function (
    ...
    if (config.delay) config.delay = transformValue(config.delay);
  ...
  this.tweens.add = function (
    ...
    if (config.duration) config.duration = transformValue(config.duration);
    if (config.delay) config.delay = transformValue(config.delay);
    if (config.repeatDelay)
      config.repeatDelay = transformValue(config.repeatDelay);
    if (config.loopDelay) config.loopDelay = transformValue(config.loopDelay);
    if (config.hold) config.hold = transformValue(config.hold);
  ...
  this.tweens.chain = function (
    ...
    if (config.tweens) {
      config.tweens.forEach((t) => {
        if (t.duration) t.duration = transformValue(t.duration);
        if (t.delay) t.delay = transformValue(t.delay);
        if (t.repeatDelay) t.repeatDelay = transformValue(t.repeatDelay);
        if (t.loopDelay) t.loopDelay = transformValue(t.loopDelay);
        if (t.hold) t.hold = transformValue(t.hold);
      });
    }
    return originalTweensChain.apply(this, [config]);
  };
  const originalAddCounter = this.tweens.addCounter;
  this.tweens.addCounter = function (

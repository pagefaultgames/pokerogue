...
export class UiInputs {
  ...
  getActionsKeyDown(): ActionKeys {
    ...
    actions[Button.CYCLE_SHINY] = () =>
      this.buttonCycleOption(Button.CYCLE_SHINY);
    actions[Button.CYCLE_FORM] = () =>
      this.buttonCycleOption(Button.CYCLE_FORM);
    actions[Button.CYCLE_GENDER] = () =>
      this.buttonCycleOption(Button.CYCLE_GENDER);
    actions[Button.CYCLE_ABILITY] = () =>
      this.buttonCycleOption(Button.CYCLE_ABILITY);
    actions[Button.CYCLE_NATURE] = () =>
      this.buttonCycleOption(Button.CYCLE_NATURE);
    actions[Button.CYCLE_VARIANT] = () =>
      this.buttonCycleOption(Button.CYCLE_VARIANT);
  ...
  buttonTouch(): void {
    this.scene.ui.processInput(Button.SUBMIT) ||
  ...
  buttonMenu(): void {
    if (this.scene.disableMenu) return;
    ...
    if (up) {
      if (this.scene.gameSpeed < 5) {
        this.scene.gameData.saveSetting(
          Setting.Game_Speed,
          settingOptions[Setting.Game_Speed].indexOf(
            `${this.scene.gameSpeed}x`,
          ) + 1,
        );

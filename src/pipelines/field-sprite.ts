...
export default class FieldSpritePipeline extends Phaser.Renderer.WebGL.Pipelines
  .MultiPipeline {
  constructor(
    game: Phaser.Game,
  ...
  onBind(gameObject: Phaser.GameObjects.GameObject): void {
    ...
    this.set3fv(
      "terrainColor",
      getTerrainColor(scene.arena.terrain?.terrainType || TerrainType.NONE).map(
        (c) => c / 255,
      ),
    );

...
export default class SpritePipeline extends FieldSpritePipeline {
  ...
  onBind(gameObject: Phaser.GameObjects.GameObject): void {
    ...
    const field = isEntityObj
      ? sprite.parentContainer.parentContainer
    ...
    position[0] +=
      -(sprite.width - sprite.frame.width) / 2 +
      sprite.frame.x +
      (!ignoreFieldPos ? sprite.x - field.x : 0);
    if (sprite.originY === 0.5)
      position[1] +=
        (sprite.height / 2) *
          ((isEntityObj ? sprite.parentContainer : sprite).scale - 1) +
    ...
    this.set1f(
      ...
      sprite.height -
        sprite.frame.height *
          (isEntityObj ? sprite.parentContainer.scale : sprite.scale),
    );
    ...
    if ((gameObject.scene as BattleScene).fusionPaletteSwaps) {
      const spriteColors = ((ignoreOverride && data["spriteColorsBase"]) ||
        data["spriteColors"] ||
        []) as number[][];
      const fusionSpriteColors = ((ignoreOverride &&
        data["fusionSpriteColorsBase"]) ||
      ...
      for (let c = 0; c < 32; c++) {
        ...
        flatFusionSpriteColors.splice(
          ...
          0,
          ...(c < fusionSpriteColors.length
            ? fusionSpriteColors[c]
            : emptyColors),
        );
      }
        ...
        for (let c = 0; c < 32; c++) {
          if (c < baseColors.length) {
            const baseColor = Array.from(
              Object.values(Utils.rgbHexToRgba(baseColors[c])),
            );
            const variantColor = Array.from(
              Object.values(
                Utils.rgbHexToRgba(variantColors[variant][baseColors[c]]),
              ),
            );
            flatBaseColors.splice(flatBaseColors.length, 0, ...baseColor);
            flatVariantColors.splice(
              flatVariantColors.length,
              0,
              ...variantColor.map((c) => c / 255.0),
            );
          } else {
  ...
  batchQuad(
    ...
    if (hasShadow) {
      ...
      const baseY =
        ((isEntityObj ? sprite.parentContainer.y : sprite.y + sprite.height) *
          6) /
        fieldScaleRatio;
      const bottomPadding =

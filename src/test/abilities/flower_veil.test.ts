...
describe("Abilities - Flower Veil", () => {
  ...
  it("should not prevent any source of self-inflicted status conditions", async () => {
    ...
    await game.scene.addModifier(modifierTypes.FLAME_ORB().newModifier(user));
    await game.scene.updateModifiers(true);
    // remove sleep status
    user.resetStatus();
    ...
    await game.scene.addModifier(modifierTypes.TOXIC_ORB().newModifier(user));
    await game.scene.updateModifiers(true);
    user.resetStatus();

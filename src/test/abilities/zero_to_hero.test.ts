...
describe("Abilities - ZERO TO HERO", () => {
  ...
  it(
    "should swap to base form on arena reset",
    async () => {
  ...
  it(
    "should swap to Hero form when switching out during a battle",
    async () => {
  ...
  it(
    "should not swap to Hero form if switching due to faint",
    async () => {
      await game.startBattle([Species.PALAFIN, Species.FEEBAS]);
  ...
  it(
    "should stay hero form if fainted and then revived",
    async () => {
      game.override.starterForms({
        [Species.PALAFIN]: heroForm,
      });

...
export function getBiomeName(biome: Biome | -1) {
  if (biome === -1) return "Somewhere you can't remember";
  switch (biome) {
    case Biome.GRASS:
      return "Grassy Field";
    case Biome.RUINS:
  ...
  [Biome.PLAINS]: {
    ...
    [BiomePoolTier.SUPER_RARE]: {
      ...
      [TimeOfDay.ALL]: [
        ...
        Species.EEVEE,
        Species.SNORLAX,
        { 1: [Species.DUNSPARCE], 72: [Species.DUDUNSPARCE] },
      ],
    },
    [BiomePoolTier.ULTRA_RARE]: {
      [TimeOfDay.DAWN]: [],
  ...
  [Biome.TALL_GRASS]: {
    ...
    [BiomePoolTier.RARE]: {
      ...
      [TimeOfDay.ALL]: [
        ...
        { 1: [Species.GIRAFARIG], 72: [Species.FARIGIRAF] },
        Species.ZANGOOSE,
        Species.KECLEON,
        Species.TROPIUS,
      ],
  ...
  [Biome.SWAMP]: {
    ...
    [BiomePoolTier.SUPER_RARE]: {
      [TimeOfDay.DAWN]: [
        ...
        { 1: [Species.HISUI_SLIGGOO], 90: [Species.HISUI_GOODRA] },
      ],
      [TimeOfDay.DAY]: [
        { 1: [Species.GALAR_SLOWPOKE], 40: [Species.GALAR_SLOWBRO] },
        { 1: [Species.HISUI_SLIGGOO], 90: [Species.HISUI_GOODRA] },
...

{
  ...
  const trainerBiomes = [
    ...
    [TrainerType.STRIKER, []],
    [TrainerType.STUDENT, [[Biome.GRASS, BiomePoolTier.COMMON]]],
  ...
  const traverseBiome = (biome: Biome, depth: integer) => {
    const linkedBiomes: (Biome | [Biome, integer])[] = Array.isArray(
      biomeLinks[biome],
    )
    ...
    for (const pb of pokemonBiomes) {
      ...
      for (const b of biomeEntries) {
        ...
        for (const tod of timesOfDay) {
          ...
          else (biomeTierPool as unknown as Species[][]).push([speciesId]);
        }
      }
    }
      ...
      for (const t of Object.keys(biomePokemonPools[b])) {
        ...
        for (const tod of Object.keys(biomePokemonPools[b][t])) {
          ...
          for (let e = 0; e < biomeTierTimePool.length; e++) {
            ...
            if (entry.length === 1) biomeTierTimePool[e] = entry[0];
            else {
              const newEntry = {
                1: [entry[0]],
              };
              for (let s = 1; s < entry.length; s++) {
                ...
                else newEntry[level].push(speciesId);
              }
  ...
  function outputPools() {
    ...
    console.log(
      beautify(pokemonOutput, null, 2, 180)
        .replace(
          /( {8}| {8}(?:\{ "\d+": \[ )?| {4}"(?:.*?)": \[ |(?:,|\[) (?:"\w+": \[ |(?:\{ )?"\d+": \[ )?)"(\w+)"(?= |,|\n)/g,
          "$1Species.$2",
        )
        ...
        .replace(/( {4})"(.*?)"/g, "$1[BiomePoolTier.$2]")
        .replace(/( {2})"(.*?)"/g, "$1[Biome.$2]"),
    );
    console.log(
      beautify(trainerOutput, null, 2, 120)
        .replace(
          /( {6}| {6}(?:\{ "\d+": \[ )?| {4}"(?:.*?)": \[ |, (?:(?:\{ )?"\d+": \[ )?)"(.*?)"/g,
          "$1TrainerType.$2",
        )

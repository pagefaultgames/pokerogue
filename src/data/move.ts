...
export default class Move implements Localizable {
  ...
  localize(): void {
    ...
    this.name = this.id
      ? `${i18next.t(`move:${i18nKey}.name`).toString()}${this.nameAppend}`
      : "";
    this.effect = this.id
      ? `${i18next.t(`move:${i18nKey}.effect`).toString()}${this.nameAppend}`
      : "";
  }
  ...
  isTypeImmune(type: Type): boolean {
    switch (type) {
      case Type.GRASS:
        if (this.hasFlag(MoveFlags.POWDER_MOVE)) return true;
        break;
    }
    return false;
  }
  ...
  checkFlag(flag: MoveFlags, user: Pokemon, target: Pokemon): boolean {
    switch (flag) {
      case MoveFlags.MAKES_CONTACT:
        if (user.hasAbilityWithAttr(IgnoreContactAbAttr)) return false;
        break;
  ...
  applyConditions(user: Pokemon, target: Pokemon, move: Move): boolean {
    for (const condition of this.conditions) {
      if (!condition.apply(user, target, move)) return false;
    }
  ...
  getFailedText(
    ...
    for (const attr of this.attrs) {
      ...
      if (failedText !== null) return failedText;
    }
    return null;
  }
  ...
  getTargetBenefitScore(user: Pokemon, target: Pokemon, move: Move): integer {
    ...
    if (attackScore) {
      if (this.category === MoveCategory.PHYSICAL) {
        const atk = new Utils.IntegerHolder(
          user.getBattleStat(Stat.ATK, target),
        );
        ...
        if (atk.value > user.getBattleStat(Stat.SPATK, target)) {
          ...
          if (statRatio <= 0.75) attackScore *= 2;
          else if (statRatio <= 0.875) attackScore *= 1.5;
        }
      } else {
        const spAtk = new Utils.IntegerHolder(
          user.getBattleStat(Stat.SPATK, target),
        );
...
export class MoveEffectAttr extends MoveAttr {
  ...
  constructor(
    ...
    super(selfTarget);
    this.trigger =
      trigger !== undefined ? trigger : MoveEffectTrigger.POST_APPLY;
...
export class TargetHalfHpDamageAttr extends FixedDamageAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    (args[0] as Utils.IntegerHolder).value = Math.max(
      Math.floor(target.hp / 2),
      1,
    );
...
export class CounterDamageAttr extends FixedDamageAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    (args[0] as Utils.IntegerHolder).value = Math.floor(
      Math.max(damage * this.multiplier, 1),
    );
...
export class RandomLevelDamageAttr extends FixedDamageAttr {
  ...
  getDamage(user: Pokemon, target: Pokemon, move: Move): number {
    return Math.max(
      Math.floor(user.level * (user.randSeedIntRange(50, 150) * 0.01)),
      1,
    );
  }
}
...
export class RecoilAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (!this.unblockable) applyAbAttrs(BlockRecoilDamageAttr, user, cancelled);

    if (cancelled.value) return false;

    const recoilDamage = Math.max(
      Math.floor(
        (!this.useHp ? user.turnData.damageDealt : user.getMaxHp()) *
          this.damageRatio,
      ),
...
export class HalfSacrificialAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    applyAbAttrs(BlockNonDirectDamageAbAttr, user, cancelled);
    if (!cancelled.value) {
      user.damageAndUpdate(
        Math.ceil(user.getMaxHp() / 2),
        HitResult.OTHER,
        false,
        true,
        true,
      ...
      user.scene.queueMessage(
        getPokemonMessage(user, " cut its own HP to power up its move!"),
...
export abstract class WeatherHealAttr extends HealAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (!user.scene.arena.weather?.isEffectSuppressed(user.scene)) {
      const weatherType =
        user.scene.arena.weather?.weatherType || WeatherType.NONE;
...
export class HitHealAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    const healAmount = Math.max(
      Math.floor(user.turnData.damageDealt * this.healRatio),
      1,
    );
    ...
    user.scene.unshiftPhase(
      new PokemonHealPhase(
        ...
        !reverseDrain
          ? getPokemonMessage(target, ` had its\nenergy drained!`)
          : undefined,
        false,
        true,
      ),
  ...
  getUserBenefitScore(user: Pokemon, target: Pokemon, move: Move): integer {
    return Math.floor(
      Math.max(1 - user.getHpRatio() - 0.33, 0) * (move.power / 5 / 4),
    );
  }
}
...
export class StrengthSapHealAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    const healAmount =
      target.stats[Stat.ATK] *
      (Math.max(2, 2 + target.summonData.battleStats[BattleStat.ATK]) /
        Math.max(2, 2 - target.summonData.battleStats[BattleStat.ATK]));
    const reverseDrain = user.hasAbilityWithAttr(ReverseDrainAbAttr);
    user.scene.unshiftPhase(
      new PokemonHealPhase(
      ...
      case MultiHitType._2_TO_5:
        {
          ...
          if (hitValue.value >= 10) hitTimes = 2;
          else if (hitValue.value >= 4) hitTimes = 3;
          else if (hitValue.value >= 2) hitTimes = 4;
          else hitTimes = 5;
        }
        break;
      ...
      case MultiHitType._1_TO_10:
        {
          ...
          const hitValue = new Utils.IntegerHolder(rand);
          applyAbAttrs(MaxMultiHitAbAttr, user, null, hitValue);
          if (hitValue.value >= 81) hitTimes = 1;
          else if (hitValue.value >= 73) hitTimes = 2;
          else if (hitValue.value >= 66) hitTimes = 3;
          else if (hitValue.value >= 60) hitTimes = 4;
          else if (hitValue.value >= 54) hitTimes = 5;
          else if (hitValue.value >= 49) hitTimes = 6;
          else if (hitValue.value >= 44) hitTimes = 7;
          else if (hitValue.value >= 40) hitTimes = 8;
          else if (hitValue.value >= 36) hitTimes = 9;
          else hitTimes = 10;
        }
        break;
    }
...
export class StatusEffectAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    const statusCheck =
      move.chance < 0 ||
      move.chance === 100 ||
      user.randSeedInt(100) < move.chance;
    if (statusCheck) {
      const pokemon = this.selfTarget ? user : target;
      if (pokemon.status) {
        if (this.overrideStatus) pokemon.resetStatus();
        else return false;
      }
      if (
        !pokemon.status ||
        (pokemon.status.effect === this.effect && move.chance < 0)
      )
  ...
  getTargetBenefitScore(user: Pokemon, target: Pokemon, move: Move): number {
    return !(this.selfTarget ? user : target).status &&
      (this.selfTarget ? user : target).canSetStatus(this.effect, true)
...
export class PsychoShiftEffectAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (
      !target.status ||
      (target.status.effect === statusToApply && move.chance < 0)
    ) {
  ...
  getTargetBenefitScore(user: Pokemon, target: Pokemon, move: Move): number {
    return !(this.selfTarget ? user : target).status &&
      (this.selfTarget ? user : target).canSetStatus(user.status?.effect, true)
      ? Math.floor(move.chance * -0.1)
      : 0;
  }
}
...
export class StealHeldItemChanceAttr extends MoveEffectAttr {
  ...
  apply(
    ...
    return new Promise<boolean>((resolve) => {
      ...
      if (rand >= this.chance) return resolve(false);
      const heldItems = this.getTargetHeldItems(target).filter((i) =>
      ...
      if (heldItems.length) {
        ...
        const stolenItem =
          tierHeldItems[user.randSeedInt(tierHeldItems.length)];
        user.scene
          .tryTransferHeldItemModifier(stolenItem, user, false, false)
          .then((success) => {
            if (success)
              user.scene.queueMessage(
                getPokemonMessage(
  ...
  getTargetHeldItems(target: Pokemon): PokemonHeldItemModifier[] {
    return target.scene.findModifiers(
      (m) =>
        ...
        (m as PokemonHeldItemModifier).pokemonId === target.id,
      target.isPlayer(),
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (pokemon.status && this.effects.includes(pokemon.status.effect)) {
      pokemon.scene.queueMessage(
        getPokemonMessage(
          pokemon,
          getStatusEffectHealText(pokemon.status.effect),
        ),
      );
      pokemon.resetStatus();
      pokemon.updateInfo();
...
export class TerrainChangeAttr extends MoveEffectAttr {
  ...
  getCondition(): MoveConditionFunc {
    return (user, target, move) =>
      !user.scene.arena.terrain ||
      user.scene.arena.terrain.terrainType !== this.terrainType;
  }
...
export class OneHitKOAttr extends MoveAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    if (target.isBossImmune()) return false;
...
export class ChargeAttr extends OverrideMoveEffectAttr {
  ...
  apply(
    ...
    return new Promise((resolve) => {
      ...
      if (
        ...
        (args[0] as Utils.BooleanHolder).value = true;
        new MoveChargeAnim(this.chargeAnim, move.id, user).play(
          user.scene,
          () => {
            user.scene.queueMessage(
              getPokemonMessage(
                user,
                ` ${this.chargeText.replace("{TARGET}", target.name)}`,
              ),
            );
            if (this.tagType) user.addTag(this.tagType, 1, move.id, user.id);
  ...
  usedChargeEffect(user: Pokemon, target: Pokemon, move: Move): boolean {
    if (!this.chargeEffect) return false;
...
export class ElectroShotChargeAttr extends ChargeAttr {
  ...
  constructor() {
    super(
      ChargeAnim.ELECTRO_SHOT_CHARGING,
...
export class PostVictoryStatChangeAttr extends MoveAttr {
  ...
  applyPostVictory(user: Pokemon, target: Pokemon, move: Move): void {
    if (this.condition && !this.condition(user, target, move)) return false;
    const statChangeAttr = new StatChangeAttr(
      this.stats,
      this.levels,
      this.showMessage,
    );
...
export class HalfHpStatMaxAttr extends StatChangeAttr {
  ...
  apply(
    ...
    return new Promise<boolean>((resolve) => {
      ...
      if (damage) user.scene.damageNumberHandler.add(user, damage);
      user.updateInfo().then(() => {
        const ret = super.apply(user, target, move, args);
        user.scene.queueMessage(
          getPokemonMessage(
            user,
            ` cut its own HP\nand maximized its ${getBattleStatName(
              this.stats[0],
            )}!`,
          ),
        );
...
export class CopyStatsAttr extends MoveEffectAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    else user.removeTag(BattlerTagType.CRIT_BOOST);
    target.updateInfo();
    user.updateInfo();
...
export class HpSplitAttr extends MoveEffectAttr {
  apply(
    ...
    return new Promise((resolve) => {
      if (!super.apply(user, target, move, args)) return resolve(false);
      ...
      if (target.hp < hpValue) {
        ...
        if (damage) target.scene.damageNumberHandler.add(target, damage);
      }
...
export class WeightPowerAttr extends VariablePowerAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    while (targetWeight >= weightThresholds[w]) {
      if (++w === weightThresholds.length) break;
    }
...
export class BattleStatRatioPowerAttr extends VariablePowerAttr {
  ...
  constructor(stat: Stat, invert = false) {
    super();
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (this.invert) {
      ...
      const bp = Math.floor(
        Math.min(150, (25 * target.getBattleStat(this.stat)) / userSpeed + 1),
      );
      power.value = bp;
      return true;
    }
    ...
    while (w < statThresholds.length - 1 && statRatio > statThresholds[w]) {
      if (++w === statThresholds.length) break;
    }
...
export class CompareWeightPowerAttr extends VariablePowerAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (!userWeight || userWeight === 0) return false;
...
export class HpPowerAttr extends VariablePowerAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    (args[0] as Utils.NumberHolder).value = Math.max(
      Math.floor(150 * user.getHpRatio()),
      1,
    );
...
export class OpponentHighHpPowerAttr extends VariablePowerAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    (args[0] as Utils.NumberHolder).value = Math.max(
      Math.floor(120 * target.getHpRatio()),
      1,
    );
...
export class FirstAttackDoublePowerAttr extends VariablePowerAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (
      !target
        .getLastXMoves(1)
        .find((m) => m.turn === target.scene.currentBattle.turn)
    ) {
  ...
  user.scene.executeWithSeedOffset(
    () => {
      ...
      for (; m < magnitudeThresholds.length; m++) {
        if (rand < magnitudeThresholds[m]) break;
      }
...
export class HitCountPowerAttr extends VariablePowerAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    (args[0] as Utils.NumberHolder).value +=
      Math.min(user.battleData.hitCount, 6) * 50;
...
export class StatChangeCountPowerAttr extends VariablePowerAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    const positiveStats: number = user.summonData.battleStats.reduce(
      (total, stat) => (stat > 0 && stat ? total + stat : total),
      0,
    );
...
export class TargetAtkUserAtkAttr extends VariableAtkAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    (args[0] as Utils.IntegerHolder).value = target.getBattleStat(
      Stat.ATK,
      target,
    );
    return true;
...
export class DefAtkAttr extends VariableAtkAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    (args[0] as Utils.IntegerHolder).value = user.getBattleStat(
      Stat.DEF,
      target,
    );
    return true;
...
export class PhotonGeyserCategoryAttr extends VariableMoveCategoryAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (
      user.getBattleStat(Stat.ATK, target, move) >
      user.getBattleStat(Stat.SPATK, target, move)
    ) {
...
export class ShellSideArmCategoryAttr extends VariableMoveCategoryAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const atkRatio =
      user.getBattleStat(Stat.ATK, target, move) /
      target.getBattleStat(Stat.DEF, user, move);
    const specialRatio =
      user.getBattleStat(Stat.SPATK, target, move) /
      target.getBattleStat(Stat.SPDEF, user, move);
...
export class NeutralDamageAgainstFlyingTypeMultiplierAttr extends VariableMoveTypeMultiplierAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    if (!target.getTag(BattlerTagType.IGNORE_FLYING)) {
      ...
      if (target.isOfType(Type.FLYING)) multiplier.value = 1;
...
export class OneHitKOAccuracyAttr extends VariableAccuracyAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    const accuracy = args[0] as Utils.NumberHolder;
    if (user.level < target.level) accuracy.value = 0;
    else
...
export class DisableMoveAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    while (moveQueue.length) {
      ...
      if (turnMove.virtual) continue;

      const moveIndex = target
        .getMoveset()
        .findIndex((m) => m.moveId === turnMove.move);
  ...
  getCondition(): MoveConditionFunc {
    return (user, target, move) => {
      if (target.summonData.disabledMove || target.isMax()) return false;
...
export class AddBattlerTagAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (chance < 0 || chance === 100 || user.randSeedInt(100) < chance)
      return (this.selfTarget ? user : target).addTag(
        this.tagType,
        user.randSeedInt(
          this.turnCountMax - this.turnCountMin,
          this.turnCountMin,
        ),
        move.id,
        user.id,
  ...
  getTargetBenefitScore(user: Pokemon, target: Pokemon, move: Move): integer {
    ...
    return Math.floor(
      this.getTagTargetBenefitScore(user, target, move) * (chance / 100),
    );
  }
}
  ...
  getCondition(): MoveConditionFunc {
    return (user, target, move): boolean => {
      ...
      while (moveHistory.length) {
        ...
        if (
          !allMoves[turnMove.move].getAttrs(ProtectAttr).length ||
          turnMove.result !== MoveResult.SUCCESS
        )
          break;
        timesUsed++;
      }
      if (timesUsed) return !user.randSeedInt(Math.pow(3, timesUsed));
      return true;
    };
  }
}
  ...
  getCondition(): MoveConditionFunc {
    return (user, target, move) => {
      ...
      if (
        ...
        return true;
      const tag = user.scene.arena.getTagOnSide(
        this.tagType,
        side,
      ) as ArenaTrapTag;
...
export class ForceSwitchOutAttr extends MoveEffectAttr {
    ...
    return new Promise((resolve) => {
      ...
      if (switchOutTarget instanceof PlayerPokemon) {
        if (switchOutTarget.hp) {
          ...
          (switchOutTarget as PlayerPokemon)
            .switchOut(this.batonPass, true)
            .then(() => resolve(true));
        } else resolve(false);
        return;
  ...
  getFailedText(
    ...
    return blockedByAbility.value
      ? getPokemonMessage(target, ` can't be switched out!`)
      : null;
  }
  ...
  getSwitchOutCondition(): MoveConditionFunc {
    return (user, target, move) => {
      ...
      if (!player && !user.scene.currentBattle.battleType) {
        if (this.batonPass) return false;
        // Don't allow wild opponents to flee on the boss stage since it can ruin a run early on
        if (!(user.scene.currentBattle.waveIndex % 10)) return false;
      }
...
export class CopyBiomeTypeAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    user.scene.queueMessage(
      getPokemonMessage(
        user,
        ` transformed\ninto the ${Utils.toReadableString(
          Type[biomeType],
        )} type!`,
      ),
    );
...
export class FirstMoveTypeAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    user.scene.queueMessage(
      getPokemonMessage(
        user,
        ` transformed\ninto to the ${Utils.toReadableString(
          Type[firstMoveType],
        )} type!`,
      ),
    );
...
export class RandomMovesetMoveAttr extends OverrideMoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const moves = moveset.filter(
      (m) => !m.getMove().hasFlag(MoveFlags.IGNORE_VIRTUAL),
    );
    if (moves.length) {
      const move = moves[user.randSeedInt(moves.length)];
      const moveIndex = moveset.findIndex((m) => m.moveId === move.moveId);
      const moveTargets = getMoveTargets(user, move.moveId);
      if (!moveTargets.targets.length) return false;
      ...
      switch (true) {
        ...
        default: {
          moveTargets.targets.splice(
            moveTargets.targets.indexOf(user.getAlly().getBattlerIndex()),
          );
          selectTargets = [
      ...
      user
        .getMoveQueue()
        .push({ move: move.moveId, targets: targets, ignorePP: true });
      user.scene.unshiftPhase(
        new MovePhase(user.scene, user, targets, moveset[moveIndex], true),
      );
      return true;
    }
  ...
  apply(
    ...
    return new Promise((resolve) => {
      const moveIds = Utils.getEnumValues(Moves).filter(
        (m) =>
          !allMoves[m].hasFlag(MoveFlags.IGNORE_VIRTUAL) &&
      ...
      const targets =
        moveTargets.multiple || moveTargets.targets.length === 1
          ...
          : moveTargets.targets.indexOf(target.getBattlerIndex()) > -1
            ...
            : [
                moveTargets.targets[
                  user.randSeedInt(moveTargets.targets.length)
                ],
              ];
      user
        .getMoveQueue()
        .push({ move: moveId, targets: targets, ignorePP: true });
      user.scene.unshiftPhase(
        new MovePhase(
          user.scene,
          user,
      ...
      initMoveAnim(user.scene, moveId).then(() => {
        loadMoveAnimAssets(user.scene, [moveId], true).then(() =>
          resolve(true),
        );
      });
    });
...
const lastMoveCopiableCondition: MoveConditionFunc = (user, target, move) => {
  ...
  if (!copiableMove) return false;

  if (allMoves[copiableMove].getAttrs(ChargeAttr).length) return false;
...
export class CopyMoveAttr extends OverrideMoveEffectAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    user
      .getMoveQueue()
      .push({ move: lastMove, targets: targets, ignorePP: true });
  ...
  getTargetBenefitScore(user: Pokemon, target: Pokemon, move: Move): number {
    ...
    if (lastMove) {
      ...
      if (movesetMove) {
        ...
        if (ppLeft < 4) return (value / 4) * ppLeft;
        return value;
      }
    }
...
const targetMoveCopiableCondition: MoveConditionFunc = (user, target, move) => {
  ...
  if (!targetMoves.length) return false;

  const copiableMove = targetMoves[0];

  if (!copiableMove.move) return false;

  if (
    allMoves[copiableMove.move].getAttrs(ChargeAttr).length &&
...
export class MovesetCopyMoveAttr extends OverrideMoveEffectAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (thisMoveIndex === -1) return false;
...
export class SketchAttr extends MoveEffectAttr {
  ...
  getCondition(): MoveConditionFunc {
    return (user, target, move) => {
      if (!targetMoveCopiableCondition(user, target, move)) return false;
...
export class AbilityCopyAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (
      this.copyToPartner &&
      user.scene.currentBattle?.double &&
      user.getAlly().hp
    ) {
    ...
    return (user, target, move) => {
      ...
      if (this.copyToPartner && user.scene.currentBattle?.double)
        ret =
          ret &&
          (!user.getAlly().hp ||
            !user.getAlly().getAbility().hasAttr(UnsuppressableAbilityAbAttr));
      else ret = ret && user.getAbility().id !== target.getAbility().id;
      return ret;
    };
  }
}
...
export class TransformAttr extends MoveEffectAttr {
  apply(
    ...
    return new Promise((resolve) => {
      ...
      user.summonData.stats = [user.stats[Stat.HP]].concat(
        target.stats.slice(1),
      );
...
export class DiscourageFrequentUseAttr extends MoveAttr {
  getUserBenefitScore(user: Pokemon, target: Pokemon, move: Move): integer {
    ...
    for (let m = 0; m < lastMoves.length; m++) {
      if (lastMoves[m].move === move.id) return (4 - (m + 1)) * -10;
    }
...
export class MoneyAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move): boolean {
    user.scene.currentBattle.moneyScattered +=
      user.scene.getWaveMoneyAmount(0.2);
...
const failOnMaxCondition: MoveConditionFunc = (user, target, move) =>
  !target.isMax();
...
function applyMoveAttrsInternal(
  ...
  return new Promise((resolve) => {
    ...
    for (const attr of moveAttrs) {
      ...
      if (result instanceof Promise) attrPromises.push(result);
    }
...
export function getMoveTargets(user: Pokemon, move: Moves): MoveTargetSet {
  ...
  const moveTarget = allMoves[move].getAttrs(VariableTargetAttr).length
...
export function initMoves() {
  allMoves.push(
    ...
    new AttackMove(
      ...
      .attr(
        ChargeAttr,
        ChargeAnim.RAZOR_WIND_CHARGING,
        "whipped\nup a whirlwind!",
      )
      .attr(HighCritAttr)
      .windMove()
    ...
    new SelfStatusMove(Moves.GROWTH, Type.NORMAL, -1, 20, -1, 0, 1).attr(
      GrowthStatChangeAttr,
    ),
    new AttackMove(
      Moves.RAZOR_LEAF,
      Type.GRASS,
    ...
    new AttackMove(
      ...
      .attr(
        SunlightChargeAttr,
        ChargeAnim.SOLAR_BEAM_CHARGING,
        "took\nin sunlight!",
      )
    ...
    new SelfStatusMove(Moves.MINIMIZE, Type.NORMAL, -1, 10, -1, 0, 1).attr(
      StatChangeAttr,
      BattleStat.EVA,
      2,
      true,
    ...
    new StatusMove(Moves.CONFUSE_RAY, Type.GHOST, 100, 10, -1, 0, 1).attr(
      ConfuseAttr,
    ),
    ...
    new AttackMove(
      ...
      .attr(
        ChargeAttr,
        ChargeAnim.SKULL_BASH_CHARGING,
        "lowered\nits head!",
        null,
        true,
      )
    ...
    new AttackMove(
      ...
      .attr(HitHealAttr)
      .condition(
        (user, target, move) => target.status?.effect === StatusEffect.SLEEP,
      )
      .triageMove(),
    ...
    new SelfStatusMove(Moves.REST, Type.PSYCHIC, -1, 5, -1, 0, 1)
      ...
      .condition(
        (user, target, move) =>
          user.getHpRatio() < 1 &&
          user.canSetStatus(StatusEffect.SLEEP, true, true),
      )
      .triageMove(),
    new AttackMove(
      Moves.ROCK_SLIDE,
    ...
    new SelfStatusMove(Moves.CONVERSION, Type.NORMAL, -1, 30, -1, 0, 1).attr(
      FirstMoveTypeAttr,
    ),
    new AttackMove(
      Moves.TRI_ATTACK,
    ...
    new StatusMove(Moves.MIND_READER, Type.NORMAL, -1, 5, -1, 0, 2).attr(
      IgnoreAccuracyAttr,
    ),
    ...
    new AttackMove(
      ...
      .attr(FlinchAttr)
      .condition(
        (user, target, move) => user.status?.effect === StatusEffect.SLEEP,
      )
      .soundBased(),
    ...
    new StatusMove(Moves.SPITE, Type.GHOST, 100, 10, -1, 0, 2).attr(
      ReducePpMoveAttr,
    ),
    new AttackMove(
      Moves.POWDER_SNOW,
      ...
      .attr(StatusEffectAttr, StatusEffect.FREEZE)
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    new SelfStatusMove(Moves.PROTECT, Type.NORMAL, -1, 10, -1, 4, 2).attr(
      ProtectAttr,
    ),
    new AttackMove(
      Moves.MACH_PUNCH,
    ...
    new StatusMove(Moves.SWEET_KISS, Type.FAIRY, 75, 10, -1, 0, 2).attr(
      ConfuseAttr,
    ),
    ...
    new AttackMove(
      ...
      .windMove()
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    new SelfStatusMove(Moves.DETECT, Type.FIGHTING, -1, 5, -1, 4, 2).attr(
      ProtectAttr,
    ),
    new AttackMove(
      Moves.BONE_RUSH,
      ...
      .attr(MultiHitAttr)
      .makesContact(false),
    new StatusMove(Moves.LOCK_ON, Type.NORMAL, -1, 5, -1, 0, 2).attr(
      IgnoreAccuracyAttr,
    ),
    new AttackMove(
      Moves.OUTRAGE,
    ...
    new AttackMove(
      ...
      .attr(HitHealAttr)
      .triageMove(),
    new SelfStatusMove(Moves.ENDURE, Type.NORMAL, -1, 10, -1, 4, 2).attr(
      EndureAttr,
    ),
    ...
    new AttackMove(
      ...
      .target(MoveTarget.ATTACKER),
    new StatusMove(Moves.PSYCH_UP, Type.NORMAL, -1, 10, -1, 0, 2).attr(
      CopyStatsAttr,
    ),
    new AttackMove(
      Moves.EXTREME_SPEED,
    ...
    new StatusMove(Moves.ROLE_PLAY, Type.PSYCHIC, -1, 10, -1, 0, 3).attr(
      AbilityCopyAttr,
    ),
    ...
    new AttackMove(
      ...
      .attr(HpPowerAttr)
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    new StatusMove(Moves.SKILL_SWAP, Type.PSYCHIC, -1, 10, -1, 0, 3).attr(
      SwitchAbilitiesAttr,
    ),
    new SelfStatusMove(
      Moves.IMPRISON,
    ...
    new SelfStatusMove(Moves.CAMOUFLAGE, Type.NORMAL, -1, 20, -1, 0, 3).attr(
      CopyBiomeTypeAttr,
    ),
    ...
    new AttackMove(
      ...
      .attr(MovePowerMultiplierAttr, (user, target, move) =>
        target.status?.effect === StatusEffect.SLEEP ? 2 : 1,
      )
    ...
    new AttackMove(
      ...
      target
        .getLastXMoves(1)
        .find((m) => m.turn === target.scene.currentBattle.turn) ||
      user.scene.currentBattle.turnCommands[target.getBattlerIndex()]
    ...
    new StatusMove(Moves.GASTRO_ACID, Type.POISON, 100, 10, -1, 0, 4).attr(
    ...
    new AttackMove(
      ...
      (user, target, move) =>
        ...
        allMoves[
          user.scene.currentBattle.turnCommands[target.getBattlerIndex()].move
            .move
    ...
    new SelfStatusMove(Moves.QUIVER_DANCE, Type.BUG, -1, 20, -1, 0, 5)
      .attr(
        StatChangeAttr,
        [BattleStat.SPATK, BattleStat.SPDEF, BattleStat.SPD],
        1,
        true,
      )
      .danceMove(),
    ...
    new AttackMove(
      ...
      .ballBombMove(),
    new StatusMove(Moves.SOAK, Type.WATER, 100, 20, -1, 0, 5).attr(
      ChangeTypeAttr,
      Type.WATER,
    ),
    new AttackMove(
    ...
    new StatusMove(Moves.ENTRAINMENT, Type.NORMAL, 100, 15, -1, 0, 5).attr(
      AbilityGiveAttr,
    ),
    ...
    new AttackMove(
      ...
      Math.max(
        1,
        2 - 0.2 * user.getHeldItems().reduce((v, m) => v + m.stackCount, 0),
      ),
    ),
    new StatusMove(Moves.REFLECT_TYPE, Type.NORMAL, -1, 15, -1, 0, 5).attr(
      CopyTypeAttr,
    ),
    new AttackMove(
      Moves.RETALIATE,
    ...
    new AttackMove(
      ...
      .attr(
        ChargeAttr,
        ChargeAnim.FREEZE_SHOCK_CHARGING,
        "became cloaked\nin a freezing light!",
      )
      .attr(StatusEffectAttr, StatusEffect.PARALYSIS)
      .makesContact(false),
    ...
    new StatusMove(Moves.PARTING_SHOT, Type.DARK, 100, 20, 100, 0, 6)
      ...
      .attr(ForceSwitchOutAttr, true, false)
      .soundBased(),
    new StatusMove(Moves.TOPSY_TURVY, Type.DARK, -1, 20, -1, 0, 6).attr(
      InvertStatsAttr,
    ),
    new AttackMove(
      Moves.DRAINING_KISS,
    ...
    new StatusMove(Moves.HOLD_HANDS, Type.NORMAL, -1, 40, -1, 0, 6).target(
    ...
    new StatusMove(Moves.STRENGTH_SAP, Type.GRASS, 100, 10, 100, 0, 7)
      ...
      .condition(
        (user, target, move) =>
          target.summonData.battleStats[BattleStat.ATK] > -6,
      )
      .triageMove(),
    new AttackMove(
      Moves.SOLAR_BLADE,
    ...
    new AttackMove(
      ...
      .punchingMove(),
    /* Unused */
    new SelfStatusMove(Moves.MAX_GUARD, Type.NORMAL, -1, 10, -1, 4, 8).attr(
      ProtectAttr,
    ),
    /* End Unused */
    new AttackMove(
    ...
    new AttackMove(
      ...
      .attr(MovePowerMultiplierAttr, (user, target, move) =>
        user.scene.arena.getTerrainType() === TerrainType.PSYCHIC &&
        user.isGrounded()
          ? 1.5
          : 1,
      )
    ...
    new AttackMove(
      ...
      .target(MoveTarget.ALL_NEAR_OTHERS)
      .attr(MovePowerMultiplierAttr, (user, target, move) =>
        user.scene.arena.getTerrainType() === TerrainType.MISTY &&
        user.isGrounded()
          ? 1.5
          : 1,
      )
    ...
    new AttackMove(
      ...
      user.scene.arena.getTerrainType() === TerrainType.ELECTRIC &&
      target.isGrounded()
        ? 2
        : 1,
    ),
    ...
    new SelfStatusMove(Moves.VICTORY_DANCE, Type.FIGHTING, -1, 10, 100, 0, 8)
      .attr(
        StatChangeAttr,
        [BattleStat.ATK, BattleStat.DEF, BattleStat.SPD],
        1,
        true,
      )
      .danceMove(),
      ...
      .attr(MovePowerMultiplierAttr, (user, target, move) => {
        return user.scene
          ...
          .reduce(
            (acc, pokemonInParty) =>
              acc +
              (pokemonInParty.status?.effect == StatusEffect.FAINT ? 1 : 0),
            1,
          );
      })
      .makesContact(false),
    ...
    new AttackMove(
      ...
      .attr(RemoveArenaTrapAttr)
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    new StatusMove(Moves.DOODLE, Type.NORMAL, 100, 10, -1, 0, 9).attr(
      AbilityCopyAttr,
      true,
    ),
    ...
    new StatusMove(Moves.BURNING_BULWARK, Type.FIRE, -1, 10, 100, 4, 9).attr(
      ProtectAttr,

...
export class PartyStatusCureAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const partyPokemon = user.isPlayer() ? globalScene.getParty() : globalScene.getEnemyParty();
...
export class SacrificialFullRestoreAttr extends SacrificialAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const maxPartyMemberHp = globalScene.getParty().map(p => p.getMaxHp()).reduce((maxHp: integer, hp: integer) => Math.max(hp, maxHp), 0);
  ...
  getCondition(): MoveConditionFunc {
    return (user, target, move) => globalScene.getParty().filter(p => p.isActive()).length > globalScene.currentBattle.getBattlerCount();
  }
}
  ...
  getHitCount(user: Pokemon, target: Pokemon): integer {
    switch (this.multiHitType) {
      ...
      case MultiHitType.BEAT_UP:
        const party = user.isPlayer() ? globalScene.getParty() : globalScene.getEnemyParty();
...
export class PsychoShiftEffectAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (target.status) {
      ...
      if (canSetStatus) {
        if (user.status) {
          globalScene.queueMessage(getStatusEffectHealText(user.status.effect, getPokemonNameWithAffix(user)));
        }
        user.resetStatus();
        user.updateInfo();
...
export class EatBerryAttr extends MoveEffectAttr {
  ...
  reduceBerryModifier(target: Pokemon) {
    if (this.chosenBerry?.stackCount === 1) {
      globalScene.removeModifier(this.chosenBerry, !target.isPlayer());
    ...
    return new Promise(resolve => {
      if (args.length < 2 || !args[1]) {
        new MoveChargeAnim(this.chargeAnim, move.id, user).play(false, () => {
          ...
          globalScene.arena.addTag(this.tagType, 3, move.id, user.id, ArenaTagSide.BOTH, false, target.getBattlerIndex());
...
export class AddArenaTagAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if ((move.chance < 0 || move.chance === 100 || user.randSeedInt(100) < move.chance) && user.getLastXMoves(1)[0]?.result === MoveResult.SUCCESS) {
      globalScene.arena.addTag(this.tagType, this.turnCount, move.id, user.id, (this.selfSideTarget ? user : target).isPlayer() ? ArenaTagSide.PLAYER : ArenaTagSide.ENEMY);
      return true;
    }
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): Promise<boolean> {
    return new Promise(resolve => {
      ...
      if (user instanceof PlayerPokemon
        && globalScene.getParty().findIndex(p => p.isFainted()) > -1) {
...
export class ForceSwitchOutAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (switchOutTarget instanceof PlayerPokemon) {
      ...
      if (globalScene.getParty().filter((p) => p.isAllowedInBattle() && !p.isOnField()).length < 1) {
        return false;
      }
      ...
      if (switchOutTarget.hp > 0) {
        ...
        switchOutTarget.leaveField(this.switchType === SwitchType.SWITCH);
        globalScene.prependToPhase(new SwitchSummonPhase(this.switchType, switchOutTarget.getFieldIndex(),
  ...
  getSwitchOutCondition(): MoveConditionFunc {
    return (user, target, move) => {
      ...
      const party = player ? globalScene.getParty() : globalScene.getEnemyParty();
...
export class CopyBiomeTypeAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const biomeType = globalScene.arena.getTypeForBiome();

    user.summonData.types = [ biomeType ];
    user.updateInfo();

    globalScene.queueMessage(i18next.t("moveTriggers:transformedIntoType", { pokemonName: getPokemonNameWithAffix(user), typeName: i18next.t(`pokemonInfo:Type.${Type[biomeType]}`) }));
  ...
  allMoves.push(
    ...
    new StatusMove(Moves.ROTOTILLER, Type.GROUND, -1, 10, -1, 0, 6)
      ...
      .condition((user, target, move) => {
        ...
        return [ ...globalScene.getEnemyParty(), ...globalScene.getParty() ].some((poke) => poke.isOfType(Type.GRASS) && poke.isGrounded());
      })
    ...
    new AttackMove(Moves.UPPER_HAND, Type.FIGHTING, MoveCategory.PHYSICAL, 65, 100, 15, 100, 3, 9)
      .attr(FlinchAttr)
      .condition((user, target, move) => globalScene.currentBattle.turnCommands[target.getBattlerIndex()]?.command === Command.FIGHT && !target.turnData.acted && allMoves[globalScene.currentBattle.turnCommands[target.getBattlerIndex()]?.move?.move!].category !== MoveCategory.STATUS && allMoves[globalScene.currentBattle.turnCommands[target.getBattlerIndex()]?.move?.move!].priority > 0 ) // TODO: is this bang correct?

...
export interface ForceSwitchOutHelperArgs {
  /**
   * Whether the current effect is attempting to switch out the Pokemon itself (`true`) or another Pokemon (`false`).
...
export class ForceSwitchOutHelper implements ForceSwitchOutHelperArgs {
  ...
  private trySwitchPlayerPokemon(switchOutTarget: PlayerPokemon): void {
    ...
    // If forcibly switched out, pick a random off-field pokemon with which to replace our target.
    // If switching out by choice, queue the switch as normal and let the player pick their replacement.
    // TODO: Change to use rand seed item once that gets a seeded version
    const switchInIndex =
      this.switchType === SwitchType.FORCE_SWITCH
        ? reservePartyMembers[switchOutTarget.randBattleSeedInt(reservePartyMembers.length)]
        : undefined;

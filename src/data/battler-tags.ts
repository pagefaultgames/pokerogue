...
export class TrappedTag extends BattlerTag {
  ...
  canAdd(pokemon: Pokemon): boolean {
    return (
      !pokemon.isOfType(Type.GHOST) && !pokemon.getTag(BattlerTagType.TRAPPED)
    );
  }
...
export class ConfusedTag extends BattlerTag {
  ...
  canAdd(pokemon: Pokemon): boolean {
    return (
      pokemon.scene.arena.terrain?.terrainType !== TerrainType.MISTY ||
      !pokemon.isGrounded()
    );
  }
  ...
  onRemove(pokemon: Pokemon): void {
    ...
    pokemon.scene.queueMessage(
      getPokemonMessage(pokemon, " snapped\nout of confusion!"),
    );
  }
...
export class InfatuatedTag extends BattlerTag {
  ...
  canAdd(pokemon: Pokemon): boolean {
    return pokemon.isOppositeGender(
      pokemon.scene.getPokemonById(this.sourceId),
    );
  }
  ...
  onAdd(pokemon: Pokemon): void {
    ...
    pokemon.scene.queueMessage(
      getPokemonMessage(
        pokemon,
        ` fell in love\nwith ${
          pokemon.scene.getPokemonById(this.sourceId).name
        }!`,
      ),
    );
  }
...
export class SeedTag extends BattlerTag {
  ...
  onAdd(pokemon: Pokemon): void {
    ...
    this.sourceIndex = pokemon.scene
      ...
      if (source) {
        ...
        if (!cancelled.value) {
          ...
          const damage = pokemon.damageAndUpdate(
            Math.max(Math.floor(pokemon.getMaxHp() / 8), 1),
          );
...
export class EncoreTag extends BattlerTag {
  ...
  canAdd(pokemon: Pokemon): boolean {
    ...
    if (
      allMoves[repeatableMove.move].getAttrs(ChargeAttr).length &&
      repeatableMove.result === MoveResult.OTHER
    )
      return false;
  ...
  onAdd(pokemon: Pokemon): void {
    ...
    const movePhase = pokemon.scene.findPhase(
...
export class DrowsyTag extends BattlerTag {
  ...
  canAdd(pokemon: Pokemon): boolean {
    return (
      pokemon.scene.arena.terrain?.terrainType !== TerrainType.ELECTRIC ||
      !pokemon.isGrounded()
    );
  }
...
export abstract class DamagingTrapTag extends TrappedTag {
  ...
  canAdd(pokemon: Pokemon): boolean {
    return (
      !pokemon.isOfType(Type.GHOST) &&
      !pokemon.findTag((t) => t instanceof DamagingTrapTag)
    );
  }
  ...
  lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (lapseType === BattlerTagLapseType.CUSTOM) {
      ...
      if (
        ...
        effectPhase.move.getMove().hasFlag(MoveFlags.MAKES_CONTACT)
      ) {
        const attacker = effectPhase.getPokemon();
        attacker.damageAndUpdate(
          Math.ceil(attacker.getMaxHp() * (1 / this.damageRatio)),
          HitResult.OTHER,
        );
      }
    }
  ...
  lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (ret)
      pokemon.scene.queueMessage(
        getPokemonMessage(
          pokemon,
          `\'s perish count fell to ${this.turnCount}.`,
        ),
      );
    else
...
export class TruantTag extends AbilityBattlerTag {
  ...
  lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (lastMove && lastMove.move !== Moves.NONE) {
      (pokemon.scene.getCurrentPhase() as MovePhase).cancel();
      pokemon.scene.unshiftPhase(
        new ShowAbilityPhase(pokemon.scene, pokemon.id, passive),
      );
      pokemon.scene.queueMessage(
        getPokemonMessage(pokemon, " is\nloafing around!"),
      );
...
export class SlowStartTag extends AbilityBattlerTag {
  ...
  lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    if (!pokemon.hasAbility(this.ability)) this.turnCount = 1;
...
export class HighestStatBoostTag extends AbilityBattlerTag {
  ...
  onAdd(pokemon: Pokemon): void {
    ...
    stats
      .map((s) => pokemon.getBattleStat(s))
      .reduce((highestValue: integer, value: integer, i: integer) => {
...
export class SaltCuredTag extends BattlerTag {
  ...
  onAdd(pokemon: Pokemon): void {
    ...
    pokemon.scene.queueMessage(
      getPokemonMessage(pokemon, " is being salt cured!"),
    );
    this.sourceIndex = pokemon.scene
      .getPokemonById(this.sourceId)
      .getBattlerIndex();
    ...
    if (ret) {
      ...
      if (!cancelled.value) {
        const pokemonSteelOrWater =
          pokemon.isOfType(Type.STEEL) || pokemon.isOfType(Type.WATER);
        pokemon.damageAndUpdate(
          Math.max(
            Math.floor(
              pokemonSteelOrWater
        ...
        pokemon.scene.queueMessage(
          getPokemonMessage(pokemon, ` is hurt by ${this.getMoveName()}!`),
        );
      }
    }
...
export function getBattlerTag(
  ...
  switch (tagType) {
    ...
    case BattlerTagType.KINGS_SHIELD:
      return new ContactStatChangeProtectedTag(
        sourceMove,
        tagType,
        BattleStat.ATK,
        -1,

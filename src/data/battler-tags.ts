  ...
  lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (ret) {
      ...
      if (pokemon.randSeedInt(3) === 0) {
        ...
        const damage = Utils.toIntValue(((((2 * pokemon.level / 5 + 2) * 40 * atk / def) / 50) + 2) * (pokemon.randSeedInt(15, 85) / 100));
    ...
    if (ret) {
      ...
      if (source) {
        ...
        if (!cancelled.value) {
          ...
          const damage = pokemon.damageAndUpdate(Utils.toIntValue(pokemon.getMaxHp() / 8));
...
export class NightmareTag extends BattlerTag {
  ...
  lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (ret) {
      ...
      if (!cancelled.value) {
        pokemon.damageAndUpdate(Utils.toIntValue(pokemon.getMaxHp() / 4));
      }
    }
    ...
    if (ret) {
      pokemon.scene.unshiftPhase(
        new PokemonHealPhase(
          ...
          Utils.toIntValue(pokemon.getMaxHp() / 16),
  ...
  */
  ...
  lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (ret) {
      ...
      if (!cancelled.value) {
        pokemon.damageAndUpdate(Utils.toIntValue(pokemon.getMaxHp() / 8));
      }
    }
    ...
    if (lapseType === BattlerTagLapseType.CUSTOM) {
      ...
      if (effectPhase instanceof MoveEffectPhase && effectPhase.move.getMove().hasFlag(MoveFlags.MAKES_CONTACT)) {
        ...
        if (!attacker.hasAbilityWithAttr(BlockNonDirectDamageAbAttr)) {
          attacker.damageAndUpdate(Utils.toIntValue(attacker.getMaxHp() * (1 / this.damageRatio)), HitResult.OTHER);
        }
      }
    }
  ...
  lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (ret) {
      ...
      if (!cancelled.value) {
        ...
        pokemon.damageAndUpdate(Utils.toIntValue(pokemonSteelOrWater ? pokemon.getMaxHp() / 4 : pokemon.getMaxHp() / 8));

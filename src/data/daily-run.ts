...
export function getDailyRunStarters(
  scene: BattleScene,
  seed: string,
): Starter[] {
    ...
    () => {
      ...
      if (/\d{18}$/.test(seed)) {
        for (let s = 0; s < 3; s++) {
          ...
          starters.push(
            getDailyRunStarter(scene, starterSpeciesForm, startingLevel),
          );
        }
        return;
      }
      ...
      for (let c = 0; c < starterCosts.length; c++) {
        ...
        const starterSpecies = getPokemonSpecies(
          getPokemonSpecies(
            Utils.randSeedItem(costSpecies),

...
export class Ability implements Localizable {
  ...
  localize(): void {
    ...
    this.name = this.id
      ? `${i18next.t(`ability:${i18nKey}.name`) as string}${this.nameAppend}`
      : "";
    this.description = this.id
      ? (i18next.t(`ability:${i18nKey}.description`) as string)
      : "";
  }
...
type PokemonAttackCondition = (
  user: Pokemon,
  target: Pokemon,
  move: Move,
) => boolean;
type PokemonDefendCondition = (
  target: Pokemon,
  user: Pokemon,
  move: Move,
) => boolean;
...
export class PostBattleInitFormChangeAbAttr extends PostBattleInitAbAttr {
  ...
  applyPostBattleInit(
    ...
    if (formIndex !== pokemon.formIndex)
      return pokemon.scene.triggerPokemonFormChange(
        pokemon,
...
export class PostBattleInitStatChangeAbAttr extends PostBattleInitAbAttr {
  ...
  applyPostBattleInit(
    ...
    for (const statChangePhase of statChangePhases) {
      ...
      else pokemon.scene.unshiftPhase(statChangePhase);
    }
...
export class PreDefendFormChangeAbAttr extends PreDefendAbAttr {
  ...
  applyPreDefend(
    ...
    const formIndex = this.formFunc(pokemon);
    if (formIndex !== pokemon.formIndex) {
      pokemon.scene.triggerPokemonFormChange(
        pokemon,
        SpeciesFormChangeManualTrigger,
        false,
      );
    ...
    if (
      (move.getMove() instanceof AttackMove ||
        move
          ...
          .find(
            (attr) =>
              (attr as StatusMoveTypeImmunityAttr).immuneType ===
              this.immuneType,
          )) &&
      move.getMove().type === this.immuneType
    ) {
...
export class TypeImmunityHealAbAttr extends TypeImmunityAbAttr {
  ...
  applyPreDefend(
    ...
    const ret = super.applyPreDefend(
      pokemon,
      passive,
      attacker,
      move,
...
export class NonSuperEffectiveImmunityAbAttr extends TypeImmunityAbAttr {
  ...
  applyPreDefend(
    ...
    if (
      move.getMove() instanceof AttackMove &&
      pokemon.getAttackTypeEffectiveness(move.getMove().type, attacker) < 2
    ) {
      cancelled.value = true;
      (args[0] as Utils.NumberHolder).value = 0;
      return true;
...
export class PostDefendDisguiseAbAttr extends PostDefendAbAttr {
  applyPostDefend(
    ...
    if (
      ...
      const recoilDamage = Math.ceil(
        pokemon.getMaxHp() / 8 - attacker.turnData.damageDealt,
      );
      ...
      pokemon.scene.queueMessage(
        getPokemonMessage(pokemon, "'s disguise was busted!"),
      );
      return true;
    }
...
export class ReverseDrainAbAttr extends PostDefendAbAttr {
  applyPostDefend(
    ...
    if (
      !!move.getMove().getAttrs(HitHealAttr).length ||
      !!move.getMove().getAttrs(StrengthSapHealAttr).length
    ) {
      pokemon.scene.queueMessage(
        getPokemonMessage(attacker, ` sucked up the liquid ooze!`),
      );
      return true;
    }
    return false;
...
export class PostDefendHpGatedStatChangeAbAttr extends PostDefendAbAttr {
  ...
  applyPostDefend(
    ...
    if (
      this.condition(pokemon, attacker, move.getMove()) &&
      pokemon.hp <= hpGateFlat &&
      pokemon.hp + lastAttackReceived.damage > hpGateFlat
    ) {
      pokemon.scene.unshiftPhase(
        new StatChangePhase(
          pokemon.scene,
...
export class PostDefendApplyArenaTrapTagAbAttr extends PostDefendAbAttr {
  ...
  applyPostDefend(
    ...
    if (this.condition(pokemon, attacker, move.getMove())) {
      ...
      if (
        !pokemon.scene.arena.getTag(this.tagType) ||
        tag.layers < tag.maxLayers
      ) {
...
export class PostDefendTypeChangeAbAttr extends PostDefendAbAttr {
  ...
  getTriggerMessage(
    ...
    return getPokemonMessage(
      pokemon,
      `'s ${abilityName}\nmade it the ${Utils.toReadableString(
        Type[pokemon.getTypes(true)[0]],
      )} type!`,
    );
  }
}
...
export class PostDefendContactApplyStatusEffectAbAttr extends PostDefendAbAttr {
  ...
  applyPostDefend(
    ...
    if (
      move.getMove().checkFlag(MoveFlags.MAKES_CONTACT, attacker, pokemon) &&
      !attacker.status &&
...
export class EffectSporeAbAttr extends PostDefendContactApplyStatusEffectAbAttr {
  ...
  applyPostDefend(
    ...
    if (
      ...
      attacker.isOfType(Type.GRASS)
    ) {
      return false;
    }
    return super.applyPostDefend(
      pokemon,
      passive,
      attacker,
      move,
...
export class PostDefendContactDamageAbAttr extends PostDefendAbAttr {
  ...
  applyPostDefend(
    ...
    if (move.getMove().checkFlag(MoveFlags.MAKES_CONTACT, attacker, pokemon)) {
      ...
      attacker.turnData.damageTaken += Math.ceil(
...
export class PostStatChangeStatChangeAbAttr extends PostStatChangeAbAttr {
  ...
  constructor(
    condition: PokemonStatChangeCondition,
...
export class MoveTypePowerBoostAbAttr extends MovePowerBoostAbAttr {
  constructor(boostedType: Type, powerMultiplier?: number) {
    super(
      (pokemon, defender, move) => move.type === boostedType,
      powerMultiplier || 1.5,
    );
  }
}
  ...
  applyPostAttack(
    ...
    return new Promise<boolean>((resolve) => {
      if (
        ...
        (!this.condition || this.condition(pokemon, defender, move.getMove()))
      ) {
        ...
        if (heldItems.length) {
          ...
          pokemon.scene
            .tryTransferHeldItemModifier(stolenItem, pokemon, false, false)
            .then((success) => {
              if (success)
                pokemon.scene.queueMessage(
                  getPokemonMessage(
...
export class PostAttackApplyBattlerTagAbAttr extends PostAttackAbAttr {
  ...
  constructor(
    contactRequired: boolean,
    chance: (user: Pokemon, target: Pokemon, move: PokemonMove) => integer,
...
export class PostDefendStealHeldItemAbAttr extends PostDefendAbAttr {
  ...
  applyPostDefend(
    ...
    return new Promise<boolean>((resolve) => {
      if (
        hitResult < HitResult.NO_EFFECT &&
        (!this.condition || this.condition(pokemon, attacker, move.getMove()))
      ) {
...
class PostVictoryStatChangeAbAttr extends PostVictoryAbAttr {
  ...
  constructor(
    stat: BattleStat | ((p: Pokemon) => BattleStat),
    levels: integer,
  ) {
    super();
...
export class IntimidateImmunityAbAttr extends AbAttr {
  ...
  getTriggerMessage(
    ...
    return getPokemonMessage(
      pokemon,
      `'s ${abilityName} prevented it from being Intimidated!`,
    );
  }
}
...
export class DownloadAbAttr extends PostSummonAbAttr {
  ...
  applyPostSummon(pokemon: Pokemon, passive: boolean, args: any[]): boolean {
    ...
    if (this.enemyDef < this.enemySpDef) this.stats = [BattleStat.ATK];
    else this.stats = [BattleStat.SPATK];
...
export class PostSummonTransformAbAttr extends PostSummonAbAttr {
  ...
  applyPostSummon(pokemon: Pokemon, passive: boolean, args: any[]): boolean {
    ...
    pokemon.summonData.stats = [pokemon.stats[Stat.HP]].concat(
...
export class ProtectStatAbAttr extends PreStatChangeAbAttr {
  ...
  getTriggerMessage(
    ...
    return getPokemonMessage(
      pokemon,
      `'s ${abilityName}\nprevents lowering its ${
        this.protectedStat !== undefined
          ? getBattleStatName(this.protectedStat)
          : "stats"
      }!`,
    );
  }
...
export class StatusEffectImmunityAbAttr extends PreSetStatusAbAttr {
  ...
  getTriggerMessage(
    ...
    return getPokemonMessage(
      pokemon,
      `'s ${abilityName}\nprevents ${
        this.immuneEffects.length
          ? getStatusEffectDescriptor(args[0] as StatusEffect)
          : "status problems"
      }!`,
    );
  }
...
export class IncrementMovePriorityAbAttr extends AbAttr {
  ...
  constructor(
    moveIncrementFunc: (pokemon: Pokemon, move: Move) => boolean,
    increaseAmount = 1,
  ) {
    super(true);
  ...
  apply(
    ...
    if (!this.moveIncrementFunc(pokemon, args[0] as Move)) return false;
  ...
  return (pokemon: Pokemon) => {
    for (const opponent of pokemon.getOpponents()) {
      for (const move of opponent.moveset) {
        ...
        if (
          move.getMove() instanceof AttackMove &&
          pokemon.getAttackTypeEffectiveness(move.getMove().type, opponent) >= 2
        ) {
          return true;
        }
        // move is a OHKO
    ...
    for (const opponent of pokemon.getOpponents()) {
      for (const move of opponent.moveset) {
        if (move.getMove() instanceof StatusMove) {
          ...
          move.getMove().findAttr((attr) => attr instanceof OneHitKOAttr)
        ) {
          movePower = 150;
        } else if (
...
export class PostWeatherChangeAddBattlerTagAttr extends PostWeatherChangeAbAttr {
  ...
  applyPostWeatherChange(
    ...
    if (!this.weatherTypes.find((w) => weather === w)) return false;
...
export class PostTerrainChangeAddBattlerTagAttr extends PostTerrainChangeAbAttr {
  ...
  applyPostTerrainChange(
    ...
    if (!this.terrainTypes.find((t) => t === terrain)) return false;
...
export class MoodyAbAttr extends PostTurnAbAttr {
  ...
  applyPostTurn(pokemon: Pokemon, passive: boolean, args: any[]): boolean {
    ...
    const increaseStatArray = selectableStats.filter(
      (s) => pokemon.summonData.battleStats[s] < 6,
    );
    let decreaseStatArray = selectableStats.filter(
      (s) => pokemon.summonData.battleStats[s] > -6,
    );

    if (increaseStatArray.length > 0) {
      const increaseStat =
        increaseStatArray[Utils.randInt(increaseStatArray.length)];
...
export class PostBattleLootAbAttr extends PostBattleAbAttr {
  applyPostBattle(pokemon: Pokemon, passive: boolean, args: any[]): boolean {
    ...
    if (postBattleLoot.length) {
      ...
      if (
        pokemon.scene.tryTransferHeldItemModifier(
          randItem,
          pokemon,
          false,
          true,
  ...
  applyPostFaint(
    ...
    if (move.getMove().checkFlag(MoveFlags.MAKES_CONTACT, attacker, pokemon)) {
      ...
      pokemon.scene
        .getField(true)
        .map((p) =>
          applyAbAttrs(FieldPreventExplosiveMovesAbAttr, p, cancelled),
        );
      if (cancelled) {
        return false;
      }
...
function applyAbAttrsInternal<TAttr extends AbAttr>(
  ...
  return new Promise((resolve) => {
    ...
    const applyNextAbAttr = () => {
      if (attrs.length) applyAbAttr(attrs.shift());
      else clearSpliceQueueAndResolve();
    };
    const applyAbAttr = (attr: TAttr) => {
      if (!canApplyAttr(pokemon, attr)) return applyNextAbAttr();
      pokemon.scene.setPhaseQueueSplice();
      const onApplySuccess = () => {
        if (
          pokemon.battleData &&
          !pokemon.battleData.abilitiesApplied.includes(ability.id)
        ) {
        ...
        if (attr.showAbility && !quiet) {
          ...
          else queueShowAbility(pokemon, passive);
        }
        if (!quiet) {
          ...
          if (message) {
            ...
            else pokemon.scene.queueMessage(message);
          }
        }
      };
      ...
      if (result instanceof Promise) {
        result.then((success) => {
          if (success) onApplySuccess();
          applyNextAbAttr();
        });
      } else {
        if (result) onApplySuccess();
        applyNextAbAttr();
      }
    };
    applyNextAbAttr();
...
export function applyPreDefendAbAttrs(
  ...
  return applyAbAttrsInternal<PreDefendAbAttr>(
    ...
    pokemon,
    (attr, passive) =>
      attr.applyPreDefend(pokemon, passive, attacker, move, cancelled, args),
    args,
    false,
    false,
    simulated,
...
export function applyPostDefendAbAttrs(
  ...
  return applyAbAttrsInternal<PostDefendAbAttr>(
    ...
    pokemon,
    (attr, passive) =>
      attr.applyPostDefend(pokemon, passive, attacker, move, hitResult, args),
    args,
  );
}
...
export function applyBattleStatMultiplierAbAttrs(
  ...
  return applyAbAttrsInternal<BattleStatMultiplierAbAttr>(
    ...
    pokemon,
    (attr, passive) =>
      attr.applyBattleStat(pokemon, passive, battleStat, statValue, args),
    args,
  );
}
...
export function applyPreAttackAbAttrs(
  ...
  return applyAbAttrsInternal<PreAttackAbAttr>(
    ...
    pokemon,
    (attr, passive) =>
      attr.applyPreAttack(pokemon, passive, defender, move, args),
    args,
  );
}
...
export function applyPostAttackAbAttrs(
  ...
  return applyAbAttrsInternal<PostAttackAbAttr>(
    ...
    pokemon,
    (attr, passive) =>
      attr.applyPostAttack(pokemon, passive, defender, move, hitResult, args),
    args,
  );
}
...
export function applyPostKnockOutAbAttrs(
  ...
  return applyAbAttrsInternal<PostKnockOutAbAttr>(
    ...
    pokemon,
    (attr, passive) =>
      attr.applyPostKnockOut(pokemon, passive, knockedOut, args),
    args,
  );
}
...
export function applyPreStatChangeAbAttrs(
  ...
  return applyAbAttrsInternal<PreStatChangeAbAttr>(
    ...
    pokemon,
    (attr, passive) =>
      attr.applyPreStatChange(pokemon, passive, stat, cancelled, args),
    args,
  );
}
...
export function applyPostStatChangeAbAttrs(
  ...
  return applyAbAttrsInternal<PostStatChangeAbAttr>(
    ...
    pokemon,
    (attr, passive) =>
      attr.applyPostStatChange(pokemon, stats, levels, selfTarget, args),
    args,
  );
}
...
export function applyPreSetStatusAbAttrs(
  ...
  return applyAbAttrsInternal<PreSetStatusAbAttr>(
    ...
    pokemon,
    (attr, passive) =>
      attr.applyPreSetStatus(pokemon, passive, effect, cancelled, args),
    args,
    false,
    false,
    !simulated,
...
export function applyPreApplyBattlerTagAbAttrs(
  ...
  return applyAbAttrsInternal<PreApplyBattlerTagAbAttr>(
    ...
    pokemon,
    (attr, passive) =>
      attr.applyPreApplyBattlerTag(pokemon, passive, tag, cancelled, args),
    args,
  );
}
...
export function applyPreWeatherEffectAbAttrs(
  ...
  return applyAbAttrsInternal<PreWeatherDamageAbAttr>(
    ...
    pokemon,
    (attr, passive) =>
      attr.applyPreWeatherEffect(pokemon, passive, weather, cancelled, args),
    args,
    false,
    true,
  );
...
export function applyPostWeatherChangeAbAttrs(
  ...
  return applyAbAttrsInternal<PostWeatherChangeAbAttr>(
    ...
    pokemon,
    (attr, passive) =>
      attr.applyPostWeatherChange(pokemon, passive, weather, args),
    args,
  );
}
...
export function applyPostWeatherLapseAbAttrs(
  ...
  return applyAbAttrsInternal<PostWeatherLapseAbAttr>(
    ...
    pokemon,
    (attr, passive) =>
      attr.applyPostWeatherLapse(pokemon, passive, weather, args),
    args,
  );
}
...
export function applyPostTerrainChangeAbAttrs(
  ...
  return applyAbAttrsInternal<PostTerrainChangeAbAttr>(
    ...
    pokemon,
    (attr, passive) =>
      attr.applyPostTerrainChange(pokemon, passive, terrain, args),
    args,
  );
}
...
export function applyCheckTrappedAbAttrs(
  ...
  return applyAbAttrsInternal<CheckTrappedAbAttr>(
    attrType,
    pokemon,
    (attr, passive) => attr.applyCheckTrapped(pokemon, passive, trapped, args),
    args,
    true,
  );
}
...
export function applyPostFaintAbAttrs(
  ...
  return applyAbAttrsInternal<PostFaintAbAttr>(
    ...
    pokemon,
    (attr, passive) =>
      attr.applyPostFaint(pokemon, passive, attacker, move, hitResult, args),
    args,
  );
}
...
export function initAbilities() {
  allAbilities.push(
    new Ability(Abilities.STENCH, 3).attr(
      ...
      false,
      (user, target, move) =>
        !move.getMove().findAttr((attr) => attr instanceof FlinchAttr) ? 10 : 0,
      BattlerTagType.FLINCHED,
    ),
    new Ability(Abilities.DRIZZLE, 3)
      .attr(PostSummonWeatherChangeAbAttr, WeatherType.RAIN)
      .attr(PostBiomeChangeWeatherChangeAbAttr, WeatherType.RAIN),
    new Ability(Abilities.SPEED_BOOST, 3).attr(
    ...
    new Ability(Abilities.OBLIVIOUS, 3)
      ...
      .ignorable(),
    new Ability(Abilities.CLOUD_NINE, 3).attr(
    ...
    new Ability(Abilities.FLASH_FIRE, 3)
      .attr(
        ...
        1,
        (pokemon: Pokemon) =>
          !pokemon.status || pokemon.status.effect !== StatusEffect.FREEZE,
      )
      .ignorable(),
    ...
    new Ability(Abilities.SHADOW_TAG, 3).attr(ArenaTrapAbAttr),
    ...
    new Ability(Abilities.SOUNDPROOF, 3)
      .attr(
        ...
        (pokemon, attacker, move) =>
          pokemon !== attacker && move.getMove().hasFlag(MoveFlags.SOUND_BASED),
      )
      .ignorable(),
    ...
    new Ability(Abilities.PLUS, 3).unimplemented(),
    new Ability(Abilities.MINUS, 3).unimplemented(),
    ...
    new Ability(Abilities.AIR_LOCK, 3).attr(SuppressWeatherEffectAbAttr, true),
    ...
    new Ability(Abilities.ANGER_POINT, 4).attr(
      PostDefendCritStatChangeAbAttr,
      BattleStat.ATK,
      6,
    ),
    ...
    new Ability(Abilities.POISON_HEAL, 4).unimplemented(),
    ...
    new Ability(Abilities.NORMALIZE, 4).attr(
      ...
      1.2,
      (user, target, move) =>
        move.id !== Moves.HIDDEN_POWER &&
        move.id !== Moves.WEATHER_BALL &&
        move.id !== Moves.NATURAL_GIFT &&
        move.id !== Moves.JUDGMENT &&
        move.id !== Moves.TECHNO_BLAST,
    ),
    ...
    new Ability(Abilities.TECHNICIAN, 4).attr(
      MovePowerBoostAbAttr,
      (user, target, move) => move.power <= 60,
      1.5,
    ),
    ...
    new Ability(Abilities.HONEY_GATHER, 4).unimplemented(),
    ...
    new Ability(Abilities.FLOWER_GIFT, 4)
      ...
      .ignorable()
      .partial(),
    new Ability(Abilities.BAD_DREAMS, 4).unimplemented(),
    new Ability(Abilities.PICKPOCKET, 5).attr(
    ...
    new Ability(Abilities.CURSED_BODY, 5).unimplemented(),
    new Ability(Abilities.HEALER, 5).unimplemented(),
    ...
    new Ability(Abilities.HARVEST, 5).unimplemented(),
    new Ability(Abilities.TELEPATHY, 5).ignorable().unimplemented(),
    ...
    new Ability(Abilities.OVERCOAT, 5)
      ...
      .attr(
        MoveImmunityAbAttr,
        (pokemon, attacker, move) =>
          pokemon !== attacker && move.getMove().hasFlag(MoveFlags.POWDER_MOVE),
      )
      .ignorable(),
    ...
    new Ability(Abilities.SAND_RUSH, 5)
      ...
      .attr(BlockWeatherDamageAttr, WeatherType.SANDSTORM)
      .condition(getWeatherCondition(WeatherType.SANDSTORM)),
    new Ability(Abilities.WONDER_SKIN, 5).ignorable().unimplemented(),
    ...
    new Ability(Abilities.JUSTIFIED, 5).attr(
      PostDefendStatChangeAbAttr,
      (target, user, move) =>
        move.type === Type.DARK && move.category !== MoveCategory.STATUS,
      BattleStat.ATK,
      1,
    ),
    ...
    new Ability(Abilities.ZEN_MODE, 5)
      .attr(PostBattleInitFormChangeAbAttr, (p) =>
        p.getHpRatio() <= 0.5 ? 1 : 0,
      )
    ...
    new Ability(Abilities.CHEEK_POUCH, 6).unimplemented(),
    new Ability(Abilities.PROTEAN, 6).unimplemented(),
    ...
    new Ability(Abilities.BULLETPROOF, 6)
      .attr(
        ...
        (pokemon, attacker, move) =>
          pokemon !== attacker &&
          move.getMove().hasFlag(MoveFlags.BALLBOMB_MOVE),
      )
      .ignorable(),
    ...
    new Ability(Abilities.REFRIGERATE, 6).attr(
      MoveTypeChangePowerMultiplierAbAttr,
      Type.NORMAL,
      Type.ICE,
      1.2,
    ),
    ...
    new Ability(Abilities.GALE_WINGS, 6).attr(
      IncrementMovePriorityAbAttr,
      (pokemon, move) =>
        pokemon.getHpRatio() === 1 && move.type === Type.FLYING,
    ),
    ...
    new Ability(Abilities.WATER_COMPACTION, 7).attr(
      PostDefendStatChangeAbAttr,
      (target, user, move) => move.type === Type.WATER,
      BattleStat.DEF,
      2,
    ),
    new Ability(Abilities.MERCILESS, 7).unimplemented(),
    ...
    new Ability(Abilities.DISGUISE, 7)
      .attr(
        ...
        (target, user, move) =>
          target.formIndex == 0 &&
          target.getAttackTypeEffectiveness(move.type, user) > 0,
      )
      .attr(PostSummonFormChangeAbAttr, (p) =>
        p.battleData.hitCount === 0 ? 0 : 1,
      )
      .attr(PostBattleInitFormChangeAbAttr, (p) =>
        p.battleData.hitCount === 0 ? 0 : 1,
      )
      .attr(PostDefendFormChangeAbAttr, (p) =>
        p.battleData.hitCount === 0 ? 0 : 1,
      )
      .attr(PreDefendFormChangeAbAttr, (p) =>
        p.battleData.hitCount === 0 ? 0 : 1,
      )
    ...
    new Ability(Abilities.POWER_CONSTRUCT, 7) // TODO: 10% Power Construct Zygarde isn't accounted for yet. If changed, update Zygarde's getSpeciesFormIndex entry accordingly
      ...
      .attr(NoFusionAbilityAbAttr)
      .partial(),
    new Ability(Abilities.CORROSION, 7).unimplemented(),
    ...
    new Ability(Abilities.QUEENLY_MAJESTY, 7)
      ...
      .ignorable(),
    new Ability(Abilities.INNARDS_OUT, 7).unimplemented(),
    new Ability(Abilities.DANCER, 7).unimplemented(),
    ...
    new Ability(Abilities.FLUFFY, 7)
      ...
      .attr(
        ReceivedMoveDamageMultiplierAbAttr,
        (target, user, move) => move.type === Type.FIRE,
        2,
      )
      .ignorable(),
    ...
    new Ability(Abilities.LIBERO, 8).unimplemented(),
    new Ability(Abilities.BALL_FETCH, 8).unimplemented(),
    new Ability(Abilities.COTTON_DOWN, 8).unimplemented(),
    new Ability(Abilities.PROPELLER_TAIL, 8).unimplemented(),
    ...
    new Ability(Abilities.GULP_MISSILE, 8)
      ...
      .attr(NoFusionAbilityAbAttr)
      .unimplemented(),
    new Ability(Abilities.STALWART, 8).unimplemented(),
    ...
    new Ability(Abilities.STEELY_SPIRIT, 8).unimplemented(),
    new Ability(Abilities.PERISH_BODY, 8).unimplemented(),
    ...
    new Ability(Abilities.UNSEEN_FIST, 8).unimplemented(),
    new Ability(Abilities.CURIOUS_MEDICINE, 8).unimplemented(),
    ...
    new Ability(Abilities.LINGERING_AROMA, 9)
      ...
      .bypassFaint(),
    new Ability(Abilities.SEED_SOWER, 9).attr(
    ...
    new Ability(Abilities.THERMAL_EXCHANGE, 9)
      .attr(
        PostDefendStatChangeAbAttr,
        (target, user, move) =>
          move.type === Type.FIRE && move.category !== MoveCategory.STATUS,
        BattleStat.ATK,
        1,
      )
      .attr(StatusEffectImmunityAbAttr, StatusEffect.BURN)
    ...
    new Ability(Abilities.WIND_RIDER, 9)
      .attr(
        ...
        (pokemon, attacker, move) =>
          pokemon !== attacker && move.getMove().hasFlag(MoveFlags.WIND_MOVE),
        BattleStat.ATK,
        1,
      )
      .ignorable()
    ...
    new Ability(Abilities.GOOD_AS_GOLD, 9)
      ...
      .ignorable()
      .partial(),
    new Ability(Abilities.VESSEL_OF_RUIN, 9).ignorable().unimplemented(),
    new Ability(Abilities.SWORD_OF_RUIN, 9).ignorable().unimplemented(),
    new Ability(Abilities.TABLETS_OF_RUIN, 9).ignorable().unimplemented(),
    new Ability(Abilities.BEADS_OF_RUIN, 9).ignorable().unimplemented(),
    ...
    new Ability(Abilities.SUPREME_OVERLORD, 9).unimplemented(),

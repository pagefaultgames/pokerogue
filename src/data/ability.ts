...
export class PostSummonChangeEnemyTypeAbAttr extends PostSummonAbAttr {
  private type: Type;

  constructor(type: Type, showAbility?: boolean) {
    super(showAbility);
  }
...
export class PostTurnHurtAbAttr extends PostTurnAbAttr {

  /**
   * Deals damage to all pokemon equal to 1/16 of their max hp (min 1)
   * @param {Pokemon} pokemon Pokemon that has this ability
   * @param {boolean} passive N/A
   * @param {any[]} args N/A
   * @returns {boolean} true if any opponents are sleeping
  ...
  applyPostTurn(pokemon: Pokemon, passive: boolean, args: any[]): boolean | Promise<boolean> {
    let hadEffect: boolean = false;
    for (const opp of pokemon.getOpponents()) {
      opp.damageAndUpdate(Math.floor(Math.max(1, opp.getMaxHp() / 16)), HitResult.OTHER);
      pokemon.scene.queueMessage(i18next.t("abilityTriggers:badDreams", {pokemonName: `${getPokemonPrefix(opp)}${opp.name}`}));
      hadEffect = true;
    }
    for (const ally of pokemon.getPlayerPokemons()) {
      ally.damageAndUpdate(Math.floor(Math.max(1, ally.getMaxHp() / 16)), HitResult.OTHER);
      pokemon.scene.queueMessage(i18next.t("abilityTriggers:badDreams", {pokemonName: `${getPokemonPrefix(ally)}${ally.name}`}));
      hadEffect = true;
    }
    return hadEffect;
  }
...
export class PostBattleGemLootAbAttr extends PostBattleAbAttr {
  applyPostBattle(pokemon: Pokemon, passive: boolean, args: any[]): boolean {
    const postBattleLoot = pokemon.scene.currentBattle.postBattleLoot;
    if (postBattleLoot.length) {
      const randItem = Utils.randSeedItem(postBattleLoot);
...
export function initAbilities() {
  allAbilities.push(
    ...
    new Ability(Abilities.ADMIN_ABUSE, 12)
      .attr(MovePowerBoostAbAttr, (user, target, move) => move.hasFlag(MoveFlags.HAMMER_MOVE), 1.2),
    new Ability(Abilities.SUDDENLY, 12)
      .unimplemented(),
    new Ability(Abilities.CACOPHONY, 10)
      .attr(MoveImmunityAbAttr, (pokemon, attacker, move) => pokemon !== attacker && move.getMove().hasFlag(MoveFlags.SOUND_BASED))
      .ignorable(),
    new Ability(Abilities.JEWELRY, 10)
      .attr(PostBattleLootAbAttr),
    new Ability(Abilities.JEWLERY, 10)
      .attr(PostBattleLootAbAttr),
    new Ability(Abilities.FLARE_HEAL, 3)
      .attr(TypeImmunityHealAbAttr, Type.FIRE)
      .partial() // Healing not blocked by Heal Block
      .ignorable(),
    new Ability(Abilities.MORE_ROOM, 10)
      .unimplemented(),
    new Ability(Abilities.MADMAN, 10)
      .attr(PostDefendContactApplyTagChanceAbAttr, 30, BattlerTagType.CONFUSED),
    new Ability(Abilities.BLADEMASTER, 10)
      .attr(MovePowerBoostAbAttr, (user, target, move) => move.hasFlag(MoveFlags.HAMMER_MOVE), 1.2),
    new Ability(Abilities.STRIKER, 10)
      .attr(MovePowerBoostAbAttr, (user, target, move) => move.hasFlag(MoveFlags.KICKING_MOVE), 1.2),
    new Ability(Abilities.METAL_BODY, 10)
      .attr(ProtectStatAbAttr)
      .ignorable(),
    new Ability(Abilities.SHADOW_GUARD, 10)
      .attr(ReceivedMoveDamageMultiplierAbAttr,(target, user, move) => target.getHpRatio() === 1, 0.5),
    new Ability(Abilities.POZZED, 10)
      .attr(TypeImmunityHealAbAttr, Type.POISON)
      .partial() // Healing not blocked by Heal Block
      .ignorable(),
    new Ability(Abilities.CONCERT, 10).attr(PostTurnHurtAbAttr),
    new Ability(Abilities.AN_ABILITY, 10),
    new Ability(Abilities.ANY_ABILITY, 10),
    new Ability(Abilities.INVERSION, 10).unimplemented(),
    new Ability(Abilities.COMPACTION, 8)
      .attr(PostDefendStatChangeAbAttr, (target, user, move) => move.category !== MoveCategory.STATUS, BattleStat.SPD, -1, false, true)
      .bypassFaint(),
    new Ability(Abilities.COMPACTION, 10)
      .attr(PostDefendStatChangeAbAttr, (target, user, move) => move.type === Type.WATER && move.category !== MoveCategory.STATUS, BattleStat.DEF, 2),
    new Ability(Abilities.POLLUTION, 10)
      .attr(MoveTypeChangeAttr, Type.POISON, 2, (user, target, move) => target.getTypes().includes(Type.WATER) && target.getTypes().length === 1)
      .attr(MoveTypeChangeAttr, Type.POISON, 1.5, (user, target, move) => target.getTypes().includes(Type.WATER) && target.getTypes().length > 1),
    new Ability(Abilities.BOOMBOX, 10)
      .attr(MovePowerBoostAbAttr, (user, target, move) => move.hasFlag(MoveFlags.SOUND_BASED), 1.2),
    new Ability(Abilities.MAJESTIC, 10)
      .attr(FieldPriorityMoveImmunityAbAttr)
      .ignorable(),
    new Ability(Abilities.BIG_GUY, 10)
      .attr(PostSummonAddBattlerTagAbAttr, BattlerTagType.GROUNDED, 10).partial(),
    new Ability(Abilities.FUK_U, 10).attr(PostSummonChangeEnemyTypeAbAttr,Type.NORMAL, true),
    new Ability(Abilities.STINK_BOMB, 10)
      .attr(PostSummonWeatherChangeAbAttr, WeatherType.STINK)
      .attr(PostBiomeChangeWeatherChangeAbAttr, WeatherType.STINK),
    new Ability(Abilities.ALCHEMY, 10)
      .attr(CopyFaintedAllyAbilityAbAttr)
      .attr(UncopiableAbilityAbAttr),
    new Ability(Abilities.PUPPETEER, 10)
      .attr(MoveTypePowerBoostAbAttr, Type.BUG, 1.3),
    new Ability(Abilities.PHANTASMA, 10)

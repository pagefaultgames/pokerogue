...
class AnimFrame {
  ...
  constructor(
    ...
    if (zoomX) this.zoomX = zoomX;
    else if (init) this.zoomX = 0;
    if (zoomY) this.zoomY = zoomY;
    else if (init) this.zoomY = 0;
    if (angle) this.angle = angle;
    else if (init) this.angle = 0;
    if (mirror) this.mirror = mirror;
    else if (init) this.mirror = false;
    if (visible) this.visible = visible;
    else if (init) this.visible = false;
    if (blendType) this.blendType = blendType;
    else if (init) this.blendType = AnimBlendType.NORMAL;
    if (!init) {
...
class AnimTimedSoundEvent extends AnimTimedEvent {
  ...
  execute(scene: BattleScene, battleAnim: BattleAnim): integer {
    ...
    if (this.resourceName) {
      ...
      return Math.ceil(
        (scene.sound.get(this.resourceName).totalDuration * 1000) / 33.33,
      );
    } else
      return Math.ceil(
...
class AnimTimedUpdateBgEvent extends AnimTimedBgEvent {
  ...
  execute(scene: BattleScene, moveAnim: MoveAnim): integer {
    const tweenProps = {};
    if (this.bgX !== undefined) tweenProps["x"] = this.bgX * 0.5 - 320;
    if (this.bgY !== undefined) tweenProps["y"] = this.bgY * 0.5 - 284;
...
class AnimTimedAddBgEvent extends AnimTimedBgEvent {
  ...
  execute(scene: BattleScene, moveAnim: MoveAnim): integer {
    if (moveAnim.bgSprite) moveAnim.bgSprite.destroy();
...
export const moveAnims = new Map<
  Moves,
  AnimConfig | [AnimConfig, AnimConfig]
>();
  ...
  return new Promise((resolve) => {
    ...
    for (let ca = 0; ca < commonAnimIds.length; ca++) {
      ...
      commonAnimFetches.push(
        scene
          .cachedFetch(
            `./battle-anims/common-${commonAnimNames[ca]
              .toLowerCase()
              .replace(/\_/g, "-")}.json`,
          )
...
export function initMoveAnim(scene: BattleScene, move: Moves): Promise<void> {
  return new Promise((resolve) => {
    if (moveAnims.has(move)) {
      if (moveAnims.get(move) !== null) resolve();
      else {
        const loadedCheckTimer = setInterval(() => {
          if (moveAnims.get(move) !== null) {
            const chargeAttr =
              (allMoves[move]
                .getAttrs(ChargeAttr)
                .find(() => true) as ChargeAttr) ||
              (allMoves[move]
                .getAttrs(DelayedAttackAttr)
                .find(() => true) as DelayedAttackAttr);
          ...
          .then((ba) => {
            ...
            const chargeAttr =
              ...
              (allMoves[move]
                ...
                .find(() => true) as DelayedAttackAttr);
            if (chargeAttr)
              initMoveChargeAnim(scene, chargeAttr.chargeAnim).then(() =>
                resolve(),
              );
            else resolve();
          });
...
export function initMoveChargeAnim(
  scene: BattleScene,
  chargeAnim: ChargeAnim,
): Promise<void> {
  return new Promise((resolve) => {
    if (chargeAnims.has(chargeAnim)) {
      if (chargeAnims.get(chargeAnim) !== null) resolve();
      else {
        const loadedCheckTimer = setInterval(() => {
      ...
      scene
        .cachedFetch(
          `./battle-anims/${ChargeAnim[chargeAnim]
            .toLowerCase()
            .replace(/\_/g, "-")}.json`,
        )
        .then((response) => response.json())
        .then((ca) => {
          ...
          } else populateMoveChargeAnim(chargeAnim, ca);
          resolve();
        });
    }
  });
...
export function loadCommonAnimAssets(
  scene: BattleScene,
  startLoad?: boolean,
): Promise<void> {
  return new Promise((resolve) => {
    loadAnimAssets(scene, Array.from(commonAnims.values()), startLoad).then(
      () => resolve(),
    );
  });
}
...
export function loadMoveAnimAssets(
  ...
  return new Promise((resolve) => {
    const moveAnimations = moveIds
      .flatMap((m) => moveAnims.get(m) as AnimConfig);
...
function loadAnimAssets(
  ...
  return new Promise((resolve) => {
    ...
    for (const a of anims) {
      ...
      for (const ms of animSounds) sounds.add(ms);
      const animBackgrounds = a.getBackgroundResourceNames();
      for (const abg of animBackgrounds) backgrounds.add(abg);
      if (a.graphic) scene.loadSpritesheet(a.graphic, "battle_anims", 96);
    }
    for (const bg of backgrounds) scene.loadImage(bg, "battle_anims");
    for (const s of sounds) scene.loadSe(s, "battle_anims", s);
    if (startLoad) {
...
export abstract class BattleAnim {
  ...
  private getGraphicFrameData(
    ...
    for (const frame of frames) {
      ...
      ret
        .get(frame.target)
        .set(key, { x: x, y: y, scaleX: scaleX, scaleY: scaleY, angle: angle });
    }
  ...
  play(scene: BattleScene, callback?: Function) {
    ...
    const cleanUpAndComplete = () => {
      ...
      if (!this.isHideUser()) userSprite.setVisible(true);
      if (
        !this.isHideTarget() &&
        (targetSprite !== userSprite || !this.isHideUser())
      )
    ...
    if (!scene.moveAnimations) return cleanUpAndComplete();
    ...
    scene.tweens.addCounter({
      ...
      onRepeat: () => {
        ...
        for (const frame of spriteFrames) {
          if (frame.target !== AnimFrameTarget.GRAPHIC) {
            ...
            if (isUser && target === user) continue;
            const sprites =
              spriteCache[
            ...
            pokemonSprite.setPosition(
              graphicFrameData.x,
              graphicFrameData.y -
                (spriteSource.height / 2) *
                  (spriteSource.parentContainer.scale - 1),
            );
        ...
        if (anim.frameTimedEvents.has(f)) {
          for (const event of anim.frameTimedEvents.get(f))
            r = Math.max(
              anim.frames.length - f + event.execute(scene, this),
              r,
            );
        }
      ...
      onComplete: () => {
        for (const ms of Object.values(spriteCache).flat()) {
          if (ms && !ms.getData("locked")) ms.destroy();
        }
        if (r) {
          scene.tweens.addCounter({
...
export async function populateAnims() {
  const commonAnimNames = Utils.getEnumKeys(CommonAnim).map((k) =>
    k.toLowerCase(),
  );
  const commonAnimMatchNames = commonAnimNames.map((k) => k.replace(/\_/g, ""));
  const commonAnimIds = Utils.getEnumValues(CommonAnim) as CommonAnim[];
  const chargeAnimNames = Utils.getEnumKeys(ChargeAnim).map((k) =>
    k.toLowerCase(),
  );
  ...
  for (let a = 0; a < animsData.length; a++) {
    ...
    if (commonAnimId) commonAnims.set(commonAnimId, anim);
      ...
      switch (fieldName) {
        ...
        case "timing":
          ...
          for (let t = 0; t < timingEntries.length; t++) {
            ...
            const frameIndex = Number.parseInt(/frame: (\d+)/.exec(timingData)[1]);
            let resourceName = /name: "(.*?)"/
              .exec(timingData)[1]
              .replace("''", "");
            const timingType = Number.parseInt(/timingType: (\d)/.exec(timingData)[1]);
            let timedEvent: AnimTimedEvent;
            switch (timingType) {
              case 0:
            ...
            while ((propMatch = propPattern.exec(timingData))) {
              ...
              if (timedEvent.hasOwnProperty(prop)) timedEvent[prop] = value;
            }
  ...
  const animReplacer = (k, v) => {
    if (k === "id" && !v) return undefined;
    if (v instanceof Map) return Object.fromEntries(v);
    if (v instanceof AnimTimedEvent) v["eventType"] = v.getEventType();
    return v;
  };

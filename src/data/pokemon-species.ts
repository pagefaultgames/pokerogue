...
export function getPokemonSpecies(species: Species): PokemonSpecies {
  if (species >= 2000) return allSpecies.find((s) => s.speciesId === species);
...
export function getPokemonSpeciesForm(
  ...
  if (formIndex < retSpecies.forms?.length) return retSpecies.forms[formIndex];
  return retSpecies;
}
...
export function getFusedSpeciesName(
  ...
  if (splitNameB.length === 1) {
    if (fragBMatch) {
      ...
      if (lastCharA === fragB[0]) {
        if (/[aiu]/.test(lastCharA)) fragB = fragB.slice(1);
        else {
          const newCharMatch = new RegExp(`[^${lastCharA}]`).exec(fragB);
          if (newCharMatch?.index > 0) fragB = fragB.slice(newCharMatch.index);
        }
      }
    } else fragB = speciesBName;
  } else fragB = splitNameB[splitNameB.length - 1];
  ...
  return `${speciesAPrefix || speciesBPrefix}${fragA}${fragB}${
    speciesBSuffix || speciesASuffix
  }`;
}
...
export abstract class PokemonSpeciesForm {
  ...
  getSpriteId(
    ...
    const showGenderDiffs =
      this.genderDiffs &&
      female &&
      ![SpeciesFormKey.MEGA, SpeciesFormKey.GIGANTAMAX].find(
        (k) => formSpriteKey === k,
      );

    const baseSpriteKey = `${showGenderDiffs ? "female__" : ""}${
      this.speciesId
    ...
    return `${back ? "back__" : ""}${
      shiny && (!variantSet || (!variant && !variantSet[variant || 0]))
        ? "shiny__"
        : ""
    }${baseSpriteKey}${
      shiny && variantSet && variantSet[variant || 0] === 2
        ? `_${variant + 1}`
        : ""
    }`;
  }
  ...
  getIconAtlasKey(
    ...
    const isVariant =
      shiny &&
      variantData[this.speciesId] &&
      variantData[this.speciesId][variant];
  ...
  validateStarterMoveset(moveset: StarterMoveset, eggMoves: integer): boolean {
    ...
    for (const moveId of moveset) {
      if (speciesEggMoves.hasOwnProperty(rootSpeciesId)) {
        ...
        if (eggMoveIndex > -1 && eggMoves & Math.pow(2, eggMoveIndex)) continue;
      }
      if (
  ...
  loadAssets(
    ...
    return new Promise((resolve) => {
      ...
      scene.loadPokemonAtlas(
        spriteKey,
        this.getSpriteAtlasPath(female, formIndex, shiny, variant),
      );
      scene.load.audio(
        this.getCryKey(formIndex),
        `audio/cry/${this.getCryKey(formIndex)}.m4a`,
      );
      scene.load.once(Phaser.Loader.Events.COMPLETE, () => {
        ...
        let spritePath = this.getSpriteAtlasPath(
          female,
          formIndex,
          shiny,
          variant,
          ...
          .replace(/_[1-3]$/, "");
        const useExpSprite =
          scene.experimentalSprites && scene.hasExpSprite(spriteKey);
        if (useExpSprite) spritePath = `exp/${spritePath}`;
        let variantSet: VariantSet;
        let config = variantData;
        spritePath
          .split("/")
  ...
  cry(
    ...
    if (cry?.pendingRemove) cry = null;
    cry = scene.playSound(cry || cryKey, soundConfig);
    if (ignorePlay) cry.stop();
    return cry;
  }
  ...
  generateCandyColors(scene: BattleScene): integer[][] {
    ...
    return Array.from(paletteColors.keys()).map(
      (c) => Object.values(rgbaFromArgb(c)) as integer[],
    );
  }
}
  ...
  getSpeciesForLevel(
    ...
    if (prevolutionLevels.length) {
      for (let pl = prevolutionLevels.length - 1; pl >= 0; pl--) {
        ...
        if (level < prevolutionLevel[1]) return prevolutionLevel[0];
      }
    }
    ...
    for (const ev of evolutions) {
      ...
      if (!forTrainer && isRegionalEvolution) evolutionChance = 0;
      else {
        if (ev.wildDelay === SpeciesWildEvolutionDelay.NONE) {
          if (strength === PartyMemberStrength.STRONGER) evolutionChance = 1;
          else {
          ...
          if (
            ...
            ].find((ev) => ev.speciesId === this.speciesId).level;
            if (prevolutionLevel > 1) evolutionLevel = prevolutionLevel;
          }
    ...
    const randValue =
      evolutionPool.size === 1 ? 0 : Utils.randSeedInt(totalWeight);
  ...
  getEvolutionLevels() {
    ...
    if (pokemonEvolutions.hasOwnProperty(this.speciesId)) {
      for (const e of pokemonEvolutions[this.speciesId]) {
        ...
        for (const npl of nextEvolutionLevels) evolutionLevels.push(npl);
      }
    }
    ...
    for (const p of allEvolvingPokemon) {
      for (const e of pokemonEvolutions[p]) {
        if (
          ...
          for (const spl of subPrevolutionLevels) prevolutionLevels.push(spl);
        }
      }
    }

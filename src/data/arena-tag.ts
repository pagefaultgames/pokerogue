...
export abstract class ArenaTag {
  ...
  onRemove(arena: Arena): void {
    arena.scene.queueMessage(
      `${this.getMoveName()}\'s effect wore off${
        this.side === ArenaTagSide.PLAYER
          ? "\non your side"
          : this.side === ArenaTagSide.ENEMY
            ? "\non the foe's side"
            : ""
      }.`,
    );
...
class ReflectTag extends WeakenMoveScreenTag {
  ...
  onAdd(arena: Arena): void {
    arena.scene.queueMessage(
      `Reflect reduced the damage of physical moves${
        this.side === ArenaTagSide.PLAYER
          ? "\non your side"
          : this.side === ArenaTagSide.ENEMY
            ? "\non the foe's side"
...
class LightScreenTag extends WeakenMoveScreenTag {
  ...
  onAdd(arena: Arena): void {
    arena.scene.queueMessage(
      `Light Screen reduced the damage of special moves${
        this.side === ArenaTagSide.PLAYER
          ? "\non your side"
          : this.side === ArenaTagSide.ENEMY
            ? "\non the foe's side"
...
class AuroraVeilTag extends WeakenMoveScreenTag {
  ...
  onAdd(arena: Arena): void {
    arena.scene.queueMessage(
      `Aurora Veil reduced the damage of moves${
        this.side === ArenaTagSide.PLAYER
          ? "\non your side"
          : this.side === ArenaTagSide.ENEMY
            ? "\non the foe's side"
...
class ToxicSpikesTag extends ArenaTrapTag {
  ...
  onRemove(arena: Arena): void {
    if (!this.neutralized) super.onRemove(arena);
  }
  ...
  getMatchupScoreMultiplier(pokemon: Pokemon): number {
    if (
      pokemon.isGrounded() ||
      !pokemon.canSetStatus(StatusEffect.POISON, true)
    )
      return 1;
    if (pokemon.isOfType(Type.POISON)) return 1.25;
...
class StickyWebTag extends ArenaTrapTag {
  ...
  activateTrap(pokemon: Pokemon): boolean {
    if (pokemon.isGrounded()) {
      ...
      if (!cancelled.value) {
        pokemon.scene.queueMessage(
          `The opposing ${pokemon.name} was caught in a sticky web!`,
        );
...
export class TrickRoomTag extends ArenaTag {
  ...
  onAdd(arena: Arena): void {
    arena.scene.queueMessage(
      getPokemonMessage(
        arena.scene.getPokemonById(this.sourceId),
        " twisted\nthe dimensions!",
      ),
    );
  }

...
import { invalidWimpOutMoves, noAbilityTypeOverrideMoves } from "#moves/invalid-moves";
...
export interface PostDamageAbAttrParams extends AbAttrBaseParams {
  /**
   * The {@linkcode Pokemon} that caused the instance of damage, or `undefined` if the damage was not from a Pokemon
   */
...
export class PostDamageForceSwitchAbAttr extends PostDamageAbAttr {
  ...
  public override canApply({ pokemon, source, damage }: PostDamageAbAttrParams): boolean {
    ...
    if (currentPhase.is("MoveEffectPhase") && !this.passesMoveChecks(source, currentPhase.move)) {
      return false;
    }

    if (!this.passesHpChecks(pokemon, damage)) {
      return false;
    }
  ...
  /**
   * Perform checks against the current move being used to determine if this ability should be allowed to switch out.
   ...
   * @returns Whether the move checks passed
   */
  private passesMoveChecks(source: Pokemon | undefined, move: Move): boolean {
    // Wimp Out and Emergency Exit...
    // will not activate from damage instances from invalid moves
    if (invalidWimpOutMoves.has(move.id)) {
      return false;
    }

    // will not activate if the Pokemon is currently in the air from Sky Drop.
    ...
    if (lastMove?.move === MoveId.SKY_DROP && lastMove.result === MoveResult.OTHER) {
      return false;
    }
  ...
  /**
   * Perform HP checks to determine if this ability should switch the user out.
   * @param user - The {@linkcode Pokemon} with this ability
   * @param damage - The amount of damage taken
   * @returns Whether the damage instance knocked the user below {@linkcode hpRatio}.
   */
  private passesHpChecks(user: Pokemon, damage: number): boolean {
    // NB: This occurs _after_ the damage instance has been dealt,
    // so `pokemon.hp` contains the post-taking damage hp value.
    const hpNeededToSwitch = user.getMaxHp() * this.hpRatio;
    return user.hp < hpNeededToSwitch && user.hp + damage >= hpNeededToSwitch;
  }

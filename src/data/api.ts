...
export async function printPokemon() {
  ...
  const abilities = Utils.getEnumKeys(Abilities).map((a) =>
  ...
  for (const p of pokemonArr) {
    ...
    if (pokemon.id > 10000) {
      ...
      if (!dexIdMatch) continue;

      const matchingSpecies = pokemonSpeciesList[Number.parseInt(dexIdMatch[1]) - 1];
      ...
      if (ignoredForms.filter((f) => formName.indexOf(f) > -1).length) continue;
      ...
      if (regionalForms.indexOf(shortFormName) > -1)
        ...
        const [formAbility1, formAbility2, formAbilityHidden] = [
          Math.max(
            abilities.indexOf(
              pokemon.abilities.find((a) => a.slot === 1)?.ability.name,
            ),
            0,
          ),
          Math.max(
            abilities.indexOf(
              pokemon.abilities.find((a) => a.slot === 2)?.ability.name,
            ),
            0,
          ),
          Math.max(
            abilities.indexOf(
              pokemon.abilities.find((a) => a.slot === 3)?.ability.name,
            ),
            0,
          ),
        ];
            ...
            moveData.version_group_details
              ...
              .forEach((verData) => {
                ...
                const moveName = moveData.move.name
                  ...
                  .replace(/\_/g, "")
                  .replace(/\-/g, "_");
                const moveId = Math.max(
                  Utils.getEnumKeys(Moves).indexOf(moveName),
                  0,
                );
                ...
                if (isMoveVer && learnMethod === "level-up")
                  speciesFormLevelMoves[speciesKey][pokemonForm.formIndex].push(
                    [verData.level_learned_at, moveId],
                  );
                ...
                if (
                  ...
                  const speciesIndex = moveTmSpecies[moveId].findIndex(
                    (s) => s[0] === speciesKey,
                  );
                  if (speciesIndex === -1)
                    moveTmSpecies[moveId].push([speciesKey, formName]);
                  else
                    (moveTmSpecies[moveId][speciesIndex] as string[]).push(
                      formName,
                    );
                }
              });
    ...
    const matchingExistingSpecies = allSpecies.find(
      (s) => Species[s.speciesId] === speciesKey,
    );
    ...
    if (!region)
      while (
        ++generationIndex < 9 &&
        dexId > generationDexNumbers[generationIndex]
      );
    else generationIndex = regionalForms.indexOf(region.toLowerCase()) + 6;
    ...
    if (moveVer) {
      pokemon.moves.forEach((moveData) => {
        ...
        switch (verData.move_learn_method.name) {
          case "level-up":
            speciesLevelMoves[speciesKey].push([
    ...
    for (const f of pokemon.forms) {
      ...
      const matchingForm =
        ...
        matchingExistingSpecies.forms.length > formIndex
          ? matchingExistingSpecies.forms.find(
              (f2) =>
                f2.formKey === form.form_name || f2.formName === form.form_name,
            ) || matchingExistingSpecies.forms[formIndex]
          : null;
      const formName = matchingForm
        ? matchingForm.formName
        : form.form_names.find((fn) => fn.language.name === "en")?.name ||
          form.form_name;
  ...
  for (const pokemonSpecies of pokemonSpeciesList) {
    ...
    enumStr += `  ${speciesKey}${
      pokemonSpecies.speciesId >= 2000 ? ` = ${pokemonSpecies.speciesId}` : ""
    },\n`;
    pokemonSpeciesStr += `    new PokemonSpecies(Species.${speciesKey}, "${
      pokemonSpecies.name
    }", ${pokemonSpecies.generation}, ${pokemonSpecies.subLegendary}, ${
      pokemonSpecies.legendary
    }, ${pokemonSpecies.mythical}, "${pokemonSpecies.species}", Type.${
      Type[pokemonSpecies.type1]
    }, ${
      pokemonSpecies.type2 ? `Type.${Type[pokemonSpecies.type2]}` : "null"
    }, ${pokemonSpecies.height}, ${pokemonSpecies.weight}, Abilities.${
      Abilities[pokemonSpecies.ability1]
    }, Abilities.${Abilities[pokemonSpecies.ability2]}, Abilities.${
      Abilities[pokemonSpecies.abilityHidden]
    }, ${pokemonSpecies.baseTotal}, ${pokemonSpecies.baseStats[0]}, ${
      pokemonSpecies.baseStats[1]
    }, ${pokemonSpecies.baseStats[2]}, ${pokemonSpecies.baseStats[3]}, ${
      pokemonSpecies.baseStats[4]
    }, ${pokemonSpecies.baseStats[5]}, ${pokemonSpecies.catchRate}, ${
      pokemonSpecies.baseFriendship
    }, ${pokemonSpecies.baseExp}, GrowthRate.${
      GrowthRate[pokemonSpecies.growthRate]
    }, ${pokemonSpecies.malePercent}, ${pokemonSpecies.genderDiffs}`;
    if (pokemonSpecies.forms.length > 1) {
      pokemonSpeciesStr += `, ${pokemonSpecies.canChangeForm},`;
      for (const form of pokemonSpecies.forms)
        pokemonSpeciesStr += `\n      new PokemonForm("${form.formName}", "${
          form.formName
        }", Type.${Type[form.type1]}, ${
          form.type2 ? `Type.${Type[form.type2]}` : "null"
        }, ${form.height}, ${form.weight}, Abilities.${
          Abilities[form.ability1]
        }, Abilities.${Abilities[form.ability2]}, Abilities.${
          Abilities[form.abilityHidden]
        }, ${form.baseTotal}, ${form.baseStats[0]}, ${form.baseStats[1]}, ${
          form.baseStats[2]
        }, ${form.baseStats[3]}, ${form.baseStats[4]}, ${form.baseStats[5]}, ${
          form.catchRate
        }, ${form.baseFriendship}, ${form.baseExp}${
          form.genderDiffs ? ", true" : ""
        }),`;
      pokemonSpeciesStr += "\n    ";
    }
    pokemonSpeciesStr += `),\n`;
  ...
  for (const species of Object.keys(speciesLevelMoves)) {
    ...
    const orderedLevelMoves = speciesLevelMoves[species].sort(
  ...
  for (const species of Object.keys(speciesFormLevelMoves)) {
    ...
    for (const f of Object.keys(speciesFormLevelMoves[species])) {
      ...
      for (const lm of orderedLevelMoves)
        speciesFormLevelMovesStr += `      [ ${lm[0]}, Moves.${
          Moves[lm[1]]
        } ],\n`;
  ...
  for (const moveId of Object.keys(moveTmSpecies)) {
    tmSpeciesStr += `  [Moves.${Moves[Number.parseInt(moveId)]}]: [\n`;
    for (const species of moveTmSpecies[moveId]) {
      if (typeof species === "string")
        tmSpeciesStr += `    Species.${species},\n`;
      else {
        const matchingExistingSpecies = allSpecies.find(
          (s) => Species[s.speciesId] === species[0],
        );
        const forms = (species as string[]).slice(1);
        if (
          matchingExistingSpecies &&
          (!pokemonFormLevelMoves.hasOwnProperty(
            matchingExistingSpecies.speciesId,
          ) ||
            matchingExistingSpecies.forms.length <= 1 ||
            (matchingExistingSpecies.forms.length === 2 &&
              matchingExistingSpecies.forms[1].formKey.indexOf(
                SpeciesFormKey.MEGA,
              ) > -1) ||
...
export async function printAbilities() {
  ...
  for (const a of abilities) {
    ...
    const abilityEnumName = ability.name
      .toUpperCase()
      .replace(/\_/g, "")
      .replace(/\-/g, "_");
    enumStr += `\n  ${abilityEnumName},`;
    ...
    let matchingLine =
      matchingLineIndex > -1 ? abilityContent.slice(matchingLineIndex) : null;
    if (matchingLine)
      ...
      for (const version of versions) {
        if (
          (flavorText = ability.flavor_text_entries.find(
            (fte) =>
              fte.language.name === "en" && fte.version_group.name === version,
          )?.flavor_text) ||
          ""
        ) {
          if (flavorText.indexOf("forgotten") > -1) continue;
          break;
        }
      }
    } else if (matchingLine) flavorText = allAbilities[ability.id].description;
    abilityStr += `\n    new Ability(Abilities.${abilityEnumName}, "${abilityName}", "${
      flavorText
        ?.replace(/\n/g, "\\n")
        .replace(/ {2}/g, " ")
        .replace(/’/g, "'") || ""
    }", ${generationMap[ability.generation.name]})`;
    if (matchingLine && matchingLine.length > 1) {
      const newLineIndex = matchingLine.indexOf("\n");
      if (newLineIndex > -1) abilityStr += matchingLine.slice(newLineIndex);
    }
    abilityStr += ",";
  }
...
export async function printMoves() {
  ...
  for (const m of moves) {
    ...
    const matchingLineIndex = moveContent.search(
      new RegExp(
        `new (?:Attack|(?:Self)?Status)Move\\\(Moves.${Moves[move.id]},`,
      ),
    );
    let matchingLine =
      matchingLineIndex > -1 ? moveContent.slice(matchingLineIndex) : null;
    if (matchingLine)
      matchingLine = matchingLine.slice(
        0,
        matchingLine.search(/,(?: \/\/.*?)?(?:\r)?\n[ \t]+(?:new|\);)/),
    ...
    } else if (matchingLine) flavorText = allMoves[move.id].effect;
    const moveTarget = targetMap[move.target.name];
    moveStr += `\n    new ${
      move.damage_class.name !== "status"
        ? "Attack"
        : (moveTarget === MoveTarget.USER ? "Self" : "") + "Status"
    }Move(Moves.${moveEnumName}, "${moveName}", Type.${move.type.name.toUpperCase()}${
      move.damage_class.name !== "status"
        ? `, MoveCategory.${move.damage_class.name.toUpperCase()}`
        : ""
    }${move.damage_class.name !== "status" ? `, ${move.power || -1}` : ""}, ${
      move.accuracy || -1
    }, ${move.pp}, "${
      flavorText
        ?.replace(/\n/g, "\\n")
        .replace(/ {2}/g, " ")
        .replace(/’/g, "'") || ""
    }", ${move.effect_chance || -1}, ${move.priority}, ${
      generationMap[move.generation.name]
    })`;
...
export async function printTmSpecies() {
  ...
  const moveIds = Object.keys(tmSpecies).map((k) => Number.parseInt(k) as Moves);
  ...
  for (const moveId of moveIds) {
    ...
    for (const species of move.learned_by_pokemon) {
      ...
      const dexId = Number.parseInt(dexIdMatch[1]);
      ...
      if (dexId < 10000) matchingSpecies = allSpecies[dexId - 1];
      else {
        const pokemon = await api.pokemon.getPokemonById(dexId);

        const speciesDexIdMatch = /\/(\d+)\//.exec(pokemon.species.url);
        if (!speciesDexIdMatch) continue;

        const speciesDexId = Number.parseInt(speciesDexIdMatch[1]);
        ...
        if (regionKey) {
          ...
          matchingSpecies = allSpecies.find(
            (s) =>
              Species[s.speciesId] ===
              `${regionKey.toUpperCase()}_${speciesKey}`,
          );
        } else matchingSpecies = allSpecies[speciesDexId - 1];
      }
      ...
      if (matchingIndex === -1)
        moveTmSpecies[moveId].push(
          !formKey ? speciesKey : [speciesKey, formKey],
        );
      else {

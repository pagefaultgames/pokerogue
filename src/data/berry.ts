...
export function getBerryPredicate(berryType: BerryType): BerryPredicate {
  ...
  case BerryType.LEPPA:
    return (pokemon: Pokemon) => {
      const threshold = new Utils.NumberHolder(0.25);
      applyAbAttrs(ReduceBerryUseThresholdAbAttr, pokemon, null, threshold);
      return !!pokemon.getMoveset().filter(m => !!m).find(m => !m.getPpRatio());
    };
  }
}
...
export function getBerryEffectFunc(berryType: BerryType): BerryEffectFunc {
  ...
  case BerryType.LEPPA:
    return (pokemon: Pokemon) => {
      ...
      const ppRestoreMove = pokemon.getMoveset().filter(m => !!m).find(m => !m.getPpRatio()) ? pokemon.getMoveset().filter(m => !!m).find(m => !m.getPpRatio()) : pokemon.getMoveset().filter(m => !!m).find(m => m.getPpRatio() < 1);

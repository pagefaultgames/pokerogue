...
export class TrainerPartyCompoundTemplate extends TrainerPartyTemplate {
  ...
  getStrength(index: integer): PartyMemberStrength {
    ...
    for (const template of this.templates) {
      if (t + template.size > index) return template.getStrength(index - t);
      t += template.size;
    }
  ...
  isSameSpecies(index: integer): boolean {
    let t = 0;
    for (const template of this.templates) {
      if (t + template.size > index) return template.isSameSpecies(index - t);
      t += template.size;
    }
  ...
  isBalanced(index: integer): boolean {
    let t = 0;
    for (const template of this.templates) {
      if (t + template.size > index) return template.isBalanced(index - t);
      t += template.size;
    }
...
export const trainerPartyTemplates = {
  ...
  THREE_WEAK_BALANCED: new TrainerPartyTemplate(
    3,
    PartyMemberStrength.WEAK,
    false,
    true,
  ),
  FOUR_WEAKER: new TrainerPartyTemplate(4, PartyMemberStrength.WEAKER),
  FOUR_WEAKER_SAME: new TrainerPartyTemplate(
    4,
    PartyMemberStrength.WEAKER,
    true,
  ),
  ...
  FOUR_WEAK_BALANCED: new TrainerPartyTemplate(
    4,
    PartyMemberStrength.WEAK,
    false,
    true,
  ...
  FIVE_WEAK_BALANCED: new TrainerPartyTemplate(
    5,
    PartyMemberStrength.WEAK,
    false,
    true,
  ...
  SIX_WEAKER_SAME: new TrainerPartyTemplate(
    6,
    PartyMemberStrength.WEAKER,
    true,
  ),
  SIX_WEAK_SAME: new TrainerPartyTemplate(6, PartyMemberStrength.WEAKER, true),
  SIX_WEAK_BALANCED: new TrainerPartyTemplate(
    6,
    PartyMemberStrength.WEAK,
    false,
    true,
...
export class TrainerConfig {
  ...
  constructor(trainerType: TrainerType, allowLegendaries?: boolean) {
    ...
    this.speciesFilter = (species) =>
      (allowLegendaries ||
        (!species.legendary && !species.subLegendary && !species.mythical)) &&
  ...
  getSpriteKey(female?: boolean): string {
    let ret = this.getKey();
    if (this.hasGenders) ret += `_${female ? "f" : "m"}`;
    return ret;
  }
  ...
  setHasGenders(
    nameFemale?: string,
    femaleEncounterBgm?: TrainerType | string,
  ): TrainerConfig {
    ...
    if (femaleEncounterBgm)
      this.femaleEncounterBgm =
        typeof femaleEncounterBgm === "number"
          ? TrainerType[femaleEncounterBgm]
              .toString()
              .replace(/\_/g, " ")
              .toLowerCase()
          : femaleEncounterBgm;
  ...
  setHasDouble(
    nameDouble: string,
    doubleEncounterBgm?: TrainerType | string,
  ): TrainerConfig {
    this.hasDouble = true;
    ...
    if (doubleEncounterBgm)
      this.doubleEncounterBgm =
        typeof doubleEncounterBgm === "number"
          ? TrainerType[doubleEncounterBgm]
              .toString()
              .replace(/\_/g, " ")
              .toLowerCase()
          : doubleEncounterBgm;
  ...
  setEncounterBgm(encounterBgm: TrainerType | string): TrainerConfig {
    this.encounterBgm =
      typeof encounterBgm === "number"
        ? TrainerType[encounterBgm].toString().toLowerCase()
        : encounterBgm;
    return this;
  }
  ...
  setSpeciesFilter(
    ...
    this.speciesFilter = allowLegendaries
      ? speciesFilter
      : (species) => speciesFilter(species) && baseFilter(species);
    return this;
  }
  ...
  setModifierRewardFuncs(
    ...modifierTypeFuncs: (() => ModifierTypeFunc)[]
  ): TrainerConfig {
  ...
  initForGymLeader(
    ...
    signatureSpecies.forEach((speciesPool, s) => {
      if (!Array.isArray(speciesPool)) speciesPool = [speciesPool];
      this.setPartyMemberFunc(-(s + 1), getRandomPartyMemberFunc(speciesPool));
    });
    if (specialtyTypes.length) {
      this.setSpeciesFilter(
        (p) => specialtyTypes.find((t) => p.isOfType(t)) !== undefined,
      );
    ...
    this.setGenModifiersFunc((party) => {
      ...
      return getRandomTeraModifiers(
        party,
        waveIndex >= 100 ? 1 : 0,
        specialtyTypes.length ? specialtyTypes : null,
      );
    });
    return this;
  }
  ...
  initForEliteFour(
    ...
    if (specialtyTypes.length) {
      this.setSpeciesFilter(
        (p) => specialtyTypes.find((t) => p.isOfType(t)) && p.baseTotal >= 450,
      );
      this.setSpecialtyTypes(...specialtyTypes);
    } else this.setSpeciesFilter((p) => p.baseTotal >= 450);
  ...
  loadAssets(scene: BattleScene, variant: TrainerVariant): Promise<void> {
    return new Promise((resolve) => {
      ...
      if (isDouble) scene.loadAtlas(partnerTrainerKey, "trainer");
...
function getWavePartyTemplate(
  ...
  return templates[
    Math.min(
      Math.max(
        Math.ceil(
          (scene.gameMode.getWaveForDifficulty(
            scene.currentBattle?.waveIndex || startingWave,
            true,
          ) -
            20) /
            30,
...
function getSpeciesFilterRandomPartyMemberFunc(
  ...
  speciesFilter = (species: PokemonSpecies) =>
    (allowLegendaries ||
      (!species.legendary && !species.subLegendary && !species.mythical)) &&
    !species.isTrainerForbidden() &&
    originalSpeciesFilter(species);
  return (
    scene: BattleScene,
    level: integer,
    strength: PartyMemberStrength,
  ) => {
    const ret = scene.addEnemyPokemon(
      getPokemonSpecies(
        scene
          .randomSpecies(
...
function getRandomTeraModifiers(
  party: EnemyPokemon[],
  count: integer,
  types?: Type[],
): PersistentModifier[] {
  ...
  for (let t = 0; t < Math.min(count, party.length); t++) {
    ...
    ret.push(
      modifierTypes
        .TERA_SHARD()
        .generateType(null, [
          Utils.randSeedItem(types ? types : party[randomIndex].getTypes()),
        ])
...
export const trainerConfigs: TrainerConfigs = {
  ...
  [TrainerType.DOCTOR]: new TrainerConfig(++t)
    ...
    .setEncounterBgm(TrainerType.CLERK)
    .setSpeciesFilter(
      (s) => !!s.getLevelMoves().find((plm) => plm[1] === Moves.HEAL_PULSE),
    ),
  ...
  [TrainerType.HARLEQUIN]: new TrainerConfig(++t)
    ...
    .setSpeciesFilter(
      (s) => tmSpecies[Moves.TRICK_ROOM].indexOf(s.speciesId) > -1,
    ),
  ...
  [TrainerType.MUSICIAN]: new TrainerConfig(++t)
    .setEncounterBgm(TrainerType.ROUGHNECK)
    .setSpeciesFilter(
      (s) => !!s.getLevelMoves().find((plm) => plm[1] === Moves.SING),
    ),
  ...
  [TrainerType.STRIKER]: new TrainerConfig(++t)
    .setMoneyMultiplier(1.2)
    .setEncounterBgm(TrainerType.CYCLIST),
  [TrainerType.STUDENT]: new TrainerConfig(++t)
    .setMoneyMultiplier(0.75)
    .setEncounterBgm(TrainerType.YOUNGSTER)
    .setHasGenders(undefined, "lass")
    .setHasDouble("Students")
    .setSpeciesPools({
      [TrainerPoolTier.COMMON]: [
        Species.ODDISH,
        Species.EXEGGCUTE,
  ...
  [TrainerType.BROCK]: new TrainerConfig((t = TrainerType.BROCK))
    .initForGymLeader([Species.GEODUDE, Species.ONIX], Type.ROCK)
    .setBattleBgm("battle_kanto_gym"),
  [TrainerType.MISTY]: new TrainerConfig(++t)
    .initForGymLeader([Species.STARYU, Species.PSYDUCK], Type.WATER)
    .setBattleBgm("battle_kanto_gym"),
  [TrainerType.LT_SURGE]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.VOLTORB, Species.PIKACHU, Species.ELECTABUZZ],
      Type.ELECTRIC,
    )
    .setBattleBgm("battle_kanto_gym"),
  [TrainerType.ERIKA]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.ODDISH, Species.BELLSPROUT, Species.TANGELA, Species.HOPPIP],
      Type.GRASS,
    )
    .setBattleBgm("battle_kanto_gym"),
  [TrainerType.JANINE]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.VENONAT, Species.SPINARAK, Species.ZUBAT],
      Type.POISON,
    )
    .setBattleBgm("battle_kanto_gym"),
  [TrainerType.SABRINA]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.ABRA, Species.MR_MIME, Species.ESPEON],
      Type.PSYCHIC,
    )
    .setBattleBgm("battle_kanto_gym"),
  [TrainerType.BLAINE]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.GROWLITHE, Species.PONYTA, Species.MAGMAR],
      Type.FIRE,
    )
    .setBattleBgm("battle_kanto_gym"),
  [TrainerType.GIOVANNI]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.SANDILE, Species.MURKROW, Species.NIDORAN_M, Species.NIDORAN_F],
      Type.DARK,
    )
    .setBattleBgm("battle_kanto_gym"),
  [TrainerType.FALKNER]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.PIDGEY, Species.HOOTHOOT, Species.DODUO],
      Type.FLYING,
    )
    .setBattleBgm("battle_johto_gym"),
  [TrainerType.BUGSY]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.SCYTHER, Species.HERACROSS, Species.SHUCKLE, Species.PINSIR],
      Type.BUG,
    )
    .setBattleBgm("battle_johto_gym"),
  [TrainerType.WHITNEY]: new TrainerConfig(++t)
    .initForGymLeader([Species.GIRAFARIG, Species.MILTANK], Type.NORMAL)
    .setBattleBgm("battle_johto_gym"),
  [TrainerType.MORTY]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.GASTLY, Species.MISDREAVUS, Species.SABLEYE],
      Type.GHOST,
    )
    .setBattleBgm("battle_johto_gym"),
  [TrainerType.CHUCK]: new TrainerConfig(++t)
    .initForGymLeader([Species.POLIWRATH, Species.MANKEY], Type.FIGHTING)
    .setBattleBgm("battle_johto_gym"),
  [TrainerType.JASMINE]: new TrainerConfig(++t)
    .initForGymLeader([Species.MAGNEMITE, Species.STEELIX], Type.STEEL)
    .setBattleBgm("battle_johto_gym"),
  [TrainerType.PRYCE]: new TrainerConfig(++t)
    .initForGymLeader([Species.SEEL, Species.SWINUB], Type.ICE)
    .setBattleBgm("battle_johto_gym"),
  [TrainerType.CLAIR]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.DRATINI, Species.HORSEA, Species.GYARADOS],
      Type.DRAGON,
    )
    .setBattleBgm("battle_johto_gym"),
  [TrainerType.ROXANNE]: new TrainerConfig(++t)
    .initForGymLeader([Species.GEODUDE, Species.NOSEPASS], Type.ROCK)
    .setBattleBgm("battle_hoenn_gym"),
  [TrainerType.BRAWLY]: new TrainerConfig(++t)
    .initForGymLeader([Species.MACHOP, Species.MAKUHITA], Type.FIGHTING)
    .setBattleBgm("battle_hoenn_gym"),
  [TrainerType.WATTSON]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.MAGNEMITE, Species.VOLTORB, Species.ELECTRIKE],
      Type.ELECTRIC,
    )
    .setBattleBgm("battle_hoenn_gym"),
  [TrainerType.FLANNERY]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.SLUGMA, Species.TORKOAL, Species.NUMEL],
      Type.FIRE,
    )
    .setBattleBgm("battle_hoenn_gym"),
  [TrainerType.NORMAN]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.SLAKOTH, Species.SPINDA, Species.CHANSEY, Species.KANGASKHAN],
      Type.NORMAL,
    )
    .setBattleBgm("battle_hoenn_gym"),
  [TrainerType.WINONA]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.SWABLU, Species.WINGULL, Species.TROPIUS, Species.SKARMORY],
      Type.FLYING,
    )
    .setBattleBgm("battle_hoenn_gym"),
  [TrainerType.TATE]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.SOLROCK, Species.NATU, Species.CHIMECHO, Species.GALLADE],
      Type.PSYCHIC,
    )
    .setBattleBgm("battle_hoenn_gym"),
  [TrainerType.LIZA]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.LUNATONE, Species.SPOINK, Species.BALTOY, Species.GARDEVOIR],
      Type.PSYCHIC,
    )
    .setBattleBgm("battle_hoenn_gym"),
  [TrainerType.JUAN]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.HORSEA, Species.BARBOACH, Species.SPHEAL, Species.RELICANTH],
      Type.WATER,
    )
    .setBattleBgm("battle_hoenn_gym"),
  [TrainerType.ROARK]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.CRANIDOS, Species.LARVITAR, Species.GEODUDE],
      Type.ROCK,
    )
    .setBattleBgm("battle_sinnoh_gym"),
  [TrainerType.GARDENIA]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.ROSELIA, Species.TANGELA, Species.TURTWIG],
      Type.GRASS,
    )
    .setBattleBgm("battle_sinnoh_gym"),
  [TrainerType.MAYLENE]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.LUCARIO, Species.MEDITITE, Species.CHIMCHAR],
      Type.FIGHTING,
    )
    .setBattleBgm("battle_sinnoh_gym"),
  [TrainerType.CRASHER_WAKE]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.BUIZEL, Species.MAGIKARP, Species.PIPLUP],
      Type.WATER,
    )
    .setBattleBgm("battle_sinnoh_gym"),
  [TrainerType.FANTINA]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.MISDREAVUS, Species.DRIFLOON, Species.SPIRITOMB],
      Type.GHOST,
    )
    .setBattleBgm("battle_sinnoh_gym"),
  [TrainerType.BYRON]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.SHIELDON, Species.BRONZOR, Species.AGGRON],
      Type.STEEL,
    )
    .setBattleBgm("battle_sinnoh_gym"),
  [TrainerType.CANDICE]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.SNEASEL, Species.SNOVER, Species.SNORUNT],
      Type.ICE,
    )
    .setBattleBgm("battle_sinnoh_gym"),
  [TrainerType.VOLKNER]: new TrainerConfig(++t)
    .initForGymLeader(
      [Species.SHINX, Species.CHINCHOU, Species.ROTOM],
      Type.ELECTRIC,
    )
    .setBattleBgm("battle_sinnoh_gym"),
  [TrainerType.CILAN]: new TrainerConfig(++t).initForGymLeader(
    [Species.PANSAGE, Species.COTTONEE, Species.PETILIL],
    Type.GRASS,
  ),
  [TrainerType.CHILI]: new TrainerConfig(++t).initForGymLeader(
    [Species.PANSEAR, Species.DARUMAKA, Species.HEATMOR],
    Type.FIRE,
  ),
  [TrainerType.CRESS]: new TrainerConfig(++t).initForGymLeader(
    [Species.PANPOUR, Species.BASCULIN, Species.TYMPOLE],
    Type.WATER,
  ),
  [TrainerType.CHEREN]: new TrainerConfig(++t).initForGymLeader(
    [Species.LILLIPUP, Species.MINCCINO, Species.PATRAT],
    Type.NORMAL,
  ),
  [TrainerType.LENORA]: new TrainerConfig(++t).initForGymLeader(
    [Species.KANGASKHAN, Species.DEERLING, Species.AUDINO],
    Type.NORMAL,
  ),
  [TrainerType.ROXIE]: new TrainerConfig(++t).initForGymLeader(
    [Species.VENIPEDE, Species.TRUBBISH, Species.SKORUPI],
    Type.POISON,
  ),
  [TrainerType.BURGH]: new TrainerConfig(++t).initForGymLeader(
    [Species.SEWADDLE, Species.SHELMET, Species.KARRABLAST],
    Type.BUG,
  ),
  [TrainerType.ELESA]: new TrainerConfig(++t).initForGymLeader(
    [Species.EMOLGA, Species.BLITZLE, Species.JOLTIK],
    Type.ELECTRIC,
  ),
  [TrainerType.CLAY]: new TrainerConfig(++t).initForGymLeader(
    [Species.DRILBUR, Species.SANDILE, Species.GOLETT],
    Type.GROUND,
  ),
  [TrainerType.SKYLA]: new TrainerConfig(++t).initForGymLeader(
    [Species.DUCKLETT, Species.WOOBAT, Species.RUFFLET],
    Type.FLYING,
  ),
  [TrainerType.BRYCEN]: new TrainerConfig(++t).initForGymLeader(
    [Species.CRYOGONAL, Species.VANILLITE, Species.CUBCHOO],
    Type.ICE,
  ),
  [TrainerType.DRAYDEN]: new TrainerConfig(++t).initForGymLeader(
    [Species.DRUDDIGON, Species.AXEW, Species.DEINO],
    Type.DRAGON,
  ),
  [TrainerType.MARLON]: new TrainerConfig(++t).initForGymLeader(
    [Species.WAILMER, Species.FRILLISH, Species.TIRTOUGA],
    Type.WATER,
  ),
  [TrainerType.VIOLA]: new TrainerConfig(++t).initForGymLeader(
    [Species.SURSKIT, Species.SCATTERBUG],
    Type.BUG,
  ),
  [TrainerType.GRANT]: new TrainerConfig(++t).initForGymLeader(
    [Species.AMAURA, Species.TYRUNT],
    Type.ROCK,
  ),
  [TrainerType.KORRINA]: new TrainerConfig(++t).initForGymLeader(
    [Species.HAWLUCHA, Species.LUCARIO, Species.MIENFOO],
    Type.FIGHTING,
  ),
  [TrainerType.RAMOS]: new TrainerConfig(++t).initForGymLeader(
    [Species.SKIDDO, Species.HOPPIP, Species.BELLSPROUT],
    Type.GRASS,
  ),
  [TrainerType.CLEMONT]: new TrainerConfig(++t).initForGymLeader(
    [Species.HELIOPTILE, Species.MAGNEMITE, Species.EMOLGA],
    Type.ELECTRIC,
  ),
  [TrainerType.VALERIE]: new TrainerConfig(++t).initForGymLeader(
    [Species.SYLVEON, Species.MAWILE, Species.MR_MIME],
    Type.FAIRY,
  ),
  [TrainerType.OLYMPIA]: new TrainerConfig(++t).initForGymLeader(
    [Species.ESPURR, Species.SIGILYPH, Species.SLOWKING],
    Type.PSYCHIC,
  ),
  [TrainerType.WULFRIC]: new TrainerConfig(++t).initForGymLeader(
    [Species.BERGMITE, Species.SNOVER, Species.CRYOGONAL],
    Type.ICE,
  ),
  [TrainerType.MILO]: new TrainerConfig(++t).initForGymLeader(
    [Species.GOSSIFLEUR, Species.APPLIN, Species.BOUNSWEET],
    Type.GRASS,
  ),
  [TrainerType.NESSA]: new TrainerConfig(++t).initForGymLeader(
    [Species.CHEWTLE, Species.ARROKUDA, Species.WIMPOD],
    Type.WATER,
  ),
  [TrainerType.KABU]: new TrainerConfig(++t).initForGymLeader(
    [Species.SIZZLIPEDE, Species.VULPIX, Species.TORKOAL],
    Type.FIRE,
  ),
  [TrainerType.BEA]: new TrainerConfig(++t).initForGymLeader(
    [Species.GALAR_FARFETCHD, Species.MACHOP, Species.CLOBBOPUS],
    Type.FIGHTING,
  ),
  [TrainerType.ALLISTER]: new TrainerConfig(++t).initForGymLeader(
    [Species.GALAR_YAMASK, Species.GALAR_CORSOLA, Species.GASTLY],
    Type.GHOST,
  ),
  [TrainerType.OPAL]: new TrainerConfig(++t).initForGymLeader(
    [Species.MILCERY, Species.TOGETIC, Species.GALAR_WEEZING],
    Type.FAIRY,
  ),
  [TrainerType.BEDE]: new TrainerConfig(++t).initForGymLeader(
    [Species.HATENNA, Species.GALAR_PONYTA, Species.GARDEVOIR],
    Type.FAIRY,
  ),
  [TrainerType.GORDIE]: new TrainerConfig(++t).initForGymLeader(
    [Species.ROLYCOLY, Species.STONJOURNER, Species.BINACLE],
    Type.ROCK,
  ),
  [TrainerType.MELONY]: new TrainerConfig(++t).initForGymLeader(
    [Species.SNOM, Species.GALAR_DARUMAKA, Species.GALAR_MR_MIME],
    Type.ICE,
  ),
  [TrainerType.PIERS]: new TrainerConfig(++t).initForGymLeader(
    [Species.GALAR_ZIGZAGOON, Species.SCRAGGY, Species.INKAY],
    Type.DARK,
  ),
  [TrainerType.MARNIE]: new TrainerConfig(++t).initForGymLeader(
    [Species.IMPIDIMP, Species.PURRLOIN, Species.MORPEKO],
    Type.DARK,
  ),
  [TrainerType.RAIHAN]: new TrainerConfig(++t).initForGymLeader(
    [Species.DURALUDON, Species.TURTONATOR, Species.GOOMY],
    Type.DRAGON,
  ),
  [TrainerType.KATY]: new TrainerConfig(++t).initForGymLeader(
    [Species.NYMBLE, Species.TAROUNTULA, Species.HERACROSS],
    Type.BUG,
  ),
  [TrainerType.BRASSIUS]: new TrainerConfig(++t).initForGymLeader(
    [Species.SMOLIV, Species.SHROOMISH, Species.ODDISH],
    Type.GRASS,
  ),
  [TrainerType.IONO]: new TrainerConfig(++t).initForGymLeader(
    [Species.TADBULB, Species.WATTREL, Species.VOLTORB],
    Type.ELECTRIC,
  ),
  [TrainerType.KOFU]: new TrainerConfig(++t).initForGymLeader(
    [Species.VELUZA, Species.WIGLETT, Species.WINGULL],
    Type.WATER,
  ),
  [TrainerType.LARRY]: new TrainerConfig(++t).initForGymLeader(
    [Species.STARLY, Species.DUNSPARCE, Species.KOMALA],
    Type.NORMAL,
  ),
  [TrainerType.RYME]: new TrainerConfig(++t).initForGymLeader(
    [Species.GREAVARD, Species.SHUPPET, Species.MIMIKYU],
    Type.GHOST,
  ),
  [TrainerType.TULIP]: new TrainerConfig(++t).initForGymLeader(
    [Species.GIRAFARIG, Species.FLITTLE, Species.RALTS],
    Type.PSYCHIC,
  ),
  [TrainerType.GRUSHA]: new TrainerConfig(++t).initForGymLeader(
    [Species.CETODDLE, Species.ALOLA_VULPIX, Species.CUBCHOO],
    Type.ICE,
  ),

  [TrainerType.LORELEI]: new TrainerConfig(
    (t = TrainerType.LORELEI),
  ).initForEliteFour(
    [Species.SLOWBRO, Species.LAPRAS, Species.DEWGONG, Species.ALOLA_SANDSLASH],
    Type.ICE,
  ),
  [TrainerType.BRUNO]: new TrainerConfig(++t).initForEliteFour(
    [Species.ONIX, Species.HITMONCHAN, Species.HITMONLEE, Species.ALOLA_GOLEM],
    Type.FIGHTING,
  ),
  [TrainerType.AGATHA]: new TrainerConfig(++t).initForEliteFour(
    [Species.GENGAR, Species.ARBOK, Species.CROBAT, Species.ALOLA_MAROWAK],
    Type.GHOST,
  ),
  [TrainerType.LANCE]: new TrainerConfig(++t).initForEliteFour(
    [
      Species.DRAGONITE,
      Species.GYARADOS,
      Species.AERODACTYL,
  ...
  [TrainerType.WILL]: new TrainerConfig(++t).initForEliteFour(
    [Species.XATU, Species.JYNX, Species.SLOWBRO, Species.EXEGGUTOR],
    Type.PSYCHIC,
  ),
  [TrainerType.KOGA]: new TrainerConfig(++t).initForEliteFour(
    [Species.WEEZING, Species.VENOMOTH, Species.CROBAT, Species.TENTACRUEL],
    Type.POISON,
  ),
  [TrainerType.KAREN]: new TrainerConfig(++t).initForEliteFour(
    [Species.UMBREON, Species.HONCHKROW, Species.HOUNDOOM, Species.WEAVILE],
    Type.DARK,
  ),
  [TrainerType.SIDNEY]: new TrainerConfig(++t).initForEliteFour(
    [Species.SHIFTRY, Species.SHARPEDO, Species.ABSOL, Species.ZOROARK],
    Type.DARK,
  ),
  [TrainerType.PHOEBE]: new TrainerConfig(++t).initForEliteFour(
    [Species.SABLEYE, Species.DUSKNOIR, Species.BANETTE, Species.CHANDELURE],
    Type.GHOST,
  ),
  [TrainerType.GLACIA]: new TrainerConfig(++t).initForEliteFour(
    [Species.GLALIE, Species.WALREIN, Species.FROSLASS, Species.ABOMASNOW],
    Type.ICE,
  ),
  [TrainerType.DRAKE]: new TrainerConfig(++t).initForEliteFour(
    [Species.ALTARIA, Species.SALAMENCE, Species.FLYGON, Species.KINGDRA],
    Type.DRAGON,
  ),
  [TrainerType.AARON]: new TrainerConfig(++t).initForEliteFour(
    [Species.SCIZOR, Species.HERACROSS, Species.VESPIQUEN, Species.DRAPION],
    Type.BUG,
  ),
  [TrainerType.BERTHA]: new TrainerConfig(++t).initForEliteFour(
    [Species.WHISCASH, Species.HIPPOWDON, Species.GLISCOR, Species.RHYPERIOR],
    Type.GROUND,
  ),
  [TrainerType.FLINT]: new TrainerConfig(++t).initForEliteFour(
    [Species.FLAREON, Species.HOUNDOOM, Species.RAPIDASH, Species.INFERNAPE],
    Type.FIRE,
  ),
  [TrainerType.LUCIAN]: new TrainerConfig(++t).initForEliteFour(
    [Species.MR_MIME, Species.GALLADE, Species.BRONZONG, Species.ALAKAZAM],
    Type.PSYCHIC,
  ),
  [TrainerType.SHAUNTAL]: new TrainerConfig(++t).initForEliteFour(
    [Species.COFAGRIGUS, Species.CHANDELURE, Species.GOLURK, Species.DRIFBLIM],
    Type.GHOST,
  ),
  [TrainerType.MARSHAL]: new TrainerConfig(++t).initForEliteFour(
    [Species.TIMBURR, Species.MIENFOO, Species.THROH, Species.SAWK],
    Type.FIGHTING,
  ),
  [TrainerType.GRIMSLEY]: new TrainerConfig(++t).initForEliteFour(
    [Species.LIEPARD, Species.KINGAMBIT, Species.SCRAFTY, Species.KROOKODILE],
    Type.DARK,
  ),
  [TrainerType.CAITLIN]: new TrainerConfig(++t).initForEliteFour(
    [Species.MUSHARNA, Species.GOTHITELLE, Species.SIGILYPH, Species.REUNICLUS],
    Type.PSYCHIC,
  ),
  [TrainerType.MALVA]: new TrainerConfig(++t).initForEliteFour(
    [Species.PYROAR, Species.TORKOAL, Species.CHANDELURE, Species.TALONFLAME],
    Type.FIRE,
  ),
  [TrainerType.SIEBOLD]: new TrainerConfig(++t).initForEliteFour(
    [Species.CLAWITZER, Species.GYARADOS, Species.BARBARACLE, Species.STARMIE],
    Type.WATER,
  ),
  [TrainerType.WIKSTROM]: new TrainerConfig(++t).initForEliteFour(
    [Species.KLEFKI, Species.PROBOPASS, Species.SCIZOR, Species.AEGISLASH],
    Type.STEEL,
  ),
  [TrainerType.DRASNA]: new TrainerConfig(++t).initForEliteFour(
    [Species.DRAGALGE, Species.DRUDDIGON, Species.ALTARIA, Species.NOIVERN],
    Type.DRAGON,
  ),
  [TrainerType.HALA]: new TrainerConfig(++t).initForEliteFour(
    [Species.HARIYAMA, Species.BEWEAR, Species.CRABOMINABLE, Species.POLIWRATH],
    Type.FIGHTING,
  ),
  [TrainerType.MOLAYNE]: new TrainerConfig(++t).initForEliteFour(
    [
      Species.KLEFKI,
      Species.MAGNEZONE,
      Species.METAGROSS,
  ...
  [TrainerType.OLIVIA]: new TrainerConfig(++t).initForEliteFour(
    [Species.ARMALDO, Species.CRADILY, Species.ALOLA_GOLEM, Species.LYCANROC],
    Type.ROCK,
  ),
  [TrainerType.ACEROLA]: new TrainerConfig(++t).initForEliteFour(
    [Species.BANETTE, Species.DRIFBLIM, Species.DHELMISE, Species.PALOSSAND],
    Type.GHOST,
  ),
  [TrainerType.KAHILI]: new TrainerConfig(++t).initForEliteFour(
    [Species.BRAVIARY, Species.HAWLUCHA, Species.ORICORIO, Species.TOUCANNON],
    Type.FLYING,
  ),
  [TrainerType.RIKA]: new TrainerConfig(++t).initForEliteFour(
    [Species.WHISCASH, Species.DONPHAN, Species.CAMERUPT, Species.CLODSIRE],
    Type.GROUND,
  ),
  [TrainerType.POPPY]: new TrainerConfig(++t).initForEliteFour(
    [
      Species.COPPERAJAH,
      Species.BRONZONG,
      Species.CORVIKNIGHT,
  ...
  [TrainerType.LARRY_ELITE]: new TrainerConfig(++t).initForEliteFour(
    [Species.STARAPTOR, Species.FLAMIGO, Species.ALTARIA, Species.TROPIUS],
    Type.NORMAL,
    Type.FLYING,
  ),
  [TrainerType.HASSEL]: new TrainerConfig(++t).initForEliteFour(
    [Species.NOIVERN, Species.HAXORUS, Species.DRAGALGE, Species.BAXCALIBUR],
    Type.DRAGON,
  ),
  [TrainerType.CRISPIN]: new TrainerConfig(++t).initForEliteFour(
    [Species.TALONFLAME, Species.CAMERUPT, Species.MAGMORTAR, Species.BLAZIKEN],
    Type.FIRE,
  ),
  [TrainerType.AMARYS]: new TrainerConfig(++t).initForEliteFour(
    [Species.SKARMORY, Species.EMPOLEON, Species.SCIZOR, Species.METAGROSS],
    Type.STEEL,
  ),
  [TrainerType.LACEY]: new TrainerConfig(++t).initForEliteFour(
    [
      Species.EXCADRILL,
      Species.PRIMARINA,
      Species.ALCREMIE,
  ...
  [TrainerType.DRAYTON]: new TrainerConfig(++t).initForEliteFour(
    [Species.DRAGONITE, Species.ARCHALUDON, Species.FLYGON, Species.SCEPTILE],
    Type.DRAGON,
  ),
  ...
  [TrainerType.STEVEN]: new TrainerConfig(++t)
    .initForChampion([
      Species.METAGROSS,
      [Species.DIALGA, Species.PALKIA],
      Species.SKARMORY,
      Species.AGGRON,
      Species.CARBINK,
    ])
    .setBattleBgm("battle_hoenn_champion"),
  ...
  [TrainerType.ALDER]: new TrainerConfig(++t).initForChampion([
    Species.VOLCARONA,
    Species.GROUDON,
    Species.BOUFFALANT,
    Species.ACCELGOR,
  ...
  [TrainerType.DIANTHA]: new TrainerConfig(++t).initForChampion([
    Species.HAWLUCHA,
    Species.XERNEAS,
    Species.GOURGEIST,
    Species.GOODRA,
  ...
  [TrainerType.HAU]: new TrainerConfig(++t).initForChampion([
  ...
  [TrainerType.GEETA]: new TrainerConfig(++t).initForChampion([
    Species.GLIMMORA,
    Species.MIRAIDON,
    Species.ESPATHRA,
    Species.VELUZA,
  ...
  [TrainerType.NEMONA]: new TrainerConfig(++t).initForChampion([
    Species.LYCANROC,
    Species.KORAIDON,
    Species.KOMMO_O,
    Species.PAWMOT,
  ...
  [TrainerType.KIERAN]: new TrainerConfig(++t).initForChampion([
    Species.POLITOED,
    [Species.OGERPON, Species.TERAPAGOS],
    Species.HYDRAPPLE,
    Species.PORYGON_Z,
    Species.GRIMMSNARL,
  ]),
  [TrainerType.LEON]: new TrainerConfig(++t).initForChampion([
    Species.DRAGAPULT,
    [Species.ZACIAN, Species.ZAMAZENTA],
    Species.SEISMITOAD,
    Species.AEGISLASH,
    Species.CHARIZARD,
  ]),
  ...
  [TrainerType.RIVAL_4]: new TrainerConfig(++t)
    ...
    .setGenModifiersFunc((party) => {
      ...
      return [
        modifierTypes
          .TERA_SHARD()
          .generateType(null, [starter.species.type1])
          .withIdFromFunc(modifierTypes.TERA_SHARD)

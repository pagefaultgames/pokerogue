...
import {
  STARTING_WAVE_OVERRIDE,
  OPP_SPECIES_OVERRIDE,
  SEED_OVERRIDE,
  STARTING_BIOME_OVERRIDE,
  DOUBLE_BATTLE_OVERRIDE,
} from "./overrides";
...
export default class BattleScene extends SceneBase {
  ...
  loadPokemonAtlas(key: string, atlasPath: string, experimental?: boolean) {
    if (experimental === undefined) experimental = this.experimentalSprites;
    const variant = atlasPath.includes("variant/") || /_[0-3]$/.test(atlasPath);
    if (experimental) experimental = this.hasExpSprite(key);
    if (variant) atlasPath = atlasPath.replace("variant/", "");
    this.load.atlas(
      key,
  ...
  async preload() {
    if (DEBUG_RNG) {
      ...
      Phaser.Math.RND.realInRange = function (
        ...
        args.push(
          `seed: ${scene.rngSeedOverride || scene.waveSeed || scene.seed}`,
        );
        if (scene.rngOffset) args.push(`offset: ${scene.rngOffset}`);
        console.log(...args);
        return ret;
      };
    }
  ...
  launchBattle() {
    ...
    [
      ...
      if (a instanceof Phaser.GameObjects.Sprite) a.setOrigin(0, 0);
      field.add(a);
    });
    ...
    const trainer = this.addFieldSprite(
      ...
      0,
      `trainer_${
        this.gameData.gender === PlayerGender.FEMALE ? "f" : "m"
      }_back`,
    );
  ...
  initSession(): void {
    if (this.sessionPlayTime === null) this.sessionPlayTime = 0;
    ...
    this.playTimeTimer = this.time.addEvent({
      ...
      repeat: -1,
      callback: () => {
        if (this.gameData) this.gameData.gameStats.playTime++;
        if (this.sessionPlayTime !== null) this.sessionPlayTime++;
      },
    });
  ...
  async initExpSprites(): Promise<void> {
    if (expSpriteKeys.length) return;
    this.cachedFetch("./exp-sprites.json")
      .then((res) => res.json())
      .then((keys) => {
        if (Array.isArray(keys)) expSpriteKeys.push(...keys);
        Promise.resolve();
      });
  }
  ...
  initStarterColors(): Promise<void> {
    return new Promise((resolve) => {
      if (starterColors) return resolve();
  ...
  hasExpSprite(key: string): boolean {
    ...
    if (keyMatch[1]) k += "b";
    if (keyMatch[3]) k += "f";
    if (keyMatch[5]) k += keyMatch[5];
    if (!expSpriteKeys.includes(k)) return false;
    return true;
  }
  ...
  getField(activeOnly = false): Pokemon[] {
  ...
  addPlayerPokemon(
    ...
    if (postProcess) postProcess(pokemon);
    pokemon.init();
    return pokemon;
  }
  ...
  addEnemyPokemon(
    ...
    if (OPP_SPECIES_OVERRIDE) species = getPokemonSpecies(OPP_SPECIES_OVERRIDE);
  ...
  addPokemonIcon(
    ...
    if (pokemon.isFusion()) {
      ...
      if (originX !== 0.5) container.x -= originalWidth * (originX - 0.5);
      if (originY !== 0) container.y -= originalHeight * originY;
    } else {
      if (originX !== 0.5) container.x -= icon.width * (originX - 0.5);
      if (originY !== 0) container.y -= icon.height * originY;
    }
  ...
  reset(
    ...
    if (clearData) this.gameData = new GameData(this);
    ...
    for (const p of this.getParty()) p.destroy();
    this.party = [];
    for (const p of this.getEnemyParty()) p.destroy();
    ...
    if (reloadI18n) {
      ...
      for (const item of localizable) item.localize();
    }
  ...
  newBattle(
    ...
    const newWaveIndex =
      waveIndex || (this.currentBattle?.waveIndex || startingWave - 1) + 1;
    ...
    if (
      this.gameMode.hasFixedBattles &&
      fixedBattles.hasOwnProperty(newWaveIndex) &&
      ...
      if (newTrainer) this.field.add(newTrainer);
    } else {
      if (!this.gameMode.hasTrainers) newBattleType = BattleType.WILD;
      else if (battleType === undefined)
        newBattleType = this.gameMode.isWaveTrainer(newWaveIndex, this.arena)
          ? BattleType.TRAINER
          : BattleType.WILD;
      else newBattleType = battleType;

      if (newBattleType === BattleType.TRAINER) {
        const trainerType = this.arena.randomTrainerType(newWaveIndex);
        let doubleTrainer = false;
        if (trainerConfigs[trainerType].doubleOnly) doubleTrainer = true;
          ...
          playerField.forEach((p) =>
            applyAbAttrs(DoubleBattleChanceAbAttr, p, null, doubleChance),
          );
    ...
    } else if (!battleConfig) newDouble = !!double;

    if (DOUBLE_BATTLE_OVERRIDE) newDouble = true;
    ...
    if (!waveIndex && lastBattle) {
      ...
      for (const pokemon of this.getParty()) {
        if (pokemon) {
          if (resetArenaState) pokemon.resetBattleData();
  ...
  setFieldScale(scale: number, instant = false): Promise<void> {
    return new Promise((resolve) => {
      scale *= 6;
      if (this.field.scale === scale) return resolve();
  ...
  getSpeciesFormIndex(
    ...
    if (!species.forms?.length) return 0;
  ...
  getEncounterBossSegments(
    ...
    if (this.gameMode.isDaily && this.gameMode.isWaveFinal(waveIndex)) return 5;
  ...
  trySpreadPokerus(): void {
    ...
    party.forEach((pokemon, p) => {
      if (!pokemon.pokerus || infectedIndexes.indexOf(p) > -1) return;

      this.executeWithSeedOffset(
        () => {
          if (p) spread(p, -1);
          if (p < party.length - 1) spread(p, 1);
        },
  ...
  updateAndShowLuckText(duration: integer): void {
    ...
    if (luckValue < 14) this.luckText.setTint(getLuckTextTint(luckValue));
    else this.luckText.setTint(0x83a55a, 0xee384a, 0x5271cd, 0x7b487b);
  ...
  updateUIPositions(): void {
    const enemyModifierCount = this.enemyModifiers.filter((m) =>
      m.isIconVisible(this),
    ).length;
    this.waveCountText.setY(
  ...
  addFaintedEnemyScore(enemy: EnemyPokemon): void {
    let scoreIncrease =
      ...
      ((enemy.ivs.reduce((iv: integer, total: integer) => (total += iv), 0) /
        93) *
        0.2 +
        0.8);
    this.findModifiers(
    ...
    if (enemy.isBoss()) scoreIncrease *= Math.sqrt(enemy.bossSegments);
    this.currentBattle.battleScore += Math.ceil(scoreIncrease);
  }

  getMaxExpLevel(ignoreLevelCap?: boolean): integer {
    if (ignoreLevelCap) return Number.MAX_SAFE_INTEGER;
  ...
  randomSpecies(
    ...
    if (fromArenaPool) return this.arena.randomSpecies(waveIndex, level);
  ...
  generateRandomBiome(waveIndex: integer): Biome {
    ...
    const biomes = Utils.getEnumValues(Biome).slice(
      1,
      Utils.getEnumValues(Biome).filter((b) => b >= 40).length * -1,
    );
    ...
    for (const biome of biomes) {
      if (randInt < biomeThresholds[biome]) return biome;
    }
  ...
  playBgm(bgmName?: string, fadeOut?: boolean): void {
    ...
    if (fadeOut && !this.bgm) fadeOut = false;
    ...
    this.load.once(Phaser.Loader.Events.COMPLETE, () => {
      loaded = true;
      if (!fadeOut || !this.bgm.isPlaying) playNewBgm();
    });
    if (fadeOut) {
      const onBgmFaded = () => {
    ...
    if (this.sound) {
      for (const sound of this.sound.getAllPlaying())
        (sound as AnySound).setVolume(
          this.masterVolume *
            (this.bgmCache.has(sound.key) ? this.bgmVolume : this.seVolume),
        );
    }
  }

  fadeOutBgm(duration: integer = 500, destroy = true): boolean {
  ...
  playSound(sound: string | AnySound, config?: object): AnySound {
    if (config) {
      ...
      else config["volume"] = this.masterVolume * this.seVolume;
    } else config = { volume: this.masterVolume * this.seVolume };
  ...
  playSoundWithoutBgm(soundName: string, pauseDuration?: integer): AnySound {
    ...
    if (this.bgmResumeTimer) this.bgmResumeTimer.destroy();
    if (resumeBgm) {
  ...
  toggleInvert(invert: boolean): void {
    if (invert) this.cameras.main.setPostPipeline(InvertPostFX);
    else this.cameras.main.removePostPipeline("InvertPostFX");
  }
  ...
  shiftPhase(): void {
    ...
    if (!this.phaseQueue.length) this.populatePhaseQueue();
    this.currentPhase = this.phaseQueue.shift();
    this.currentPhase.start();
  }

  overridePhase(phase: Phase): boolean {
    if (this.standbyPhase) return false;
  ...
  pushMovePhase(movePhase: MovePhase, priorityOverride?: integer): void {
    const movePriority = new Utils.IntegerHolder(
      priorityOverride !== undefined
        ? priorityOverride
        : movePhase.move.getMove().priority,
    );
    applyAbAttrs(
      IncrementMovePriorityAbAttr,
      movePhase.pokemon,
    ...
    const lowerPriorityPhase = this.phaseQueue.find(
      (p) =>
        p instanceof MovePhase &&
        p.move.getMove().priority < movePriority.value,
    );
    if (lowerPriorityPhase)
      this.phaseQueue.splice(
        this.phaseQueue.indexOf(lowerPriorityPhase),
        0,
        movePhase,
      );
    else this.pushPhase(movePhase);
  }
  ...
  queueMessage(
    ...
    if (!defer) this.unshiftPhase(phase);
    else this.pushPhase(phase);
  }
  ...
  addModifier(
    ...
    return new Promise((resolve) => {
      ...
      if (modifier instanceof PersistentModifier) {
        ...
        for (const rm of modifiersToRemove) this.removeModifier(rm);
        ...
        if (playSound && !this.sound.get(soundName)) this.playSound(soundName);
  ...
  addEnemyModifier(
    ...
    return new Promise((resolve) => {
      ...
      if (
        (modifier as PersistentModifier).add(this.enemyModifiers, false, this)
      ) {
        if (
          modifier instanceof PokemonFormChangeItemModifier ||
          modifier instanceof TerastallizeModifier
        ...
        for (const rm of modifiersToRemove) this.removeModifier(rm, true);
      }
      if (!ignoreUpdate)
  ...
  tryTransferHeldItemModifier(
    ...
    target: Pokemon,
    transferStack: boolean,
    playSound: boolean,
    instant?: boolean,
    ignoreUpdate?: boolean,
  ): Promise<boolean> {
    return new Promise((resolve) => {
      const source = itemModifier.pokemonId
        ? itemModifier.getPokemon(target.scene)
        : null;
      const cancelled = new Utils.BooleanHolder(false);
      Utils.executeIf(source && source.isPlayer() !== target.isPlayer(), () =>
        applyAbAttrs(BlockItemTheftAbAttr, source, cancelled),
      ).then(() => {
        if (cancelled.value) return resolve(false);
        ...
        if (matchingModifier) {
          ...
          const countTaken = transferStack
            ? Math.min(
  ...
  generateEnemyModifiers(): Promise<void> {
    return new Promise((resolve) => {
      ...
      if (isFinalBoss) chances = Math.ceil(chances * 2.5);
      ...
      party.forEach((enemyPokemon: EnemyPokemon, i: integer) => {
        ...
        if (isBoss) upgradeChance /= 2;
        if (isFinalBoss) upgradeChance /= 8;
        ...
        for (let c = 0; c < chances; c++) {
          if (!Utils.randSeedInt(modifierChance)) count++;
        }
        if (isBoss) count = Math.max(count, Math.floor(chances / 2));
        getEnemyModifierTypesForWave(
          difficultyWaveIndex,
          count,
          [enemyPokemon],
          ...
          mt.newModifier(enemyPokemon).add(this.enemyModifiers, false, this),
        );
      });
  ...
  updateModifiers(player?: boolean, instant?: boolean): Promise<void> {
    if (player === undefined) player = true;
    return new Promise((resolve) => {
      ...
      this.updatePartyForModifiers(
        player ? this.getParty() : this.getEnemyParty(),
        instant,
      ).then(() => {
        (player ? this.modifierBar : this.enemyModifierBar).updateModifiers(
          modifiers,
        );
        if (!player) this.updateUIPositions();
        resolve();
      });
    });
  }
  ...
  findModifiers(
    ...
    return (player ? this.modifiers : this.enemyModifiers).filter((m) =>
      (modifierFilter as ModifierPredicate)(m),
    );
  }
  ...
  findModifier(
    ...
    return (player ? this.modifiers : this.enemyModifiers).find((m) =>
      (modifierFilter as ModifierPredicate)(m),
    );
  }
  ...
  triggerPokemonFormChange(
    ...
    if (pokemonFormChanges.hasOwnProperty(pokemon.species.speciesId)) {
      const matchingFormChange = pokemonFormChanges[
      ...
      if (matchingFormChange) {
        ...
        else if (delayed) this.pushPhase(phase);
        else this.unshiftPhase(phase);
        return true;
      }
    }
  ...
  validateAchvs(
    achvType: { new (...args: any[]): Achv },
    ...args: any[]
  ): void {
    const filteredAchvs = Object.values(achvs).filter(
      (a) => a instanceof achvType,
    );
    for (const achv of filteredAchvs) this.validateAchv(achv, args);
  }

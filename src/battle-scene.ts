...
export default class BattleScene extends SceneBase {
    ...
    public gameSpeed: integer = 10000;
    ...
    public moveAnimations: boolean = false;
    public expGainsSpeed: integer = 100;
    public skipSeenDialogues: boolean = true;
    ...
    private phaseChangeCallbacks: Array<(newPhase: Phase) => void> = [];

    addPhaseChangeListener(callback: (newPhase: Phase) => void) {
        this.phaseChangeCallbacks.push(callback);
    }

    private notifyPhaseChange(newPhase: Phase) {
        this.phaseChangeCallbacks.forEach((callback) => callback(newPhase));
    }
    ...
    loadPokemonAtlas(key: string, atlasPath: string, experimental?: boolean) {
        ...
        const variant =
            atlasPath.includes("variant/") || /_[0-3]$/.test(atlasPath);
        if (experimental) {
    ...
    async preload() {
        if (DEBUG_RNG) {
            ...
            Phaser.Math.RND.realInRange = function (
                ...
                args.push(
                    `seed: ${
                        scene.rngSeedOverride || scene.waveSeed || scene.seed
                    }`
                );
                if (scene.rngOffset) {
    ...
    launchBattle() {
        ...
        this.anims.create({
            key: "prompt",
            frames: this.anims.generateFrameNumbers("prompt", {
                start: 1,
                end: 4,
            }),
            frameRate: 6,
    ...
    getPokemonById(pokemonId: integer): Pokemon {
        ...
        return (
            findInParty(this.getParty()) || findInParty(this.getEnemyParty())
        );
    }
    ...
    addEnemyPokemon(
        ...
        if (boss && !dataSource) {
            const secondaryIvs = Utils.getIvsFromId(
                Utils.randSeedInt(4294967295)
            );
    ...
    addPokemonIcon(
        ...
        const icon = this.add.sprite(
            0,
            0,
            pokemon.getIconAtlasKey(ignoreOverride)
        );
    ...
    setSeed(seed: string): void {
        ...
        this.offsetGym =
            this.gameMode.isClassic && this.getGeneratedOffsetGym();
    }
    ...
    newBattle(
        ...
        const newWaveIndex =
            waveIndex ||
            (this.currentBattle?.waveIndex || _startingWave - 1) + 1;
        ...
        if (
            ...
            if (!this.gameMode.hasTrainers) {
                ...
                newBattleType = this.gameMode.isWaveTrainer(
                    newWaveIndex,
                    this.arena
                )
                    ? BattleType.TRAINER
        ...
        if (
            ...
            Overrides.SINGLE_BATTLE_OVERRIDE
        ) {
            newDouble = false;
        }
        ...
        if (!waveIndex && lastBattle) {
            ...
            if (
                ...
                let w =
                    lastBattle.waveIndex - ((lastBattle.waveIndex % 10) - 1);
                let biomeWaves = 1;
            ...
            this.getEnemyParty().forEach((enemyPokemon) =>
    ...
    getSpeciesFormIndex(
        ...
        switch (species.speciesId) {
            ...
            case Species.TOXTRICITY:
                ...
                if (
                    nature !== undefined &&
                    lowkeyNatures.indexOf(nature) > -1
                ) {
                    return 1;
                }
                return 0;
                ...
                isBoss =
                    ...
                    (this.gameMode.hasRandomBosses &&
                        Utils.randSeedInt(100) <
                            Math.min(
                                Math.max(Math.ceil((waveIndex - 250) / 50), 0) *
                                    2,
                                30
                            ));
            }, waveIndex << 2);
    ...
    animateMoneyChanged(positiveChange: boolean): void {
        ...
        const deltaScale =
            this.moneyText.scale * 0.14 * (positiveChange ? 1 : -1);
        ...
        this.tweens.add({
            ...
            yoyo: true,
            onComplete: (_) =>
                this.moneyText.setShadowColor(
                    getTextColor(TextStyle.MONEY, true)
                ),
        });
    }
    ...
    updateUIPositions(): void {
        ...
        const biomeWaveTextHeight =
            this.biomeWaveText.getBottomLeft().y -
            this.biomeWaveText.getTopLeft().y;
        this.biomeWaveText.setY(
            -(this.game.canvas.height / 6) +
                (enemyModifierCount
                    ? enemyModifierCount <= 12
                        ? 15
                        : 24
                    : 0) +
        ...
        [this.luckLabelText, this.luckText].map((l) =>
            l.setY(
                (this.scoreText.visible ? this.scoreText : this.moneyText).y +
                    10
            )
        );
        const offsetY =
    ...
    addFaintedEnemyScore(enemy: EnemyPokemon): void {
        let scoreIncrease =
            ...
            ((enemy.ivs.reduce(
                (iv: integer, total: integer) => (total += iv),
                0
            ) /
                93) *
                0.2 +
        ...
        this.findModifiers(
            (m) =>
                ...
                m.pokemonId === enemy.id,
            false
        ).map(
            (m) =>
                (scoreIncrease *= (
                    m as PokemonHeldItemModifier
                ).getScoreMultiplier())
        );
        if (enemy.isBoss()) {
    ...
    getMaxExpLevel(ignoreLevelCap?: boolean): integer {
        ...
        const waveIndex =
            Math.ceil((this.currentBattle?.waveIndex || 1) / 10) * 10;
        const difficultyWaveIndex =
            this.gameMode.getWaveForDifficulty(waveIndex);
        const baseLevel =
            (1 +
                difficultyWaveIndex / 2 +
                Math.pow(difficultyWaveIndex / 25, 2)) *
            1.2;
                      ...
                      allSpecies
                          ...
                          .map((s) => {
                              if (!filterAllEvolutions) {
                                  while (
                                      pokemonPrevolutions.hasOwnProperty(
                                          s.speciesId
                                      )
                                  ) {
                                      s = getPokemonSpecies(
                                          pokemonPrevolutions[s.speciesId]
                                      );
                                  }
                              }
                              return s;
        ...
        const depthWeights = new Array(maxDepth + 1)
            ...
            .map(
                (_, i: integer) =>
                    ((1 -
                        Math.min(
                            Math.abs(i / (maxDepth - 1) - relWave / 250) + 0.25,
                            1
                        )) /
                        0.75) *
                    250
    ...
    playBgm(bgmName?: string, fadeOut?: boolean): void {
        if (bgmName === undefined) {
            bgmName =
                this.currentBattle?.getBgmOverride(this) || this.arena?.bgm;
        }
        ...
        if (fadeOut) {
            ...
            this.time.delayedCall(
                this.fadeOutBgm(500, true) ? 750 : 250,
                onBgmFaded
            );
        }
    ...
    playSound(sound: string | AnySound, config?: object): AnySound {
        ...
        if (
            (typeof sound === "string" ? sound : sound.key).startsWith(
                "PRSFX- "
            )
        ) {
            config["volume"] *= 0.5;
    ...
    populatePhaseQueue(): void {
        if (this.nextCommandPhaseQueue.length) {
            ...
            this.nextCommandPhaseQueue.splice(
                0,
                this.nextCommandPhaseQueue.length
            );
        }
    ...
    addModifier(
        ...
        return new Promise((resolve) => {
            ...
            if (modifier instanceof PersistentModifier) {
                ...
                if (
                    (modifier as PersistentModifier).add(
                        this.modifiers,
                        !!virtual,
                        this
                    )
                ...
                if (modifier instanceof ConsumablePokemonModifier) {
                    for (const p in this.party) {
                        ...
                        if (modifier instanceof PokemonHpRestoreModifier) {
                            if (
                                !(modifier as PokemonHpRestoreModifier).fainted
                            ) {
                                const hpRestoreMultiplier =
                                    new Utils.IntegerHolder(1);
                                this.applyModifiers(
                                    HealingBoosterModifier,
                                    true,
                                    hpRestoreMultiplier
    ...
    tryTransferHeldItemModifier(
        ...
        return new Promise((resolve) => {
            ...
            Utils.executeIf(
                source && source.isPlayer() !== target.isPlayer(),
                () => applyAbAttrs(BlockItemTheftAbAttr, source, cancelled)
            ).then(() => {
                if (cancelled.value) {
                    return resolve(false);
                }
                const newItemModifier =
                    itemModifier.clone() as PokemonHeldItemModifier;
                newItemModifier.pokemonId = target.id;
                const matchingModifier = target.scene.findModifier(
                    (m) =>
                        m instanceof PokemonHeldItemModifier &&
                        (m as PokemonHeldItemModifier).matchType(
                            itemModifier
                        ) &&
    ...
    generateEnemyModifiers(): Promise<void> {
        return new Promise((resolve) => {
            ...
            if (this.currentBattle.trainer) {
                const modifiers =
                    this.currentBattle.trainer.genModifiers(party);
            ...
            party.forEach((enemyPokemon: EnemyPokemon, i: integer) => {
                ...
                const modifierChance =
                    this.gameMode.getEnemyModifierChance(isBoss);
                ...
                getEnemyModifierTypesForWave(
                    ...
                    mt
                        ...
                        .add(this.enemyModifiers, false, this)
                );
            });
    ...
    setModifiersVisible(visible: boolean) {
        [this.modifierBar, this.enemyModifierBar].map((m) =>
        ...
        return new Promise((resolve) => {
            ...
            for (let m = 0; m < modifiers.length; m++) {
                ...
                if (
                    ...
                    !this.getPokemonById(
                        (modifier as PokemonHeldItemModifier).pokemonId
                    )
                ) {
    ...
    applyModifiers(
        ...
        const modifiers = (
            player ? this.modifiers : this.enemyModifiers
        ).filter((m) => m instanceof modifierType && m.shouldApply(args));
        return this.applyModifiersInternal(modifiers, player, args);
    }
    ...
    triggerPokemonFormChange(
        ...
        if (pokemonFormChanges.hasOwnProperty(pokemon.species.speciesId)) {
            const matchingFormChange = pokemonFormChanges[
                ...
                (fc) =>
                    fc.findTrigger(formChangeTriggerType) &&
            ...
            if (matchingFormChange) {
                ...
                if (
                    ...
                    phase = new QuietFormChangePhase(
                        this,
                        pokemon,
                        matchingFormChange
                    );

...
const tutorialHandlers = {
  ...
  [Tutorial.Access_Menu]: (scene: BattleScene) => {
    return new Promise<void>((resolve) => {
      if (scene.enableTouchControls) return resolve();
      scene
      ...
      scene.ui.setModeWithoutClear(Mode.MESSAGE).then(() => {
        scene.ui.showText(
          ...
          () =>
            scene.ui.showText("", null, () =>
              scene.ui
                .setModeWithoutClear(Mode.MODIFIER_SELECT)
                .then(() => resolve()),
            ),
          null,
          true,
...
export function handleTutorial(
  ...
  return new Promise<boolean>((resolve) => {
    if (!scene.enableTutorials) return resolve(false);

    if (scene.gameData.getTutorialFlags()[tutorial]) return resolve(false);
    ...
    tutorialHandlers[tutorial](scene).then(() => {
      ...
      if (handler instanceof AwaitableUiHandler) handler.tutorialActive = false;
      resolve(true);
    });
  });
}

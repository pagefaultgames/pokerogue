...
export default class Battle {
  ...
  randSeedGaussForLevel(value: number): number {
    let rand = 0;
    for (let i = value; i > 0; i--) rand += Phaser.Math.RND.realInRange(0, 1);
    return rand / value;
  }
  ...
  incrementTurn(scene: BattleScene): void {
    ...
    this.turnCommands = Object.fromEntries(
      Utils.getEnumValues(BattlerIndex).map((bt) => [bt, null]),
    );
    this.battleSeedState = null;
  }
  ...
  pickUpScatteredMoney(scene: BattleScene): void {
    const moneyAmount = new Utils.IntegerHolder(
      scene.currentBattle.moneyScattered,
    );
    ...
    scene.queueMessage(
      `You picked up ₽${moneyAmount.value.toLocaleString("en-US")}!`,
      null,
      true,
    );
  ...
  addBattleScore(scene: BattleScene): void {
    ...
    if (this.double) partyMemberTurnMultiplier /= 1.5;
    for (const p of scene.getEnemyParty()) {
      if (p.isBoss())
        partyMemberTurnMultiplier *=
          p.bossSegments / 1.5 / scene.getEnemyParty().length;
    }
    const turnMultiplier = Phaser.Tweens.Builders.GetEaseFunction(
      "Sine.easeIn",
    )(
      1 -
        Math.min(this.turn - 2, 10 * partyMemberTurnMultiplier) /
          (10 * partyMemberTurnMultiplier),
    );
    const finalBattleScore = Math.ceil(this.battleScore * turnMultiplier);
    scene.score += finalBattleScore;
    console.log(
      `Battle Score: ${finalBattleScore} (${this.turn - 1} Turns x${
        Math.floor(turnMultiplier * 100) / 100
      })`,
    );
  ...
  getBgmOverride(scene: BattleScene): string {
    ...
    for (const pokemon of battlers) {
      if (this.battleSpec === BattleSpec.FINAL_BOSS) {
        if (pokemon.formIndex) return "battle_final";
      ...
      if (
        ...
        if (
          pokemon.species.speciesId === Species.RESHIRAM ||
          pokemon.species.speciesId === Species.ZEKROM
        )
          return "battle_legendary_res_zek";
        if (pokemon.species.speciesId === Species.KYUREM)
          return "battle_legendary_kyurem";
        if (pokemon.species.legendary) return "battle_legendary_res_zek";
        return "battle_legendary_unova";
      }
    }

    if (scene.gameMode.isClassic && this.waveIndex <= 4) return "battle_wild";
  ...
  randSeedInt(scene: BattleScene, range: integer, min: integer = 0): integer {
    ...
    if (this.battleSeedState) Phaser.Math.RND.state(this.battleSeedState);
    else {
      Phaser.Math.RND.sow([
        Utils.shiftCharCodes(this.battleSeed, this.turn << 6),
      ]);
...
export class FixedBattle extends Battle {
  constructor(
    scene: BattleScene,
    waveIndex: integer,
    config: FixedBattleConfig,
  ) {
    super(
      scene.gameMode,
      waveIndex,
      config.battleType,
      config.battleType === BattleType.TRAINER
        ? config.getTrainer(scene)
        : null,
      config.double,
    );
    if (config.getEnemyParty) this.enemyParty = config.getEnemyParty(scene);
  }
}
...
export class FixedBattleConfig {
  ...
  setGetEnemyPartyFunc(
    getEnemyPartyFunc: GetEnemyPartyFunc,
  ): FixedBattleConfig {

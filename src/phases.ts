      ...
      (response) => {
        ...
        if (!success) {
          ...
          this.scene.gameData.loadSystem().then((success) => {
            if (success || bypassLogin) this.end();
            else {
      ...
      },
      {
        ...
        handler: () => {
          this.scene.ui.setOverlayMode(
            ...
            SaveSlotUiMode.LOAD,
            (slotId: integer) => {
              if (slotId === -1) return this.showOptions();
...
export class ReloadSessionPhase extends Phase {
  ...
  start(): void {
    ...
    this.scene.time.delayedCall(Utils.fixedInt(1500), () => {
      if (loaded) this.end();
      else delayElapsed = true;
    });

    this.scene.gameData.loadSystem().then(() => {
      if (delayElapsed) this.end();
      else loaded = true;
    });
  }
}
        ...
        this.scene.ui.setMode(
          ...
          (slotId: integer) => {
            ...
            starters.forEach((starter: Starter, i: integer) => {
              if (!i && STARTER_SPECIES_OVERRIDE)
                starter.species = getPokemonSpecies(
                  STARTER_SPECIES_OVERRIDE as Species,
                );
              const starterProps = this.scene.gameData.getSpeciesDexAttrProps(
                starter.species,
                starter.dexAttr,
            ...
            Promise.all(loadPokemonAssets).then(() => {
              ...
              else this.scene.gameData.gameStats.endlessSessionsPlayed++;
              this.scene.newBattle();
              this.scene.arena.init();
              this.scene.sessionPlayTime = 0;
              this.end();
...
export abstract class FieldPhase extends BattlePhase {
  getOrder(): BattlerIndex[] {
    ...
    if (speedReversed.value) orderedTargets = orderedTargets.reverse();
...
export class EncounterPhase extends BattlePhase {
      ...
      this.scene.ui.setMode(Mode.MESSAGE).then(() => {
        if (!this.loaded) {
          this.scene.gameData.saveSystem().then((success) => {
            ...
            this.scene.gameData
              .saveSession(this.scene, true)
              .then(() => this.doEncounter());
          });
        } else this.doEncounter();
      });
    });
  }
  ...
  doEncounter() {
    ...
    for (const pokemon of this.scene.getParty()) {
      if (pokemon) pokemon.resetBattleData();
    }
    ...
    this.scene.tweens.add({
      ...
      duration: 2000,
      onComplete: () => {
        if (!this.tryOverrideForBattleSpec()) this.doEncounterCommon();
      },
    });
  }
  ...
  doEncounterCommon(showEncounterMessage = true) {
    ...
    if (this.scene.currentBattle.battleType === BattleType.WILD) {
      enemyField.forEach((enemyPokemon) => {
        ...
        if (enemyPokemon.isShiny()) this.scene.validateAchv(achvs.SEE_SHINY);
      });
    ...
    if (!this.loaded) {
      ...
      if (availablePartyMembers[0].isOnField())
        applyPostBattleInitAbAttrs(
          PostBattleInitAbAttr,
          availablePartyMembers[0],
        );
      else this.scene.pushPhase(new SummonPhase(this.scene, 0));
      ...
      if (this.scene.currentBattle.double) {
        if (availablePartyMembers.length > 1) {
          ...
          else this.scene.pushPhase(new SummonPhase(this.scene, 1));
        }
      } else {
        if (
      ...
      if (
        this.scene.currentBattle.waveIndex > startingWave &&
        this.scene.currentBattle.battleType !== BattleType.TRAINER
      ) {
        this.scene.pushPhase(
          new CheckSwitchPhase(this.scene, 0, this.scene.currentBattle.double),
  ...
  doEncounter(): void {
    ...
    this.scene.tweens.add({
      ...
      onComplete: () => {
        ...
        if (this.scene.lastEnemyTrainer) this.scene.lastEnemyTrainer.destroy();
...
export class SelectBiomePhase extends BattlePhase {
  ...
  start() {
    ...
    if (
      ...
      } else setNextBiome(biomes[Utils.randSeedInt(biomes.length)]);
    } else if (biomeLinks.hasOwnProperty(currentBiome))
      setNextBiome(biomeLinks[currentBiome] as Biome);
    else setNextBiome(this.generateNextBiome());
  }

  generateNextBiome(): Biome {
    if (!(this.scene.currentBattle.waveIndex % 50)) return Biome.END;
...
export class SwitchBiomePhase extends BattlePhase {
  ...
  start() {
    ...
    if (this.nextBiome === undefined) return this.end();
...
export class SummonPhase extends PartyMemberPokemonPhase {
  ...
  preSummon(): void {
    ...
    if (this.player) {
      ...
      if (this.player) this.scene.pbTray.hide();
  ...
  summon(): void {
    ...
    if (this.fieldIndex === 1) pokemon.setFieldPosition(FieldPosition.RIGHT, 0);
    else {
...
export class SwitchSummonPhase extends SummonPhase {
  ...
  preSummon(): void {
    ...
    if (
      ...
      if (this.player) return this.switchAndSummon();
      else {
  ...
  switchAndSummon() {
    ...
    if (switchedPokemon) {
      ...
      if (this.player) showTextAndSummon();
      else {
  ...
  onEnd(): void {
    ...
    if (this.batonPass && pokemon) pokemon.transferSummon(this.lastPokemon);
...
export class CommandPhase extends FieldPhase {
  ...
  start() {
    ...
    if (moveQueue.length) {
      ...
      if (!queuedMove.move) this.handleCommand(Command.FIGHT, -1, false);
      else {
        ...
        } else this.scene.ui.setMode(Mode.COMMAND, this.fieldIndex);
      }
    } else this.scene.ui.setMode(Mode.COMMAND, this.fieldIndex);
  }
    ...
    switch (command) {
      case Command.FIGHT:
        ...
        if (
          ...
          if (!moveId) turnCommand.targets = [this.fieldIndex];
  ...
  start() {
    ...
    if (trainer && !enemyPokemon.getMoveQueue().length) {
      ...
      opponents.forEach((playerPokemon) =>
        applyCheckTrappedAbAttrs(CheckTrappedAbAttr, playerPokemon, trapped),
      );
  ...
  start() {
    ...
    this.scene.ui.setMode(
      ...
      (cursor: integer) => {
        ...
        } else turnCommand.targets = [cursor];
...
export class TurnStartPhase extends FieldPhase {
  ...
  start() {
      ...
      switch (turnCommand.command) {
        ...
        case Command.RUN:
          ...
          if (isSwitch)
            ...
            this.scene.unshiftPhase(
              new AttemptRunPhase(this.scene, pokemon.getFieldIndex()),
            );
          break;
      }
    }
    ...
    if (this.scene.arena.weather)
      this.scene.pushPhase(
        new WeatherEffectPhase(this.scene, this.scene.arena.weather),
      );
...
export class TurnEndPhase extends FieldPhase {
  ...
  start() {
    ...
    const handlePokemon = (pokemon: Pokemon) => {
      ...
      const hasUsableBerry = !!this.scene.findModifier(
        (m) => m instanceof BerryModifier && m.shouldApply([pokemon]),
        pokemon.isPlayer(),
      );
      if (hasUsableBerry)
        this.scene.unshiftPhase(
          new BerryPhase(this.scene, pokemon.getBattlerIndex()),
        );
...
export class BattleEndPhase extends BattlePhase {
  start() {
    ...
    for (const pokemon of this.scene.getField()) {
      if (pokemon) pokemon.resetBattleSummonData();
    }
    ...
    for (const m of lapsingModifiers) {
      ...
      if (!m.lapse(args)) this.scene.removeModifier(m);
    }
...
export class MovePhase extends BattlePhase {
  ...
  start() {
      ...
      if (this.targets.indexOf(p.getBattlerIndex()) > -1) {
        ...
        if (
          ...
          !this.move
            ...
            .length &&
          !p.hasAbilityWithAttr(AlwaysHitAbAttr) &&
          !this.pokemon.hasAbilityWithAttr(AlwaysHitAbAttr)
        )
          return false;
        return true;
      }
    ...
    const doMove = () => {
      ...
      if (this.move.moveId) this.showMoveText();
      ...
      if (!moveQueue.length || !moveQueue.shift().ignorePP) {
        ...
        for (const opponent of targetedOpponents) {
          if (this.move.ppUsed === this.move.getMove().pp) break;
          if (opponent.hasAbilityWithAttr(IncreasePpAbAttr))
            this.move.ppUsed = Math.min(
              this.move.ppUsed + 1,
              this.move.getMovePp(),
            );
        }
      ...
      if (success) this.scene.unshiftPhase(this.getEffectPhase());
      else {
        this.pokemon.pushMoveHistory({
        ...
        if (!cancelled.value) this.showFailedText(failedText);
      }
...
export class MoveEffectPhase extends PokemonPhase {
  ...
  start() {
    ...
    applyMoveAttrs(
      ...
      if (overridden.value) return this.end();
  ...
  hitCheck(target: Pokemon): boolean {
    if (this.move.getMove().moveTarget === MoveTarget.USER) return true;

    const user = this.getUserPokemon();

    // Hit check only calculated on first hit for multi-hit moves
    if (user.turnData.hitsLeft < user.turnData.hitCount) return true;
    ...
    if (
      ...
      (user.getLastXMoves().find(() => true)?.targets || []).indexOf(
    ...
    if (moveAccuracy.value === -1) return true;
...
export class MoveAnimTestPhase extends BattlePhase {
  ...
  playMoveAnim(moveQueue: Moves[], player: boolean) {
    ...
    if (moveId === undefined) {
      ...
      return;
    } else if (player) console.log(Moves[moveId]);
    ...
    initMoveAnim(this.scene, moveId).then(() => {
      loadMoveAnimAssets(this.scene, [moveId], true).then(() => {
        new MoveAnim(
          ...
          if (player) this.playMoveAnim(moveQueue, false);
          else this.playMoveAnim(moveQueue, true);
        });
      });
    });
  }
...
export class StatChangePhase extends PokemonPhase {
  ...
  start() {
    ...
    if (!pokemon.isActive(true)) return this.end();
    ...
    const end = () => {
      if (this.showMessage) {
        ...
        for (const message of messages) this.scene.queueMessage(message);
      }
    ...
    if (relLevels.filter((l) => l).length && this.scene.moveAnimations) {
      ...
      const statSprite = this.scene.add.tileSprite(
        ...
        filteredStats.length > 1
          ? "mix"
          : BattleStat[filteredStats[0]].toLowerCase(),
      );
  ...
  aggregateStatChanges(random = false): void {
    ...
    if (this.stats.length === 1) {
      while (
        ...
        if (existingPhase.stats[0] === BattleStat.RAND) {
          ...
          if (existingPhase.stats[0] !== this.stats[0]) continue;
        }
        this.levels += existingPhase.levels;

        if (!this.scene.tryRemovePhase((p) => p === existingPhase)) break;
      }
    }
    while (
      (existingPhase = this.scene.findPhase(
  ...
  getStatChangeMessages(
    ...
    for (let rl = 0; rl < relLevels.length; rl++) {
      ...
      if (!relLevelStatIndexes[relLevel]) relLevelStatIndexes[relLevel] = [];
    ...
    Object.keys(relLevelStatIndexes).forEach((rl) => {
      ...
      } else statsFragment = getBattleStatName(relLevelStats[0]);
      messages.push(
        getPokemonMessage(
          this.getPokemon(),
          `'s ${statsFragment} ${getBattleStatLevelChangeDescription(
            Math.abs(Number.parseInt(rl)),
            levels >= 1,
          )}!`,
        ),
      );
...
export class WeatherEffectPhase extends CommonAnimPhase {
  ...
  constructor(scene: BattleScene, weather: Weather) {
    super(
      ...
      undefined,
      undefined,
      CommonAnim.SUNNY + (weather.weatherType - 1),
    );
    ...
    if (this.weather.isDamaging()) {
      ...
      if (!cancelled.value) {
        ...
        this.executeForAll((pokemon: Pokemon) => {
          ...
          if (!immune) inflictDamage(pokemon);
        });
      }
    }
  ...
  start() {
    ...
    if (!pokemon.status) {
      if (pokemon.trySetStatus(this.statusEffect)) {
        if (this.cureTurn) pokemon.status.cureTurn = this.cureTurn;
...
export class MessagePhase extends Phase {
  ...
  start() {
    ...
    if (this.scene.abilityBar.shown) this.scene.abilityBar.hide();
...
export class FaintPhase extends PokemonPhase {
  ...
  start() {
    ...
    if (!this.tryOverrideForBattleSpec()) this.doFaint();
  }
  ...
  doFaint(): void {
    ...
    pokemon.faintCry(() => {
      if (pokemon instanceof PlayerPokemon) pokemon.addFriendship(-10);
      pokemon.hideInfo();
  ...
  start() {
    ...
    if (participantIds.size) {
      ...
      for (const partyMember of nonFaintedPartyMembers) {
        ...
        if (participated) partyMember.addFriendship(2);
        if (!expPartyMembers.includes(partyMember)) continue;
        ...
        if (partyMember.pokerus) expMultiplier *= 1.5;
...
export class MoneyRewardPhase extends BattlePhase {
  ...
  start() {
    ...
    this.scene.ui.showText(
      `You got ₽${moneyAmount.value.toLocaleString("en-US")}\nfor winning!`,
      null,
      () => this.end(),
      null,
      true,
...
export class GameOverPhase extends BattlePhase {
  ...
  start() {
    ...
    if (this.victory || !this.scene.enableRetries) this.handleClearSession();
    else {
      this.scene.ui.showText(
  ...
  handleClearSession(): void {
    this.scene.gameData
      ...
      .then((success: boolean | [boolean, boolean]) => {
        if (!success[0]) return this.scene.reset(true);
  ...
  start() {
    ...
    if (this.scene.expGainsSpeed < 3) {
      this.scene.partyExpBar.showPokemonExp(pokemon, exp.value).then(() => {
        if (newLevel > lastLevel) this.end();
        else
          setTimeout(
            () => this.end(),
...
export class LearnMovePhase extends PlayerPartyMemberPokemonPhase {
  ...
  start() {
    ...
    if (existingMoveIndex > -1) return this.end();
                          ...
                          () => {
                            this.scene.ui.setModeWithoutClear(
                              ...
                              (moveIndex: integer) => {
                                ...
                                this.scene.ui.setMode(messageMode).then(() => {
                                  this.scene.ui.showText(
                                    "@d{32}1, @d{15}2, and@d{15}… @d{15}… @d{15}… @d{15}@s{pb_bounce_1}Poof!",
                                    null,
                                    () => {
                                      this.scene.ui.showText(
                                        i18next.t(
...
export class BerryPhase extends CommonAnimPhase {
  ...
  start() {
    ...
    if (cancelled.value)
      ...
      (berryModifiers = this.scene.applyModifiers(
        BerryModifier,
        this.player,
        pokemon,
      ) as BerryModifier[])
      ...
      for (const berryModifier of berryModifiers) {
        if (berryModifier.consumed) {
          ...
          else berryModifier.consumed = false;
          this.scene.updateModifiers(this.player);
        }
      }
      return super.start();
...
export class PokemonHealPhase extends CommonAnimPhase {
  ...
  start() {
    ...
    if (this.message) this.scene.queueMessage(this.message);
...
export class AttemptCapturePhase extends PokemonPhase {
  ...
  failCatch(shakeCount: integer) {
    ...
    if (pokemon.species.mythical) this.scene.validateAchv(achvs.CATCH_MYTHICAL);
      ...
      () => {
        ...
        Promise.all([
          ...
          if (this.scene.getParty().length === 6) {
            const promptRelease = () => {
              this.scene.ui.showText(
                `Your party is full.\nRelease a Pokémon to make room for ${pokemon.name}?`,
                null,
                () => {
                  this.scene.ui.setMode(
                    Mode.CONFIRM,
...
export class SelectModifierPhase extends BattlePhase {
  ...
  start() {
    ...
    if (!this.rerollCount) this.updateSeed();
    ...
    const modifierSelectCallback = (rowCursor: integer, cursor: integer) => {
      ...
      const applyModifier = (
        ...
        if (cost) {
          ...
          if (result instanceof Promise) result.then(() => doEnd());
          else doEnd();
        }
      };
      ...
      } else applyModifier(modifierType.newModifier());
  ...
  getRerollCost(
    ...
    if (lockRarities) {
      ...
      for (const opt of typeOptions) baseValue += tierValues[opt.type.tier];
    } else baseValue = 250;
    return Math.min(
...
export class PartyHealPhase extends BattlePhase {
  ...
  start() {
    ...
    if (bgmPlaying) this.scene.fadeOutBgm(1000, false);
    this.scene.ui.fadeOut(1000).then(() => {
      for (const pokemon of this.scene.getParty()) {
        pokemon.hp = pokemon.getMaxHp();
        pokemon.resetStatus();
        for (const move of pokemon.moveset) move.ppUsed = 0;
        pokemon.updateInfo(true);
      }
      ...
      this.scene.time.delayedCall(
        ...
        () => {
          healSong.destroy();
          if (this.resumeBgm && bgmPlaying) this.scene.playBgm();
...
export class ScanIvsPhase extends PokemonPhase {
  ...
  start() {
    ...
    if (!this.shownIvs) return this.end();
  ...
  start() {
    ...
    for (const t of Object.keys(trainerConfigs)) {
      ...
      [
        ...
        if (messages?.length) testMessages.push(...messages);
      });
    }

  ...
  handleCommand(command: Command, cursor: integer, ...args: any[]): boolean {
    ...
    switch (command) {
      case Command.FIGHT:
        ...
        break;
      case Command.BALL:
        if (this.scene.arena.biomeType === Biome.END && (!this.scene.gameMode.isClassic || (this.scene.getEnemyField().filter(p => p.isActive(true)).some(p => !p.scene.gameData.dexData[p.species.speciesId].caughtAttr || (p.isFusion() && !p.scene.gameData.dexData[p.fusionSpecies.speciesId].caughtAttr)) && this.scene.gameData.getStarterCount(d => !!d.caughtAttr) < Object.keys(speciesStarters).length - 1))) {
          this.scene.ui.setMode(Mode.COMMAND, this.fieldIndex);
          this.scene.ui.setMode(Mode.MESSAGE);

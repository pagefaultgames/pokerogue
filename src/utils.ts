...
export function randomString(length: integer, seeded = false) {
  const characters =
...
export function padInt(
  ...
  while (valueStr.length < length) valueStr = `${padWith}${valueStr}`;
  return valueStr;
}
...
export function randSeedEasedWeightedItem<T>(
  ...
  const easedValue =
    Phaser.Tweens.Builders.GetEaseFunction(easingFunction)(value);
...
export function binToDec(input: string): integer {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    decimalNum += place[i] * Number.parseInt(binary[i]);
  }
...
export function formatLargeNumber(count: integer, threshold: integer): string {
  ...
  const decimalNumber = Number.parseInt(ret.slice(digits, digits + (3 - digits)));
  return `${ret.slice(0, digits)}${
    decimalNumber ? `.${decimalNumber}` : ""
  }${suffix}`;
}

export function formatStat(stat: integer, forHp = false): string {
...
export function apiFetch(
  ...
  return new Promise((resolve, reject) => {
    ...
    if (authed) {
      ...
      if (sId) request["headers"] = { Authorization: sId };
    }
    fetch(`${!fallback ? apiUrl : fallbackApiUrl}/${path}`, request)
      ...
      .catch((err) => {
        if (fallback) reject(err);
        else apiFetch(path, authed, true).then((res) => resolve(res));
      });
  });
}
...
export function apiPost(
  ...
  return new Promise((resolve, reject) => {
    ...
    fetch(`${!fallback ? apiUrl : fallbackApiUrl}/${path}`, {
      method: "POST",
      headers: headers,
      body: data,
    })
      ...
      .catch((err) => {
        if (fallback) reject(err);
          ...
          apiPost(path, data, contentType, authed, true).then((res) =>
            resolve(res),
          );
      });
  });

  ...
  getOrInferTier(
    ...
    for (const type of poolTypes) {
      ...
      for (const tier of Utils.getEnumValues(ModifierTier)) {
        if (!pool.hasOwnProperty(tier)) continue;
        if (
          pool[tier].find(
            (m) =>
              (m as WeightedModifierType).modifierType.id ===
              (this.generatorId || this.id),
          )
        )
...
class AddPokeballModifierType extends ModifierType implements Localizable {
  ...
  constructor(pokeballType: PokeballType, count: integer, iconImage?: string) {
    super(
      ...
      "",
      (_type, _args) =>
        new Modifiers.AddPokeballModifier(this, pokeballType, count),
      iconImage,
      "pb",
      "pb_bounce_1",
    );
  ...
  localize(): void {
    ...
    this.description = `Receive ${getPokeballName(this.pokeballType)} x${
      this.count
    } (Inventory: {AMOUNT}) \nCatch Rate: ${
  ...
  getDescription(scene: BattleScene): string {
    ...
    return this.description.replace(
      "{AMOUNT}",
      scene.pokeballCounts[this.pokeballType].toString(),
    );
  }
}
...
class AddVoucherModifierType extends ModifierType {
  constructor(voucherType: VoucherType, count: integer) {
    super(
      `${count}x ${getVoucherTypeName(voucherType)}`,
      `Receive ${getVoucherTypeName(voucherType)} x${count}`,
      (_type, _args) =>
        new Modifiers.AddVoucherModifier(this, voucherType, count),
  ...
  constructor(
    ...
    super(
      ...
      (pokemon: PlayerPokemon) => {
        ...
        if (!maxStackCount) return `${pokemon.name} can\'t take\nthis item!`;
  ...
  constructor(
    ...
    super(
      ...
      restorePoints
        ...
        : `Fully restores HP for one Pokémon${
            healStatus ? " and heals any status ailment" : ""
          }`,
      newModifierFunc ||
        ((_type, args) =>
        ...
        ((pokemon: PlayerPokemon) => {
          if (
            ...
            (pokemon.hp >= pokemon.getMaxHp() &&
              (!this.healStatus || !pokemon.status))
          )
...
export class PokemonReviveModifierType extends PokemonHpRestoreModifierType {
  constructor(name: string, restorePercent: integer, iconImage?: string) {
    super(
      ...
      (pokemon: PlayerPokemon) => {
        if (!pokemon.isFainted()) return PartyUiHandler.NoEffectMessage;
        return null;
      },
      iconImage,
      "revive",
    ...
    this.selectFilter = (pokemon: PlayerPokemon) => {
      if (pokemon.hp) return PartyUiHandler.NoEffectMessage;
      return null;
    };
  }
}
...
export class PokemonStatusHealModifierType extends PokemonModifierType {
  constructor(name: string) {
    super(
      name,
      `Heals any status ailment for one Pokémon`,
      (_type, args) =>
        new Modifiers.PokemonStatusHealModifier(
          this,
          (args[0] as PlayerPokemon).id,
        ),
      (pokemon: PlayerPokemon) => {
...
export class PokemonPpRestoreModifierType extends PokemonMoveModifierType {
  ...
  constructor(name: string, restorePoints: integer, iconImage?: string) {
    super(
      name,
      `Restores ${
        restorePoints > -1 ? restorePoints : "all"
      } PP for one Pokémon move`,
      (_type, args) =>
      ...
      (pokemonMove: PokemonMove) => {
        if (!pokemonMove.ppUsed) return PartyUiHandler.NoEffectMessage;
        return null;
      },
      iconImage,
      "ether",
...
export class PokemonAllMovePpRestoreModifierType extends PokemonModifierType {
  ...
  constructor(name: string, restorePoints: integer, iconImage?: string) {
    super(
      ...
      `Restores ${
        restorePoints > -1 ? restorePoints : "all"
      } PP for all of one Pokémon's moves`,
      (_type, args) =>
...
export class PokemonNatureChangeModifierType extends PokemonModifierType {
  ...
  constructor(nature: Nature) {
    super(
      `${getNatureName(nature)} Mint`,
      `Changes a Pokémon\'s nature to ${getNatureName(
        nature,
        true,
        true,
        true,
      )} and permanently unlocks the nature for the starter.`,
      (_type, args) =>
        new Modifiers.PokemonNatureChangeModifier(
          this,
          (args[0] as PlayerPokemon).id,
          this.nature,
        ),
...
export class RememberMoveModifierType extends PokemonModifierType {
  constructor(
    ...
    super(
      ...
      description,
      (type, args) =>
        new Modifiers.RememberMoveModifier(
          type,
          (args[0] as PlayerPokemon).id,
          args[1] as integer,
        ),
...
export class DoubleBattleChanceBoosterModifierType extends ModifierType {
  ...
  constructor(name: string, battleCount: integer) {
    super(
      ...
      (_type, _args) =>
        new Modifiers.DoubleBattleChanceBoosterModifier(this, this.battleCount),
      null,
      "lure",
    );
  ...
  implements GeneratedPersistentModifierType
{
  ...
  constructor(tempBattleStat: TempBattleStat) {
    super(
      ...
      `Increases the ${getTempBattleStatName(
        tempBattleStat,
      )} of all party members by 1 stage for 5 battles`,
      (_type, _args) =>
        new Modifiers.TempBattleStatBoosterModifier(this, this.tempBattleStat),
      getTempBattleStatBoosterItemName(tempBattleStat)
        .replace(/\./g, "")
        .replace(/[ ]/g, "_")
        .toLowerCase(),
  ...
  implements GeneratedPersistentModifierType
{
  ...
  constructor(berryType: BerryType) {
    super(
      getBerryName(berryType),
      getBerryEffectDescription(berryType),
      (type, args) =>
        new Modifiers.BerryModifier(type, (args[0] as Pokemon).id, berryType),
      null,
      "berry",
    );
  ...
  implements GeneratedPersistentModifierType
{
  ...
  constructor(moveType: Type, boostPercent: integer) {
    super(
      getAttackTypeBoosterItemName(moveType),
      `Increases the power of a Pokémon's ${Utils.toReadableString(
        Type[moveType],
      )}-type moves by 20%`,
      (_type, args) =>
...
export class PokemonLevelIncrementModifierType extends PokemonModifierType {
  constructor(name: string, iconImage?: string) {
    super(
      ...
      (_type, args) =>
        new Modifiers.PokemonLevelIncrementModifier(
          this,
          (args[0] as PlayerPokemon).id,
        ),
...
export class AllPokemonLevelIncrementModifierType extends ModifierType {
  constructor(name: string, iconImage?: string) {
    super(
      name,
      `Increases all party members' level by 1`,
  ...
  implements GeneratedPersistentModifierType
{
  ...
  constructor(name: string, stat: Stat, _iconImage?: string) {
    super(
      name,
      `Increases the holder's base ${getStatName(
        stat,
      )} by 10%. The higher your IVs, the higher the stack limit.`,
      (_type, args) =>
        new Modifiers.PokemonBaseStatModifier(
          this,
          (args[0] as Pokemon).id,
          this.stat,
        ),
...
class AllPokemonFullHpRestoreModifierType extends ModifierType {
  constructor(
    ...
    super(
      ...
      newModifierFunc ||
        ((_type, _args) =>
          new Modifiers.PokemonHpRestoreModifier(this, -1, 0, 100, false)),
      iconImage,
    );
  }
}
...
class AllPokemonFullReviveModifierType extends AllPokemonFullHpRestoreModifierType {
  constructor(name: string, iconImage?: string) {
    super(
      name,
      `Revives all fainted Pokémon, fully restoring HP`,
      (_type, _args) =>
        new Modifiers.PokemonHpRestoreModifier(this, -1, 0, 100, false, true),
      iconImage,
    );
  }
}
...
export class MoneyRewardModifierType extends ModifierType {
  ...
  constructor(
    ...
    moneyMultiplierDescriptor: string,
    iconImage?: string,
  ) {
    super(
      name,
      `Grants a ${moneyMultiplierDescriptor} amount of money (₽{AMOUNT})`,
      (_type, _args) =>
        new Modifiers.MoneyRewardModifier(this, moneyMultiplier),
      iconImage,
      "money",
      "buy",
    );
  ...
  getDescription(scene: BattleScene): string {
    return this.description.replace(
      "{AMOUNT}",
      scene.getWaveMoneyAmount(this.moneyMultiplier).toLocaleString("en-US"),
    );
  }
}
...
export class ExpBoosterModifierType extends ModifierType {
  constructor(name: string, boostPercent: integer, iconImage?: string) {
    super(
      name,
      `Increases gain of EXP. Points by ${boostPercent}%`,
      () => new Modifiers.ExpBoosterModifier(this, boostPercent),
      iconImage,
    );
  }
}
...
export class PokemonExpBoosterModifierType extends PokemonHeldItemModifierType {
  constructor(name: string, boostPercent: integer, iconImage?: string) {
    super(
      name,
      `Increases the holder's gain of EXP. Points by ${boostPercent}%`,
      (_type, args) =>
        new Modifiers.PokemonExpBoosterModifier(
          this,
          (args[0] as Pokemon).id,
          boostPercent,
        ),
...
export class PokemonFriendshipBoosterModifierType extends PokemonHeldItemModifierType {
  constructor(name: string, iconImage?: string) {
    super(
      name,
      "Increases friendship gain per victory by 50%",
      (_type, args) =>
        new Modifiers.PokemonFriendshipBoosterModifier(
          this,
          (args[0] as Pokemon).id,
        ),
      iconImage,
...
export class PokemonMoveAccuracyBoosterModifierType extends PokemonHeldItemModifierType {
  constructor(
    ...
    super(
      name,
      `Increases move accuracy by ${amount} (maximum 100)`,
      (_type, args) =>
        new Modifiers.PokemonMoveAccuracyBoosterModifier(
          this,
          (args[0] as Pokemon).id,
          amount,
        ),
...
export class TmModifierType extends PokemonModifierType {
  ...
  constructor(moveId: Moves) {
    super(
      `TM${Utils.padInt(
        Object.keys(tmSpecies).indexOf(moveId.toString()) + 1,
        3,
      )} - ${allMoves[moveId].name}`,
      `Teach ${allMoves[moveId].name} to a Pokémon`,
      (_type, args) =>
        new Modifiers.TmModifier(this, (args[0] as PlayerPokemon).id),
      (pokemon: PlayerPokemon) => {
        if (
  ...
  implements GeneratedPersistentModifierType
{
  ...
  constructor(evolutionItem: EvolutionItem) {
    super(
      Utils.toReadableString(EvolutionItem[evolutionItem]),
      `Causes certain Pokémon to evolve`,
      (_type, args) =>
        new Modifiers.EvolutionItemModifier(
          this,
          (args[0] as PlayerPokemon).id,
        ),
      (pokemon: PlayerPokemon) => {
  ...
  implements GeneratedPersistentModifierType
{
  ...
  constructor(formChangeItem: FormChangeItem) {
    super(
      Utils.toReadableString(FormChangeItem[formChangeItem]),
      `Causes certain Pokémon to change form`,
      (_type, args) =>
        ...
        if (
          ...
          !!pokemonFormChanges[pokemon.species.speciesId].find(
            (fc) =>
              ...
              (fc.trigger as SpeciesFormChangeItemTrigger).item ===
                this.formChangeItem,
          )
        )
          return null;
...
export class FusePokemonModifierType extends PokemonModifierType {
  constructor(name: string, iconImage?: string) {
    super(
      ...
      (pokemon: PlayerPokemon) => {
        if (pokemon.isFusion()) return PartyUiHandler.NoEffectMessage;
        return null;
      },
      iconImage,
    );
...
class AttackTypeBoosterModifierTypeGenerator extends ModifierTypeGenerator {
  constructor() {
    super((party: Pokemon[], pregenArgs?: any[]) => {
      ...
      if (!attackMoveTypes.length) return null;
      ...
      for (const t of attackMoveTypes) {
        ...
        } else attackMoveTypeWeights.set(t, 1);
        totalWeight++;
      }
  ...
  constructor(tier: ModifierTier) {
    super((party: Pokemon[]) => {
      const partyMemberCompatibleTms = party.map((p) =>
        (p as PlayerPokemon).compatibleTms.filter(
          (tm) => !p.moveset.find((m) => m.moveId === tm),
        ),
      );
      ...
      if (!tierUniqueCompatibleTms.length) return null;
...
class EvolutionItemModifierTypeGenerator extends ModifierTypeGenerator {
  constructor(rare: boolean) {
    super((party: Pokemon[], pregenArgs?: any[]) => {
      ...
      if (!evolutionItemPool.length) return null;
...
class FormChangeItemModifierTypeGenerator extends ModifierTypeGenerator {
  constructor() {
    super((party: Pokemon[], pregenArgs?: any[]) => {
            ...
            .filter(
              (fc) =>
                ((fc.formKey.indexOf(SpeciesFormKey.MEGA) === -1 &&
                  ...
                  party[0].scene.getModifiers(
                    Modifiers.MegaEvolutionAccessModifier,
                  ).length) &&
                ((fc.formKey.indexOf(SpeciesFormKey.GIGANTAMAX) === -1 &&
                  fc.formKey.indexOf(SpeciesFormKey.ETERNAMAX) === -1) ||
                  party[0].scene.getModifiers(
                    Modifiers.GigantamaxAccessModifier,
                  ).length),
            )
            .map(
              (fc) =>
      ...
      if (!formChangeItemPool.length) return null;
  ...
  implements GeneratedPersistentModifierType
{
  ...
  constructor(teraType: Type) {
    super(
      `${Utils.toReadableString(Type[teraType])} Tera Shard`,
      `${Utils.toReadableString(
        Type[teraType],
      )} Terastallizes the holder for up to 10 battles`,
      (type, args) =>
...
export class TurnHeldItemTransferModifierType extends PokemonHeldItemModifierType {
  constructor(
    ...
    super(
      name,
      "Every turn, the holder acquires one held item from the foe.",
      (type, args) =>
        new Modifiers.TurnHeldItemTransferModifier(
          type,
          (args[0] as Pokemon).id,
        ),
      iconImage,
...
export class EnemyAttackStatusEffectChanceModifierType extends ModifierType {
  constructor(
    ...
    super(
      name,
      `Adds a ${chancePercent}% chance to inflict ${getStatusEffectDescriptor(
        effect,
      )} with attack moves`,
      (type, args) =>
...
export class EnemyEndureChanceModifierType extends ModifierType {
  constructor(name: string, chancePercent: number, iconImage?: string) {
    super(
      name,
      `Adds a ${chancePercent}% chance of enduring a hit`,
      (type, _args) =>
        new Modifiers.EnemyEndureChanceModifier(type, chancePercent),
      iconImage,
      "enemy_endure",
    );
  }
...
export const modifierTypes = {
  ...
  ROGUE_BALL: () =>
    new AddPokeballModifierType(PokeballType.ROGUE_BALL, 5, "rb"),
  MASTER_BALL: () =>
  ...
  MAP: () =>
    new ModifierType(
      "Map",
      "Allows you to choose your destination at a crossroads",
  ...
  FULL_RESTORE: () =>
    new PokemonHpRestoreModifierType("Full Restore", 0, 100, true),
  ...
  REVIVER_SEED: () =>
    new PokemonHeldItemModifierType(
      "Reviver Seed",
      "Revives the holder for 1/2 HP upon fainting",
      (type, args) =>
        new Modifiers.PokemonInstantReviveModifier(
          type,
          (args[0] as Pokemon).id,
        ),
    ),
  ...
  TEMP_STAT_BOOSTER: () =>
    new ModifierTypeGenerator((party: Pokemon[], pregenArgs?: any[]) => {
      if (pregenArgs)
        return new TempBattleStatBoosterModifierType(
          pregenArgs[0] as TempBattleStat,
        );
  ...
  BASE_STAT_BOOSTER: () =>
    new ModifierTypeGenerator((party: Pokemon[], pregenArgs?: any[]) => {
      if (pregenArgs) {
        ...
        return new PokemonBaseStatBoosterModifierType(
          getBaseStatBoosterItemName(stat),
          stat,
        );
      }
      const randStat = Utils.randSeedInt(6) as Stat;
      return new PokemonBaseStatBoosterModifierType(
        getBaseStatBoosterItemName(randStat),
        randStat,
      );
    }),
  ...
  MINT: () =>
    new ModifierTypeGenerator((party: Pokemon[], pregenArgs?: any[]) => {
      ...
      return new PokemonNatureChangeModifierType(
        Utils.randSeedInt(Utils.getEnumValues(Nature).length) as Nature,
      );
    }),
  ...
  TERA_SHARD: () =>
    new ModifierTypeGenerator((party: Pokemon[], pregenArgs?: any[]) => {
      ...
      if (
        !party[0].scene.getModifiers(Modifiers.TerastallizeAccessModifier)
          .length
      )
        return null;
      let type: Type;
      if (!Utils.randSeedInt(3)) {
        const partyMemberTypes = party
          .flatMap((p) => p.getTypes(false, false, true));
        type = Utils.randSeedItem(partyMemberTypes);
      } else
        type = Utils.randSeedInt(64)
          ? (Utils.randSeedInt(18) as Type)
          : Type.STELLAR;
  ...
  BERRY: () =>
    new ModifierTypeGenerator((party: Pokemon[], pregenArgs?: any[]) => {
      if (pregenArgs) return new BerryModifierType(pregenArgs[0] as BerryType);
      const berryTypes = Utils.getEnumValues(BerryType);
      let randBerryType: BerryType;
      const rand = Utils.randSeedInt(12);
      if (rand < 2) randBerryType = BerryType.SITRUS;
      else if (rand < 4) randBerryType = BerryType.LUM;
      else if (rand < 6) randBerryType = BerryType.LEPPA;
      else
        randBerryType =
          berryTypes[Utils.randSeedInt(berryTypes.length - 3) + 2];
  ...
  MEMORY_MUSHROOM: () =>
    new RememberMoveModifierType(
      "Memory Mushroom",
      "Recall one Pokémon's forgotten move",
      "big_mushroom",
    ),
  ...
  EXP_SHARE: () =>
    new ModifierType(
      "EXP. All",
      "Non-participants receive 20% of a single participant's EXP. Points.",
  ...
  EXP_BALANCE: () =>
    new ModifierType(
      "EXP. Balance",
      "Weighs EXP. Points received from battles towards lower-leveled party members",
  ...
  SOUL_DEW: () =>
    new PokemonHeldItemModifierType(
      "Soul Dew",
      "Increases the influence of a Pokémon's nature on its stats by 10% (additive)",
      (type, args) =>
        new Modifiers.PokemonNatureWeightModifier(
          type,
          (args[0] as Pokemon).id,
        ),
    ),
  ...
  AMULET_COIN: () =>
    new ModifierType(
      "Amulet Coin",
      "Increases money rewards by 20%",
      (type, _args) => new Modifiers.MoneyMultiplierModifier(type),
    ),
  GOLDEN_PUNCH: () =>
    new PokemonHeldItemModifierType(
      "Golden Punch",
      "Grants 50% of damage inflicted as money",
      (type, args) =>
        new Modifiers.DamageMoneyRewardModifier(type, (args[0] as Pokemon).id),
    ),
  COIN_CASE: () =>
    new ModifierType(
      "Coin Case",
      "After every 10th battle, receive 10% of your money in interest.",
  ...
  LOCK_CAPSULE: () =>
    new ModifierType(
      "Lock Capsule",
      "Allows you to lock item rarities when rerolling items",
      (type, _args) => new Modifiers.LockModifierTiersModifier(type),
      "lock_capsule",
    ),

  GRIP_CLAW: () =>
    new ContactHeldItemTransferChanceModifierType("Grip Claw", 10),
  WIDE_LENS: () =>
    new PokemonMoveAccuracyBoosterModifierType("Wide Lens", 5, "wide_lens"),
  ...
  CANDY_JAR: () =>
    new ModifierType(
      "Candy Jar",
      "Increases the number of levels added by Rare Candy items by 1",
  ...
  BERRY_POUCH: () =>
    new ModifierType(
      "Berry Pouch",
      "Adds a 25% chance that a used berry will not be consumed",
  ...
  FOCUS_BAND: () =>
    new PokemonHeldItemModifierType(
      "Focus Band",
      "Adds a 10% chance to survive with 1 HP after being damaged enough to faint",
      (type, args) =>
        new Modifiers.SurviveDamageModifier(type, (args[0] as Pokemon).id),
    ),

  KINGS_ROCK: () =>
    new PokemonHeldItemModifierType(
      "King's Rock",
      "Adds a 10% chance an attack move will cause the opponent to flinch",
      (type, args) =>
        new Modifiers.FlinchChanceModifier(type, (args[0] as Pokemon).id),
    ),

  LEFTOVERS: () =>
    new PokemonHeldItemModifierType(
      "Leftovers",
      "Heals 1/16 of a Pokémon's maximum HP every turn",
      (type, args) =>
        new Modifiers.TurnHealModifier(type, (args[0] as Pokemon).id),
    ),
  SHELL_BELL: () =>
    new PokemonHeldItemModifierType(
      "Shell Bell",
      "Heals 1/8 of a Pokémon's dealt damage",
      (type, args) =>
        new Modifiers.HitHealModifier(type, (args[0] as Pokemon).id),
    ),
  ...
  BATON: () =>
    new PokemonHeldItemModifierType(
      ...
      (type, args) =>
        new Modifiers.SwitchEffectTransferModifier(
          type,
          (args[0] as Pokemon).id,
        ),
      "stick",
  ...
  SHINY_CHARM: () =>
    new ModifierType(
      "Shiny Charm",
      "Dramatically increases the chance of a wild Pokémon being Shiny",
      (type, _args) => new Modifiers.ShinyRateBoosterModifier(type),
    ),
  ABILITY_CHARM: () =>
    new ModifierType(
      "Ability Charm",
      "Dramatically increases the chance of a wild Pokémon having a Hidden Ability",
  ...
  MINI_BLACK_HOLE: () =>
    new TurnHeldItemTransferModifierType("Mini Black Hole"),
  ...
  GOLDEN_POKEBALL: () =>
    new ModifierType(
      `Golden ${getPokeballName(PokeballType.POKEBALL)}`,
      "Adds 1 extra item option at the end of every battle",
  ...
  ENEMY_DAMAGE_REDUCTION: () =>
    new ModifierType(
      "Protection Token",
      "Reduces incoming damage by 2.5%",
  ...
  ENEMY_HEAL: () =>
    new ModifierType(
      "Recovery Token",
      "Heals 3% of max HP every turn",
      (type, _args) => new Modifiers.EnemyTurnHealModifier(type, 3),
      "wl_potion",
    ),
  ...
  ENEMY_STATUS_EFFECT_HEAL_CHANCE: () =>
    new ModifierType(
      "Full Heal Token",
      "Adds a 10% chance every turn to heal a status condition",
      (type, _args) =>
        new Modifiers.EnemyStatusEffectHealChanceModifier(type, 10),
      "wl_full_heal",
    ),
  ENEMY_ENDURE_CHANCE: () =>
    new EnemyEndureChanceModifierType("Endure Token", 2.5, "wl_reset_urge"),
  ENEMY_FUSED_CHANCE: () =>
    new ModifierType(
      "Fusion Token",
      "Adds a 1% chance that a wild Pokémon will be a fusion",
...
const modifierPool: ModifierPool = {
  [ModifierTier.COMMON]: [
    ...
    new WeightedModifierType(
      ...
      (party: Pokemon[]) => {
        const thresholdPartyMemberCount = Math.min(
          party.filter((p) => p.getInverseHp() >= 10 || p.getHpRatio() <= 0.875)
            .length,
          3,
        );
    ...
    new WeightedModifierType(
      ...
      (party: Pokemon[]) => {
        const thresholdPartyMemberCount = Math.min(
          party.filter((p) => p.getInverseHp() >= 25 || p.getHpRatio() <= 0.75)
            .length,
          3,
        );
      ...
      (party: Pokemon[]) => {
        const thresholdPartyMemberCount = Math.min(
          party.filter(
            (p) =>
              p.hp &&
              p.getMoveset().filter((m) => m.getMove().pp - m.ppUsed <= 5)
                .length,
          ).length,
          3,
        );
  ...
  [ModifierTier.GREAT]: [
    ...
    new WeightedModifierType(
      ...
      (party: Pokemon[]) => {
        const statusEffectPartyMemberCount = Math.min(
          party.filter((p) => p.hp && !!p.status).length,
          3,
        );
    ...
    new WeightedModifierType(
      ...
      (party: Pokemon[]) => {
        const thresholdPartyMemberCount = Math.min(
          party.filter(
            (p) => p.getInverseHp() >= 100 || p.getHpRatio() <= 0.625,
          ).length,
          3,
        );
        return thresholdPartyMemberCount * 3;
    ...
    new WeightedModifierType(
      ...
      (party: Pokemon[]) => {
        const thresholdPartyMemberCount = Math.min(
          party.filter((p) => p.getInverseHp() >= 150 || p.getHpRatio() <= 0.5)
            .length,
          3,
        );
        return thresholdPartyMemberCount;
    ...
    new WeightedModifierType(
      ...
      (party: Pokemon[]) => {
        ...
        const thresholdPartyMemberCount = Math.floor(
          (Math.min(
            party.filter(
              (p) => p.getInverseHp() >= 150 || p.getHpRatio() <= 0.5,
            ).length,
            3,
          ) +
    ...
    new WeightedModifierType(
      ...
      (party: Pokemon[]) => {
        return Math.min(
          Math.ceil(party[0].scene.currentBattle.waveIndex / 15),
          8,
        );
      },
      8,
    ...
    new WeightedModifierType(
      modifierTypes.MAP,
      (party: Pokemon[]) => (party[0].scene.gameMode.isClassic ? 1 : 0),
      1,
    ),
    ...
    new WeightedModifierType(
      ...
      (party: Pokemon[]) => {
        if (!party.find((p) => p.getLearnableLevelMoves().length)) return 0;
        const highestPartyLevel = party
          .map((p) => p.level)
          .reduce(
            (highestLevel: integer, level: integer) =>
  ...
  [ModifierTier.ULTRA]: [
    ...
    new WeightedModifierType(
      ...
      (party: Pokemon[]) =>
        Math.min(
          Math.ceil(party[0].scene.currentBattle.waveIndex / 15) * 4,
          32,
        ),
      32,
    ),
    ...
    new WeightedModifierType(
      ...
      (party: Pokemon[]) =>
        Math.min(
          Math.max(
            Math.floor(party[0].scene.currentBattle.waveIndex / 50) * 2,
            1,
          ),
          4,
        ),
    ...
    new WeightedModifierType(
      ...
      (party: Pokemon[], rerollCount: integer) =>
        !party[0].scene.gameMode.isDaily ? Math.max(3 - rerollCount, 0) : 0,
      3,
    ),
  ].map((m) => {
  ...
  [ModifierTier.ROGUE]: [
    ...
    new WeightedModifierType(
      ...
      (party: Pokemon[]) =>
        Math.min(Math.ceil(party[0].scene.currentBattle.waveIndex / 50), 4) * 8,
      32,
    ),
    new WeightedModifierType(
  ...
  [ModifierTier.MASTER]: [
    ...
    new WeightedModifierType(
      ...
      (party: Pokemon[], rerollCount: integer) =>
        !party[0].scene.gameMode.isDaily ? Math.max(9 - rerollCount * 3, 0) : 0,
      9,
    ),
    new WeightedModifierType(
    ...
    new WeightedModifierType(
      ...
      (party: Pokemon[]) =>
        party[0].scene.gameData.unlocks[Unlockables.MINI_BLACK_HOLE] ? 1 : 0,
      1,
    ),
  ].map((m) => {
...
export function regenerateModifierPoolThresholds(
      ...
      Object.keys(pool).map((t) => {
        ...
        pool[t].reduce((total: integer, modifierType: WeightedModifierType) => {
          ...
          const existingModifiers = party[0].scene.findModifiers(
            (m) =>
              (m.type.generatorId || m.type.id) ===
              weightedModifierType.modifierType.id,
          ...
          const weight =
            ...
            existingModifiers.find(
              (m) => m.stackCount < m.getMaxStackCount(party[0].scene, true),
            )
          ...
          if (weightedModifierType.maxWeight) {
            const modifierId =
              weightedModifierType.modifierType.generatorId ||
              weightedModifierType.modifierType.id;
  ...
  if (outputModifierData) console.table(modifierTableData);
  switch (poolType) {
    case ModifierPoolType.PLAYER:
...
export function getEnemyBuffModifierForWave(
  ...
  const tierStackCount =
    tier === ModifierTier.ULTRA ? 5 : tier === ModifierTier.GREAT ? 3 : 1;
  const retryCount = 50;
  let candidate = getNewModifierTypeOption(
    null,
  ...
  const modifier =
    candidate.type.newModifier() as Modifiers.EnemyPersistentModifier;
...
function getNewModifierTypeOption(
  ...
  if (tier === undefined) {
    ...
    while (
      ...
      tier--;
      if (upgradeCount) upgradeCount--;
    }
  } else if (upgradeCount === undefined && player) {
    upgradeCount = 0;
    if (tier < ModifierTier.MASTER) {
      const partyShinyCount = party.filter(
        (p) => p.isShiny() && !p.isFainted(),
      ).length;
      ...
      while (
        ...
        if (!Utils.randSeedInt(upgradeOdds)) upgradeCount++;
        else break;
      }
      tier += upgradeCount;
    }
  ...
  if (modifierType instanceof ModifierTypeGenerator) {
    ...
    if (modifierType === null) {
      if (player) console.log(ModifierTier[tier], upgradeCount);
...
export class ModifierTypeOption {
  ...
  constructor(type: ModifierType, upgradeCount: integer, cost = 0) {
    this.type = type;

...
const modifierPool: ModifierPool = {
  [ModifierTier.COMMON]: [
    ...
    //new WeightedModifierType(modifierTypes.FORM_CHANGE_ITEM, (party: Pokemon[]) => Math.min(Math.ceil(party[0].scene.currentBattle.waveIndex / 20), 10) * 10, 100),
  ...
  [ModifierTier.GREAT]: [
    ...
    new WeightedModifierType(modifierTypes.FORM_CHANGE_ITEM, (party: Pokemon[]) => Math.min(Math.ceil(party[0].scene.currentBattle.waveIndex / 20), 10) * 10, 100),
    new WeightedModifierType(modifierTypes.SHINY_CHARM, 14),
    new WeightedModifierType(modifierTypes.SUPER_EXP_CHARM, 18),
    ...
    new WeightedModifierType(modifierTypes.TERA_SHARD, 10),
    //new WeightedModifierType(modifierTypes.DNA_SPLICERS, (party: Pokemon[]) => party[0].scene.gameMode.isSplicedOnly && party.filter(p => !p.fusionSpecies).length > 1 ? 10 : 0),
  ...
  [ModifierTier.ROGUE]: [
    ...
    new WeightedModifierType(modifierTypes.FORM_CHANGE_ITEM, (party: Pokemon[]) => Math.min(Math.ceil(party[0].scene.currentBattle.waveIndex / 50), 4) * 10, 40),
...
const tierWeights = [ 1 / 1024, 12 / 1024, 48 / 1024, 192 / 1024, 769 / 1024 ];
...
export function getPlayerShopModifierTypeOptionsForWave(waveIndex: integer, baseCost: integer): ModifierTypeOption[] {
  ...
  const options = [
    [
      new ModifierTypeOption(modifierTypes.POTION(), 0, baseCost * 0.1),
      new ModifierTypeOption(modifierTypes.ETHER(), 0, baseCost * 0.1),
      new ModifierTypeOption(modifierTypes.REVIVE(), 0, baseCost * 0.1)
    ],
    [
      new ModifierTypeOption(modifierTypes.SUPER_POTION(), 0, baseCost * 0.1),
      new ModifierTypeOption(modifierTypes.FULL_HEAL(), 0, baseCost * 0.1),
    ],
    [
      new ModifierTypeOption(modifierTypes.ELIXIR(), 0, baseCost * 0.1),
      new ModifierTypeOption(modifierTypes.MAX_ETHER(), 0, baseCost * 0.1)
    ],
    [
      new ModifierTypeOption(modifierTypes.HYPER_POTION(), 0, baseCost * 0.1),
      new ModifierTypeOption(modifierTypes.MAX_REVIVE(), 0, baseCost * 0.1)
    ],
    [
      new ModifierTypeOption(modifierTypes.MAX_POTION(), 0, baseCost * 0.1),
      new ModifierTypeOption(modifierTypes.MAX_ELIXIR(), 0, baseCost * 0.1)
    ],
    [
      new ModifierTypeOption(modifierTypes.FULL_RESTORE(), 0, baseCost * 0.1)
    ],
    [
      new ModifierTypeOption(modifierTypes.SACRED_ASH(), 0, baseCost * 0.1)
    ]
  ];
...
export function getPartyLuckValue(party: Pokemon[]): integer {
  const luck = Phaser.Math.Clamp(party.map(p => p.isFainted() ? 0 : p.getLuck())
    .reduce((total: integer, value: integer) => total += value, 0), 14, 14);
  return luck || 14;
}

...
export class EggHatchPhase extends Phase {
  ...
  start() {
    ...
    this.scene.ui.setModeForceTransition(Mode.EGG_HATCH_SCENE).then(() => {
      if (!this.egg) return this.end();

      const eggIndex = this.scene.gameData.eggs.findIndex(
        (e) => e.id === this.egg.id,
      );
      ...
      this.eggSprite = this.scene.add.sprite(
        0,
        0,
        "egg",
        `egg_${this.egg.getKey()}`,
      );
          ...
          this.scene.tweens.add({
            ...
            onComplete: () => {
              ...
              if (count < repeatCount)
                return this.doEggShake(intensity, repeatCount, count).then(() =>
                  resolve(),
                );
              this.scene.tweens.add({
                targets: this.eggContainer,
  ...
  trySkip(): boolean {
    if (!this.canSkip || this.skipped) return false;
    this.skipped = true;
    if (!this.hatched) this.doHatch();
    else this.doReveal();
    return true;
  ...
  doReveal(): void {
    ...
    if (isShiny) this.scene.validateAchv(achvs.HATCH_SHINY);
    ...
    this.scene.time.delayedCall(Utils.fixedInt(250), () => {
      ...
      if (isShiny) {
        this.scene.time.delayedCall(Utils.fixedInt(500), () => {
          this.pokemonShinySparkle.play(
            `sparkle${
              this.pokemon.variant ? `_${this.pokemon.variant + 1}` : ""
            }`,
          );
      ...
      this.scene.time.delayedCall(
        Utils.fixedInt(
          !this.skipped ? (!isShiny ? 1250 : 1750) : !isShiny ? 250 : 750,
        ),
        () => {
    ...
    this.scene.executeWithSeedOffset(
      () => {
        if (this.egg.isManaphyEgg()) {
          ...
          this.egg.tier === EggTier.MASTER &&
          this.egg.gachaType === GachaType.LEGENDARY
        ) {
          if (!Utils.randSeedInt(2))
            speciesOverride = getLegendaryGachaSpeciesForTimestamp(
              this.scene,
              this.egg.timestamp,
            );
        }
        ...
        if (speciesOverride) {
          ...
          const speciesPool = Object.keys(speciesStarters)
            .filter(
              (s) =>
                speciesStarters[s] >= minStarterValue &&
                speciesStarters[s] <= maxStarterValue,
            )
        ...
        this.eggMoveIndex = Utils.randSeedInt(
          baseChance * Math.pow(2, 3 - this.egg.tier),
        )
          ? Utils.randSeedInt(3)
          : 3;
      },

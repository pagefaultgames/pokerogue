...
export class EvolutionPhase extends Phase {
  ...
  constructor(
    ...
    evolution: SpeciesFormEvolution,
    lastLevel: integer,
  ) {
    super(scene);
  ...
  start() {
    ...
    this.setMode().then(() => {
      if (!this.validate()) return this.end();

      this.scene.fadeOutBgm(null, false);

      const evolutionHandler =
        this.scene.ui.getHandler() as EvolutionSceneHandler;
      ...
      this.evolutionBgOverlay = this.scene.add.rectangle(
        0,
        0,
      ...
      this.evolutionContainer.add(
        (this.pokemonTintSprite = getPokemonSprite()),
      );
      this.evolutionContainer.add((this.pokemonEvoSprite = getPokemonSprite()));
      this.evolutionContainer.add(
        (this.pokemonEvoTintSprite = getPokemonSprite()),
      );
      ...
      this.evolutionOverlay = this.scene.add.rectangle(
        ...
        -this.scene.game.canvas.height / 6,
        this.scene.game.canvas.width / 6,
        this.scene.game.canvas.height / 6 - 48,
        0xffffff,
      );
  ...
  doEvolution(): void {
    ...
    this.scene.ui.showText(
      ...
      () => {
        ...
        this.pokemon
          .getPossibleEvolution(this.evolution)
          .then((evolvedPokemon) => {
                        ...
                        this.scene.time.delayedCall(1500, () => {
                          ...
                          this.doCycle(1).then((success) => {
                            if (!success) {
                              ...
                              this.scene.ui.showText(
                                `${preName} stopped evolving.`,
                                null,
                                () => {
                                  this.scene.ui.showText(
                                    `Would you like to pause evolutions for ${preName}?\nEvolutions can be re-enabled from the party screen.`,
                                    null,
                                    () => {
                                      const end = () => {
                                      ...
                                      this.scene.ui.setOverlayMode(
                                        ...
                                        () => {
                                          ...
                                          this.scene.ui.showText(
                                            `Evolutions have been paused for ${preName}.`,
                                            null,
                                            end,
                                            3000,
                                          );
                            ...
                            this.scene.time.delayedCall(900, () => {
                              ...
                              this.pokemon.evolve(this.evolution).then(() => {
                                const levelMoves = this.pokemon.getLevelMoves(
                                  this.lastLevel + 1,
                                  true,
                                );
                                              ...
                                              () => {
                                                ...
                                                this.scene.time.delayedCall(
                                                  ...
                                                  () => {
                                                    ...
                                                    evolvedPokemon.destroy();
                                                    this.scene.ui.showText(
                                                      `Congratulations!\nYour ${preName} evolved into ${this.pokemon.name}!`,
                                                      null,
                                                      () => this.end(),
                                                      null,
                                                      true,
  ...
  doArcDownward() {
    ...
    this.scene.tweens.addCounter({
      ...
      onRepeat: () => {
        if (f < 96) {
          if (f < 6) {
            for (let i = 0; i < 9; i++) this.doArcDownParticle(i * 16);
          }
          f++;
        }
      },
  ...
  doCycle(l: number, lastCycle: integer = 15): Promise<boolean> {
    return new Promise((resolve) => {
      ...
      this.scene.tweens.add({
        ...
        yoyo: !isLastCycle,
        onComplete: () => {
          if (evolutionHandler.cancelled) return resolve(false);
          if (l < lastCycle)
            this.doCycle(l + 0.5, lastCycle).then((success) =>
              resolve(success),
            );
          else {
  ...
  doCircleInward() {
    ...
    this.scene.tweens.addCounter({
      ...
      onRepeat: () => {
        if (!f) {
          for (let i = 0; i < 16; i++) this.doCircleInwardParticle(i * 16, 4);
        } else if (f === 32) {
          for (let i = 0; i < 16; i++) this.doCircleInwardParticle(i * 16, 8);
        }
        f++;
      },
    });
  ...
  doSpray() {
    ...
    this.scene.tweens.addCounter({
      ...
      onRepeat: () => {
        if (!f) {
          for (let i = 0; i < 8; i++) this.doSprayParticle(i);
        } else if (f < 50) this.doSprayParticle(Utils.randInt(8));
        f++;
      },
    });
  }
  ...
  doSprayParticle(trigIndex: integer) {
    ...
    const updateParticle = () => {
      ...
      if (trigIndex < 128) {
        ...
        particle.y += -sin(trigIndex, amp);
        if (f > 108) particle.setScale(1 - (f - 108) / 20);
        trigIndex++;
        f++;
      } else {
        particle.destroy();

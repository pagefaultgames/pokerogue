...
local function getAOProcessesRoot()
    ...
    if currentDir and not currentDir:match("^/") then
        local cwd = os.getenv("PWD") or "."
        currentDir = cwd .. "/" .. currentDir
    end
    ...
    local cwd = os.getenv("PWD") or "."
    if cwd:match("/ao%-processes/?$") then
        return cwd:gsub("/$", "")
    elseif cwd:match("/tests/?$") then
        return cwd:gsub("/tests/?$", "")
    elseif cwd:match("/integration/?$") then
        return cwd:gsub("/tests/integration/?$", "")
    else
        -- Last resort
        return cwd .. "/../.."
    end
